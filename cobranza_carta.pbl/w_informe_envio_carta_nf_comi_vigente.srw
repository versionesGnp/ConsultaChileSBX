forward
global type w_informe_envio_carta_nf_comi_vigente from window
end type
type dw_parque from datawindow within w_informe_envio_carta_nf_comi_vigente
end type
type st_16 from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type hpb_1 from hprogressbar within w_informe_envio_carta_nf_comi_vigente
end type
type st_fondo from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type st_porc from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type pb_1 from picturebutton within w_informe_envio_carta_nf_comi_vigente
end type
type sle_1 from singlelineedit within w_informe_envio_carta_nf_comi_vigente
end type
type st_4 from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type sle_titulo from singlelineedit within w_informe_envio_carta_nf_comi_vigente
end type
type st_3 from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type cb_imprimir from commandbutton within w_informe_envio_carta_nf_comi_vigente
end type
type cb_exportar from commandbutton within w_informe_envio_carta_nf_comi_vigente
end type
type cb_filtrar from commandbutton within w_informe_envio_carta_nf_comi_vigente
end type
type pb_aceptar from picturebutton within w_informe_envio_carta_nf_comi_vigente
end type
type p_fin from picture within w_informe_envio_carta_nf_comi_vigente
end type
type em_fec_fin from editmask within w_informe_envio_carta_nf_comi_vigente
end type
type st_2 from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type p_ini from picture within w_informe_envio_carta_nf_comi_vigente
end type
type em_fec_ini from editmask within w_informe_envio_carta_nf_comi_vigente
end type
type st_1 from statictext within w_informe_envio_carta_nf_comi_vigente
end type
type cb_cerrar from commandbutton within w_informe_envio_carta_nf_comi_vigente
end type
type cb_ctacte from commandbutton within w_informe_envio_carta_nf_comi_vigente
end type
type dw_lista_detalle from datawindow within w_informe_envio_carta_nf_comi_vigente
end type
type gb_1 from groupbox within w_informe_envio_carta_nf_comi_vigente
end type
type dw_lista from datawindow within w_informe_envio_carta_nf_comi_vigente
end type
end forward
...
string title = "Informe Envio Carta Cobranza Comisiones Vigentes"
...
global w_informe_envio_carta_nf_comi_vigente w_informe_envio_carta_nf_comi_vigente

type variables
long	il_row
end variables

on w_informe_envio_carta_nf_comi_vigente.create
...
on w_informe_envio_carta_nf_comi_vigente.destroy
...
gf_centrar(w_informe_envio_carta_nf_comi_vigente)
...
dw_lista_detalle.retrieve(ld_fec_ini,ld_fec_fin,ll_cod_parque)
end event
...
Long		ll_tot_reg,ll_indi,ll_new,ll_count,ll_cod_accion,ll_tot_porc,ll_tot_porc_aux,&
			ll_cod_parque
...
if trim(em_fec_ini.text)='00/00/0000' or isnull(em_fec_ini.text) then
	...
	if trim(em_fec_fin.text)='00/00/0000' or isnull(em_fec_fin.text) then
		...
		if ll_cod_parque>0 then
			if dw_lista_detalle.retrieve(ld_fec_ini,ld_fec_fin,ll_cod_parque)=0 then
				messagebox("Advertencia","No registra dato")
			else
				dw_lista_detalle.accepttext()
				dw_lista.reset()
				dw_lista.object.t_4.text		= 'INFORME ENVIO CARTA COBRANZA COMISIONES VIGENTE'
				sle_titulo.text					= ''
				...
				for ll_indi=1 to ll_tot_reg
					...
					ll_numero				= dw_lista_detalle.getitemnumber(ll_indi,'cadena_numero')
					if not isnull(ls_cod_jefe) and not isnull(ls_cod_sup) and not isnull(ls_cod_age) and &
						...
						WHERE  ( "CARTA_LOG"."BASE" = :ls_base ) AND  
								 ...
								 ( "CARTA_LOG"."FECHA_CARTA" >= :ld_fec_ini ) AND  
								 ( "CARTA_LOG"."FECHA_CARTA" <= :ld_fec_Fin ) AND
									ROWNUM < 4  
						ORDER BY "CARTA_LOG"."FECHA_CARTA" DESC  
						USing		sqlca;
						open x1;
						if sqlca.sqlcode=0 then
							DO WHILE sqlca.sqlcode=0
								fetch x1 into :ls_cod_envio, :ls_estado_env, :ldt_fecha, :ll_cod_accion, :ls_clasif;
								ll_count ++
								if not isnull(ls_cod_envio) and ls_cod_envio<>'' and not isnull(ls_estado_env) and &
									ls_estado_env<>'' and not isnull(ldt_fecha) and ll_count=1 then
									dw_lista.setitem(ll_new,'cod_envio_1',ls_cod_envio)
									dw_lista.setitem(ll_new,'estado_env_1',ls_estado_env)
									dw_lista.setitem(ll_new,'fecha_carta_1',ldt_fecha)
									dw_lista.setitem(ll_new,'cod_accion_1',ll_cod_accion)
									dw_lista.setitem(ll_new,'clasif_1',ls_clasif)
								elseif not isnull(ls_cod_envio) and ls_cod_envio<>'' and not isnull(ls_estado_env) and &
									ls_estado_env<>'' and not isnull(ldt_fecha) and ll_count=2 then
									dw_lista.setitem(ll_new,'cod_envio_2',ls_cod_envio)
									dw_lista.setitem(ll_new,'estado_env_2',ls_estado_env)
									dw_lista.setitem(ll_new,'fecha_carta_2',ldt_fecha)
									dw_lista.setitem(ll_new,'cod_accion_2',ll_cod_accion)
									dw_lista.setitem(ll_new,'clasif_2',ls_clasif)
								elseif not isnull(ls_cod_envio) and ls_cod_envio<>'' and not isnull(ls_estado_env) and &
									ls_estado_env<>'' and not isnull(ldt_fecha) and ll_count=3 then
									dw_lista.setitem(ll_new,'cod_envio_3',ls_cod_envio)
									dw_lista.setitem(ll_new,'estado_env_3',ls_estado_env)
									dw_lista.setitem(ll_new,'fecha_carta_3',ldt_fecha)
									dw_lista.setitem(ll_new,'cod_accion_3',ll_cod_accion)
									dw_lista.setitem(ll_new,'clasif_3',ls_clasif)
								elseif ll_count>3 then
									exit
								end if
								setnull(ls_cod_envio);setnull(ls_estado_env);setnull(ldt_fecha);setnull(ll_cod_accion)
								setnull(ls_clasif)
							LOOP
							setnull(ls_cod_envio);setnull(ls_estado_env);setnull(ldt_fecha);setnull(ll_cod_accion)
							setnull(ls_clasif)
							if isnull(dw_lista.getitemstring(ll_new,'cod_envio_1')) then
								dw_lista.setitem(ll_new,'cod_envio_1',ls_cod_envio)
								dw_lista.setitem(ll_new,'estado_env_1',ls_estado_env)
								dw_lista.setitem(ll_new,'fecha_carta_1',ldt_fecha)
								dw_lista.setitem(ll_new,'cod_accion_1',ll_cod_accion)
							...
							if isnull(dw_lista.getitemstring(ll_new,'cod_envio_2')) then
								dw_lista.setitem(ll_new,'cod_envio_2',ls_cod_envio)
								dw_lista.setitem(ll_new,'estado_env_2',ls_estado_env)
								dw_lista.setitem(ll_new,'fecha_carta_2',ldt_fecha)
								dw_lista.setitem(ll_new,'cod_accion_2',ll_cod_accion)
							...
							if isnull(dw_lista.getitemstring(ll_new,'cod_envio_3')) then
								dw_lista.setitem(ll_new,'cod_envio_3',ls_cod_envio)
								dw_lista.setitem(ll_new,'estado_env_3',ls_estado_env)
								dw_lista.setitem(ll_new,'fecha_carta_3',ldt_fecha)
								dw_lista.setitem(ll_new,'cod_accion_3',ll_cod_accion)
...
event clicked;close(w_informe_envio_carta_nf_comi_vigente)
end event
...
integer height = 500
string title = "none"
string dataobject = "dw_informe_envio_carta_nf_comi_vigente"
...
integer taborder = 60
string title = "none"
string dataobject = "dwe_envio_carta_cob_nf_comi_vgte"
boolean hscrollbar = true
boolean vscrollbar = true
