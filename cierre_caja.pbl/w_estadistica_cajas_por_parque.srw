forward
global type w_estadistica_cajas_por_parque from window
end type
type dw_contar_ser from datawindow within w_estadistica_cajas_por_parque
end type
type dw_contar_comp_ser from datawindow within w_estadistica_cajas_por_parque
end type
type dw_contar_comp from datawindow within w_estadistica_cajas_por_parque
end type
type st_16 from statictext within w_estadistica_cajas_por_parque
end type
type cb_exportar from commandbutton within w_estadistica_cajas_por_parque
end type
type p_hasta from picture within w_estadistica_cajas_por_parque
end type
type em_hasta from editmask within w_estadistica_cajas_por_parque
end type
type st_11 from statictext within w_estadistica_cajas_por_parque
end type
type cb_ver_detalle from commandbutton within w_estadistica_cajas_por_parque
end type
type cb_grafico from commandbutton within w_estadistica_cajas_por_parque
end type
type cb_imprimir from commandbutton within w_estadistica_cajas_por_parque
end type
type cb_aceptar from commandbutton within w_estadistica_cajas_por_parque
end type
type p_desde from picture within w_estadistica_cajas_por_parque
end type
type em_de from editmask within w_estadistica_cajas_por_parque
end type
type st_1 from statictext within w_estadistica_cajas_por_parque
end type
type cb_cerrar from commandbutton within w_estadistica_cajas_por_parque
end type
type dw_lista from datawindow within w_estadistica_cajas_por_parque
end type
type dw_contar from datawindow within w_estadistica_cajas_por_parque
end type
type dw_parque from datawindow within w_estadistica_cajas_por_parque
end type
type gb_1 from groupbox within w_estadistica_cajas_por_parque
end type
end forward
...
string title = "Estadistica Cajas por Parque"
boolean controlmenu = true
boolean minbox = true
long backcolor = 67108864
dw_contar_ser dw_contar_ser
dw_contar_comp_ser dw_contar_comp_ser
dw_contar_comp dw_contar_comp
st_16 st_16
...
global w_estadistica_cajas_por_parque w_estadistica_cajas_por_parque

type variables
datawindow dw_paso
end variables

on w_estadistica_cajas_por_parque.create
this.dw_contar_ser=create dw_contar_ser
this.dw_contar_comp_ser=create dw_contar_comp_ser
this.dw_contar_comp=create dw_contar_comp
...
this.Control[]={this.dw_contar_ser,&
this.dw_contar_comp_ser,&
this.dw_contar_comp,&
this.st_16,&
this.cb_exportar,&
this.p_hasta,&
...
on w_estadistica_cajas_por_parque.destroy
destroy(this.dw_contar_ser)
destroy(this.dw_contar_comp_ser)
destroy(this.dw_contar_comp)
destroy(this.st_16)
...
String	ls_parque
gf_centrar(w_estadistica_cajas_por_parque)
dw_lista.settransobject(sqlca)
dw_contar.settransobject(sqlca)
dw_contar_comp.settransobject(sqlca)
dw_contar_ser.settransobject(sqlca)
dw_contar_comp_ser.settransobject(sqlca)
...
integer height = 432
integer taborder = 90
string dataobject = "dw_agrupar_ingreso_por_parque_ser"
...
string dataobject = "dw_agrupar_ingreso_por_parque_comp_ser"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
string dataobject = "dw_agrupar_ingreso_por_parque_comp"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
if dw_lista.getrow()>0 and dw_lista.object.gr_grafico.Visible = '0' then
	...
	if not isnull(ls_caja) and not isnull(date(ls_fecha)) then
		ls_string	= ls_caja+'~t'+ls_fecha+'~t'+ls_fecha_fin+'~t'+string(ll_cod_parque)
		openwithparm(w_detalle_ingresos_por_caja_fecha_parque,ls_string)
	end if
end if
end event
...
event clicked;Date		ld_fecha,ld_fecha_hasta
Long		ll_cod_parque,ll_new,ll_monto_adm,ll_monto_inm,ll_cant_doc_inm,ll_cant_doc_inm2,&
			ll_cant_doc_adm,ll_monto_adm_nb,ll_monto_inm_nc,ll_total
String	ls_filtro,ls_caja,ls_parque
...
elseif ll_cod_parque=0 or isnull(ll_cod_parque) then
	...
	dw_lista.reset()
	ls_filtro							= 'cadena_cod_parque ='+string(ll_cod_parque) +' or cd_folio_cod_parque='+string(ll_cod_parque)
	SELECT	"COD_PARQ"."NOMBRE"  
	INTO 		:ls_parque  
	FROM 	"COD_PARQ"  
	WHERE 	"COD_PARQ"."CODIGO" = :ll_cod_parque   ;
		...
		if sqlca.sqlcode=0 then
			DO WHILE sqlca.sqlcode=0
				...
				if not isnull(ls_caja) then
					...
					SELECT DISTINCT	COUNT("INGRESO"."FOLIO") 
					INTO	:ll_cant_doc_inm
					FROM 	"INGRESO",   "CADENA",   "CLIENTE_REZAGO",   "CD_FOLIO"  
					WHERE	( ingreso.base = cadena.codigo (+)) and  
							...
							( "INGRESO"."TIPO_COB" = 'CU' OR
							  ...
							  "INGRESO"."TIPO_COB" = 'N1') AND 
							( "CADENA"."COD_PARQUE" = :ll_cod_parque )    ;
							  
					if isnull(ll_cant_doc_inm) then ll_cant_doc_inm=0
					dw_lista.setitem(ll_new,'cant_doc_inm',ll_cant_doc_inm)
					SELECT DISTINCT	COUNT("INGRESO"."FOLIO") 
					INTO	:ll_cant_doc_adm
					FROM 	"INGRESO",   "CADENA",   "CLIENTE_REZAGO",   "CD_FOLIO"  
					WHERE	( ingreso.base = cadena.codigo (+)) and  
							( ingreso.serie = cadena.serie (+)) and  
							( ingreso.contrato = cadena.numero (+)) and  
							...
							( "INGRESO"."TIPO_COB" = 'BL' OR 
							  ...
							  "INGRESO"."TIPO_COB" = 'B0' OR 
							  "INGRESO"."TIPO_COB" = 'BM' OR 
							  "INGRESO"."TIPO_COB" = 'NB') AND 
							( "CADENA"."COD_PARQUE" = :ll_cod_parque )    ;
...
//					if ls_caja<>'XY' and ls_caja<>'XX' then
...
//								  "INGRESO"."PAGO_HIST" = 'A' AND  
//								  "INGRESO"."COD_CAJA" = :ls_caja ) AND  
//								( "INGRESO"."FECHA_PAGO" = "INGRESO"."SERVIPAG" ) AND
//								( "INGRESO"."TIPO_COB" = 'CU' OR 
//								  "INGRESO"."TIPO_COB" = 'CC' OR 
//								  "INGRESO"."TIPO_COB" = 'CI' OR 
//								  "INGRESO"."TIPO_COB" = 'C0' OR 
...
event clicked;close(w_estadistica_cajas_por_parque)
end event
...
integer taborder = 50
string title = "none"
string dataobject = "dwe_estadistica_ingreso_cajas_2"
...
event doubleclicked;string	ls_columna,ls_fecha,ls_fecha_fin,ls_string,ls_caja
Long		ll_cod_parque
...
if row>0 and dw_lista.object.gr_grafico.Visible = '0' then
	...
	if ls_columna='suma_doc_adm' or ls_columna='suma_doc_inm' or ls_columna='suma_doc' or ls_columna='suma_pago_adm' or ls_columna='suma_pago_inm' or ls_columna='suma_pago' then
		if dw_lista.getrow()>0 and dw_lista.object.gr_grafico.Visible = '0' then
			...
			ll_cod_parque	= dw_parque.getitemnumber(1,'cod_parque')
			if not isnull(ls_caja) and not isnull(date(ls_fecha)) then
				ls_string	= 'TODAS'+'~t'+ls_fecha+'~t'+ls_fecha_fin+'~t'+string(ll_cod_parque)
				openwithparm(w_detalle_ingresos_por_caja_fecha_parque,ls_string)
			end if
		end if
	end if
...
integer width = 411
integer height = 432
string dataobject = "dw_agrupar_ingreso_por_parque"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
