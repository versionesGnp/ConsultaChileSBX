forward
global type w_exportar_archivo_contabilidad from window
end type
type lb_aux from listbox within w_exportar_archivo_contabilidad
end type
type rb_dbf3 from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_dbf2 from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_dif from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_wk1 from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_wks from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_sylk from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_sql from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_texto from radiobutton within w_exportar_archivo_contabilidad
end type
type rb_excel from radiobutton within w_exportar_archivo_contabilidad
end type
type cbx_encabezado from checkbox within w_exportar_archivo_contabilidad
end type
type st_directorio from statictext within w_exportar_archivo_contabilidad
end type
type cb_cancelar from commandbutton within w_exportar_archivo_contabilidad
end type
type cb_aceptar from commandbutton within w_exportar_archivo_contabilidad
end type
type sle_nombre from singlelineedit within w_exportar_archivo_contabilidad
end type
type st_1 from statictext within w_exportar_archivo_contabilidad
end type
type gb_formato from groupbox within w_exportar_archivo_contabilidad
end type
end forward
...
global w_exportar_archivo_contabilidad w_exportar_archivo_contabilidad
...
if isvalid(w_generar_archivo_contab) then
	...
	ll_row			= w_generar_archivo_contab.dw_lista.rowcount()
	...
	INSERT INTO "CAJA_GENERA_ARCHIVO"  
		( "FECHA_CREA",   "CORRELATIVO",   "FECHA_GENERADA",   "NOMBRE_ARCHIVO",   "CAJA",   "EMPRESA",   "CANTIDAD_REGISTRO",	"USUARIO")  
	VALUES 
		( :ld_fecha_hoy,	:ll_count,   		:ld_fecha,   		 :archivo,   			:ls_caja, :ls_empresa, :ll_row ,					:gs_user)  
	Using		sqlca;
	if sqlca.sqlcode=0 then
		commit;
	else
	...
	w_generar_archivo_contab.cb_limpiar.triggerevent(clicked!)
end if
end subroutine

event open;string 	fecha,ls_parque
Long		ll_pos
gf_centrar(w_exportar_archivo_contabilidad)
...
TipoArchivo 					= XLSX!
if isvalid(w_generar_archivo_contab) then
	ls_parque					= trim(w_generar_archivo_contab.ddlb_parque.text)
	...
	if gs_empresa='El Prado' then
		is_fecha			= mid(w_generar_archivo_contab.em_de.text,1,2)+mid(w_generar_archivo_contab.em_de.text,4,2)+mid(w_generar_archivo_contab.em_de.text,7,4)+w_generar_archivo_contab.ddlb_caja.text+mid(w_generar_archivo_contab.ddlb_empresa.text,1,1)+ls_parque
	else
		is_fecha			= mid(w_generar_archivo_contab.em_de.text,1,2)+mid(w_generar_archivo_contab.em_de.text,4,2)+mid(w_generar_archivo_contab.em_de.text,7,4)+w_generar_archivo_contab.ddlb_caja.text+mid(w_generar_archivo_contab.ddlb_empresa.text,1,1)+ls_parque
	end if
...
on w_exportar_archivo_contabilidad.create
...
on w_exportar_archivo_contabilidad.destroy
destroy(this.lb_aux)
...
event clicked;CloseWithReturn(Parent, "cancelado")
