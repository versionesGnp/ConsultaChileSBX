forward
global type w_envio_emails_bkp from window
end type
type cb_1 from commandbutton within w_envio_emails_bkp
end type
type cb_3 from commandbutton within w_envio_emails_bkp
end type
type cb_imprimir from commandbutton within w_envio_emails_bkp
end type
type cb_limpiar from commandbutton within w_envio_emails_bkp
end type
type st_cuenta from statictext within w_envio_emails_bkp
end type
type st_porc from statictext within w_envio_emails_bkp
end type
type hpb_1 from hprogressbar within w_envio_emails_bkp
end type
type dw_importar from datawindow within w_envio_emails_bkp
end type
type cb_ortografia from commandbutton within w_envio_emails_bkp
end type
type cb_cerrar from commandbutton within w_envio_emails_bkp
end type
type cb_enviar from commandbutton within w_envio_emails_bkp
end type
type cb_buscar from commandbutton within w_envio_emails_bkp
end type
type st_fondo from statictext within w_envio_emails_bkp
end type
type gb_1 from groupbox within w_envio_emails_bkp
end type
type dw_detalle from datawindow within w_envio_emails_bkp
end type
type dw_grupo from datawindow within w_envio_emails_bkp
end type
end forward
...
global w_envio_emails_bkp w_envio_emails_bkp

type variables
string named
end variables

on w_envio_emails_bkp.create
...
WHERE		"SOL_PERMISOS_USUARIO"."CODIGO_SOLICITUD" = 'ML' and
...
if sqlca.sqlcode=0 then
   ...
   USING		sqlca;
	if sqlca.sqlcode=0 then
		...
		dw_grupo.setitem(1,'tipo_email','CO')
		dw_grupo.accepttext()
...
////cb_ortografia.enabled	= false
//dw_grupo.setfocus()
w_envio_emails.triggerevent(open!)
...
event clicked;	close(w_envio_emails)
end event
...
event clicked;string	ls_res,ls_email_de,ls_asunto,ls_cuerpo_email,ls_email_para,ls_remit,ls_prioridad,ls_gestion,&
		...
		ls_base_adi,ls_glosa_adi
long		ll_tot_reg,ll_indi,ll_graba,ll_cod_parq,ll_regis,ll_grupo,ll_fila,ll_numero,ll_rut,ll_mora_cred,ll_corr,ll_mora_mant,&
			ls_serie_adi,ll_numero_adi
datetime	ldt_fech_crea,ldt_fecha_gestion
time		ld_hora
			...
			for ll_indi=1 to ll_tot_reg
               ...
               USING 	sqlca;
					...
					if ls_email_para<>'' and ls_email_para<>'-' and ls_email_para<>'NO TIENE' then
						SELECT	F_EnviarMail2(:ll_cod_parq,:ls_tipo_email,:ls_email_de,:ls_email_para,:ls_asunto,:ls_cuerpo_email,:ls_parrafo_2,:ls_parrafo_3,:ls_parrafo_4,:ll_numero,:ls_nomb,:ls_a_pater,:ls_a_mater) 
						INTO 		:ls_res
						FROM 		DUAL;
						if ls_res='OK' then
							ll_graba++
							...
							if sqlca.sqlcode=0 then
								...
								VALUES 	 ( :ll_rut,  	'2',		 :ldt_fech_crea, 'Se Envia E-mail',  :gs_user, 	:ls_base, :ls_serie, :ll_numero )  
								USING	 sqlca;
								if sqlca.sqlcode=0 then
									commit;
									UPDATE	"CLIENTE"  
										SET 	"ESTADO_COBRANZA" = '2',   
												"FECHA_COBRANZA"  = :ldt_fech_crea,   
												"GLOSA_COBRANZA"  = 'Se Envia E-mail',  
												"USUARIO_COB_TELE" = :gs_user, 
												"PROX_LLAMADO_COB" = :ldt_fech_crea + 10,
												"FECHA_PROX_LLAMADO" = :ldt_fech_crea + 10,
												"ASIGNADO_COBRANZA" = 'N'
									...
									if sqlca.sqlcode=0 then
										...
										DO WHILE Trans_2.sqlcode=0
											...
											if ll_numero_adi > 0 and not isnull(ll_numero_adi) then
												...
												ls_glosa_adi	= 'Se Envia E-mail '+' (CTTO. ORIGINAL '+gs_base+gs_serie+string(gi_numero,'###,###,###,###,###')+')'
												INSERT INTO "TELE_LOG"  
												( "RUT"  ,"ESTADO"  ,"FECHA"  ,	"GLOSA","USUARIO","BASE"  ,"SERIE"  ,"NUMERO" )  
												VALUES ( :ll_rut,'2',:ldt_fech_crea ,:ls_glosa_adi,	:gs_user,:ls_base_adi,:ls_serie_adi,:ll_numero_adi)  
												USING		Trans_1;
									...
									rollback;
									messagebox("Error Grabar","Error Grabar TELE_LOG SQL: "+sqlca.sqlerrtext)
								end if
							else
								rollback;
...
long		ll_cod_parq,ll_tot_reg,ll_indi,ll_tot_reg2,ll_numero,ll_max_grupo,ll_rut,ll_new,ll_grupo
...
integer taborder = 10
string title = "none"
string dataobject = "dw_grupo_email"
