forward
global type w_beneficios_ingreso from window
end type
type cb_2 from commandbutton within w_beneficios_ingreso
end type
type cb_1 from commandbutton within w_beneficios_ingreso
end type
type st_monto_pagado from statictext within w_beneficios_ingreso
end type
type sle_pie from statictext within w_beneficios_ingreso
end type
type sle_moneda from statictext within w_beneficios_ingreso
end type
type sle_numero from statictext within w_beneficios_ingreso
end type
type sle_serie from statictext within w_beneficios_ingreso
end type
type sle_base from statictext within w_beneficios_ingreso
end type
type st_3 from statictext within w_beneficios_ingreso
end type
type dw_ingreso from datawindow within w_beneficios_ingreso
end type
type dw_beneficios_seleccionados from datawindow within w_beneficios_ingreso
end type
type st_pie from statictext within w_beneficios_ingreso
end type
type st_1 from statictext within w_beneficios_ingreso
end type
type cb_grabar from commandbutton within w_beneficios_ingreso
end type
type cb_cerrar from commandbutton within w_beneficios_ingreso
end type
type dw_lista_beneficios from datawindow within w_beneficios_ingreso
end type
type gb_1 from groupbox within w_beneficios_ingreso
end type
end forward
...
global w_beneficios_ingreso w_beneficios_ingreso
...
forward prototypes
public subroutine wf_cargar_lista (integer al_cod_benef, integer al_cod_benef_det, integer al_est_reg, integer al_row)
end prototypes

public subroutine wf_cargar_lista (integer al_cod_benef, integer al_cod_benef_det, integer al_est_reg, integer al_row);string	ls_columna,ls_descrip
...
dw_ingreso.setitem(ll_new,'correlativo',al_row)
...
on w_beneficios_ingreso.create
...
on w_beneficios_ingreso.destroy
destroy(this.cb_2)
destroy(this.cb_1)
...
long		ll_rut,ll_indi,ll_tot_reg,ll_cod,ll_cod_det,ll_fila,ll_codigo,ll_folio,ll_corr,ll_new,ll_cod_benef,ll_cod_benef_d,ll_parque,ll_regis,ll_sum_monto,&
			ll_tot_reg_ben,ll_corre,ll_cod_ben,ll_cod_ben_det,ll_count_filas,ll_fila_activo,ll_cod_benef_ofv,ll_ret,ll_ret2,ll_pie_pac,&
...
if ll_tot_reg > 0 then
	...
	ll_cod_ori			= dw_ingreso.getitemnumber(ll_tot_reg,'cod_beneficio')
...
if ll_tot_reg = 0 then
			...
			if ll_tot_reg > 0 then
				for ll_indi=1 to ll_tot_reg
					...
					if ll_regis	= 0 then
						...
						if ll_fila > 0 then
						//	dw_ingreso.setitem(ll_fila,'estado','P') no descomentariar
						else
						//	ll_corr			= ll_corr + 1
							ll_new			= dw_ingreso.insertrow(0)
							dw_ingreso.setitem(ll_new,'correlativo',ll_new)
	...
	//		next
			...
			messagebox('Advertencia','No existe Pago')
	...
	elseif ll_codigo = 6 then
		...
		for	ll_indi=1 to ll_tot_reg
			...
			if ll_est_reg=0 then
				wf_cargar_lista(ll_cod_benef,ll_cod_benef_det,ll_est_reg,ll_indi)
			end if
			setnull(ll_est_reg);
		next
		ll_fila_activo					= dw_lista_beneficios.Find("estado_reg = 0",1,ll_tot_reg)
...
//		next
	...
	next
	messagebox('Advertencia','Contrato Con Beneficios Ingresados')
	dw_lista_beneficios.accepttext()	
end if
end event
...
if ll_tot_reg > 0 then
	...
	//			ll_corr				= il_corr_ini + il_count
				...
				dw_ingreso.setitem(ll_row,'base',is_base)
...
if is_modif = 'S' then
	ll_res 	= messagebox('Aviso','Beneficios Sin Guardar ¿Esta Seguro de Salir?',Question!,YesNo!,2)
	if ll_res = 1 then
		close(w_beneficios_ingreso)
	end if	
else
...
long	ll_cod_benef,ll_cod_benef_det,ll_tot_reg,ll_indi,ll_est_reg,ll_estado,ll_fila
...
if row > 0 then
	...
	if ls_columna = 'estado_reg' then
		...
		if ll_tot_reg>0 then
			ll_cod_benef					= dw_lista_beneficios.getitemnumber(row,'beneficios_complementario_codigo')
			ll_cod_benef_det				= dw_lista_beneficios.getitemnumber(row,'beneficios_complementario_det_codigo_beneficio')
			ll_est_reg						= dw_lista_beneficios.getitemnumber(row,'estado_reg')
			dw_lista_beneficios.accepttext()
			if ll_est_reg=0 then
				ll_est_reg				= 1
			else
			...
			if ll_est_reg=0 then
				wf_cargar_lista(ll_cod_benef,ll_cod_benef_det,ll_est_reg,row)
			elseif ll_est_reg = 1 then
				ll_fila						= dw_ingreso.Find("cod_beneficio = "+string(ll_cod_benef) +" and cod_benef_detalle = "+string(ll_cod_benef_det),1,ll_tot_reg)
				if ll_fila >0 then
					dw_ingreso.deleterow(ll_fila)
				end if
			end if
