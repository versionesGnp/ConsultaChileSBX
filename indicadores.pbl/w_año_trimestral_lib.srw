forward
global type w_año_trimestral_lib from window
end type
type ddlb_2 from dropdownlistbox within w_año_trimestral_lib
end type
type st_2 from statictext within w_año_trimestral_lib
end type
type rb_mora from radiobutton within w_año_trimestral_lib
end type
type ddlb_1 from dropdownlistbox within w_año_trimestral_lib
end type
type rb_facturacion_anual from radiobutton within w_año_trimestral_lib
end type
type cb_2 from commandbutton within w_año_trimestral_lib
end type
type cb_1 from commandbutton within w_año_trimestral_lib
end type
type gb_1 from groupbox within w_año_trimestral_lib
end type
end forward
...
global w_año_trimestral_lib w_año_trimestral_lib

type variables
date fecha
end variables

on w_año_trimestral_lib.create
...
on w_año_trimestral_lib.destroy
destroy(this.ddlb_2)
destroy(this.st_2)
...
event clicked;close(w_año_trimestral_lib)
end event
...
if trim(ddlb_2.text) = 'Primer' then
	...
	for i =1 to ddlb_1.TotalItems( )
		...
		'    FROM FACTURA_OFERTA  '+&
		"   WHERE ( FACTURA_OFERTA.FECHA_CIERRE >= TO_DATE('01-Jan-"+trim(ddlb_1.text(i))+" 00:00:00','DD-MON-YYYY HH24:MI:SS') ) AND  "+&
...
elseif trim(ddlb_2.text) = 'Segundo' then
	...
	for i =1 to ddlb_1.TotalItems( )
		...
		'         FACTURA_OFERTA.ESTADO  '+&
		'    FROM FACTURA_OFERTA  '+&
		"   WHERE ( FACTURA_OFERTA.FECHA_CIERRE >= TO_DATE('01-Apr-"+trim(ddlb_1.text(i))+" 00:00:00','DD-MON-YYYY HH24:MI:SS') ) AND  "+&
...
elseif trim(ddlb_2.text) = 'Tercer' then
	...
	for i =1 to ddlb_1.TotalItems( )
		...
		'         FACTURA_OFERTA.ESTADO  '+&
		'    FROM FACTURA_OFERTA  '+&
		"   WHERE ( FACTURA_OFERTA.FECHA_CIERRE >= TO_DATE('01-Jul-"+trim(ddlb_1.text(i))+" 00:00:00','DD-MON-YYYY HH24:MI:SS') ) AND  "+&
...
elseif trim(ddlb_2.text) = 'Cuarto' then
	...
	for i =1 to ddlb_1.TotalItems( )
		...
		'         FACTURA_OFERTA.ESTADO  '+&
		'    FROM FACTURA_OFERTA  '+&
		"   WHERE ( FACTURA_OFERTA.FECHA_CIERRE >= TO_DATE('01-Oct-"+trim(ddlb_1.text(i))+" 00:00:00','DD-MON-YYYY HH24:MI:SS') ) AND  "+&
...
OpenWithParm(w_estadisticas_fact_total_trimestral_lib,param)	
