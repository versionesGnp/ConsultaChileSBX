forward
global type w_persistencia_detalle_bk from window
end type
type cb_exportar from commandbutton within w_persistencia_detalle_bk
end type
type cb_respaldo from commandbutton within w_persistencia_detalle_bk
end type
type st_cuenta from statictext within w_persistencia_detalle_bk
end type
type hpb_1 from hprogressbar within w_persistencia_detalle_bk
end type
type st_porc from statictext within w_persistencia_detalle_bk
end type
type em_cargo from editmask within w_persistencia_detalle_bk
end type
type em_codigo from editmask within w_persistencia_detalle_bk
end type
type em_fecha_cierre from editmask within w_persistencia_detalle_bk
end type
type em_fin from editmask within w_persistencia_detalle_bk
end type
type em_ini from editmask within w_persistencia_detalle_bk
end type
type cb_proceso from commandbutton within w_persistencia_detalle_bk
end type
type cb_grabar from commandbutton within w_persistencia_detalle_bk
end type
type cb_asocia_contrato from commandbutton within w_persistencia_detalle_bk
end type
type cb_cta_cte from commandbutton within w_persistencia_detalle_bk
end type
type cb_ordenar from commandbutton within w_persistencia_detalle_bk
end type
type cb_filtrar from commandbutton within w_persistencia_detalle_bk
end type
type cb_imprimir from commandbutton within w_persistencia_detalle_bk
end type
type cb_cerrar from commandbutton within w_persistencia_detalle_bk
end type
type st_fondo from statictext within w_persistencia_detalle_bk
end type
type dw_detalle from datawindow within w_persistencia_detalle_bk
end type
end forward
...
global w_persistencia_detalle_bk w_persistencia_detalle_bk

type variables
long		il_row
end variables

event ue_calcular();string		ls_codigo,ls_base,ls_serie,ls_estado,ls_cardo,ls_base_consul,ls_serie_consul,ls_clasif,ls_cod_age_sup,&
			ls_repacta
long		ll_new,ll_cta_pag,ll_plazo,ll_cod_parque,ll_sum_pag_post,ll_ctas_pag_1,ll_dias_mora,&
			...
			ll_antiguedad,ll_cta_contr,ll_tot_reg,ll_repacta,ll_vtas_fuera,ll_correl,ll_repac,ll_licen
datetime	ldt_fec_cierre,ldt_fecha_prim,ldt_fecha_fact,ldt_fecha_venc,ldt_fec_ini,ldt_fec_fin,ldt_nulo,&
			ldt_fecha_res,ldt_fecha_ult_pago,ldt_fecha_ing,ldt_fecha_est,ldt_fec_repac,ldt_fec_prim_nue
...
ldt_fec_cierre																			= datetime(date(em_fecha_cierre.text),time('00:00:00'))
...
if ldt_fec_cierre < datetime(date(string('28/02/2011'))) then
	messagebox("Advertencia","No Registra Detalle")
else
	SELECT to_date(add_months( to_date(:ldt_fec_cierre), -11 )) INTO :ldt_fec_ini FROM dual;
	SELECT to_date(add_months( to_date(:ldt_fec_cierre), -3 )) INTO :ldt_fec_fin FROM dual;
	...
	if ll_tot_reg=0 then
		...
		if ll_licen>90 then
			...
			dw_detalle.setitem(1,'cargo',ls_cardo)
			dw_detalle.setitem(1,'fecha_cierre',ldt_fec_cierre)
			dw_detalle.setitem(1,'codigo',ls_codigo)
			dw_detalle.setitem(1,'clasifica',ls_clasif)
			dw_detalle.setitem(1,'licen_medica','C')
			dw_detalle.setitem(1,'total_ventas',0)
			dw_detalle.setitem(1,'total_mora',0)
			dw_detalle.setitem(1,'porce_mora',0)
			dw_detalle.setitem(1,'porce_persist',100 - 0)
		...
		if ls_cardo = 'A' then
			...
			WHERE	"VISTA_PERSISTENCIA"."FECHA_CIERRE" >= :ldt_fec_ini AND  
						"VISTA_PERSISTENCIA"."FECHA_CIERRE" <= :ldt_fec_fin AND
						"VISTA_PERSISTENCIA"."SERIE" <> 'R' AND 
						//( "VISTA_PERSISTENCIA"."NUMERO" = 155361 AND
						"VISTA_PERSISTENCIA"."COD_AGE" = :ls_codigo
			USING	Trans_4;			
		elseif ls_cardo = 'S' then
		...
		if Trans_4.sqlcode=0 then
			DO WHILE Trans_4.sqlcode=0
				...
				if not isnull(ls_base) and ls_base <> '' and not isnull(ls_serie) and ls_serie <> '' and not isnull(ldb_numero) and ldb_numero > 0 then
					if ls_estado= 'P' then
						if ldt_fec_cierre < ldt_fecha_res then
							...
							WHERE 	"FACTURA_OFERTA"."BASE" = :ls_base_consul AND  
										...
										"FACTURA_OFERTA"."FECHA_CIERRE" < :ldt_fec_cierre AND  
										"FACTURA_OFERTA"."ESTADO" = 'S'
							USING	sqlca;
							if isnull(ldt_fecha_est) then ldt_fecha_est= ldt_nulo
					...
					WHERE	"CADENA"."CODIGO" = "INGRESO"."BASE"and  
								...
								"INGRESO"."FECHA_PAGO" > :ldt_fec_cierre and
								"INGRESO"."FECHA_PAGO" <= :gdt_fec_sistema
					USING	sqlca;
					if isnull(ll_sum_pag_post) then ll_sum_pag_post=0
...
/*REPACTA*/	SELECT	MAX("SOL_REPACTA_CVTA"."CORRELATIVO")
					...
					WHERE	"SOL_REPACTA_CVTA"."BASE" = :ls_base_consul AND   
								...
								"SOL_REPACTA_CVTA"."NUMERO" = :ll_numero_consul 
					USING	sqlca;
					if isnull(ll_repac) then ll_repac=0
					...
					if ll_repac > 0 then
						...
						WHERE	"SOL_REPACTA_CVTA"."BASE" = :ls_base_consul AND   
									...
									"SOL_REPACTA_CVTA"."FECHA_SOLICITUD" <= :ldt_fec_cierre
						USING	sqlca;
						if isnull(ll_correl) then ll_correl=0
						if ll_correl>0 then
							if ll_correl>0 then
								SELECT	"FECHA_SOLICITUD", 	"FECREP_1_VCTO"
								INTO		:ldt_fec_repac,		:ldt_fecha_prim
								FROM 	"SOL_REPACTA_CVTA"
								WHERE	"SOL_REPACTA_CVTA"."BASE" = :ls_base_consul AND   
											"SOL_REPACTA_CVTA"."SERIE" = :ls_serie_consul AND   
											"SOL_REPACTA_CVTA"."NUMERO" = :ll_numero_consul AND
								...
								USING	sqlca;
								if not isnull(ldt_fec_repac) and not isnull(ldt_fecha_prim) then
									ls_repacta									= 'Repactado'
									ldt_fecha_prim								= ldt_fecha_prim
								end if
							else
								SELECT	"FECHA_SOLICITUD", 	"FECVTA_1_VCTO",	"FECREP_1_VCTO"
								INTO		:ldt_fec_repac,				:ldt_fecha_prim,		:ldt_fec_prim_nue 
								FROM 	"SOL_REPACTA_CVTA"
								WHERE	"SOL_REPACTA_CVTA"."BASE" = :ls_base_consul AND   
											"SOL_REPACTA_CVTA"."SERIE" = :ls_serie_consul AND   
											"SOL_REPACTA_CVTA"."NUMERO" = :ll_numero_consul AND
								...
								USING	sqlca;
								if not isnull(ldt_fec_repac) and not isnull(ldt_fecha_prim) then
									if ldt_fec_repac>ldt_fec_cierre then
										ls_repacta								= ''
										ldt_fecha_prim							= ldt_fecha_prim
									elseif ldt_fec_repac<ldt_fec_cierre then
										ls_repacta								= ''
										ldt_fecha_prim							= ldt_fec_prim_nue
									end if
								end if
							end if
						else
					...
					SELECT	to_date(add_months( to_date(:ldt_fecha_prim), :ll_ctas_pag_1 )) INTO :ldt_fecha_venc FROM dual;
					ll_dias_mora														= DaysAfter(date(ldt_fecha_venc), date(ldt_fec_cierre))
					...
					USING	sqlca;
					if isnull(ll_vtas_fuera) then ll_vtas_fuera=0
					...
					if ll_dias_mora <= 5 or ls_estado='S' or ll_vtas_fuera>0 then
						...
						ll_dia_01															= day(date(ldt_fec_cierre))
						ll_mes_01														= month(date(ldt_fec_cierre))
						ll_ano_01														= year(date(ldt_fec_cierre))
					...
					ll_tot_venta															= 0
					ll_tot_mora															= 0
					if ldt_fec_cierre <= datetime(date(string('31/08/2011'))) then
						if ls_estado <> 'P' then 
							ll_tot_venta													= 1
					...
					dw_detalle.setitem(ll_new,"fecha_cierre",ldt_fec_cierre)	
					dw_detalle.setitem(ll_new,"fecha_ini",ldt_fec_ini)	
					dw_detalle.setitem(ll_new,"fecha_fin",ldt_fec_fin)
					...
					ll_antiguedad 																= DaysAfter(date(ldt_fecha_ing), date(ldt_fec_cierre))
					...
					dw_detalle.setitem(ll_new,'licen_medica',ll_licen)
					dw_detalle.AcceptText()
				end if
...
on w_persistencia_detalle_bk.create
...
on w_persistencia_detalle_bk.destroy
...
event open;string	ls_codigo,ls_cardo
...
ldt_fecha_cierre																	= datetime(ld_fecha,time('00:00:00'))
ls_cardo																				= substr(1,3,Message.StringParm)
dw_detalle.dataobject															= 'dwe_persistencia_detalle'
dw_detalle.settransobject(sqlca)

em_codigo.text																		= ls_codigo
em_fecha_cierre.text																= string(ld_fecha)
em_cargo.text																		= ls_cardo
...
integer height = 1860
integer taborder = 10
string dataobject = "dwe_persistencia_detalle"
