forward
global type w_detalle_agrupacion_por_fecha_cobranza from window
end type
type cb_ordenar from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type cb_filtrar from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type cb_exportar from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type cb_imprimir from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type cb_ctacte from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type dw_lista from datawindow within w_detalle_agrupacion_por_fecha_cobranza
end type
type cb_cerrar from commandbutton within w_detalle_agrupacion_por_fecha_cobranza
end type
type gb_1 from groupbox within w_detalle_agrupacion_por_fecha_cobranza
end type
end forward
...
global w_detalle_agrupacion_por_fecha_cobranza w_detalle_agrupacion_por_fecha_cobranza

type variables
Long	il_row,il_parque
end variables

on w_detalle_agrupacion_por_fecha_cobranza.create
...
on w_detalle_agrupacion_por_fecha_cobranza.destroy
...
gf_centrar(w_detalle_agrupacion_por_fecha_cobranza)
dw_lista.settransobject(sqlca)
ld_fecha 	= date(substr(1,1,Message.StringParm))
ls_opcion	= substr(1,2,Message.StringParm)
il_parque	= Long(substr(1,3,Message.StringParm))
CHOOSE CASE ls_opcion
	CASE 'T'
		dw_lista.dataobject	= 'dw_detalle_por_fecha_cobranza'
		this.title				= 'Detalle Cobranza por Fecha'
	CASE 'C'
		dw_lista.dataobject	= 'dw_detalle_por_fecha_envio_carta'
		this.title				= 'Detalle Envio Carta Cobranza por Fecha'
	CASE 'P'
		dw_lista.dataobject	= 'dw_detalle_por_fecha_post_venta'
		this.title				= 'Detalle Post-Venta por Fecha'
END CHOOSE
dw_lista.settransobject(sqlca)
dw_lista.retrieve(ld_fecha,il_parque)
SetPointer(Arrow!)
end event
...
long 	 ll_rut
if il_row > 0 then
	if dw_lista.dataobject='dw_detalle_por_fecha_cobranza' then
	...
	elseif dw_lista.dataobject='dw_detalle_por_fecha_envio_carta' then
	...
	elseif dw_lista.dataobject='dw_detalle_por_fecha_post_venta' then
...
integer taborder = 10
string title = "none"
string dataobject = "dw_detalle_por_fecha_post_venta"
...
event clicked;close(w_detalle_agrupacion_por_fecha_cobranza)
end event
