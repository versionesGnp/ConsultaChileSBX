forward
global type w_listas_generadas from window
end type
type cb_cerrar from commandbutton within w_listas_generadas
end type
type cb_anular from commandbutton within w_listas_generadas
end type
type cb_ordenar from commandbutton within w_listas_generadas
end type
type cb_filtrar from commandbutton within w_listas_generadas
end type
type cb_exportar from commandbutton within w_listas_generadas
end type
type cb_imprimir from commandbutton within w_listas_generadas
end type
type st_procesar from statictext within w_listas_generadas
end type
type pb_aceptar from picturebutton within w_listas_generadas
end type
type p_fin from picture within w_listas_generadas
end type
type em_fec_fin from editmask within w_listas_generadas
end type
type p_ini from picture within w_listas_generadas
end type
type st_2 from statictext within w_listas_generadas
end type
type em_fec_ini from editmask within w_listas_generadas
end type
type st_1 from statictext within w_listas_generadas
end type
type dw_lista from datawindow within w_listas_generadas
end type
type gb_1 from groupbox within w_listas_generadas
end type
end forward
...
global w_listas_generadas w_listas_generadas

type variables
long	il_row
end variables

on w_listas_generadas.create
...
event clicked;close(w_listas_generadas)
end event
...
if il_row>0 then
	...
	if ls_estado ='P' then
		...
		IF ll_resp = 1 THEN
			...
			SELECT 	"GESTION_COBRANZA"."RUT",
						"GESTION_COBRANZA"."FECHA_ASIGNACION"
			FROM 	"GESTION_COBRANZA"  
			WHERE 	"GESTION_COBRANZA"."CODIGO_GRUPO" = :ll_codigo   
			USING	sqlca;
			open 		x1;
			do while sqlca.sqlcode=0 
				fetch x1 into :ll_rut, :ldt_fecha_asig;
				if not isnull(ll_rut) and ll_rut>0 then
					ld_fecha_ori	= RelativeDate( date(ldt_fecha_asig) , -30)
					UPDATE	"CLIENTE"  
			...
			if dw_lista.update()=1 then
				commit;
				// Eliminar Clietes Asignados Tabla Gestion_telefonica	
			else
				rollback;
			end if
		END IF
...
integer height = 1348
integer taborder = 40
string dataobject = "dw_lista_generadas_cobranza"
...
if ls_columna='t_rojo' or ls_columna='t_text_anular' then
	if dw_lista.object.t_text_anular.text='Listas Pendientes' then
		gs_formula	= "estado = '"+"P"+"'"
		dw_lista.object.t_text_anular.text='Mostrar Todo'
	else
		gs_formula	= ''
		dw_lista.object.t_text_anular.text='Listas Pendientes'
	end if
	dw_lista.SETfilter(gs_formula)
	dw_lista.filter()
	dw_lista.sort()
...
if row>0 then
	il_row		= row
	ll_codigo	= this.getitemnumber(il_row,'codigo')
	ls_string	= string(ll_codigo)
	OpenWithParm(w_detalle_lista_generada_gestion_cobranz, ls_string)
end if
end event
