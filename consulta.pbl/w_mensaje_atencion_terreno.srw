forward
global type w_mensaje_atencion_terreno from window
end type
type st_5 from statictext within w_mensaje_atencion_terreno
end type
type st_4 from statictext within w_mensaje_atencion_terreno
end type
type st_3 from statictext within w_mensaje_atencion_terreno
end type
type st_2 from statictext within w_mensaje_atencion_terreno
end type
type st_1 from statictext within w_mensaje_atencion_terreno
end type
type em_1 from editmask within w_mensaje_atencion_terreno
end type
type cb_2 from commandbutton within w_mensaje_atencion_terreno
end type
type cb_1 from commandbutton within w_mensaje_atencion_terreno
end type
type ddlb_1 from dropdownlistbox within w_mensaje_atencion_terreno
end type
type mle_1 from multilineedit within w_mensaje_atencion_terreno
end type
end forward
...
global w_mensaje_atencion_terreno w_mensaje_atencion_terreno
...
on w_mensaje_atencion_terreno.create
...
on w_mensaje_atencion_terreno.destroy
destroy(this.st_5)
destroy(this.st_4)
destroy(this.st_3)
destroy(this.st_2)
...
//else
	ls_sql 			= 'SELECT ESTADO_ATENCION.COD_ESATDO, ESTADO_ATENCION.GLOSA_ESTADO FROM ESTADO_ATENCION ORDER BY ESTADO_ATENCION.COD_ESATDO ASC'
...
//		end if
//	next
	gf_centrar(w_mensaje_atencion_terreno)
	mle_1.text 	= ' '
//end if
end event
...
long backcolor = 16711680
boolean enabled = false
string text = "Estados  Atención Terreno"
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
...
if ll_ret = 1 then
	...
	if sqlca.sqlcode <> 0 then
		...
		if isvalid(w_cobranza_historica) then 
			w_cobranza_historica.tab_1.tabpage_4.dw_at_terreno.retrieve (ll_rut)
		end if
	end if
	ii_i = 0
	close(w_mensaje_atencion_terreno)
else
	ii_i ++
end if
end event
...
if ii_i > 0 then
	...
	if ll_ret = 1 then
		...
		if isvalid(w_cobranza_historica) then
			...
			CloseWithReturn(w_mensaje_atencion_terreno, trim(ddlb_1.Text))
		end if
	elseif ll_ret = 2 then
		if isvalid(w_cobranza_historica) then
			close(w_mensaje_atencion_terreno)
		else
			CloseWithReturn(w_mensaje_atencion_terreno, trim(ls_estado))
		end if
	end if
else
	if isvalid(w_cobranza_historica) then
