forward
global type w_principal_liberador from window
end type
type st_2 from statictext within w_principal_liberador
end type
type cb_filtrar_soli from commandbutton within w_principal_liberador
end type
type cb_4 from commandbutton within w_principal_liberador
end type
type cb_3 from commandbutton within w_principal_liberador
end type
type cb_2 from commandbutton within w_principal_liberador
end type
type cb_1 from commandbutton within w_principal_liberador
end type
type cb_cta_cte from commandbutton within w_principal_liberador
end type
type cb_refrescar from commandbutton within w_principal_liberador
end type
type st_1 from statictext within w_principal_liberador
end type
type cb_salir from commandbutton within w_principal_liberador
end type
type p_1 from picture within w_principal_liberador
end type
type dw_listado_liberador from datawindow within w_principal_liberador
end type
type gb_1 from groupbox within w_principal_liberador
end type
end forward
...
global w_principal_liberador w_principal_liberador

on w_principal_liberador.create
...
on w_principal_liberador.destroy
destroy(this.st_2)
...
gf_centrar(w_principal_liberador)
dw_listado_liberador.SetTransObject(SQLCA)
gs_ventana	= 'w_principal_liberador'
f_valida_objeto()
Open(w_espera)
...
if ll_total > 0 then
	for i =1 to ll_total
		...
		serie  = dw_listado_liberador.getitemstring(i,'anexo_liberador_serie_m')
		numero = dw_listado_liberador.getitemnumber(i,'anexo_liberador_nro_liberador')
...
string text = "Listado General Anexo Liberador de Mantención"
...
st_1.text	= string(dw_listado_liberador.rowcount(),"###,###,###,###,##0")+" Anexos liberadores"
...
event clicked;if dw_listado_liberador.getrow() > 0 then open(w_cuenta_corriente_liberador)
end event
...
event clicked;Close(w_principal_liberador)
...
integer height = 1716
integer taborder = 40
string dataobject = "d_listado_liberador"
...
event clicked;IF row > 0 THEN
	...
	gs_base 		= "L"
	gs_serie 	= GetItemString(row, "anexo_liberador_serie_m")
	gi_numero 	= GetItemNumber(row, "anexo_liberador_nro_liberador")
...
event rowfocuschanged;IF this.getrow() > 0 THEN
	...
	gs_base 		= "L"
	gs_serie 	= GetItemString(this.getrow(), "anexo_liberador_serie_m")
	gi_numero 	= GetItemNumber(this.getrow(), "anexo_liberador_nro_liberador")
