forward
global type w_cuenta_corriente_pagare from window
end type
type mdi_1 from mdiclient within w_cuenta_corriente_pagare
end type
type mditbb_1 from tabbedbar within w_cuenta_corriente_pagare
end type
type mdirbb_1 from ribbonbar within w_cuenta_corriente_pagare
end type
type st_at_gestion from statictext within w_cuenta_corriente_pagare
end type
type cb_documento from commandbutton within w_cuenta_corriente_pagare
end type
type cb_planimetria from commandbutton within w_cuenta_corriente_pagare
end type
type st_help_abono from statictext within w_cuenta_corriente_pagare
end type
type cb_consul_hist from commandbutton within w_cuenta_corriente_pagare
end type
type st_pac from statictext within w_cuenta_corriente_pagare
end type
type pb_datos_cliente from picturebutton within w_cuenta_corriente_pagare
end type
type pb_ubicacion from picturebutton within w_cuenta_corriente_pagare
end type
type cb_11 from commandbutton within w_cuenta_corriente_pagare
end type
type cb_est_cheque from commandbutton within w_cuenta_corriente_pagare
end type
type cb_cupones from commandbutton within w_cuenta_corriente_pagare
end type
type cb_operaciones from commandbutton within w_cuenta_corriente_pagare
end type
type cb_5 from commandbutton within w_cuenta_corriente_pagare
end type
type cb_6 from commandbutton within w_cuenta_corriente_pagare
end type
type cb_2 from commandbutton within w_cuenta_corriente_pagare
end type
type cb_1 from commandbutton within w_cuenta_corriente_pagare
end type
type cb_imprimir from commandbutton within w_cuenta_corriente_pagare
end type
type cb_cerrar from commandbutton within w_cuenta_corriente_pagare
end type
type dw_estado_cta_cte from datawindow within w_cuenta_corriente_pagare
end type
type cb_convenio from commandbutton within w_cuenta_corriente_pagare
end type
type dw_abono from datawindow within w_cuenta_corriente_pagare
end type
type dw_encabezado_cuenta_cte from datawindow within w_cuenta_corriente_pagare
end type
type dw_listado_cuenta_cte from datawindow within w_cuenta_corriente_pagare
end type
type dw_listado_cuenta_cte_cta from datawindow within w_cuenta_corriente_pagare
end type
type dw_cons_docu_w from datawindow within w_cuenta_corriente_pagare
end type
end forward
...
string title = "Cuenta Corriente PAGARE (base = P)"
string menuname = "m_menu_cuenta_corriente_pagare"
...
global w_cuenta_corriente_pagare w_cuenta_corriente_pagare
...
on w_cuenta_corriente_pagare.create
if this.MenuName = "m_menu_cuenta_corriente_pagare" then this.MenuID = create m_menu_cuenta_corriente_pagare
...
on w_cuenta_corriente_pagare.destroy
...
event open;Long		ll_count,ll_count_reg,ll_status,ll_count_cup,ll_count_hist,ll_obj_id_estado
String		ls_base_aux,ls_pac,ls_serie_asoc,ls_cod_usuario,	ls_nom_usuario,ls_tipo_cobro,ls_usuario,ls_nombre,ls_descrip_depto  
...
if gs_serie<>'' and gi_numero>0 and gs_base<>'' then
	...
	gf_centrar (w_cuenta_corriente_pagare)
	dw_encabezado_cuenta_cte.SetTransObject(SQLCA)
	if dw_encabezado_cuenta_cte.Retrieve(gs_serie, gi_numero)=0 then
		messagebox("Advertencia","No Registra Datos en Pagaré "+gs_base+'-'+gs_serie+'-'+string(gi_numero,"###,###,###,###"))
		close(w_cuenta_corriente_pagare)
	else
		...
		gs_ventana	= 'w_cuenta_corriente_pagare'
		f_valida_objeto()
		...
		if ll_count_cup > 0 and ( gs_depto = 'E' or gs_depto = 'I' or gs_depto = 'P' or (gs_depto = 'O' and gs_digita='1')) then
			m_menu_cuenta_corriente_pagare.m_imprimircuponera.enabled	= true
		else
			m_menu_cuenta_corriente_pagare.m_imprimircuponera.enabled	= false
		end if
		...
		if ll_obj_id_estado=0 or isnull(ll_obj_id_estado) then
			m_menu_cuenta_corriente_pagare.m_consultar.m_documentación.enabled	= false
		else
			m_menu_cuenta_corriente_pagare.m_consultar.m_documentación.enabled	= true
		end if
		SELECT	COUNT("DETALLE_CONVENIO_PROTESTO"."BASE_PAGARE")  
		INTO 		:ll_count_reg  
		FROM 		"DETALLE_CONVENIO_PROTESTO"  
		WHERE  ( "DETALLE_CONVENIO_PROTESTO"."BASE_PAGARE" = :gs_base ) AND  
				 ( "DETALLE_CONVENIO_PROTESTO"."SERIE_PAGARE" = :gs_serie ) AND  
				 ( "DETALLE_CONVENIO_PROTESTO"."NUMERO_PAGARE" = :gi_numero )   
		USING		sqlca;
		...
		ls_serie_asoc					= dw_encabezado_cuenta_cte.getitemstring(1,'pagare_serie')
		SELECT	Count("PAGO_HISTORICO"."RUT_CLIENTE")  
		INTO 		:ll_count_hist  
		FROM 		"PAGO_HISTORICO"  
		WHERE  ( "PAGO_HISTORICO"."RUT_CLIENTE" = :gi_rut ) AND  
				 ( "PAGO_HISTORICO"."CORRELATIVO" = :ls_serie_asoc )   
		USING		sqlca;
		if ll_count_hist > 0 then
			m_menu_cuenta_corriente_pagare.m_consultar.m_pagoshistóricos.enabled		= true
			cb_consul_hist.enabled	= true
		else
			m_menu_cuenta_corriente_pagare.m_consultar.m_pagoshistóricos.enabled		= false
		...
		if ll_status>0 then
			...
			m_menu_cuenta_corriente_pagare.m_consultar.m_geo-referenciación.enabled	= true
		else
			pb_ubicacion.enabled		= false
			m_menu_cuenta_corriente_pagare.m_consultar.m_geo-referenciación.enabled	= false
		end if
		if cb_est_cheque.enabled=true then
			m_menu_cuenta_corriente_pagare.m_consultar.m_estadocheques.enabled		= true
		else
			m_menu_cuenta_corriente_pagare.m_consultar.m_estadocheques.enabled		= false
		end if
		if cb_convenio.enabled=true then
			m_menu_cuenta_corriente_pagare.m_consultar.m_conveniopagocheuqe.enabled	= true
		else
			m_menu_cuenta_corriente_pagare.m_consultar.m_conveniopagocheuqe.enabled	= false
		end if
		if gs_depto='I' then
			m_menu_cuenta_corriente_pagare.m_mantenedor.m_actualizarmora.visible		= true
			m_menu_cuenta_corriente_pagare.m_mantenedor.m_-2.visible						= true
		else
			m_menu_cuenta_corriente_pagare.m_mantenedor.m_actualizarmora.visible		= false
			m_menu_cuenta_corriente_pagare.m_mantenedor.m_-2.visible						= false
		end if
...
event close;if gs_serie<>'' and gi_numero>0 and gs_base<>'' and dw_encabezado_cuenta_cte.rowcount()>0 then
	...
	gs_serie 				= dw_encabezado_cuenta_cte.GetItemString(1, "pagare_serie_p")
	gi_numero 				= dw_encabezado_cuenta_cte.GetItemNumber(1, "pagare_nro_pagare")
...
event clicked;String	ls_string,ls_serie_asoc
Long	ll_cod_parque
...
ls_string			= gs_base+'~t'+ls_serie_asoc+'~t'+string(gi_numero)+'~t'+string(ll_cod_parque)
...
event clicked;gs_datawindow_a_imprimir = 'd_rep_impresion_ctta_cte_pagare'
...
event clicked;Close(w_cuenta_corriente_pagare)
end event
...
integer width = 658
integer height = 528
string dataobject = "d_estado_cta_cte_pagare"
...
integer height = 984
integer taborder = 10
string dataobject = "d_encabezado_cuenta_cte_pagare"
boolean border = false
boolean livescroll = true
end type
...
integer taborder = 40
boolean titlebar = true
string title = "Detalle Total Cta. Cte"
