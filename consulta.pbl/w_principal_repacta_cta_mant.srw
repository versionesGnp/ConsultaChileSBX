forward
global type w_principal_repacta_cta_mant from window
end type
type cb_filtrar_soli from commandbutton within w_principal_repacta_cta_mant
end type
type st_2 from statictext within w_principal_repacta_cta_mant
end type
type cb_4 from commandbutton within w_principal_repacta_cta_mant
end type
type cb_3 from commandbutton within w_principal_repacta_cta_mant
end type
type cb_2 from commandbutton within w_principal_repacta_cta_mant
end type
type cb_1 from commandbutton within w_principal_repacta_cta_mant
end type
type cb_cta_cte from commandbutton within w_principal_repacta_cta_mant
end type
type st_1 from statictext within w_principal_repacta_cta_mant
end type
type cb_salir from commandbutton within w_principal_repacta_cta_mant
end type
type p_1 from picture within w_principal_repacta_cta_mant
end type
type dw_listado_liberador from datawindow within w_principal_repacta_cta_mant
end type
type gb_1 from groupbox within w_principal_repacta_cta_mant
end type
end forward
...
string title = "REPROGRAMACION CUOTA DE MANTENCION"
...
global w_principal_repacta_cta_mant w_principal_repacta_cta_mant

on w_principal_repacta_cta_mant.create
...
this.Control[]={this.cb_filtrar_soli,&
this.st_2,&
this.cb_4,&
this.cb_3,&
this.cb_2,&
...
on w_principal_repacta_cta_mant.destroy
...
gf_centrar(w_principal_repacta_cta_mant)
dw_listado_liberador.SetTransObject(SQLCA)
gs_ventana					= 'w_principal_repacta_cta_mant'
f_valida_objeto()
Open(w_espera)
ll_total = dw_listado_liberador.Retrieve()
if ll_total > 0 then
	for i =1 to ll_total
	 	base   			= dw_listado_liberador.getitemstring(i,'repacta_mantencion_base')
		serie  			= dw_listado_liberador.getitemstring(i,'repacta_mantencion_serie')
		numero 			= dw_listado_liberador.getitemnumber(i,'repacta_mantencion_numero')
...
Close(w_espera)
st_1.text = String(ll_total,"###,###,###") + " Reprogramación Ctas.Mant."
setpointer(arrow!)
...
st_1.text	= string(dw_listado_liberador.rowcount(),"###,###,###,###,##0")+" Reprogramación Ctas.Mant."
end event
...
string text = "Listado General Reprogramación Cuota de Mantención"
...
long backcolor = 79741120
boolean enabled = false
string text = "0 Repactación Cta.Mant."
...
event clicked;Close(w_principal_repacta_cta_mant)
...
integer height = 1716
integer taborder = 40
string dataobject = "d_listado_repacta_cta_mant"
