forward
global type w_cuenta_corriente_aumento_capacidad from window
end type
type mdi_1 from mdiclient within w_cuenta_corriente_aumento_capacidad
end type
type mditbb_1 from tabbedbar within w_cuenta_corriente_aumento_capacidad
end type
type mdirbb_1 from ribbonbar within w_cuenta_corriente_aumento_capacidad
end type
type cb_aval from picturebutton within w_cuenta_corriente_aumento_capacidad
end type
type pb_clasif_aumento from picturebutton within w_cuenta_corriente_aumento_capacidad
end type
type st_at_gestion from statictext within w_cuenta_corriente_aumento_capacidad
end type
type cb_documento from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_planimetria from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type st_help_abono from statictext within w_cuenta_corriente_aumento_capacidad
end type
type dw_abono from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type st_pac from statictext within w_cuenta_corriente_aumento_capacidad
end type
type cb_ctas_cred from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type pb_datos_cliente from picturebutton within w_cuenta_corriente_aumento_capacidad
end type
type pb_ubicacion from picturebutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_11 from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_convenio from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_est_cheque from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_detalle from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_cupones from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_rescomi_vta from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_operaciones from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_agente from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_5 from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_2 from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_1 from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type cb_imprimir from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type dw_encabezado_cuenta_cte from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type cb_cerrar from commandbutton within w_cuenta_corriente_aumento_capacidad
end type
type dw_estado_cta_cte from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type dw_agente_super from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type dw_listado_cuenta_cte from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type dw_listado_cuenta_cte_cta from datawindow within w_cuenta_corriente_aumento_capacidad
end type
type dw_cons_docu_w from datawindow within w_cuenta_corriente_aumento_capacidad
end type
end forward
...
string title = "Cta.Cte. AUMENTO CAPACIDAD (base = A)"
string menuname = "m_menu_cuenta_corriente_aumento"
...
mdirbb_1 mdirbb_1
cb_aval cb_aval
pb_clasif_aumento pb_clasif_aumento
...
global w_cuenta_corriente_aumento_capacidad w_cuenta_corriente_aumento_capacidad
...
on w_cuenta_corriente_aumento_capacidad.create
if this.MenuName = "m_menu_cuenta_corriente_aumento" then this.MenuID = create m_menu_cuenta_corriente_aumento
this.mdi_1=create mdi_1
this.mditbb_1=create mditbb_1
this.mdirbb_1=create mdirbb_1
this.cb_aval=create cb_aval
this.pb_clasif_aumento=create pb_clasif_aumento
...
this.cb_ctas_cred=create cb_ctas_cred
...
on w_cuenta_corriente_aumento_capacidad.destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.mdi_1)
destroy(this.mditbb_1)
destroy(this.mdirbb_1)
destroy(this.cb_aval)
destroy(this.pb_clasif_aumento)
...
if gs_serie<>'' and gi_numero>0 and gs_base<>'' then
	...
	gf_centrar (w_cuenta_corriente_aumento_capacidad)
	if dw_encabezado_cuenta_cte.Retrieve(gs_serie, gi_numero)=0 then
		messagebox("Advertencia","No Registra Datos Aumento Capacidad "+gs_base+'-'+gs_serie+'-'+string(gi_numero,"###,###,###,###"))
		close(w_cuenta_corriente_aumento_capacidad)
	else
		...
		gs_ventana	= 'w_cuenta_corriente_aumento_capacidad'
		f_valida_objeto()
...
//		end if
		...
		if ll_count_cup > 0 and ( gs_depto = 'E' or gs_depto = 'I' or gs_depto = 'P' or (gs_depto = 'O' and gs_digita='1')) then
			m_menu_cuenta_corriente_aumento.m_imprimircuponera.enabled			= true
		else
			m_menu_cuenta_corriente_aumento.m_imprimircuponera.enabled			= false
		end if
		...
		if ll_obj_id_estado=0 or isnull(ll_obj_id_estado) then
			m_menu_cuenta_corriente_aumento.m_consultar.m_documentación.enabled	= false
		else
			m_menu_cuenta_corriente_aumento.m_consultar.m_documentación.enabled	= true
		end if
		...
		if ll_status>0 then
			...
			m_menu_cuenta_corriente_aumento.m_consultar.m_geo-referenciación.enabled	= true
		else
			pb_ubicacion.enabled		= false
			m_menu_cuenta_corriente_aumento.m_consultar.m_geo-referenciación.enabled	= false
		end if
		if cb_agente.enabled=true then
			m_menu_cuenta_corriente_aumento.m_consultar.m_pagoscomisiónagente.enabled	= true
		else
			m_menu_cuenta_corriente_aumento.m_consultar.m_pagoscomisiónagente.enabled	= false
		end if
		if cb_rescomi_vta.enabled=true then
			m_menu_cuenta_corriente_aumento.m_consultar.m_pagosventas.enabled				= true
		else
			m_menu_cuenta_corriente_aumento.m_consultar.m_pagosventas.enabled				= false
		end if
		if cb_est_cheque.enabled=true then
			m_menu_cuenta_corriente_aumento.m_consultar.m_estadocheques.enabled			= true
		else
			m_menu_cuenta_corriente_aumento.m_consultar.m_estadocheques.enabled			= false
		end if
		if cb_convenio.enabled=true then
			m_menu_cuenta_corriente_aumento.m_consultar.m_conveniopagocheuqe.enabled	= true
		else
			m_menu_cuenta_corriente_aumento.m_consultar.m_conveniopagocheuqe.enabled	= false
		end if
		if gs_depto='I' then
			m_menu_cuenta_corriente_aumento.m_mantenedor.m_actualizarmora.visible		= true
			m_menu_cuenta_corriente_aumento.m_mantenedor.m_-2.visible						= true
		else
			m_menu_cuenta_corriente_aumento.m_mantenedor.m_actualizarmora.visible		= false
			m_menu_cuenta_corriente_aumento.m_mantenedor.m_-2.visible						= false
		end if
		if date(dw_encabezado_cuenta_cte.getitemdatetime(1,'anexo_aumento_cierre_venta')) >= date('31/01/2011') then
			pb_clasif_aumento.enabled		= true
			m_menu_cuenta_corriente_aumento.m_consultar.m_clasificaciónventa.enabled	= true
		else
			pb_clasif_aumento.enabled		= false
			m_menu_cuenta_corriente_aumento.m_consultar.m_clasificaciónventa.enabled	= false
		end if
		if gi_rut>0 then 
...
event close;if gs_serie<>'' and gi_numero>0 and gs_base<>'' and dw_encabezado_cuenta_cte.rowcount()>0 then
	gs_base 					= 'A'
	gs_serie 				= dw_encabezado_cuenta_cte.GetItemString(1, "anexo_aumento_serie_m")
	gi_numero 				= dw_encabezado_cuenta_cte.GetItemNumber(1, "anexo_aumento_nro_aumento")
...
string picturename = "ver_aval.bmp"
string disabledname = "ver_aval_no.bmp"
alignment htextalign = left!
end type

event ue_mousemove;//w_cuenta_corriente_oferta.SetMicroHelp('Muestra Aval Asociado')
...
//st_clasif_promesa.visible				= false
...
gs_estado				= dw_encabezado_cuenta_cte.GetItemString(1, "cadena_estado")
gl_cod_parque_cta		= dw_encabezado_cuenta_cte.GetItemNumber(1, "cadena_cod_parque")
if dw_encabezado_cuenta_cte.getitemnumber(1,'anexo_aumento_rut_aval')>0 then 
	if isvalid(w_aval) then close(w_aval)
	OpenWithParm(w_aval, STRING(dw_encabezado_cuenta_cte.getitemnumber(1,'anexo_aumento_rut_aval')))
else
...
ret = iinet_base.HyperlinkToURL("http://intra2.nuestrosparques.cl/geomod/dbutil/ccte.php?barra="+gs_base+gs_serie+string(gi_numero))
if ret = -1 then
	messagebox('No Existe','No existe el plano del sector buscado.',stopsign!)
	return
end if
...
gs_estado				= dw_encabezado_cuenta_cte.GetItemString(1, "cadena_estado")
gl_cod_parque_cta		= dw_encabezado_cuenta_cte.GetItemNumber(1, "cadena_cod_parque")
ls_string				= dw_encabezado_cuenta_cte.getitemstring(1,'anexo_aumento_cod_agente')+'~t'+&
				  			  dw_encabezado_cuenta_cte.getitemstring(1,'anexo_aumento_cod_super')
...
event clicked;dw_listado_cuenta_cte_cta.SetTransObject(SQLCA)
dw_listado_cuenta_cte_cta.Retrieve(gs_serie, gi_numero,gs_base)
...
gs_estado				= dw_encabezado_cuenta_cte.GetItemString(1, "cadena_estado")
gl_cod_parque_cta		= dw_encabezado_cuenta_cte.GetItemNumber(1, "cadena_cod_parque")
gs_datawindow_a_imprimir = 'd_rep_impresion_ctta_cte_aumento_capac'
...
integer height = 764
integer taborder = 10
string dataobject = "d_encabezado_cuenta_cte_aumento_capacida"
boolean border = false
boolean livescroll = true
end type
...
ls_columna					= dwo.name
if ls_columna='pago_liberador_pie_t' or ls_columna='pago_aumento_pie' or ls_columna='pago_aumento_ctas_pagadas_pie' or ls_columna='pago_aumento_ctas_pactadas_pie' then
	lld_pie			= double(dw_encabezado_cuenta_cte.getitemnumber(1,'pago_aumento_pie'))
	ls_moneda		= dw_encabezado_cuenta_cte.getitemstring(1,'pago_aumento_moneda')
	if lld_pie>0 then
		ls_uso		= trim(dw_encabezado_cuenta_cte.getitemstring(1,'anexo_aumento_uso'))
	...
	gs_estado				= dw_encabezado_cuenta_cte.GetItemString(1, "cadena_estado")
	gl_cod_parque_cta		= dw_encabezado_cuenta_cte.GetItemNumber(1, "cadena_cod_parque")
	agente 					= getitemstring(1,"anexo_aumento_cod_agente")
	superv 					= getitemstring(1,"anexo_aumento_cod_super")
...
event clicked;Close(w_cuenta_corriente_aumento_capacidad)
end event
...
integer width = 786
integer height = 716
string dataobject = "d_estado_cta_cte_aumento_capacidad"
boolean border = false
boolean livescroll = true
end type
