forward
global type w_conexion_bkp from window
end type
type rb_parque_stgo from radiobutton within w_conexion_bkp
end type
type st_1 from statictext within w_conexion_bkp
end type
type mle_1 from multilineedit within w_conexion_bkp
end type
type rb_serena from radiobutton within w_conexion_bkp
end type
type rb_santiago from radiobutton within w_conexion_bkp
end type
type cb_2 from commandbutton within w_conexion_bkp
end type
type cb_1 from commandbutton within w_conexion_bkp
end type
type gb_1 from groupbox within w_conexion_bkp
end type
end forward
...
global w_conexion_bkp w_conexion_bkp
...
string facename = "Arial TUR"
long backcolor = 80269524
string text = "Prado / Manantial / Santiago"
...
elseif rb_parque_stgo.checked=true then
	...
	if isvalid(w_principal) then w_principal.title=gs_app_name+fill(' ',5)+'( Conexión Santiago )'+fill(' ',5)+'Usuario: '+gs_nom_comp_usuario+fill(' ',5)+'Fecha Hoy: '+string(gdt_fec_sistema,"dd/mm/yyyy")+'  UF: '+string(gd_uf_dia,"###,###,##0.0###")
end if	
close(w_conexion)
end event
