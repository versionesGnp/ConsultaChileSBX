...
type mditbb_1 from tabbedbar within w_principal
end type
type mdirbb_1 from ribbonbar within w_principal
end type
end forward
...
event open;String		ls_opcion_dig,ls_depto,ls_cod_aux,ls_canal,ls_usuario_odbc,ls_clave_odbc,ls_estado
...
if gs_depto='I' or gs_depto='O' or gs_depto='F' or gs_depto='P' or gs_depto='E' or gs_depto='T' or gs_depto='Q' then 
...
if gs_depto='I' then
	...
	m_menu_principal.m_herramienta.m_validaciónlogonclave.visible						= true
	m_menu_principal.m_herramienta.m_sap.visible											= true
else
	...
	m_menu_principal.m_herramienta.m_validaciónlogonclave.visible						= false
	m_menu_principal.m_herramienta.m_sap.visible											= false
end if
if gs_depto='I' or gs_depto='G' or (gs_depto='O' and gs_opera='1') then
	m_menu_principal.m_operaciones0.m_informesmorosidad912.visible					= true
else
	m_menu_principal.m_operaciones0.m_informesmorosidad912.visible					= false
end if
SELECT 	"FICHA_USUARIO_ESTADISTICO"."ESTADO"  
INTO 		:ls_estado  
FROM 	"FICHA_USUARIO_ESTADISTICO"  
WHERE 	"FICHA_USUARIO_ESTADISTICO"."USUARIO" = :gs_user 
USING	sqlca;
if sqlca.sqlcode=0 then
	if ls_estado='I' then
		m_menu_principal.m_fallecidos0.m_fichaimplementación.visible					= false
	else
		m_menu_principal.m_fallecidos0.m_fichaimplementación.visible					= true
	end if
else
	if gs_depto='I' or gs_depto='X' or gs_depto='P' then
		m_menu_principal.m_fallecidos0.m_fichaimplementación.visible					= true
	else
		m_menu_principal.m_fallecidos0.m_fichaimplementación.visible					= false
	end if
end if
SELECT "TASA"."DIRECTORIO_EXE"  INTO :gs_directorio_exe  FROM "TASA"  WHERE "TASA"."LOOK" = 1   ;

//SELECT 	"SAP_ODBC"."USUARIO", 	"SAP_ODBC"."CLAVE"  
//INTO 		:ls_usuario_odbc,   			:ls_clave_odbc  
//FROM 	"SAP_ODBC" 
//WHERE	"SAP_ODBC"."SERVERNAME" = :gs_servername ;
//
...
//Trans_hana.DBParm			= "ConnectString='DSN=Sap;UID="+ls_usuario_odbc+";PWD="+ls_clave_odbc+"'"
//Trans_hana.DBParm			= "ConnectString='DSN=Sap_Hana;UID="+ls_usuario_odbc+";PWD="+ls_clave_odbc+"'"
end event
