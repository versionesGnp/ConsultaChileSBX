forward
global type w_imprime_promesas from window
end type
type cb_7 from commandbutton within w_imprime_promesas
end type
type dw_2 from datawindow within w_imprime_promesas
end type
type pb_fin from picture within w_imprime_promesas
end type
type pb_antes from picture within w_imprime_promesas
end type
type pb_sigue from picture within w_imprime_promesas
end type
type pb_primer from picture within w_imprime_promesas
end type
type cb_6 from commandbutton within w_imprime_promesas
end type
type cb_5 from commandbutton within w_imprime_promesas
end type
type cb_4 from commandbutton within w_imprime_promesas
end type
type cb_3 from commandbutton within w_imprime_promesas
end type
type cb_2 from commandbutton within w_imprime_promesas
end type
type cb_1 from commandbutton within w_imprime_promesas
end type
type dw_1 from datawindow within w_imprime_promesas
end type
end forward
...
global w_imprime_promesas w_imprime_promesas

on w_imprime_promesas.create
...
event open;gf_centrar(w_imprime_promesas)
gs_ventana		= 'w_imprime_promesas'
f_valida_objeto()
this.title = gs_app_name+' - Promesas'
dw_1.settransobject (sqlca)
dw_2.settransobject (sqlca)
if dw_1.retrieve() = 0 then
	messagebox('No hay Registros','No hay Registros para imprimir',stopsign!)
	close(w_imprime_promesas)
else
	dw_2.retrieve()
end if
end event
...
if ll_tot_reg > 0 then
	...
	next
	dw_2.accepttext()
	dw_2.SaveAs("C:\Mod_cp\cuponera\promesa"+string(day(today()))+'_'+string(month(today()))+'_'+string(year(today()))+'.xls',Excel! , true)
...
integer height = 360
integer taborder = 20
string dataobject = "dw_etiquetas_listado"
...
event clicked;close(w_imprime_promesas)
end event
...
integer height = 2096
integer taborder = 70
string dataobject = "dw_etiquetas"
