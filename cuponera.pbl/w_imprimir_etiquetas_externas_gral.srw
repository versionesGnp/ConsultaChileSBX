forward
global type w_imprimir_etiquetas_externas_gral from window
end type
type cb_exportar from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type cb_ordenar from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type cb_setup from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type dw_no_encontrados from datawindow within w_imprimir_etiquetas_externas_gral
end type
type dw_base from datawindow within w_imprimir_etiquetas_externas_gral
end type
type cb_limpiar from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type cb_imprimir from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type cb_cerrar from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type cb_cargar from commandbutton within w_imprimir_etiquetas_externas_gral
end type
type dw_etiqueta from datawindow within w_imprimir_etiquetas_externas_gral
end type
type gb_1 from groupbox within w_imprimir_etiquetas_externas_gral
end type
end forward
...
string title = "Imprimir Etiquetas con BASE"
...
global w_imprimir_etiquetas_externas_gral w_imprimir_etiquetas_externas_gral

on w_imprimir_etiquetas_externas_gral.create
...
on w_imprimir_etiquetas_externas_gral.destroy
...
event open;gf_centrar(w_imprimir_etiquetas_externas_gral)
...
event clicked;close(w_imprimir_etiquetas_externas_gral)
end event
...
String		docname,named,ls_base,ls_serie,ls_nombres,ls_ap_pat,ls_ap_mat,ls_direc_p,&
			...
			ls_nom_comp,ls_dir_comp,ls_estado_tit,ls_fono_p,ls_fono_c,ls_celular
...
if ll_tot_reg>0 then
	for ll_indi=1 to ll_tot_reg
		...
		if sqlca.sqlcode=0 then
			SELECT	"CLIENTE"."NOMBRE",	"CLIENTE"."A_PATERNO",	"CLIENTE"."A_MATERNO",	"CLIENTE"."DIRECCION_P",	"CLIENTE"."POBLACION",	"CLIENTE"."SEXO",	"CLIENTE"."NUMERO_PARTICULAR",	"CLIENTE"."DEPTO_PARTICULAR",	"CLIENTE"."BLOCK_PARTICULAR",	"TIPO_VIA"."DESCRIPCION",	"COMUNA"."SIGLA_COMUNA",	"CIUDAD"."SIGLA_CIUDAD",	"CLIENTE"."ESTADO_TITULAR",	"CLIENTE"."FONO_P",	"CLIENTE"."FONO_C",	"CLIENTE"."CELULAR"
			INTO 		:ls_nombres,			:ls_ap_pat,					:ls_ap_mat,					:ls_direc_p,					:ls_pob,						:ls_sexo,			:ls_nro_p,								:ls_depto_p,						:ls_block_p,						:ls_tipo_via,					:ls_comuna,						:ls_ciudad,						:ls_estado_tit,					:ls_fono_p,				:ls_fono_c,				:ls_celular
			FROM 		"CLIENTE",   
						"CIUDAD",   
						"COMUNA",   
						"TIPO_VIA"  
			...
			if sqlca.sqlcode=0 then
				...
				dw_etiqueta.setitem(ll_new,'fono_particular',ls_fono_p)
				dw_etiqueta.setitem(ll_new,'fono_comercial',ls_fono_c)
				dw_etiqueta.setitem(ll_new,'celular',ls_celular)
				dw_etiqueta.setitem(ll_new,'fecha',today())
...
integer taborder = 10
string title = "none"
string dataobject = "dwe_imprimir_etiqueta_externa_con_base"
