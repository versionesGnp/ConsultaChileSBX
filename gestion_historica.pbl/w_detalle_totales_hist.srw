forward
global type w_detalle_totales_hist from window
end type
type cb_4 from commandbutton within w_detalle_totales_hist
end type
type cb_consulta from commandbutton within w_detalle_totales_hist
end type
type cb_3 from commandbutton within w_detalle_totales_hist
end type
type cb_2 from commandbutton within w_detalle_totales_hist
end type
type cb_filtrar from commandbutton within w_detalle_totales_hist
end type
type cb_1 from commandbutton within w_detalle_totales_hist
end type
type dw_lista from datawindow within w_detalle_totales_hist
end type
end forward
...
global w_detalle_totales_hist w_detalle_totales_hist
...
close(w_detalle_totales_hist)
end subroutine

on w_detalle_totales_hist.create
...
on w_detalle_totales_hist.destroy
destroy(this.cb_4)
...
gf_centrar(w_detalle_totales_hist)
gs_ventana		= 'w_detalle_totales_hist'
f_valida_objeto_1()
...
CHOOSE CASE ls_tipo_cargo
	CASE '1'
		this.title					= 'Detalle Informe Gestion Histórico por Jefe de Venta'
		if is_base='O' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe_hist'
		elseif is_base='A' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe_aumento_hi'
		end if
		dw_lista.settransobject(sqlca)
		
	CASE '2'
		this.title					= 'Detalle Informe Gestion Histórico por Supervisor'
		if is_base='O' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_supervisor_hist'
		elseif is_base='A' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_super_aume_hist'
		end if
		dw_lista.settransobject(sqlca)

	CASE '3'
		this.title					= 'Detalle Informe Gestion Histórico por Agente'
		if is_base='O' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_agente_hist'
		elseif is_base='A' then
			dw_lista.dataobject	= 'dw_lista_detalle_gestion_agente_aume_his'
		end if
		dw_lista.settransobject(sqlca)
END CHOOSE
...
integer height = 1804
integer taborder = 10
string dataobject = "dw_lista_detalle_gestion_agente_aume_his"
