forward
global type w_inf_gestion_hist_bkp from window
end type
type rb_promesas from radiobutton within w_inf_gestion_hist_bkp
end type
type rb_aumento from radiobutton within w_inf_gestion_hist_bkp
end type
type hpb_1 from hprogressbar within w_inf_gestion_hist_bkp
end type
type st_porc from statictext within w_inf_gestion_hist_bkp
end type
type st_grupo from statictext within w_inf_gestion_hist_bkp
end type
type pb_ordenar from picturebutton within w_inf_gestion_hist_bkp
end type
type pb_exportar from picturebutton within w_inf_gestion_hist_bkp
end type
type pb_aceptar from picturebutton within w_inf_gestion_hist_bkp
end type
type pb_imprimir from picturebutton within w_inf_gestion_hist_bkp
end type
type pb_salir from picturebutton within w_inf_gestion_hist_bkp
end type
type dw_detalle from datawindow within w_inf_gestion_hist_bkp
end type
type st_nombre from statictext within w_inf_gestion_hist_bkp
end type
type pb_buscar from picturebutton within w_inf_gestion_hist_bkp
end type
type em_codigo from editmask within w_inf_gestion_hist_bkp
end type
type em_1 from editmask within w_inf_gestion_hist_bkp
end type
type p_1 from picture within w_inf_gestion_hist_bkp
end type
type em_2 from editmask within w_inf_gestion_hist_bkp
end type
type p_2 from picture within w_inf_gestion_hist_bkp
end type
type st_1 from statictext within w_inf_gestion_hist_bkp
end type
type st_2 from statictext within w_inf_gestion_hist_bkp
end type
type rb_agente from radiobutton within w_inf_gestion_hist_bkp
end type
type rb_supervisor from radiobutton within w_inf_gestion_hist_bkp
end type
type rb_jefe from radiobutton within w_inf_gestion_hist_bkp
end type
type dw_lista from datawindow within w_inf_gestion_hist_bkp
end type
type gb_2 from groupbox within w_inf_gestion_hist_bkp
end type
type gb_1 from groupbox within w_inf_gestion_hist_bkp
end type
type em_grupo from editmask within w_inf_gestion_hist_bkp
end type
type gb_3 from groupbox within w_inf_gestion_hist_bkp
end type
type st_fondo from statictext within w_inf_gestion_hist_bkp
end type
type st_volver from statictext within w_inf_gestion_hist_bkp
end type
type dw_informe from datawindow within w_inf_gestion_hist_bkp
end type
type pb_licencias from picturebutton within w_inf_gestion_hist_bkp
end type
type gb_4 from groupbox within w_inf_gestion_hist_bkp
end type
end forward
...
global w_inf_gestion_hist_bkp w_inf_gestion_hist_bkp
...
String is_estado_age, is_estado_sup
...
Long il_tot_dia, il_tot_mora, il_tot_cuotas_p, il_sw, il_po_precio
...
dw_informe.object.titulo.text = 'Detalle Histórico de Cuotas en Mora'
...
public subroutine wf_informe (datawindow adw_detalle, string as_ventana, datawindow adw_informe);//adw_detalle
//as_ventana 1=fefe 2=supervisor 3=agente
...
if adw_informe.dataobject = 'dw_informe' then
	...
	if rb_jefe.checked = true then
		...
		adw_informe.object.titulo.text = 'Informe Gestion Histórica Jefe de Venta'
	end if
	if rb_supervisor.checked = true then
		adw_informe.Modify("cod_supervisor.font.italic='1'")
		adw_informe.object.titulo.text = 'Informe Gestion Histórica Supervisor'
	end if
	if rb_agente.checked = true then
		adw_informe.Modify("cod_agente.font.italic='1'")
		adw_informe.setitem(li_new,'clasificacion',is_clasif)
		adw_informe.object.titulo.text = 'Informe Gestion Histórica Agente'
	end if
elseif adw_informe.dataobject = 'dw_detalle_ciotas_pagadas' then
	wf_cargar_moras(li_new)
	
elseif adw_informe.dataobject = 'dw_resumen_promesas_por_jefe_hist' or &
	adw_informe.dataobject = 'dw_resumen_promesas_por_supervisor_hist' then
	if rb_jefe.checked = true then
		adw_informe.dataobject = 'dw_resumen_promesas_por_jefe_hist'
		adw_informe.settransobject(sqlca)
		adw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
		adw_informe.object.titulo.text = 'Resumen Histórico de Supervisores por Jefe Venta'
		adw_informe.object.usuario.text = gs_user
	end if
	if rb_supervisor.checked = true then
		adw_informe.dataobject = 'dw_resumen_promesas_por_supervisor_hist'
		adw_informe.settransobject(sqlca)
		adw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
		adw_informe.object.titulo.text = 'Resumen Histórico de Agentes por Supervisor'
		adw_informe.object.usuario.text = gs_user
	end if
	if rb_agente.checked = true then
...
event open;gf_centrar(w_inf_gestion_hist)
...
CHOOSE CASE is_ventana
	CASE '1' //jefe
		this.title = "Informe Gestion Histórica por Jefe de Venta"
		rb_jefe.checked 							= true
		rb_supervisor.checked 					= false
		rb_agente.checked 						= false

	CASE '2' //supervisor
		this.title = "Informe Gestion Histórica por Supervisor"
		rb_jefe.checked 							= false
		rb_supervisor.checked 					= true
		rb_agente.checked 						= false

	CASE '3' //agente
		this.title = "Informe Gestion Histórica por Agente"
		rb_jefe.checked 							= false
		rb_supervisor.checked 					= false
...
gs_ventana											= 'w_inf_gestion_hist'
f_valida_objeto_1()


end event

on w_inf_gestion_hist_bkp.create
...
on w_inf_gestion_hist_bkp.destroy
...
if gd_fecha_inicio <= gd_fecha_fin then
	if em_codigo.text <> '' then
		CHOOSE CASE is_ventana
			CASE '1'
				if rb_promesas.checked=true then
					dw_detalle.dataobject = 'dw_ofertas_periodo_33_buscar_jefe_hist'
				else
					dw_detalle.dataobject = 'dw_aumento_periodo_33_buscar_jefe_hist'
				end if
				dw_detalle.settransobject(sqlca)
			CASE '2'
				if rb_promesas.checked=true then
					dw_detalle.dataobject = 'dw_ofertas_periodo_33_buscar_sup_hist'
				else
					dw_detalle.dataobject = 'dw_aumento_periodo_33_buscar_super_hist'
				end if
				dw_detalle.settransobject(sqlca)
			CASE '3'
				if rb_promesas.checked=true then
					dw_detalle.dataobject = 'dw_ofertas_periodo_33_buscar_agente_hist'
				else
					dw_detalle.dataobject = 'dw_aumento_periodo_33_buscar_agente_hist'
				end if
				dw_detalle.settransobject(sqlca)
		END CHOOSE
...
event clicked;if dw_informe.rowcount() > 0 then 
	...
	f_printdlg(dw_informe,gstr_print,parent)
...
integer width = 123
integer height = 116
string dataobject = "dw_aumento_periodo_33_buscar_jefe_hist"
...
em_codigo.setfocus()
is_ventana				= '3'
w_inf_gestion_hist.title 	= "Informe Gestion Histórica por Agente"
if dw_informe.dataobject = 'dw_informe' then
	dw_informe.object.c_por_agente.Visible	= 0
	dw_informe.object.c_clasif.Visible 		= 1
	dw_informe.object.c_rescomi.x				= 892
	dw_informe.object.c_clasif.x 				= 1527
	dw_informe.object.titulo.text 			= 'Informe Gestion Histórica Agente'
end if
end event
...
em_codigo.setfocus()
is_ventana				= '2'
w_inf_gestion_hist.title 	= "Informe Gestion Histórica por Supervisor"
...
if dw_informe.dataobject = 'dw_informe' then
	...
	dw_informe.object.c_rescomi.x				= 663
	dw_informe.object.c_por_agente.text 	= 'Por Agente'
	dw_informe.object.titulo.text 			= 'Informe Gestion Histórica Supervisor'
end if
end event
...
em_codigo.setfocus()
is_ventana											= '1'
w_inf_gestion_hist.title 	= "Informe Gestion Histórica por Jefe de Venta"
...
if dw_informe.dataobject = 'dw_informe' then
	...
	dw_informe.object.c_clasif.Visible 		= 0
	dw_informe.object.c_por_agente.text 	= 'Por Supervisor'
	dw_informe.object.titulo.text 			= 'Informe Gestion Histórica Jefe de Venta'
end if
end event
...
if trim(this.text) <> '' then
	CHOOSE CASE dw_informe.dataobject
		CASE 'dw_resumen_promesas_por_jefe_hist','dw_resumen_aumento_por_jefe_hist'
			if rb_promesas.checked=true then
				ll_row	= dw_informe.Find("oferta_v_cod_sup ='" +trim(this.text)+"'", 1, dw_informe.RowCount())
			elseif rb_aumento.checked=true then
				ll_row	= dw_informe.Find("anexo_aumento_cod_super ='" +trim(this.text)+"'", 1, dw_informe.RowCount())
			end if
			if ll_row > 0 then
				ls_string	= '1'
				...
				OpenWithParm(w_muestra_detalle_gestion_hist,ls_string)
			else
				messagebox("Advertencia","No existe Código")
			end if
		CASE 'dw_resumen_promesas_por_supervisor_hist','dw_resumen_aumento_por_supervisor_hist'
			if rb_promesas.checked=true then
				ll_row	= dw_informe.Find("oferta_v_cod_age ='" +trim(this.text)+"'", 1, dw_informe.RowCount())
			elseif rb_aumento.checked=true then
				ll_row	= dw_informe.Find("anexo_aumento_cod_agente ='" +trim(this.text)+"'", 1, dw_informe.RowCount())
			end if
			if ll_row > 0 then
				ls_string	= '2'
				ll_nro_caracter	= len(trim(this.text))
...
integer width = 3063
integer height = 1736
string dataobject = "dw_detalle_ciotas_pagadas"
...
if this.rowcount() > 0 then
		...
		CHOOSE CASE ls_columna
			...
			CASE 'c_por_agente'
				if this.object.c_por_agente.text = 'Por Supervisor' then
					if rb_promesas.checked=true then
						pb_ordenar.visible 		= true
						dw_informe.dataobject	= 'dw_resumen_promesas_por_jefe_hist'
						dw_informe.settransobject(sqlca)
						dw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
						dw_informe.object.titulo.text	= 'Resumen Histórico de Supervisores por Jefe Venta'
						ls_colx = dw_informe.describe("fec_ing_t.x")
						dw_informe.Modify("datawindow.horizontalscrollsplit=" + ls_colx)
						dw_informe.object.usuario.text = gs_user
					elseif rb_aumento.checked=true then
						pb_ordenar.visible 		= true
						dw_informe.dataobject	= 'dw_resumen_aumento_por_jefe_hist'
						dw_informe.settransobject(sqlca)
						dw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
						dw_informe.object.titulo.text	= 'Resumen Histórico de Supervisores por Jefe Venta'
						ls_colx = dw_informe.describe("fec_ing_t.x")
				...
				elseif this.object.c_por_agente.text = 'Por Agente' then
					if rb_promesas.checked=true then
						pb_ordenar.visible 		= true
						dw_informe.dataobject	= 'dw_resumen_promesas_por_supervisor_hist'
						dw_informe.settransobject(sqlca)
						dw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
						dw_informe.object.titulo.text	= 'Resumen Histórico de Agentes por Supervisor'
						ls_colx = dw_informe.describe("fec_ing_t.x")
						dw_informe.Modify("datawindow.horizontalscrollsplit=" + ls_colx)
						dw_informe.object.usuario.text = gs_user
					elseif rb_aumento.checked=true then
						pb_ordenar.visible 		= true
						dw_informe.dataobject	= 'dw_resumen_aumento_por_supervisor_hist'
						dw_informe.settransobject(sqlca)
						dw_informe.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
						dw_informe.object.titulo.text	= 'Resumen Histórico de Agentes por Supervisor'
						ls_colx = dw_informe.describe("fec_ing_t.x")
	...
	elseif this.dataobject = 'dw_resumen_promesas_por_jefe_hist' or this.dataobject = 'dw_resumen_aumento_por_jefe_hist' or &
		this.dataobject = 'dw_resumen_promesas_por_supervisor_hist' or this.dataobject = 'dw_resumen_aumento_por_supervisor_hist'then
		CHOOSE CASE ls_columna
			CASE 'c_volver'
				pb_ordenar.visible 		= false
				wf_cargar_gestion1(dw_informe)
...
if this.dataobject = 'dw_resumen_promesas_por_jefe_hist' or &
	this.dataobject = 'dw_resumen_aumento_por_jefe_hist' then 
	st_grupo.text		= ' Buscar Cód.Superv. '
 	st_grupo.visible 	= true
	em_grupo.visible 	= true
	em_grupo.setfocus()
end if
if this.dataobject = 'dw_resumen_promesas_por_supervisor_hist' or &
	this.dataobject = 'dw_resumen_aumento_por_supervisor_hist' then
	st_grupo.text		= ' Buscar Cód.Agente '
	st_grupo.visible = true
	em_grupo.visible = true
	em_grupo.setfocus()
...
event doubleclicked;String	ls_columna, ls_cadena, ls_codigo, ls_formula,ls_op_base
	...
	if this.dataobject = 'dw_informe' then
		...
		CHOOSE CASE ls_columna
			CASE 'tot_vtas','c_cien','tot_vtas_t'
				ls_cadena	= ls_cadena + '%'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
	
			CASE 'tot_nulas_n','c_por_resciliados','tot_nulas_n_t'
				ls_cadena	= ls_cadena + 'N'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
				
			CASE 'tot_resueltas','c_tot_resuelta','tot_resueltas_t'
				ls_cadena	= ls_cadena + 'R'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
				
			CASE 'tot_nulas_p','c_por_nulas_p','tot_nulas_p_t'
				ls_cadena	= ls_cadena + 'P'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
			CASE 'tot_canceladas','c_por_canc','tot_canceladas_t'
				ls_cadena	= ls_cadena + 'C'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
				
			CASE 'tot_vigentes','c_por_vigente','tot_vigentes_t'
				ls_cadena	= ls_cadena + 'V'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
				
			CASE 'tot_al_dia','porce_vig','tot_al_dia_t'
				ls_cadena	= ls_cadena + 'A'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
				
			CASE 'tot_moras','porce_mora','tot_en_mora_t'
				ls_cadena	= ls_cadena + 'M'+'~t'+ls_op_base
				OpenWithParm(w_detalle_totales_hist, ls_cadena)
	...
	elseif this.dataobject = 'dw_detalle_ciotas_pagadas' then
		if mid(ls_columna,1,1) = 'c' then
			...
			if ll_valor > 0 then
				...
				ls_formula			= ls_formula+'~t'+ls_cadena+'~t'+ls_columna+'~t'+ls_op_base
				if il_sw	= 0 then	openwithparm(w_muestra_detalle_cuota_hist,ls_formula)
			end if
		end if
	end if
end if
