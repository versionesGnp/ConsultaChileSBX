forward
global type w_muestra_detalle_gestion_hist from window
end type
type hpb_1 from hprogressbar within w_muestra_detalle_gestion_hist
end type
type st_porc from statictext within w_muestra_detalle_gestion_hist
end type
type dw_lista_agente from datawindow within w_muestra_detalle_gestion_hist
end type
type dw_lista from datawindow within w_muestra_detalle_gestion_hist
end type
type st_cod_agente from statictext within w_muestra_detalle_gestion_hist
end type
type em_cod_age from editmask within w_muestra_detalle_gestion_hist
end type
type cb_sort from commandbutton within w_muestra_detalle_gestion_hist
end type
type cb_exportar from commandbutton within w_muestra_detalle_gestion_hist
end type
type cb_imprimir from commandbutton within w_muestra_detalle_gestion_hist
end type
type cb_salir from commandbutton within w_muestra_detalle_gestion_hist
end type
type st_volver_age from statictext within w_muestra_detalle_gestion_hist
end type
type st_volver from statictext within w_muestra_detalle_gestion_hist
end type
type st_fondo from statictext within w_muestra_detalle_gestion_hist
end type
type dw_muestra from datawindow within w_muestra_detalle_gestion_hist
end type
end forward
...
cb_salir cb_salir
st_volver_age st_volver_age
st_volver st_volver
st_fondo st_fondo
dw_muestra dw_muestra
end type
global w_muestra_detalle_gestion_hist w_muestra_detalle_gestion_hist

type variables
datawindow dw_paso
Long il_empresa
string is_codigo, is_estado, is_conexion, is_opcion
...
public subroutine wf_informe (datawindow adw_lista, datawindow adw_muestra);long 		ll_tot_reg, ll_indi
...
for ll_indi = 1 to ll_tot_reg
	...
	elseif is_opcion='A' then
		ll_tot_cuotas_p	= adw_lista.getitemNumber(ll_indi,'anexo_aumento_cta_pag_m')
	end if
...
dw_muestra.object.titulo.text = 'Detalle Histórico de Cuotas en Mora'
if dw_muestra.dataobject = 'dw_informe_external_supervisor' then 
	dw_muestra.object.codigo_t.text = 'Supervisor'
...
if dw_muestra.dataobject = 'dw_informe_external_agente' then 
	dw_muestra.object.codigo_t.text = 'Agente'
	dw_muestra.setitem(al_fila,'codigo',is_codigo_age)
	dw_muestra.setitem(al_fila,'nombre',is_nom_age)
	dw_muestra.setitem(al_fila,'estado',is_estado_age)
end if

end subroutine

public subroutine wf_volver_sup ();if is_opcion='O' then
	w_muestra_detalle_gestion_hist.title	= "Resumen Histórico de Agentes por Supervisor"
	dw_muestra.dataobject						= 'dw_resumen_promesas_por_gral_sup_hist'
	dw_muestra.settransobject(sqlca)
	dw_muestra.object.titulo.text 			= 'Resumen Histórico de Agentes por Supervisor'
	dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
	dw_muestra.object.usuario.text 			= gs_user
	dw_muestra.object.tot_vtas_t.text		= 'Total~rPromesas'
	em_cod_age.visible							= false
	st_cod_agente.visible						= false
	cb_salir.visible								= true
elseif is_opcion='A' then
	w_muestra_detalle_gestion_hist.title	= "Resumen Histórico de Agentes por Supervisor"
	dw_muestra.dataobject						= 'dw_resumen_aumento_por_gral_sup_hist'
	dw_muestra.settransobject(sqlca)
	dw_muestra.object.titulo.text 			= 'Resumen Histórico de Agentes por Supervisor'
	dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
	dw_muestra.object.usuario.text 			= gs_user
	dw_muestra.object.tot_vtas_t.text		= 'Total~rAum.Cap.'
	em_cod_age.visible							= false
	st_cod_agente.visible						= false
	cb_salir.visible								= true
end if
...
on w_muestra_detalle_gestion_hist.create
...
this.st_volver_age=create st_volver_age
...
on w_muestra_detalle_gestion_hist.destroy
destroy(this.hpb_1)
...
gf_centrar(w_muestra_detalle_gestion_hist)
em_cod_age.visible		= false
st_cod_agente.visible	= false
...
ls_texto 					= substr(1,1,Message.StringParm)
is_opcion					= substr(1,2,Message.StringParm)
...
if is_conexion = '1' then // jefe
	if is_opcion='O' then
		this.title									= "Resumen Histórico de Agentes por Supervisor"
		dw_muestra.dataobject					= 'dw_resumen_promesas_por_gral_sup_hist'
		dw_muestra.settransobject(sqlca)
		dw_muestra.object.tot_vtas_t.text	= 'Total~rPromesas'
...
if is_conexion = '2' then // agente
	if is_opcion='O' then
		this.title									= "Resumen Histórico de Promesas por Agente"
		dw_muestra.dataobject					= 'dw_resumen_promesas_por_agente_hist'
		dw_muestra.settransobject(sqlca)
		dw_muestra.object.tot_vtas_t.text	= 'Total~rPromesas'
		if dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa) = 0 then 
			cb_salir.triggerevent(clicked!)
		else
			dw_muestra.object.titulo.text 	= 'Resumen Histórico de Promesas por Agente'
			dw_muestra.object.usuario.text 	= gs_user
		end if
		cb_sort.visible							= false
	elseif is_opcion='A' then
		this.title									= "Resumen Histórico de Aumento Capacidad por Agente"
		dw_muestra.dataobject					= 'dw_resumen_aumento_por_agente_hist'
		dw_muestra.settransobject(sqlca)
		dw_muestra.object.tot_vtas_t.text	= 'Total~rAum.Cap.'
		if dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa) = 0 then 
			cb_salir.triggerevent(clicked!)
		else
			dw_muestra.object.titulo.text 	= 'Resumen Histórico de Aumento Capacidad por Agente'
			dw_muestra.object.usuario.text 	= gs_user
		end if
		cb_sort.visible							= false
	end if
end if
gs_ventana											= 'w_muestra_detalle_gestion_hist'
f_valida_objeto_1()
end event
...
integer height = 108
integer taborder = 70
string dataobject = "dw_aumento_periodo_agente"
...
integer height = 116
integer taborder = 50
string dataobject = "dw_aumento_periodo_supervisor"
boolean livescroll = true
end type
...
if trim(this.text) <> '' then
	...
	CHOOSE CASE dw_muestra.dataobject
		CASE 'dw_resumen_promesas_por_supervisor_hist','dw_resumen_aumento_por_supervisor_hist'
			if is_opcion='O' then
				ll_row	= dw_muestra.Find("oferta_v_cod_age ='" +trim(this.text)+"'", 1, dw_muestra.RowCount())
			elseif is_opcion='A' then
				ll_row	= dw_muestra.Find("anexo_aumento_cod_agente ='" +trim(this.text)+"'", 1, dw_muestra.RowCount())
			end if
			if ll_row > 0 then
				ls_string	= '2'
				...
				ls_string			= ls_string+'~t'+is_opcion
				OpenWithParm(w_muestra_detalle_gestion_agente_hist,ls_string)
			else
...
if is_opcion='O' then
	cb_salir.visible								= true
	w_muestra_detalle_gestion_hist.title	= "Resumen Histórico de Promesas por Agente"
	dw_muestra.dataobject						= 'dw_resumen_promesas_por_agente_hist'
	dw_muestra.settransobject(sqlca)
	if dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa) = 0 then 
		cb_salir.triggerevent(clicked!)
...
elseif is_opcion='A' then
	cb_salir.visible								= true
	w_muestra_detalle_gestion_hist.title	= "Resumen Histórico de Aumento Capacidad por Agente"
	dw_muestra.dataobject						= 'dw_resumen_aumento_por_agente_hist'
	dw_muestra.settransobject(sqlca)
	if dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa) = 0 then 
		cb_salir.triggerevent(clicked!)
...
integer height = 1760
integer taborder = 80
string dataobject = "dw_resumen_aumento_por_agente_hist"
...
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_gral_sup_hist' or this.dataobject = 'dw_resumen_aumento_por_gral_sup_hist' then
		...
		CHOOSE CASE ls_columna
			...
			CASE 'c_detalle'
				if is_opcion='O' then
					dw_muestra.dataobject	= 'dw_resumen_promesas_por_supervisor_hist'
				elseif is_opcion='A' then
					dw_muestra.dataobject	= 'dw_resumen_aumento_por_supervisor_hist'
				end if
				dw_muestra.settransobject(sqlca)
			...
			CASE 'c_moras'
				...
				cb_salir.visible			= false
				if is_opcion='O' then
					dw_lista.dataobject	= 'dw_ofertas_periodo_supervisor_hist'
				elseif is_opcion='A' then
					dw_lista.dataobject	= 'dw_aumento_periodo_supervisor_hist'
				end if 
				dw_lista.settransobject(sqlca)
	...
	elseif this.dataobject = 'dw_resumen_promesas_por_agente_hist' or this.dataobject = 'dw_resumen_aumento_por_agente_hist' then
		f_color_titulo(dw_muestra,ls_columna)
		CHOOSE CASE ls_columna
			CASE 'c_grafico'
				if this.object.c_grafico.text = 'Mostrar Gráfico' then
			...
			CASE 'c_moras'
				...
				cb_salir.visible					= false
				if is_opcion='O' then
					dw_lista_agente.dataobject	= 'dw_ofertas_periodo_agente_hist'
				elseif is_opcion='A' then
					dw_lista_agente.dataobject	= 'dw_aumento_periodo_agente_hist'
				end if
				dw_lista_agente.settransobject(sqlca)
			...
			CASE 'c_rescomi'
				cb_salir.visible			= false
				st_volver_age.visible	= true
				is_codigo_age				= this.getitemstring(1,'agentes_cod_age')
				is_estado_age				= this.getitemstring(1,'agentes_estado')
				il_rut_agente				= this.getitemNumber(1,'agentes_rut')
				is_nom_age					= this.getitemstring(1,'nom_comp_age')
				id_fec_ini_age				= date(this.getitemDatetime(1,'agentes_fecha_ini'))
				dw_muestra.dataobject 	= 'dw_lista_res_comi'
				dw_muestra.settransobject(sqlca)
				dw_muestra.retrieve(is_codigo_age)
				dw_muestra.object.c_codigo.text		= is_codigo_age
				dw_muestra.object.c_estado.text		= is_estado_age
				dw_muestra.object.c_rut.text			= string(il_rut_agente)
				dw_muestra.object.c_nombre.text		= is_nom_age
				dw_muestra.object.c_fecha_ini.text	= string(id_fec_ini_age)
			CASE 'c_clasif'
				cb_salir.visible			= false
				st_volver_age.visible	= true
				is_codigo_age				= this.getitemstring(1,'agentes_cod_age')
				...
				is_nom_age					= this.getitemstring(1,'nom_comp_age')
				id_fec_ini_age				= date(this.getitemDatetime(1,'agentes_fecha_ini'))
				dw_muestra.dataobject 	= 'dw_lista_clasif_hist'  //agente
				dw_muestra.settransobject(sqlca)
				dw_muestra.retrieve(is_codigo_age)
				dw_muestra.object.c_codigo.text		= is_codigo_age
				dw_muestra.object.c_estado.text		= is_estado_age
	...
	elseif this.dataobject = 'dw_resumen_promesas_por_supervisor_hist' then
		em_cod_age.setfocus()
	end if
end if
end event
...
ls_columna	= dwo.name
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_gral_sup_hist' or dw_muestra.dataobject	= 'dw_resumen_promesas_por_agente_hist' or &
		this.dataobject = 'dw_resumen_aumento_por_gral_sup_hist' or dw_muestra.dataobject	= 'dw_resumen_aumento_por_agente_hist' then
		ll_nro_caracter	= len(trim(is_codigo))
		ll_nro_caracter	= (5 - ll_nro_caracter)
		is_codigo			= trim(is_codigo) + space(ll_nro_caracter)
		if dw_muestra.dataobject = 'dw_resumen_promesas_por_gral_sup_hist' or dw_muestra.dataobject = 'dw_resumen_aumento_por_gral_sup_hist' then ls_cadena = '2'
		if dw_muestra.dataobject = 'dw_resumen_promesas_por_agente_hist' or dw_muestra.dataobject = 'dw_resumen_aumento_por_agente_hist' then	ls_cadena = '3'
	...
	elseif this.dataobject = 'dw_informe_external_supervisor' or this.dataobject = 'dw_informe_external_agente' then
		if mid(ls_columna,1,1) = 'c' then
			...
			if ll_valor > 0 then
				...
				ls_formula			= ls_formula+'~t'+ls_cadena+'~t'+ls_columna+'~t'+is_opcion
