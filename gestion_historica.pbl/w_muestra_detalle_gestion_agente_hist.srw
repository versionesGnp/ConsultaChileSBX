forward
global type w_muestra_detalle_gestion_agente_hist from window
end type
type hpb_1 from hprogressbar within w_muestra_detalle_gestion_agente_hist
end type
type st_porc from statictext within w_muestra_detalle_gestion_agente_hist
end type
type dw_lista from datawindow within w_muestra_detalle_gestion_agente_hist
end type
type cb_sort from commandbutton within w_muestra_detalle_gestion_agente_hist
end type
type cb_exportar from commandbutton within w_muestra_detalle_gestion_agente_hist
end type
type cb_imprimir from commandbutton within w_muestra_detalle_gestion_agente_hist
end type
type cb_salir from commandbutton within w_muestra_detalle_gestion_agente_hist
end type
type st_volver from statictext within w_muestra_detalle_gestion_agente_hist
end type
type st_fondo from statictext within w_muestra_detalle_gestion_agente_hist
end type
type dw_muestra from datawindow within w_muestra_detalle_gestion_agente_hist
end type
end forward
...
global w_muestra_detalle_gestion_agente_hist w_muestra_detalle_gestion_agente_hist

type variables
datawindow dw_paso
Long il_empresa, il_sw
...
String is_codigo_age,is_codigo_sup,is_codigo_jef,is_estado_age
String is_estado_sup,is_clasif,is_clasif_sup
Long il_rut_agente,il_rut_supervisor
...
forward prototypes
public subroutine wf_rescatar_datos (datawindow adw_lista)
...
public subroutine wf_rescatar_datos (datawindow adw_lista);is_nom_jefe			= adw_lista.getitemstring(1,'c_nom_jef')
is_nom_sup			= adw_lista.getitemstring(1,'c_nom_sup')
is_nom_age			= adw_lista.getitemstring(1,'c_nom_age')
id_fec_ini_age		= date(adw_lista.getitemdatetime(1,'agentes_fecha_ini'))
id_fec_fin_age		= date(adw_lista.getitemdatetime(1,'agentes_fecha_fin'))
id_fec_ini_sup		= date(adw_lista.getitemdatetime(1,'supervisor_fecha_ing'))
id_fec_fin_sup		= date(adw_lista.getitemdatetime(1,'supervisor_fecha_fin'))
is_codigo_age		= adw_lista.getitemstring(1,'agentes_cod_age')
is_codigo_sup		= adw_lista.getitemstring(1,'supervisor_cod_sup')
is_codigo_jef		= adw_lista.getitemstring(1,'jefe_ventas_jefe_ventas')
is_estado_age		= adw_lista.getitemstring(1,'agentes_estado')
is_estado_sup		= adw_lista.getitemstring(1,'supervisor_estado')
is_clasif			= adw_lista.getitemstring(1,'agentes_clasificacion')
is_clasif_sup		= adw_lista.getitemstring(1,'supervisor_clasifica')
il_rut_agente		= adw_lista.getitemNumber(1,'agentes_rut')
il_rut_supervisor	= adw_lista.getitemNumber(1,'supervisor_rut')
end subroutine
...
if adw_muestra.dataobject = 'dw_informe_external_agente' then
...
on w_muestra_detalle_gestion_agente_hist.create
...
on w_muestra_detalle_gestion_agente_hist.destroy
destroy(this.hpb_1)
...
gf_centrar(w_muestra_detalle_gestion_agente_hist)
...
if is_conexion = '2' then // agente
	if is_opcion='O' then
		this.title									= "Resumen Histórica de Promesas por Agente"
	...
	elseif is_opcion='A' then
		this.title									= "Resumen Histórica de Aumento Capacidad por Agente"
...
gs_ventana											= 'w_muestra_detalle_gestion_agente_hist'
f_valida_objeto_1()
end event
...
event clicked;if is_opcion='O' then
...
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_agente_hist' or this.dataobject = 'dw_resumen_aumento_por_agente_hist' then
...
event clicked;String	ls_columna, ls_cadena
Int		li_new
	...
	if this.dataobject = 'dw_resumen_promesas_por_agente_hist' or this.dataobject = 'dw_resumen_aumento_por_agente_hist' then
		...
		CHOOSE CASE ls_columna
			...
			CASE 'c_moras'
				...
				cb_salir.visible			= false
				if is_opcion='O' then
					dw_lista.dataobject	= 'dw_ofertas_periodo_agente_hist'
				elseif is_opcion='A' then
					dw_lista.dataobject	= 'dw_aumento_periodo_agente_hist'
				end if
				dw_lista.settransobject(sqlca)
