forward
global type w_inf_gestion_hist from window
end type
type rb_promesas from radiobutton within w_inf_gestion_hist
end type
type rb_aumento from radiobutton within w_inf_gestion_hist
end type
type hpb_1 from hprogressbar within w_inf_gestion_hist
end type
type st_porc from statictext within w_inf_gestion_hist
end type
type st_grupo from statictext within w_inf_gestion_hist
end type
type pb_ordenar from picturebutton within w_inf_gestion_hist
end type
type pb_exportar from picturebutton within w_inf_gestion_hist
end type
type pb_aceptar from picturebutton within w_inf_gestion_hist
end type
type pb_imprimir from picturebutton within w_inf_gestion_hist
end type
type pb_salir from picturebutton within w_inf_gestion_hist
end type
type dw_detalle from datawindow within w_inf_gestion_hist
end type
type st_nombre from statictext within w_inf_gestion_hist
end type
type pb_buscar from picturebutton within w_inf_gestion_hist
end type
type em_codigo from editmask within w_inf_gestion_hist
end type
type em_1 from editmask within w_inf_gestion_hist
end type
type p_1 from picture within w_inf_gestion_hist
end type
type em_2 from editmask within w_inf_gestion_hist
end type
type p_2 from picture within w_inf_gestion_hist
end type
type st_1 from statictext within w_inf_gestion_hist
end type
type st_2 from statictext within w_inf_gestion_hist
end type
type rb_agente from radiobutton within w_inf_gestion_hist
end type
type rb_supervisor from radiobutton within w_inf_gestion_hist
end type
type rb_jefe from radiobutton within w_inf_gestion_hist
end type
type dw_lista from datawindow within w_inf_gestion_hist
end type
type gb_2 from groupbox within w_inf_gestion_hist
end type
type gb_1 from groupbox within w_inf_gestion_hist
end type
type em_grupo from editmask within w_inf_gestion_hist
end type
type gb_3 from groupbox within w_inf_gestion_hist
end type
type st_fondo from statictext within w_inf_gestion_hist
end type
type st_volver from statictext within w_inf_gestion_hist
end type
type dw_informe from datawindow within w_inf_gestion_hist
end type
type pb_licencias from picturebutton within w_inf_gestion_hist
end type
type gb_4 from groupbox within w_inf_gestion_hist
end type
end forward
...
global w_inf_gestion_hist w_inf_gestion_hist
...
if ls_cod <> '' then
	...
	if is_cod_ejec	= 'J' then
		if rb_supervisor.checked=true then
			...
			INTO 		:ls_cod_aux  
			FROM		"SUPERVISOR"  
			WHERE  	"SUPERVISOR"."COD_SUP" = :ls_cod
			USING		sqlca;
			if gs_user <> ls_cod_aux then
				messagebox("Advertencia","Supervisor No Corresponde al Jefe de Venta "+gs_user)
				ls_pasa			= 'N'
				em_codigo.text	= ''
			end if
		...
		elseif rb_agente.checked=true then
			...
			WHERE  	"SUPERVISOR"."COD_JEFE" = "JEFE_VENTAS"."JEFE_VENTAS" and  
					 	...
					 	"AGENTES"."COD_AGE" = :ls_cod
			USING		sqlca;
			if gs_user <> ls_cod_aux then
				messagebox("Advertencia","Agente No Corresponde al Jefe de Venta "+gs_user)
				ls_pasa			= 'N'
				em_codigo.text	= ''
			end if
	...
	elseif is_cod_ejec = 'S' then
		if rb_agente.checked=true then
			...
			INTO 		:ls_cod_aux  
			FROM 		"AGENTES"  
			WHERE  	"AGENTES"."COD_AGE" = :ls_cod
			USING		sqlca;
			if gs_user <> ls_cod_aux then
				messagebox("Advertencia","Agente No Corresponde al Supervisor "+gs_user)
				ls_pasa			= 'N'
				em_codigo.text	= ''
			end if
		end if
