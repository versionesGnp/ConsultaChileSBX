forward
global type w_premio_incremento_comi_jefe from window
end type
type cb_filtrar from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_2 from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_1 from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_exportar from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_actualiza_pag from commandbutton within w_premio_incremento_comi_jefe
end type
type em_termino from editmask within w_premio_incremento_comi_jefe
end type
type st_cuenta from statictext within w_premio_incremento_comi_jefe
end type
type st_porc from statictext within w_premio_incremento_comi_jefe
end type
type hpb_1 from hprogressbar within w_premio_incremento_comi_jefe
end type
type cb_actualiz from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_cta_cte from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_respal from commandbutton within w_premio_incremento_comi_jefe
end type
type dw_fecha_cierre from datawindow within w_premio_incremento_comi_jefe
end type
type dw_parque from datawindow within w_premio_incremento_comi_jefe
end type
type cb_premio_pago from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_grabar from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_imprimir from commandbutton within w_premio_incremento_comi_jefe
end type
type cb_cerrar from commandbutton within w_premio_incremento_comi_jefe
end type
type pb_aceptar from picturebutton within w_premio_incremento_comi_jefe
end type
type dw_actualiza from datawindow within w_premio_incremento_comi_jefe
end type
type st_fondo from statictext within w_premio_incremento_comi_jefe
end type
type dw_premio_jefe from datawindow within w_premio_incremento_comi_jefe
end type
type gb_1 from groupbox within w_premio_incremento_comi_jefe
end type
end forward
...
global w_premio_incremento_comi_jefe w_premio_incremento_comi_jefe
...
public function long wf_cta_vtas (datetime adt_fecha_cierre, string as_cod_jefe, double adb_porce_ing, long al_cod_parque, long al_unidad, string as_canal)
public function long wf_calcula_premio (double adb_factor, datetime adt_fec_bono, string as_cod_jef, string as_tabla, string as_canal_sup)
public function double wf_producc_total (datetime adt_fec_comi, string as_cod_jef, string as_tabla, string as_canal_sup)
public function double wf_vta_equival (datetime adt_fecha_cierre, string as_cod_jefe, string as_canal, double adb_porce_ing, long al_new)
end prototypes
...
public subroutine wf_lista_premiados (datetime adt_fecha_cierre, long al_cod_parque, string as_tabla);string		ls_cod_jef,ls_nombre,ls_a_pater,ls_a_mater,ls_canal,ls_descrip
long		ll_cta_vtas_ugn,ll_mes,ll_ano,ll_rut,ll_unidad,ll_vtas_pres,ll_cta_vtas,ll_new,ll_mes_comi,ll_ano_comi,ll_presu_equival
datetime	ldt_fec_comi
double	ldb_porc_ing,ldb_tot_prod,ldb_porc,ldb_factor,ldb_premio
...
SELECT		"JEFE_VENTAS"."JEFE_VENTAS","JEFE_VENTAS"."NOMBRE","JEFE_VENTAS"."A_PATERNO","JEFE_VENTAS"."A_MATERNO","JEFE_VENTAS"."RUT","PRESUPUESTO_UNIDAD_VENTAS"."CANAL","PRESUPUESTO"."UNIDAD_VENTAS","PRESUPUESTO_UNIDAD_VENTAS"."PORCE_ING_CIERRE","PRESUPUESTO"."CANTIDAD_VENTAS","PRESUPUESTO_UNIDAD_VENTAS"."DESCRIPCION","PRESUPUESTO"."NRO_VENTA_EQUIVAL" 
...
ORDER BY	"JEFE_VENTAS"."JEFE_VENTAS"
USING		Trans_1;
open x1;
if Trans_1.sqlcode=0 then
	DO WHILE Trans_1.sqlcode=0
	fetch x1 INTO :ls_cod_jef,:ls_nombre,:ls_a_pater,:ls_a_mater,:ll_rut,:ls_canal, :ll_unidad, :ldb_porc_ing,:ll_vtas_pres,:ls_descrip, :ll_presu_equival;
		if not isnull(ls_cod_jef) and  ls_cod_jef<>'' and not isnull(ll_rut) and ll_rut>0 then
			
			ll_new									= dw_premio_jefe.insertrow(0)
			dw_premio_jefe.scrolltorow(ll_new)
			dw_premio_jefe.setitem(ll_new,'cod_jefe',ls_cod_jef)
			...
			if adt_fecha_cierre<=datetime(date(string('31/12/2016'))) then
				ll_cta_vtas							= wf_cta_vtas(adt_fecha_cierre,ls_cod_jef,ldb_porc_ing,al_cod_parque,ll_unidad,ls_canal)
				if ll_cta_vtas>0 then
					ldb_porc							= round(ll_cta_vtas / ll_vtas_pres * 100,2)
				else
					ldb_porc							= 0
				end if
			else
				ll_cta_vtas							= wf_vta_equival(adt_fecha_cierre,ls_cod_jef,ls_canal,ldb_porc_ing,ll_new)
				if ll_cta_vtas>0 then
					ldb_porc							= round(ll_cta_vtas / ll_presu_equival * 100,2)
				else
					ldb_porc							= 0
			end if
				
			end if
			ldb_tot_prod							= wf_producc_total(ldt_fec_comi,ls_cod_jef,as_tabla,ls_canal)
			
			SELECT	"FACTOR"  
			INTO		:ldb_factor  
			FROM 	"PREMIO_INCREM_COMI_JEFE"  
			WHERE  	( "PREMIO_INCREM_COMI_JEFE"."TRAMO_INICIO" <= :ldb_porc ) AND  
						( "PREMIO_INCREM_COMI_JEFE"."TRAMO_FIN" > :ldb_porc ) AND
						( "PREMIO_INCREM_COMI_JEFE"."CANAL" = :ls_canal )
			USING	sqlca;
			if isnull(ldb_factor) then ldb_factor=0
			
			ldb_premio								= wf_calcula_premio(ldb_porc,ldt_fec_comi,ls_cod_jef,as_tabla,ls_canal)
			
			dw_premio_jefe.setitem(ll_new,'porce_ing_cierre',ldb_porc_ing)
			dw_premio_jefe.setitem(ll_new,'vtas_presupuesto',ll_vtas_pres)
			dw_premio_jefe.setitem(ll_new,'vtas_aquivalente',ll_presu_equival)
			
			dw_premio_jefe.setitem(ll_new,'vtas_producc',ll_cta_vtas)
			dw_premio_jefe.setitem(ll_new,'porce_pago',ldb_porc)
			dw_premio_jefe.setitem(ll_new,'factor',ldb_factor)
			dw_premio_jefe.setitem(ll_new,'producc_uf',ldb_tot_prod)
		...
		setnull(ls_cod_jef);setnull(ls_nombre);setnull(ls_a_pater);setnull(ls_a_mater);setnull(ll_rut);setnull(ls_canal);	setnull(ll_unidad);setnull(ldb_porc_ing);setnull(ll_vtas_pres);setnull(ls_descrip)
	LOOP
end if
close x1;
...
public function long wf_cta_vtas (datetime adt_fecha_cierre, string as_cod_jefe, double adb_porce_ing, long al_cod_parque, long al_unidad, string as_canal);string	ls_tipo_vta,ls_canal_ni
long		ll_cta_vtas
...
WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
			...
			("VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :as_canal or "VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :ls_canal_ni) AND
...
public function long wf_calcula_premio (double adb_factor, datetime adt_fec_bono, string as_cod_jef, string as_tabla, string as_canal_sup);double	ldb_premio

//wf_calcula_premio(adb_factor,adt_fec_bono,as_cod_jef,as_tabla)
if as_tabla='O' then //OFERTA_V
	SELECT	SUM(ROUND("VISTA_PRODUCC_MENSUAL"."PRECIO_UF" * "VISTA_PRODUCC_MENSUAL"."VALOR_UF" * "PREMIO_INCREM_COMI_JEFE"."FACTOR" / 100,0))
	INTO		:ldb_premio	
	FROM		"VISTA_PRODUCC_MENSUAL","PREMIO_INCREM_COMI_JEFE"
	WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
				"PREMIO_INCREM_COMI_JEFE"."CANAL" = "VISTA_PRODUCC_MENSUAL"."CANAL_JEF" and
				"VISTA_PRODUCC_MENSUAL"."FECHA_BONO_JEF" = :adt_fec_bono AND
				"VISTA_PRODUCC_MENSUAL"."COD_JEF" = :as_cod_jef and
				"PREMIO_INCREM_COMI_JEFE"."TRAMO_INICIO" <= :adb_factor and
				"PREMIO_INCREM_COMI_JEFE"."TRAMO_FIN" > :adb_factor and
				 "PREMIO_INCREM_COMI_JEFE"."CANAL" = :as_canal_sup
	USING	sqlca;
elseif  as_tabla='R' then //RES_COMI
	SELECT	SUM(round("RES_COMI"."TOTAL_PAGO",0))
	INTO		:ldb_premio
	FROM		"VISTA_PRODUCC_MENSUAL","RES_COMI"    
	WHERE	"VISTA_PRODUCC_MENSUAL"."BASE" = "RES_COMI"."BASE" AND  
				"VISTA_PRODUCC_MENSUAL"."SERIE" = "RES_COMI"."SERIE" AND  
				"VISTA_PRODUCC_MENSUAL"."NUMERO" = "RES_COMI"."CONTRATO" AND  
...
public function double wf_producc_total (datetime adt_fec_comi, string as_cod_jef, string as_tabla, string as_canal_sup);string		ls_canal_ni
double	ldb_producc

//wf_producc_total(adt_fec_comi,as_cod_jef,as_tabla)


if as_canal_sup='UG' then
	ls_canal_ni						= 'NI'
else
	ls_canal_ni						= as_canal_sup
end if


if as_tabla='O' then
	SELECT	SUM("VISTA_PRODUCC_MENSUAL"."PRECIO_UF")
	INTO		:ldb_producc	
	FROM		"VISTA_PRODUCC_MENSUAL"
	WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
				"VISTA_PRODUCC_MENSUAL"."FECHA_BONO_JEF" = :adt_fec_comi AND
				("VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :as_canal_sup or "VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :ls_canal_ni) AND 
				"VISTA_PRODUCC_MENSUAL"."COD_JEF" = :as_cod_jef
	USING		sqlca;
...
public function double wf_vta_equival (datetime adt_fecha_cierre, string as_cod_jefe, string as_canal, double adb_porce_ing, long al_new);string		ls_canal_ni
long		ll_cta_a,ll_cta_b,ll_cta_c,ll_cta_vtas
double	ldb_valor_vta_a,ldb_valor_vta_b,ldb_valor_vta_c

//wf_vta_equival(adt_fecha_cierre,as_cod_jefe,as_canal,adb_porce_ing)
...
WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
			...
			"VISTA_PRODUCC_MENSUAL"."CLASIFICA_VENTA" = 'A'
USING		sqlca;	
if isnull(ll_cta_a) then ll_cta_a=0
...
WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
			...
			("VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :as_canal or "VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :ls_canal_ni) AND
			"VISTA_PRODUCC_MENSUAL"."PORCE_ING_CIERRE" >= :adb_porce_ing AND
			"VISTA_PRODUCC_MENSUAL"."CLASIFICA_VENTA" = 'B'
USING		sqlca;	
if isnull(ll_cta_b) then ll_cta_b=0
...
WHERE 	("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
			...
			("VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :as_canal or "VISTA_PRODUCC_MENSUAL"."CANAL_SUP" = :ls_canal_ni) AND
			"VISTA_PRODUCC_MENSUAL"."PORCE_ING_CIERRE" >= :adb_porce_ing AND
			"VISTA_PRODUCC_MENSUAL"."CLASIFICA_VENTA" = 'C'
USING		sqlca;	
if isnull(ll_cta_c) then ll_cta_c=0
	
SELECT	"CLASIFICACION"."VALOR_VTA_EQUIVALENTE"
INTO		:ldb_valor_vta_a
FROM		"CLASIFICACION"  
WHERE	( "CLASIFICACION"."ESTADO" = 'V' ) AND  
       		( "CLASIFICACION"."TIPO_CLASIFICA" = 2 )  AND
	 		( "CLASIFICACION"."CLASIFICA" = 'A' )
USING		sqlca;	
if isnull(ldb_valor_vta_a) then ldb_valor_vta_a=0
	
SELECT	"CLASIFICACION"."VALOR_VTA_EQUIVALENTE"
INTO		:ldb_valor_vta_b
FROM		"CLASIFICACION"  
WHERE	( "CLASIFICACION"."ESTADO" = 'V' ) AND  
        		( "CLASIFICACION"."TIPO_CLASIFICA" = 2 )  AND
	 		( "CLASIFICACION"."CLASIFICA" = 'B' )
USING		sqlca;	
if isnull(ldb_valor_vta_b) then ldb_valor_vta_b=0
	
SELECT	"CLASIFICACION"."VALOR_VTA_EQUIVALENTE"
INTO		:ldb_valor_vta_c
FROM		"CLASIFICACION"  
WHERE	( "CLASIFICACION"."ESTADO" = 'V' ) AND  
        		( "CLASIFICACION"."TIPO_CLASIFICA" = 2 )  AND
	 		( "CLASIFICACION"."CLASIFICA" = 'C' )
USING		sqlca;	
if isnull(ldb_valor_vta_c) then ldb_valor_vta_c=0

ldb_valor_vta_a	= round(ll_cta_a * ldb_valor_vta_a,0)
ldb_valor_vta_b	= round(ll_cta_b * ldb_valor_vta_b,0)
ldb_valor_vta_c		= round(ll_cta_c * ldb_valor_vta_c,0)
	
ll_cta_vtas			=  ldb_valor_vta_a + ldb_valor_vta_b + ldb_valor_vta_c

dw_premio_jefe.setitem(al_new,'cta_a',ll_cta_a)
dw_premio_jefe.setitem(al_new,'cta_b',ll_cta_b)
dw_premio_jefe.setitem(al_new,'cta_c',ll_cta_c)

dw_premio_jefe.setitem(al_new,'valor_a',ldb_valor_vta_a)
dw_premio_jefe.setitem(al_new,'valor_b',ldb_valor_vta_b)
dw_premio_jefe.setitem(al_new,'valor_c',ldb_valor_vta_c)
dw_premio_jefe.accepttext()
...
on w_premio_incremento_comi_jefe.create
...
on w_premio_incremento_comi_jefe.destroy
...
setnull (nulo)
dw_premio_jefe.SETfilter(NULO)
dw_premio_jefe.filter()
end event
...
event clicked;//string	ls_tipo_vta
...
long		ll_graba,ll_tot_reg,ll_res,ll_dia,ll_indi
		...
		if ll_res = 1 then
			...
			if ll_tot_reg>0 then
				...
				for ll_indi=1 to ll_tot_reg
					...
					ldb_numero				= dw_actualiza.getitemnumber(ll_indi,'cadena_numero')
...
event clicked;string		ls_graba='N',ls_base,ls_serie
long		ll_tot_reg,ll_res,ll_graba=0,ll_mes,ll_ano
...
if not isnull(ldt_fecha_cierre) then
	...
	if ll_res=1 then
		...
		INTO		:ll_tot_reg
		FROM		"VISTA_PRODUCC_MENSUAL"  
		WHERE	"VISTA_PRODUCC_MENSUAL"."FECHA_BONO_JEF" <= :ldt_fec_comi AND  
					"VISTA_PRODUCC_MENSUAL"."ESTADO_BONO_JEF" = 'N'
		USING	sqlca;
...
CHOOSE CASE ls_columna
	CASE 'cierre_ventas'
		...
		INTO		:ll_cta
   		FROM		"VISTA_PRODUCC_MENSUAL"  
   		WHERE 	( "VISTA_PRODUCC_MENSUAL"."FECHA_BONO_JEF" =:ldt_fec_fin ) AND  
         			( "VISTA_PRODUCC_MENSUAL"."ESTADO_BONO_JEF" = 'N' )
		USING	sqlca;
...
//		ldt_fec_7porc						= datetime(date(string('15/01/2017')))
//		gdt_fec_sistema					= datetime(date(string('17/02/2017')))
		
		SELECT to_date(add_months( to_date(:ldt_fec_7porc), +1 )) INTO :ldt_fec_7porc FROM dual;
		ldt_fec_7porc						= 	datetime((RelativeDate(date(ldt_fec_7porc), + 2)),time('00:00:00'))	
		
		
		if ldt_fec_fin >= gdt_fec_sistema then
			...
			if ldt_fec_7porc <= gdt_fec_sistema and gl_proceso>=4 then
...
event clicked;string		ls_cod_jef,ls_canal,ls_cod_cont,ls_serie,ls_haber,ls_moneda,ls_codigo_com,ls_estado,ls_cod_age_sup,ls_base
...
datetime	ldt_fecha_cierre,ldt_fec_comi,ldt_fecha_ini
...
if ll_tot_reg=0 then
	...
	if ll_res = 1 then
			...
			if sqlca.sqlcode=0 then
				if ll_count=0 then
					if not isnull(ls_cod_jef) and ls_cod_jef<>'' and not isnull(ls_canal) and ls_canal<>'' and not isnull(ldt_fecha_cierre) then
						DECLARE x1 CURSOR FOR  
						SELECT	"VISTA_PRODUCC_MENSUAL"."COD_JEF",
									"JEFE_VENTAS"."RUT",
									(	SELECT 	sum("INGRESO"."MONTO")
										FROM		"INGRESO"
										WHERE	"INGRESO"."BASE" = "VISTA_PRODUCC_MENSUAL"."BASE"  AND
													"INGRESO"."SERIE" = "VISTA_PRODUCC_MENSUAL"."SERIE" AND
													"INGRESO"."CONTRATO" = "VISTA_PRODUCC_MENSUAL"."NUMERO" AND
													("INGRESO"."TIPO_MOV" = 'A' OR  "INGRESO"."TIPO_MOV" = 'E') AND
													"INGRESO"."FECHA_PAGO" <= :ldt_fec_comi) as tot_pagado, 
									"VISTA_PRODUCC_MENSUAL"."NUMERO",
									"VISTA_PRODUCC_MENSUAL"."SERIE",
									round((	SELECT	sum("INGRESO"."MONTO")
												...
												WHERE	"INGRESO"."BASE" = "VISTA_PRODUCC_MENSUAL"."BASE" AND
															...
															"INGRESO"."CONTRATO" = "VISTA_PRODUCC_MENSUAL"."NUMERO" AND
															("INGRESO"."TIPO_MOV" = 'A' OR  "INGRESO"."TIPO_MOV" = 'E') AND
															"INGRESO"."FECHA_PAGO" <= :ldt_fec_comi) / round("VISTA_PRODUCC_MENSUAL"."PRECIO_PESO",2),2),
									"VISTA_PRODUCC_MENSUAL"."PRECIO_UF",
									"VISTA_PRODUCC_MENSUAL"."VALOR_UF",
									'H',
									"VISTA_PRODUCC_MENSUAL"."MONEDA",
									'P',
									'A',
									"VISTA_PRODUCC_MENSUAL"."BASE"
						FROM		"VISTA_PRODUCC_MENSUAL","JEFE_VENTAS"
						WHERE	"VISTA_PRODUCC_MENSUAL"."COD_JEF" = "JEFE_VENTAS"."JEFE_VENTAS" AND
									("VISTA_PRODUCC_MENSUAL"."BASE" = 'O' or "VISTA_PRODUCC_MENSUAL"."BASE" = 'A') and 
									"VISTA_PRODUCC_MENSUAL"."FECHA_BONO_JEF" = :ldt_fec_comi AND  
									"VISTA_PRODUCC_MENSUAL"."COD_JEF" = :ls_cod_jef
						USING	Trans_1;
						open x1;
						if Trans_1.sqlcode=0 then
							DO WHILE Trans_1.sqlcode=0
							fetch x1 INTO	:ls_cod_age_sup,:ll_rut,:ll_monto_ing,:ll_contr,:ls_serie,:ldb_porc_ing,:ldb_precio,:ldb_valor_uf,:ls_haber,:ls_moneda, :ls_codigo_com,:ls_estado,:ls_base;
								if not isnull(ls_cod_age_sup) and ls_cod_age_sup<>'' then
									ll_premio 			= round(ldb_precio * ldb_valor_uf * ldb_factor/ 100,0)
									if f_graba_res_comi(ls_cod_age_sup,ll_rut,ll_monto_ing,ls_cod_cont,string(ll_mes),ldt_fecha_cierre,ll_contr,ls_serie,ldb_porc_ing,ldb_factor,ldb_precio,ll_premio,ldb_valor_uf,ls_haber,ls_moneda,ls_codigo_com,ls_estado,ls_base,ll_cod_parque,0,0)=-1 then
										ll_malos++
									else
										ll_cont++
									end if
		...
		VALUES		( :ldt_fecha_ini,	:ldt_fecha_cierre,		:gdt_fec_sistema,		10,							:ll_cod_parque,		:gs_user,			'J' )  ;
		if sqlca.sqlcode = 0 then
			commit using sqlca;
		else
			rollback using sqlca;
...
long		ll_cod_parque,ll_cta,ll_tot_reg
datetime	ldt_fec_fin,ldt_fec_comi

dw_premio_jefe.reset()
SetPointer(HourGlass!)
dw_parque.accepttext()
dw_fecha_cierre.accepttext()
ldt_fec_fin										= dw_fecha_cierre.getitemdatetime(1,'cierre_ventas')
ll_cod_parque									= dw_parque.getitemnumber(1,'parque')
...
if ls_pasa='S' then
	SELECT 	count("FECHA_TERMINO")
	INTO		:ll_cta
	FROM		"CIERRE_PAGOS_VENTAS"
	WHERE	"CIERRE_PAGOS_VENTAS"."TIPO_PREMIO" = 10 AND
				"CIERRE_PAGOS_VENTAS"."FECHA_TERMINO" = :ldt_fec_fin AND
				"CIERRE_PAGOS_VENTAS"."COD_PARQUE" = :ll_cod_parque AND
				"CIERRE_PAGOS_VENTAS"."TIPO_EMPLEADO" = 'J'
	USING	sqlca;
	if isnull(ll_cta) then ll_cta=0
	if ll_cta > 0 then
		cb_grabar.enabled										= false
		if (gl_proceso=3 or gl_proceso=2) and (gs_depto='B' or gs_depto='X') then
			cb_premio_pago.visible							= false
			dw_premio_jefe.dataobject						= 'dw_bono_jefe_presupuesto_detalle'
			dw_premio_jefe.settransobject(sqlca)
			ll_tot_reg											= dw_premio_jefe.retrieve(ldt_fec_fin,gs_user,ldt_fec_comi)
			dw_premio_jefe.object.usuario.text			= gs_user
			if ll_tot_reg=0 then
				messagebox("Advertencia","No Registra Datos")
			end if
		elseif (gs_depto='D' or gs_depto='I' and gl_proceso>=4 )then
			...
			if ldt_fec_fin<=datetime(date(string('31/12/2016'))) then
				dw_premio_jefe.dataobject						= 'dw_bono_jefe_presupuesto'
			else
				dw_premio_jefe.dataobject						= 'dw_bono_jefe_presup_equival'
			end if
			dw_premio_jefe.settransobject(sqlca)
			wf_lista_premiados(ldt_fec_fin,ll_cod_parque,'R')
		end if
	else
		...
		if (gs_depto='D' or gs_depto='I' and gl_proceso>=4 )then
			...
			dw_premio_jefe.settransobject(sqlca)
			wf_lista_premiados(ldt_fec_fin,ll_cod_parque,'O')
		else
			messagebox("Advertencia","No Registra Datos")
		end if
	end if
...
integer taborder = 40
string title = "none"
string dataobject = "dw_bono_jefe_presup_equival"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = stylelowered!
end type
