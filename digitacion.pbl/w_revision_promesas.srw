forward
global type w_revision_promesas from window
end type
type hpb_1 from hprogressbar within w_revision_promesas
end type
type st_fondo from statictext within w_revision_promesas
end type
type st_porc from statictext within w_revision_promesas
end type
type cb_ctacte from commandbutton within w_revision_promesas
end type
type em_desde from editmask within w_revision_promesas
end type
type pb_procesar from picturebutton within w_revision_promesas
end type
type p_2 from picture within w_revision_promesas
end type
type p_1 from picture within w_revision_promesas
end type
type em_final from editmask within w_revision_promesas
end type
type st_2 from statictext within w_revision_promesas
end type
type st_1 from statictext within w_revision_promesas
end type
type st_16 from statictext within w_revision_promesas
end type
type dw_parque from datawindow within w_revision_promesas
end type
type cb_cerrar from commandbutton within w_revision_promesas
end type
type dw_lista from datawindow within w_revision_promesas
end type
type cb_filtrar from commandbutton within w_revision_promesas
end type
type cb_imprimir from commandbutton within w_revision_promesas
end type
type cb_ordenar from commandbutton within w_revision_promesas
end type
type cb_exportar from commandbutton within w_revision_promesas
end type
type gb_1 from groupbox within w_revision_promesas
end type
end forward
...
global w_revision_promesas w_revision_promesas

type variables
Long	il_parque,il_row
end variables

on w_revision_promesas.create
...
on w_revision_promesas.destroy
destroy(this.hpb_1)
...
Long		ll_tot_reg,ll_indi,ll_tot_porc,ll_tot_porc_aux,ll_capacidad,ll_dias,ll_nro_cuotas,&
			ll_cod_seguro,ll_numero,ll_count_reg
String	ls_age,ls_tipo_venta,ls_uso,ls_tipo_sepultura,ls_area,ls_moneda,ls_tipo_const,ls_base,&
			ls_serie
Double	ldb_porce_age,ldb_uf_dia,ldb_precio_lista,lld_factor,lld_saldo_finan,ldb_pie_uf,&
			ldb_descuento_uf
	...
	if il_parque>0 then
		...
		if ll_tot_reg>0 then
			...
			for ll_indi=1 to ll_tot_reg
				...
				ldb_pie_uf					= double(dw_lista.getitemnumber(ll_indi,'pie_uf'))
				ldb_descuento_uf			= double(dw_lista.getitemnumber(ll_indi,'descuento_por_uf'))
				...
				ll_cod_seguro				= dw_lista.getitemnumber(ll_indi,'pago_oferta_codigo_tipo_seguro')
				SELECT	COUNT("BENEFICIARIOS_SEGURO"."BASE")
				INTO 		:ll_count_reg  
				FROM 		"BENEFICIARIOS_SEGURO"  
				WHERE  ( "BENEFICIARIOS_SEGURO"."BASE" = :ls_base ) AND  
						 ( "BENEFICIARIOS_SEGURO"."SERIE" = :ls_serie ) AND  
						 ( "BENEFICIARIOS_SEGURO"."NUMERO" = :ll_numero ) AND  
						 ( "BENEFICIARIOS_SEGURO"."CODIGO_SEGURO" = :ll_cod_seguro )   
				USING		sqlca;
				if ll_count_reg>0 then
					dw_lista.setitem(ll_indi,'tiene_beneficiarios_ingre','Si ('+string(ll_count_reg)+')')
				else
					dw_lista.setitem(ll_indi,'tiene_beneficiarios_ingre','No Tiene')
				end if
				if isnull(lld_saldo_finan) then lld_saldo_finan=0
				if isnull(ldb_pie_uf) then ldb_pie_uf=0
				
				
				
				
				
				// ver diferencia por factor aplicado
				...
				if ll_nro_cuotas>=1 and ll_nro_cuotas<=11 then
					...
					if ll_nro_cuotas=1 then
						...
						dw_lista.setitem(ll_indi,'valor_cuota_lista',round(lld_saldo_finan + ldb_pie_uf,4))
					end if
				else
					if ll_dias>=30 then
						...
						if lld_factor > 0 and lld_saldo_finan > 0 then
							lld_saldo_finan	= round((lld_factor * lld_saldo_finan),4)	//round((lld_factor * lld_saldo_finan),5)
							dw_lista.setitem(ll_indi,'dif_dias',ll_dias)
							dw_lista.setitem(ll_indi,'factor_cuota',lld_factor)
							dw_lista.setitem(ll_indi,'valor_cuota_lista',lld_saldo_finan)
						else
