forward
global type w_ingresar_benef_seguro_funeraria from window
end type
type st_salir from statictext within w_ingresar_benef_seguro_funeraria
end type
type cb_salir from picturebutton within w_ingresar_benef_seguro_funeraria
end type
type st_grabar from statictext within w_ingresar_benef_seguro_funeraria
end type
type cb_grabar from picturebutton within w_ingresar_benef_seguro_funeraria
end type
type st_1 from statictext within w_ingresar_benef_seguro_funeraria
end type
type cb_borrar from picturebutton within w_ingresar_benef_seguro_funeraria
end type
type st_borrar_benef from statictext within w_ingresar_benef_seguro_funeraria
end type
type cb_insertar from picturebutton within w_ingresar_benef_seguro_funeraria
end type
type st_insert_benef from statictext within w_ingresar_benef_seguro_funeraria
end type
type dw_beneficiarios from datawindow within w_ingresar_benef_seguro_funeraria
end type
end forward
...
string title = "Ingreso Beneficiarios Seguro Funeraria"
...
global w_ingresar_benef_seguro_funeraria w_ingresar_benef_seguro_funeraria
...
on w_ingresar_benef_seguro_funeraria.create
...
on w_ingresar_benef_seguro_funeraria.destroy
...
if dw_beneficiarios.Retrieve(is_serie,il_numero,is_base)=0 then
...
event clicked;close(w_ingresar_benef_seguro_funeraria)
end event
...
string text = "Lista Beneficiarios Seguro Funerario"
boolean focusrectangle = false
end type
...
String		ls_nombre,ls_a_pat,ls_a_mat,ls_dv,ls_sw_pasa='N'
Double	ll_rut
...
if not isnull(is_serie) and not isnull(il_numero) and il_numero>0 then
	...
	if ll_tot_reg>0 then
		for ll_indi=1 to ll_tot_reg
			...
			ll_rut				= dw_beneficiarios.getitemnumber(ll_indi,'rut')
			ls_dv				= trim(dw_beneficiarios.getitemstring(ll_indi,'dv'))
			if isnull(ll_rut) or ll_rut=0 then
				messagebox("Advertencia","Debe Ingresar Rut Beneficiario en Fila Nº "+string(ll_indi))
				dw_beneficiarios.setfocus()
				dw_beneficiarios.scrolltorow(ll_indi)
				dw_beneficiarios.setcolumn('rut')
				ls_sw_pasa	= 'S'
				ll_indi		= ll_tot_reg
			elseif isnull(ls_dv) or ls_dv='' then
				messagebox("Advertencia","Debe Ingresar Digito verificador Beneficiario en Fila Nº "+string(ll_indi))
				dw_beneficiarios.setfocus()
				dw_beneficiarios.scrolltorow(ll_indi)
				dw_beneficiarios.setcolumn('dv')
				ls_sw_pasa	= 'S'
				ll_indi		= ll_tot_reg
	...
	if ls_sw_pasa='N' then
		...
		ll_contador 		= 0
		dw_beneficiarios.SetItem(ll_fila, "base", is_base)
...
integer height = 796
integer taborder = 10
string dataobject = "dw_ingreso_benef_seguro_funeraria"
...
if ls_columna='rut' or ls_columna='dv' then
	...
	if (not isnull(ll_rut) or ll_rut>0 ) and (ls_val_dv <> ls_dv) then
		...
		dw_beneficiarios.setitem(1,'dv',ls_nulo)
	end if
end if
this.accepttext()
end event
