forward
global type w_ingresar_pagare from window
end type
type pb_2 from picturebutton within w_ingresar_pagare
end type
type dw_seguro from datawindow within w_ingresar_pagare
end type
type cb_ver_cuenta from commandbutton within w_ingresar_pagare
end type
type p_1 from picture within w_ingresar_pagare
end type
type st_help_memo from statictext within w_ingresar_pagare
end type
type st_memo from commandbutton within w_ingresar_pagare
end type
type st_buscar from statictext within w_ingresar_pagare
end type
type pb_buscar from picturebutton within w_ingresar_pagare
end type
type pb_print from picturebutton within w_ingresar_pagare
end type
type st_imprimir from statictext within w_ingresar_pagare
end type
type st_limpiar from statictext within w_ingresar_pagare
end type
type pb_limpiar from picturebutton within w_ingresar_pagare
end type
type st_salir from statictext within w_ingresar_pagare
end type
type cb_cerrar from picturebutton within w_ingresar_pagare
end type
type st_grabar from statictext within w_ingresar_pagare
end type
type cb_aceptar from picturebutton within w_ingresar_pagare
end type
type dw_1 from datawindow within w_ingresar_pagare
end type
type dw_clientes from datawindow within w_ingresar_pagare
end type
type st_seg_fune from statictext within w_ingresar_pagare
end type
type dw_pagare from datawindow within w_ingresar_pagare
end type
end forward
...
global w_ingresar_pagare w_ingresar_pagare

type variables
date		id_fecha_dia
Long		il_tipo_cliente=1,il_modif,il_modif_fec,il_modif_plazo,il_estado,il_cod_seguro
String		is_error_cod
end variables
...
public subroutine wf_mostrar_pagare ();Long		ll_count,ll_cod_seguro,ll_estado_reg,ll_reg
Double	ldb_monto

cb_aceptar.text = "Actualizar"
gi_rut = Long(dw_pagare.GetItemNumber(1, "rut"))
if dw_clientes.Retrieve(gi_rut)>0 then dw_pagare.SetItem(1,"compute",upper(dw_clientes.GetItemstring(1,"dv")))
dw_pagare.setitem(1,'estado_rut',1)
dw_pagare.setitem(1,'estado_pagare',1)
dw_pagare.setitem(1,'estado_contrato',1)
if dw_pagare.getitemnumber(1,'nro_resc') = 99 then
...
public subroutine wf_log_pagare ();String	ls_serie_p,ls_serie,ls_moneda,ls_base,ls_dv_aval
Long		ll_nro_oferta,ll_rut,ll_plazo,ll_nro_pagare,ll_rut_aval,ll_cod_parque
Double	ll_kapital,ll_tasa,ll_valor_cuo,ll_pie
Datetime	ldt_fecha,ldt_fecha_prim,ldt_fecha_hoy,ldt_fecha_ult
...
ls_serie_p				= dw_pagare.getitemstring(1,'serie_p')
ls_serie					= dw_pagare.getitemstring(1,'serie')
ls_moneda				= dw_pagare.getitemstring(1,'moneda')
ls_base					= dw_pagare.getitemstring(1,'base')
ls_dv_aval				= dw_pagare.getitemstring(1,'dv_aval')

ll_nro_pagare			= dw_pagare.getitemnumber(1,'nro_pagare')
ll_nro_oferta			= dw_pagare.getitemnumber(1,'nro_oferta')
ll_rut					= dw_pagare.getitemnumber(1,'rut')
ll_kapital				= dw_pagare.getitemnumber(1,'kapital')
ll_plazo					= dw_pagare.getitemnumber(1,'plazo')
ll_valor_cuo			= dw_pagare.getitemnumber(1,'valor_cuo')
ll_pie					= dw_pagare.getitemnumber(1,'pie')
ll_tasa					= dw_pagare.getitemnumber(1,'tasa')
ll_rut_aval				= dw_pagare.getitemnumber(1,'rut_aval')
ll_cod_parque			= dw_pagare.getitemnumber(1,'cadena_cod_parque')

ldt_fecha				= dw_pagare.getitemdatetime(1,'fecha')
ldt_fecha_prim			= dw_pagare.getitemdatetime(1,'fecha_prim')
ldt_fecha_ult			= dw_pagare.getitemdatetime(1,'fecha_ult')
...
if isnull(ls_dv_aval) then ls_dv_aval=' '
...
INSERT INTO	"LOG_PAGARE"  
			( "SERIE_P",   "NRO_PAGARE",   "RUT",   "BASE",   "SERIE",   "NRO_OFERTA",   "USUARIO_CREA",   "FECHA_CREA",   "FECHA",   "MONEDA",   "PLAZO",   "PIE",   "VALOR_CUO",   "FECHA_PRIM",   "KAPITAL",   "TASA",   "FECHA_ULT",   "RUT_AVAL",   "DV_AVAL",		"COD_PARQUE" )  
VALUES 	( :ls_serie_p, :ll_nro_pagare, :ll_rut, :ls_base, :ls_serie, :ll_nro_oferta, :gs_user,  		  :ldt_fecha_hoy, :ldt_fecha,:ls_moneda, :ll_plazo, :ll_pie, :ll_valor_cuo, :ldt_fecha_prim,:ll_kapital, :ll_tasa, :ldt_fecha_ult,:ll_rut_aval, :ls_dv_aval,	:ll_cod_parque )  
USING		sqlca;
if sqlca.sqlcode=0 then
	commit;
else
...
Double	lld_tasa,lld_kapital,lld_valor_cuo,lld_pie
dw_pagare.AcceptText()
dw_clientes.AcceptText()
ls_moneda	= dw_pagare.getitemstring(1,'moneda')
lld_tasa		= double(dw_pagare.getitemnumber(1,'tasa'))
if lld_tasa>100 then
	messagebox("Error","Valor Tasa Inválida")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('tasa')
	RETURN FALSE
end if
if ls_moneda='2' then //uf
	lld_kapital				= double(dw_pagare.getitemnumber(1,'kapital'))
	lld_valor_cuo			= double(dw_pagare.getitemnumber(1,'valor_cuo'))
	lld_pie					= double(dw_pagare.getitemnumber(1,'pie'))
	if lld_kapital>=500 then 
		messagebox("Error","Recuerde los Valores deben ser ingresados en U.F. (Verificar Precio)")
		dw_pagare.setfocus()
		dw_pagare.setcolumn('kapital')
		RETURN FALSE
	elseif lld_valor_cuo>=500 then
		messagebox("Error","Recuerde los Valores deben ser ingresados en U.F. (Verificar Valor Cuota)")
		dw_pagare.setfocus()
		dw_pagare.setcolumn('valor_cuo')
		RETURN FALSE
	elseif lld_pie>=500 then
		messagebox("Error","Recuerde los Valores deben ser ingresados en U.F. (Verificar Pie)")
		dw_pagare.setfocus()
		dw_pagare.setcolumn('pie')
		RETURN FALSE
	end if
end if
IF IsNull(dw_pagare.GetItemNumber(1, "rut")) THEN
	MessageBox("Error", "Debe ingresar el rut del cliente")
	RETURN FALSE
END IF
//IF IsNull(dw_pagare.GetItemString(1, "serie")) THEN
//	MessageBox("Error", "Debe ingresar la serie del contrato asociado")
//	RETURN FALSE
//END IF
//IF IsNull(dw_pagare.GetItemNumber(1, "nro_oferta")) THEN
//	MessageBox("Error", "Debe ingresar el número del contrato asociado")
//	RETURN FALSE
//END IF
IF IsNull(dw_pagare.GetItemString(1, "moneda")) THEN
	MessageBox("Error", "Debe ingresar el tipo de moneda")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('moneda')
	RETURN FALSE
END IF
IF (dw_pagare.GetItemNumber(1, "kapital"))=0 THEN
	MessageBox("Error", "Debe ingresar el Capital")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('kapital')
	RETURN FALSE
END IF
//IF (dw_pagare.GetItemNumber(1, "tasa"))=0 THEN
//	MessageBox("Error", "Debe ingresar el interes")
//	dw_pagare.setfocus()
//	dw_pagare.setcolumn('tasa')
//	RETURN FALSE
//END IF
IF (dw_pagare.GetItemNumber(1, "valor_cuo"))=0 THEN
	MessageBox("Error", "Debe ingresar el Valor Cuota")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('valor_cuo')
	RETURN FALSE
END IF
IF (dw_pagare.GetItemNumber(1, "plazo"))=0 THEN
	MessageBox("Error", "Debe ingresar Nº Cuotas")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('plazo')
	RETURN FALSE
END IF
IF IsNull(dw_pagare.GetItemDateTime(1, "fecha_prim")) THEN
	MessageBox("Error", "Debe ingresar la fecha del primer Vencimiento")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('fecha_prim')
	RETURN FALSE
END IF
IF IsNull(dw_pagare.GetItemDateTime(1, "fecha_ult")) THEN
	MessageBox("Error", "Debe ingresar la fecha del Ultimo Vencimiento")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('fecha_ult')
	RETURN FALSE
END IF
IF IsNull(dw_pagare.GetItemDateTime(1, "fecha")) THEN
	MessageBox("Error", "Debe ingresar la fecha")
	dw_pagare.setfocus()
	dw_pagare.setcolumn('fecha')
	RETURN FALSE
END IF
...
public function boolean f_cadena (integer ai_modo);string 	ls_barra,ls_base,ls_serie
Double	ll_numero

ls_barra 		= 'P' + gs_serie + String(gi_numero)
ls_base			= dw_pagare.getitemstring(1,'base')
ls_serie			= dw_pagare.getitemstring(1,'serie')
ll_numero		= dw_pagare.getitemnumber(1,'nro_oferta')
gl_cod_parque	= dw_pagare.getitemnumber(1,'cadena_cod_parque')
if isnull(gl_cod_parque) or gl_cod_parque=0 then gl_cod_parque	= 0
IF ai_modo = 0 THEN // Es un Insert
	INSERT INTO cadena
			(barra,		rut,  	codigo, 	serie, 		numero, 		estado, 	print,	est_cup,		nro_carta,	fecha_carta,	estado_carta,	fecha_act_estado,
			cuota_cupon,	fecha_res,	a_demandar,	estado_contable,	estado_titulo,	fecha_titulo,	nro_carta_m,	fecha_carta_m,
			estado_carta_m,	fecha_act_estado_m,	cod_parque)
	VALUES 
			(:ls_barra, :gi_rut,	'P',		:gs_serie,	:gi_numero,	'V',		'N',		'0',			'SC',			Null,				'0',				Null,
			0,					Null,			'0',			'A',					'N',				Null,				'SC',				Null,	
			'0', 					Null,						:gl_cod_parque);
...
public subroutine wf_nuevo_pagare ();String	ls_null,ls_parque
Long		ll_null

ls_parque					= UPPER(gs_empresa)
Setnull(ls_null);setnull(ll_null)
SELECT	"MEMBRETE_EMPRESA"."COD_PARQUE"  
INTO 		:gi_cod_parque  
FROM 		"MEMBRETE_EMPRESA"  
WHERE 	"MEMBRETE_EMPRESA"."PARQUE" = :ls_parque   
USING		sqlca;
...
dw_pagare.InsertRow(0)
dw_pagare.SetItem(1, "serie_p", gs_serie)
dw_pagare.SetItem(1, "nro_pagare", gi_numero)
dw_pagare.SetItem(1, "serie", ls_null)
dw_pagare.SetItem(1, "nro_oferta", 0)
dw_pagare.SetItem(1, "rut_aval", 0)
dw_pagare.SetItem(1, "rut", ll_null)
dw_pagare.SetItem(1, "kapital", 0)
dw_pagare.SetItem(1, "tasa", 0)
dw_pagare.SetItem(1, "valor_cuo", 0)
//dw_pagare.SetItem(1, "fecha_prim", ld_null)
//dw_pagare.SetItem(1, "fecha_ult", ld_null)
dw_pagare.SetItem(1, "plazo", 0)
dw_pagare.SetItem(1, "tot_pagado", 0)
dw_pagare.SetItem(1, "pie_pagado", 0)
dw_pagare.SetItem(1, "cta_pag_la", 0)
dw_pagare.SetItem(1, "cta_pag_isa", 0)
dw_pagare.SetItem(1, "abono_pag", 0)
dw_pagare.SetItem(1, "usuario", gs_user)
dw_pagare.SetItem(1, "fechas_ingreso", id_fecha_dia)
dw_pagare.SetItem(1, "fecha", id_fecha_dia)
dw_pagare.SetItem(1, "estado", "V")
dw_pagare.SetItem(1, "nro_resc", 0)
dw_pagare.SetItem(1, "parque", gi_cod_parque)
dw_pagare.setitem(1, "dueno","I")
dw_pagare.setitem(1, "origen","I")
dw_pagare.SetItem(1, "moneda", ls_null)
dw_pagare.SetItem(1, "sector", ls_null)
dw_pagare.SetItem(1, "sepultura", ls_null)
dw_pagare.SetItem(1, "pie", 0)
end subroutine

on w_ingresar_pagare.create
...
if sqlca.sqlcode<>0 then
	...
	dw_pagare.getchild('serie',idw_detalle)
	idw_detalle.settransobject(sqlca)
	if idw_detalle.rowcount()=0 then
		idw_detalle.retrieve('O')
	end if
	dw_pagare.getchild('serie_p',idw_detalle2)
...
gs_serie		= dw_pagare.GetItemString(1,"serie_p")
gi_numero	= dw_pagare.GetItemNumber(1,"nro_pagare")
gs_base		= 'P'
...
event clicked;string	ls_base_contrato,ls_string,ls_serie_contrato
Long		ll_nro,ll_nro_aux

gs_serie				= dw_pagare.GetItemString(1,"serie_p")
gi_numero			= dw_pagare.GetItemNumber(1,"nro_pagare")
ls_base_contrato 	= dw_pagare.GetItemString(1,"base")
ls_serie_contrato = dw_pagare.GetItemString(1,"serie")
ll_nro 				= dw_pagare.GetItemNumber(1,"nro_oferta")
if not isnull(gs_serie) and gs_serie<>'' and gi_numero>0 and not isnull(ls_base_contrato) and &
	not isnull(ls_serie_contrato) and ll_nro>0 then
	SELECT	"PAGARE"."NRO_PAGARE"  
	INTO 		:ll_nro_aux  
	FROM 		"CADENA",   
				"PAGARE"  
	...
	USING		sqlca;
	if sqlca.sqlcode=0 then
		ls_string 	= 'P'+'~t'+gs_serie+'~t'+string(gi_numero)
		OpenWithParm(w_ingreso_cd_memorandum_digitacion,ls_string)
		w_ingreso_cd_memorandum_digitacion.setfocus()
	else
		messagebox("Advertencia","Antes de Ingresar Memorandum debe Grabar Pagaré")
		cb_aceptar.setfocus()
	end if
end if
end event
...
event clicked;openwithparm(w_listado_contrato,'P')
end event
...
ls_serie		= dw_pagare.getitemstring(1,'serie_p') //	trim(em_serie.text)
ll_numero	= dw_pagare.getitemnumber(1,'nro_pagare')//  long(trim(em_numero.text))
...
setnull(gs_base);setnull(gs_serie);setnull(gi_numero);setnull(gl_cod_parque)
gs_base			= 'P'
wf_limpiar()
//dw_seguro.reset()
//ll_tot_reg	= dw_seguro.retrieve()
//if ll_tot_reg > 0 then
//	for ll_indi=1 to ll_tot_reg
//		dw_seguro.setitem(ll_indi,'c_estado_reg',1)
//	next
...
event clicked;close(w_ingresar_pagare)
end event
...
event clicked;long 		ll_contador,rut,numero,rut_busca,ll_resp,li_ret1, li_ret4, i,ll_estado_cd,ll_tot_reg,ll_indi,ll_est_reg,ll_codigo,ll_count,&
			ll_estado_graba,ll_plazo,ll_rut,ll_cod_parque,ll_count_vig
string 	serie,ls_depto_asignado,ls_age,ls_sup,ls_jefe,ls_glosa,ls_memo,ls_ref,ls_dv,ls_estado_cadena,ls_moneda,ls_modif,ls_forma_pago
double 	ld_porcent_age, ld_porcent_sup,ldb_valor_cuota,ldb_valor_cuota_mant,ldb_valor_pie,ldb_calculo,ldb_precio,ldb_monto,ld_saldo_finan,ldb_tasa,ldb_tasa_cta
datetime	ldt_fecha_prim,ldt_fec_venc_mant

gs_serie		= dw_pagare.getitemstring(1,'serie_p')
gi_numero	= dw_pagare.getitemnumber(1,'nro_pagare')
gi_rut			= dw_pagare.getitemnumber(1,'rut')
SELECT	sysdate
INTO 		:gdt_fec_sistema
FROM 		"TASA"  ;
if not isnull(gs_base) and not isnull(gs_serie) and gi_numero>0 and gi_rut>0 then
	if String(dw_pagare.getitemstring(1,'compute')) <> String(gf_obtener_dv(long(dw_pagare.GetItemNumber(1, "rut")))) then
		messagebox('Error','El Rut no es valido',stopsign!)
		dw_pagare.setfocus()
		dw_pagare.SetColumn('rut')
		return
	end if
	gl_cod_parque	= dw_pagare.GetItemNumber(1,"cadena_cod_parque")
	if isnull(gl_cod_parque) or gl_cod_parque=0 then
		...
		dw_pagare.setfocus()
		dw_pagare.setcolumn('cadena_cod_parque')
		return
	end if
	ll_plazo					= dw_pagare.getitemnumber(1,'plazo')
	ldb_valor_cuota		= dw_pagare.getitemnumber(1,'valor_cuo')
	ldb_valor_pie			= dw_pagare.getitemnumber(1,'pie')
	ldb_precio				= dw_pagare.getitemnumber(1,'kapital')
	ldb_calculo				= round(ldb_valor_pie + (ll_plazo * ldb_valor_cuota),2)
	...
	dw_pagare.accepttext ()
	if isnull(dw_pagare.getitemstring(1,'base')) or isnull(dw_pagare.getitemstring(1,'serie')) or &
		isnull(dw_pagare.getitemnumber(1,'nro_oferta')) then 
		ll_resp	= messagebox("Grabar Pagaré","Desea Actualizar los datos ingresados, tenga presente que falta ingresar Contrato Asociado",Question!, YesNo!)
	else
		ll_resp	= MessageBox("Grabar Pagaré", "Desea Actualizar los datos ingresados", Question!, YesNo!)
	end if
	IF ll_resp = 1 THEN
		dw_pagare.SetItem(1, "parque", gi_cod_parque)
		dw_pagare.setitem(1, "dueno","I")
...
//		ldb_tasa				= ldb_tasa / 100
		dw_pagare.setitem(1,'pagare_tasa_interes_cuota',ldb_tasa_cta)
		dw_pagare.accepttext()
		li_ret1 	= dw_pagare.Update()
		IF li_ret1 = 1 THEN
			rut 			= dw_pagare.GetItemNumber(1,'rut')
			Commit using SQLCA;
			wf_log_pagare()
			IF f_validar() THEN
				...
				END IF
			END IF
			serie 			= dw_pagare.getitemstring(1,'serie_p') 
			numero 		= dw_pagare.getitemnumber(1,'nro_pagare') 
			IF cb_aceptar.text = "Actualizar" THEN 
				if dw_1.retrieve(serie,numero) > 0 then 
//						if 1 = MessageBox("Imprimir", 'Desea imprimir',Exclamation!, YesNo!, 2) then
//							f_printdlg(dw_1,gstr_print,w_ingresar_pagare)
//						end if
					ll_plazo					= dw_pagare.getitemnumber(1,'plazo')
					ll_rut						= dw_pagare.getitemnumber(1,'rut')
					ldb_valor_cuota		= dw_pagare.getitemnumber(1,'valor_cuo')
					...
					ls_dv						= dw_pagare.getitemString(1,'compute')
					ll_cod_parque			= dw_pagare.getitemnumber(1,'cadena_cod_parque')
					ls_moneda				= dw_pagare.getitemstring(1,'moneda')
					ls_forma_pago			= dw_pagare.getitemstring(1,'pagare_forma_pago')
...
//														ldb_monto	= dw_seguro.getitemnumber(ll_indi,'factor_prima')
					...
					IF MessageBox("Grabar", "Grabación Exitosa, desea Ingresar otro Pagaré", Question!, YesNo!, 2) = 1 THEN
						setnull(gs_base);setnull(gs_serie);setnull(gi_numero)
						gs_base				= 'P'
						wf_limpiar()
						dw_pagare.setfocus()
...
integer height = 360
boolean titlebar = true
string dataobject = "dw_imprime_pagare"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
...
integer height = 1672
integer taborder = 10
string dataobject = "d_ingreso_pagare"
boolean border = false
boolean livescroll = true
end type
...
event itemchanged;long 		ll_cant, ll_fila,  ll_nro, ll_rut_aval,ll_resp,ll_nro_p,ll_nro_cuotas,ll_estado,ll_plazo_rev,ll_cod_parque_rev,ll_rut,ll_tot_reg,ll_indi,ll_dias_tope_def,ll_dias_inicio_def
string 	ls_columna, ls_base_contrato, ls_serie_contrato, ls_null,ls_serie_p,ls_fecha,ls_mensaje,ls_moneda,ls_descrip,ls_agente,ls_string,ls_base_aux,ls_moneda_rev,ls_forma_pago_rev,&
			ls_area_rev,	ls_base_asoc,ls_serie_asoc
date 		ld_null,ld_fecha_prim,ld_fecha_min
Datetime	ldt_fecha_prim,ldt_fecha_ctto_rev,ldt_fecha_prim_rev
Double	lld_tasa,lld_kapital,lld_valor_cuo,lld_pie,ldb_uf_ctto_rev,ldb_pie_rev,ldb_gasto_adm_rev,ldb_saldo_financiar_rev,ldb_valor_cuota_rev,ldb_precio_ctto_rev,ldb_factor_cuota_rev,&
			ldb_int_peso,ldb_int_uf,ll_nro_contrato,ll_numero_asoc
...
IF ls_columna = "rut" THEN
	...
	if dw_clientes.Retrieve(gi_rut)=0 then
		...
		IF ll_resp = 1 THEN
			...
			this.setcolumn('rut')
			dw_pagare.setitem(1, 'estado_rut',0)
		END IF
	else
		dw_pagare.setitem(1,'estado_rut',1)
		dw_pagare.setitem(1,'compute',dw_clientes.getitemstring(1,'dv'))
	END IF
end if
if ls_columna = 'cadena_cod_parque' then
	gl_cod_parque	= dw_pagare.getitemnumber(1,'cadena_cod_parque')
	ll_tot_reg	= dw_seguro.retrieve(gl_cod_parque,'P')
	if ll_tot_reg > 0 then
		for ll_indi=1 to ll_tot_reg
			dw_seguro.setitem(ll_indi,'c_estado_reg',1)
		next
...
if ls_columna='fecha_prim' then
	ld_fecha_prim	= date(dw_pagare.getitemdatetime(1,'fecha_prim'))
	...
	if ld_fecha_prim < ld_fecha_min then
		...
		dw_pagare.setitem(1,'fecha_prim',date(gdt_fec_sistema))
		dw_pagare.accepttext()
	end if
end if
...
IF ls_columna = "nro_pagare" or ls_columna='serie_p' THEN
	ls_serie_p	= GetItemString(1,"serie_p")
	ll_nro_p		= GetItemNumber(1,"nro_pagare")
	gs_base		= 'P'
	gs_serie 	= ls_serie_p
	gi_numero	= ll_nro_p
	if not isnull(ls_serie_p) and not isnull(ll_nro_p) and ll_nro_p>0 then
		SELECT 	"CADENA"."NUMERO"
		INTO 		:ll_nro_contrato
		FROM 		"CADENA"
		WHERE	 ( "CADENA"."CODIGO" = 'P') and 
				 ( "CADENA"."SERIE" = :ls_serie_p) and
				 ( "CADENA"."NUMERO" = :ll_nro_p)
		using		sqlca;
		if sqlca.sqlcode=0 then
			if dw_pagare.retrieve(ls_serie_p,ll_nro_p)>0 then
				if isnull(dw_pagare.getitemstring(1,'usuario')) or dw_pagare.getitemstring(1,'usuario')='' then dw_pagare.setitem(1,'usuario',gs_user)
				wf_mostrar_pagare()
			else
				pb_limpiar.triggerevent(clicked!)
				dw_pagare.setitem(1,'estado_pagare',0)
			end if
			dw_pagare.setcolumn('nro_pagare')
		ELSE
			ll_resp	= MessageBox("Advertencia", "Pagaré Nº: P-"+ls_serie_p+'-'+string(ll_nro_p,"###,###,###,###")+' No Existe, desea Ingresarlo', Exclamation!, YesNo!, 2)
			IF ll_resp = 1 THEN
				gi_numero	= ll_nro_p
				gs_serie 	= ls_serie_p
				gs_base 		= 'P'
				...
				if sqlca.sqlcode=100 then
					...
					USING		sqlca;
					if sqlca.sqlcode=0 then
						dw_pagare.SetItem(1,'base',ls_base_asoc)
						dw_pagare.SetItem(1,'serie',ls_serie_asoc)
						dw_pagare.SetItem(1,'nro_oferta',ll_numero_asoc)
						dw_pagare.SetItem(1,'fecha',ldt_fecha_ctto_rev)
						dw_pagare.SetItem(1,'moneda',ls_moneda_rev)
						dw_pagare.SetItem(1,'pie',ldb_pie_rev)
						dw_pagare.SetItem(1,'plazo',ll_plazo_rev)
						dw_pagare.SetItem(1,'valor_cuo',ldb_valor_cuota_rev)
						dw_pagare.SetItem(1,'kapital',ldb_precio_ctto_rev)
						dw_pagare.SetItem(1,'cadena_cod_parque',ll_cod_parque_rev)
						dw_pagare.SetItem(1,'fecha_prim',ldt_fecha_prim_rev)
						...
						USING		sqlca;
						if sqlca.sqlcode=0 then
							if dw_pagare.getitemstring(1,'pagare_forma_pago')='1' then
								ldb_int_peso	= 0
								ldb_int_uf		= 0
							end if
							if ls_moneda_rev='1' then //peso
								dw_pagare.setitem(1,'tasa',ldb_int_peso)
							elseif ls_moneda_rev='2' then //UF
								dw_pagare.setitem(1,'tasa',ldb_int_uf)
							end if
						end if
						ls_base_contrato 					= dw_pagare.getitemstring(1,'base')
						ls_serie_contrato 				= dw_pagare.getitemstring(1,'serie')
						...
						if not isnull(ls_base_contrato) and not isnull(ls_serie_contrato) and not isnull(ll_nro) and ll_nro>0 then
							...
							if sqlca.sqlcode=0 then
								if gi_rut = ll_rut then
									dw_pagare.setitem(1,'estado_contrato',0)
									cb_aceptar.enabled	= true
									dw_seguro.enabled	= true
								else
									ll_resp	= messagebox("Advertencia","Pagaré Nº "+gs_base+"-"+gs_serie+"-"+string(gi_numero,"###,###,###,###")+&
								...
								MessageBox("Pagaré" , "No Existe el Contrato Asociado")	
								dw_pagare.setitem(1,'estado_contrato',1)
								cb_aceptar.enabled	= false
								dw_seguro.enabled	= false
							end if
...
IF ls_columna = "nro_oferta" or ls_columna='serie' or ls_columna='base' THEN
	IF ls_columna = "base" THEN
		ls_base_contrato	= this.getitemstring(1,'base')
		if not isnull(ls_base_contrato) then 
			dw_pagare.getchild('serie',idw_detalle)
			idw_detalle.settransobject(sqlca)
			idw_detalle.retrieve(ls_base_contrato)
		end if
	...
	if not isnull(ls_serie_contrato) and ll_nro>0 and not isnull(ls_base_contrato) then
		SELECT 	"CADENA"."NUMERO"
		INTO 		:ll_nro_contrato
		FROM 		"CADENA"
		WHERE  ( "CADENA"."CODIGO" = :ls_base_contrato) and 
		...
		IF ll_nro <> ll_nro_contrato THEN
			CHOOSE CASE ls_base_contrato
				CASE 'O'
					ls_mensaje	= 'No existe Promesa Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'D'
					ls_mensaje	= 'No existe Derecho Especial Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'L'
					ls_mensaje	= 'No existe Liberador Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'C'
					ls_mensaje	= 'No existe Contrato IsaCruz Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'P'
					ls_mensaje	= 'No existe Pagaré Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'R'
					ls_mensaje	= 'No existe Reprogramación de Mantención Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
				CASE 'A'
					ls_mensaje	= 'No existe Aumento Capacidad Nº: '+ls_base_contrato+'-'+ls_serie_contrato+'-'+string(ll_nro,"###,###,###,###")
			END CHOOSE
		...
		ELSE
			...
			CHOOSE CASE ls_base_contrato
				...
				CASE 'D','P','R'
					messagebox("Advertencia","Contrato NO tiene Agente Asociado, Verifique Código PARQUE")
				CASE 'L'
					SELECT	"ANEXO_LIBERADOR"."COD_AGENTE"  
					INTO 		:ls_agente  
					FROM 		"ANEXO_LIBERADOR"  
					WHERE  ( "ANEXO_LIBERADOR"."SERIE_M" = :ls_serie_contrato ) AND  
							 ( "ANEXO_LIBERADOR"."NRO_LIBERADOR" = :ll_nro )   ;

				CASE 'C'
					SELECT	"CONTRATO"."AGENTE"  
					INTO 		:ls_agente  
					FROM 		"CONTRATO"  
					WHERE  ( "CONTRATO"."SERIE_C" = :ls_serie_contrato ) AND  
							 ( "CONTRATO"."NRO_CONTRATO" = :ll_nro )   ;

				CASE 'A'
					SELECT	"ANEXO_AUMENTO"."COD_AGENTE"  
					INTO 		:ls_agente  
					FROM 		"ANEXO_AUMENTO"  
					WHERE  ( "ANEXO_AUMENTO"."SERIE_M" = :ls_serie_contrato ) AND  
							 ( "ANEXO_AUMENTO"."NRO_AUMENTO" = :ll_nro )   ;
			...
			if not isnull(ls_agente) then
				...
				USING		sqlca;
				if sqlca.sqlcode=0 then dw_pagare.setitem(1,'cadena_cod_parque',gl_cod_parque)
			end if
		END IF
	end if
end if
if ls_columna='dv_aval' then
	if String(getitemstring(1,'dv_aval')) <> String(gf_obtener_dv(long(GetItemNumber(1, "rut_aval")))) then
		messagebox('Error','El Rut no es valido',stopsign!)
		this.setfocus()
		this.SetColumn('rut_aval')
	else
		ll_rut_aval	= dw_pagare.getitemnumber(1,'rut_aval')
		open(w_aval)
		if Message.StringParm <> '1' then
			dw_pagare.setitem(1,'rut_aval',0)
			dw_pagare.setitem(1,'dv_aval','')
		end if 
	end if
end if	
if ls_columna='fecha_prim' or ls_columna='plazo' then
	ldt_fecha_prim	= dw_pagare.getitemdatetime(1,'fecha_prim')
	ll_nro_cuotas	= dw_pagare.getitemnumber(1,'plazo')
	if not isnull(ldt_fecha_prim) and not isnull(ll_nro_cuotas) and ll_nro_cuotas>0 then
		...
		if not isnull(ls_fecha) and ls_fecha<>'//' and ls_fecha<>'' then
			dw_pagare.setitem(1,'fecha_ult',date(ls_fecha))
		end if
	end if
END IF
if ls_columna='valor_cuo' then
	SELECT	COUNT("LOG_PAGARE"."NRO_PAGARE")  
	INTO 		:il_modif  
	FROM 		"LOG_PAGARE"  
	WHERE  ( "LOG_PAGARE"."SERIE_P" = :gs_serie ) AND  
			 ( "LOG_PAGARE"."NRO_PAGARE" = :gi_numero )   
	USING		sqlca;
end if
...
if ls_columna='kapital' or ls_columna='tasa' or ls_columna='valor_cuo' or ls_columna='pie' then
...
if ls_columna='plazo' or ls_columna='valor_cuo' then
	SELECT	COUNT("LOG_PROMESA"."NRO_OFERTA")  
	INTO 		:il_modif  
	FROM 		"LOG_PROMESA"  
	WHERE  ( "LOG_PROMESA"."SERIE" = :gs_serie ) AND  
			 ( "LOG_PROMESA"."NRO_OFERTA" = :gi_numero )   
	USING		sqlca;
end if
...
//	gd_uf_dia
	if gd_uf_dia>0 then
		if ls_moneda='1' then //peso
			if lld_kapital<500 then 
				dw_pagare.setitem(1,'kapital',long((lld_kapital * gd_uf_dia)))
			end if
			if lld_valor_cuo<500 then
				dw_pagare.setitem(1,'valor_cuo',long((lld_valor_cuo * gd_uf_dia)))
			end if
			if lld_pie<500 then
				dw_pagare.setitem(1,'pie',long((lld_pie * gd_uf_dia)))
			end if
		elseif ls_moneda='2' then //uf
			if lld_kapital>=500 then 
				dw_pagare.setitem(1,'kapital',(lld_kapital / gd_uf_dia))
			end if
			if lld_valor_cuo>=500 then
				dw_pagare.setitem(1,'valor_cuo',(lld_valor_cuo / gd_uf_dia))
			end if
			if lld_pie>=500 then
				dw_pagare.setitem(1,'pie',(lld_pie / gd_uf_dia))
			end if
		end if
		dw_pagare.accepttext()
	end if
...
ls_columna	= dwo.name
if dw_pagare.getitemnumber(1,'estado_rut')=1 and dw_pagare.getitemnumber(1,'estado_contrato')=1 and dw_pagare.getitemnumber(1,'estado_pagare')=1 then
	CHOOSE CASE ls_columna
		CASE 'p_2'
			ls_fecha					= string(date(dw_pagare.getitemdatetime(1,'fecha')))
			if isnull(ls_fecha) then ls_fecha = string(id_fecha_dia,"dd/mm/yyyy")
			if f_valida_fecha(ls_fecha)=-1 then 
				dw_pagare.setitem(1,'fecha',datetime(string(id_fecha_dia,gs_formato_fecha)))
				return
			end if
			OpenWithParm(w_calendar,ls_fecha)
			IF not isnull(Message.StringParm) THEN
				ls_fecha				= trim(Message.StringParm)
				dw_pagare.setitem(1,'fecha',date(ls_fecha))
			END IF
			dw_pagare.setcolumn('fecha')
		...
		CASE 'p_3'
			...
			if ld_fecha_prim < ld_fecha_min then
				...
				ls_fecha					= string(date(dw_pagare.getitemdatetime(1,'fecha_prim')))
				if isnull(ls_fecha) then ls_fecha = string(id_fecha_dia,"dd/mm/yyyy")
				if f_valida_fecha(ls_fecha)=-1 then 
					dw_pagare.setitem(1,'fecha_prim',datetime(string(id_fecha_dia,gs_formato_fecha)))
					return
				end if
				OpenWithParm(w_calendar,ls_fecha)
				IF not isnull(Message.StringParm) THEN
					ls_fecha				= trim(Message.StringParm)
					dw_pagare.setitem(1,'fecha_prim',date(ls_fecha))
				END IF
				ldt_fecha_prim	= dw_pagare.getitemdatetime(1,'fecha_prim')
				ll_nro_cuotas	= dw_pagare.getitemnumber(1,'plazo')
				if not isnull(ldt_fecha_prim) and not isnull(ll_nro_cuotas) and ll_nro_cuotas>0 then
		...
		CASE 'p_4'
			ls_fecha					= string(date(dw_pagare.getitemdatetime(1,'fecha_ult')))
			if isnull(ls_fecha) then ls_fecha = string(id_fecha_dia,"dd/mm/yyyy")
			if f_valida_fecha(ls_fecha)=-1 then 
				dw_pagare.setitem(1,'fecha_ult',datetime(string(id_fecha_dia,gs_formato_fecha)))
				return
			end if
			OpenWithParm(w_calendar,ls_fecha)
			IF not isnull(Message.StringParm) THEN
