forward
global type w_cd_enviar_email from window
end type
type em_buscar from editmask within w_cd_enviar_email
end type
type st_6 from statictext within w_cd_enviar_email
end type
type em_asunto from editmask within w_cd_enviar_email
end type
type em_copia from editmask within w_cd_enviar_email
end type
type em_para from editmask within w_cd_enviar_email
end type
type st_5 from statictext within w_cd_enviar_email
end type
type st_4 from statictext within w_cd_enviar_email
end type
type st_3 from statictext within w_cd_enviar_email
end type
type st_2 from statictext within w_cd_enviar_email
end type
type st_1 from statictext within w_cd_enviar_email
end type
type pb_eliminar from picturebutton within w_cd_enviar_email
end type
type pb_agregar from picturebutton within w_cd_enviar_email
end type
type cb_enviar from commandbutton within w_cd_enviar_email
end type
type cb_cerrar from commandbutton within w_cd_enviar_email
end type
type dw_destinatarios from datawindow within w_cd_enviar_email
end type
type dw_encargados from datawindow within w_cd_enviar_email
end type
type gb_1 from groupbox within w_cd_enviar_email
end type
type mle_texto from multilineedit within w_cd_enviar_email
end type
type gb_2 from groupbox within w_cd_enviar_email
end type
end forward
...
global w_cd_enviar_email w_cd_enviar_email
...
event open;String	ls_asunto,ls_glosa

gf_centrar(w_cd_enviar_email)
ls_asunto		= trim(substr(1,1,Message.StringParm))
ls_glosa			= trim(substr(1,2,Message.StringParm))
is_base			= trim(substr(1,3,Message.StringParm))
is_serie			= trim(substr(1,4,Message.StringParm))
...
String	ls_email,ls_correos,ls_asunto,ls_texto
ll_tot_reg	= dw_destinatarios.rowcount()

if ll_tot_reg>0 then
	ls_correos					= 'SE ENVIO EMAIL A LOS USUARIO(S): '+trim(em_para.text)+';'+trim(em_copia.text)+' EL DIA '+STRING(gdt_fec_sistema,"dd/mm/yyyy")
	...
	ls_texto						= 'MENSAJE ENVIADO: '+ls_texto
	if len(ls_texto) > 1500 then ls_texto=mid(ls_texto,1,1500)
	if len(ls_correos) > 500 then ls_texto=mid(ls_correos,1,500)
	...
	VALUES	( :is_base,	:is_serie,	:il_numero,	:gdt_fec_sistema,	:gs_user,			98,			:gdt_fec_sistema,	:ls_texto,	:ls_asunto,	'',				'',						'',						:ls_correos )  
	USING		sqlca;
...
event clicked;close(w_cd_enviar_email)
end event
...
string dataobject = "dw_lista_email_encargados"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = stylelowered!
end type
