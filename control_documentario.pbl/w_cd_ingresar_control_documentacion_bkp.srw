forward
global type w_cd_ingresar_control_documentacion_bkp from window
end type
type cb_revision from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_anular from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_aceptar from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_cancelar from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cbx_tipo_rechazo from checkbox within w_cd_ingresar_control_documentacion_bkp
end type
type cb_email from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_memo from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_cerrar from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type cb_grabar from commandbutton within w_cd_ingresar_control_documentacion_bkp
end type
type st_2 from statictext within w_cd_ingresar_control_documentacion_bkp
end type
type st_1 from statictext within w_cd_ingresar_control_documentacion_bkp
end type
type p_flecha from picture within w_cd_ingresar_control_documentacion_bkp
end type
type dw_lista_rechazo from datawindow within w_cd_ingresar_control_documentacion_bkp
end type
type gb_3 from groupbox within w_cd_ingresar_control_documentacion_bkp
end type
type dw_nuevo from datawindow within w_cd_ingresar_control_documentacion_bkp
end type
type dw_actual from datawindow within w_cd_ingresar_control_documentacion_bkp
end type
end forward
...
global w_cd_ingresar_control_documentacion_bkp w_cd_ingresar_control_documentacion_bkp
...
on w_cd_ingresar_control_documentacion_bkp.create
...
on w_cd_ingresar_control_documentacion_bkp.destroy
...
if gs_conexion="Parque El Prado" then
	if idw_detalle.retrieve(gs_depto)=0 then idw_detalle.insertrow(0)
elseif gs_conexion = "Parque La Foresta" then
	if gs_depto='O' or gs_depto='P' then
		if gs_depto='O' then ls_depto='P'
		if gs_depto='P' then ls_depto='O'
		if idw_detalle.retrieve(gs_depto,ls_depto)=0 then idw_detalle.insertrow(0)
	else
		if idw_detalle.retrieve(gs_depto,gs_depto)=0 then idw_detalle.insertrow(0)
	end if
end if
...
OpenWithParm(w_validar_promesa_operaciones,ls_string)
end event
...
event clicked;close(w_cd_ingresar_control_documentacion)
end event
...
event clicked;String	ls_usuario,ls_glosa,ls_memo,ls_age,ls_sup,ls_jefe,ls_age_actual,ls_sup_actual,&
			ls_jefe_actual,ls_cod_aux,ls_parametro
Datetime	ldt_fecha_estado
Long		ll_estado,ll_count_fila,ll_resp,ll_mes,ll_ano,ll_sw_pasa=0
...
if isnull(dw_nuevo.getitemstring(1,'glosa')) then
	...
	if ll_estado=14 or ll_estado=15 then
...
integer height = 1144
integer taborder = 30
string dataobject = "dw_ingreso_detalle_cd"
...
event itemchanged;String	ls_columna,ls_glosa,ls_age_actual,ls_sup_actual,ls_jefe_actual,&
			...
			ls_ult_glosa,ls_parametro,ls_inserta_fila	= 'S'
...
CHOOSE CASE ls_columna
	CASE 'estado'
		...
		if ll_estado=5 or ll_estado=10 then
		...
		if ll_estado>=100 then
			...
			if ll_ult_estado=19 and ll_estado<>14 then
				messagebox("Advertencia","No es posible Seleccionar esta Opcion por encontrarse en Estado "+string(ll_ult_estado)+'-'+ls_descrip+", debe Asignar Agente")
				dw_nuevo.reset()
				ll_new						= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
				dw_nuevo.setitem(ll_new,'usuario_crea',gs_user)
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_ult_estado=2 and ll_estado<>3 and ll_estado<>4 and ll_estado<>5 and ll_estado<>7 and ll_estado<>12 and ll_estado<>13 and ll_estado<>19 and ll_estado<>14 then
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado>5 and ll_ult_estado<4 and ll_estado<>14 and ll_estado<>15 and ll_estado<>19 then
				messagebox("Advertencia","No es posible Seleccionar esta Opcion por encontrarse en Estado "+string(ll_ult_estado)+'-'+ls_descrip)
				dw_nuevo.reset()
				ll_new							= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado<>3 and ll_estado<>19 and ll_estado<>4 and ll_estado<>5 and ll_estado<>14 and ll_estado<100  and ll_ult_estado=2 then
				messagebox("Advertencia","No es posible Seleccionar esta Opcion por encontrarse en Estado "+string(ll_ult_estado)+'-'+ls_descrip)
				dw_nuevo.reset()
				ll_new						= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado<>13 and ll_estado<>4 and ll_estado<>5 and ll_estado<>14 and ll_ult_estado=12 then
				messagebox("Advertencia","No es posible Seleccionar esta Opcion por encontrarse en Estado "+string(ll_ult_estado)+'-'+ls_descrip)
				dw_nuevo.reset()
				ll_new						= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif (ll_estado=8 and ll_ult_estado=16) or (ll_estado=18 and ll_ult_estado=4) or (ll_estado=10 and ll_ult_estado=16) or (ll_estado=17 and ll_ult_estado=4) then
			...
			elseif ll_ult_estado=16 and (ll_estado<>17 and ll_estado<>9 and ll_estado<>15 and ll_estado<>18) or &
				ll_ult_estado=17 and (ll_estado<>16 and ll_estado<>15 and ll_estado<>18) then //				ll_ult_estado=8 and (ll_estado<>17 and ll_estado=16) and &
				messagebox("Advertencia","No es posible Seleccionar esta Opcion por encontrarse en Estado "+string(ll_ult_estado)+'-'+ls_descrip)
				dw_nuevo.reset()
				ll_new						= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
			...
			elseif ll_estado<>16 and ll_ult_estado=6 then
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado=7 and ll_ult_estado<>10 then
			...
			elseif ll_estado=7 and ll_ult_estado=10 then
				...
				dw_nuevo.accepttext()
			elseif ll_estado=5 and ll_ult_estado=7 then
				dw_nuevo.setitem(1,'glosa',ls_ult_glosa)
				dw_nuevo.accepttext()
			elseif ll_estado=13 and ll_ult_estado<>12 then
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado=9 and ll_ult_estado<>4 and ll_ult_estado<>8 and ll_ult_estado<>18 then
				messagebox("Advertencia","Para esta Opción el Contrato debe estar en Estado 4-Aceptado Contrato y Entregar a Operaciones, Estado 8-Recepción Ctto.por Depto.Operaciones a Depto.Adm.Vtas. o Estado 18-Recepción Ctto.por Depto.Operaciones a Depto.At.Cliente")
				dw_nuevo.reset()
				ll_new						= dw_nuevo.insertrow(0)
				dw_nuevo.scrolltorow(ll_new)
				dw_nuevo.setitem(ll_new,'usuario_crea',gs_user)
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado=8 and ll_ult_estado<>4 then
				...
				dw_nuevo.setfocus()
				dw_nuevo.setcolumn('estado')
			elseif ll_estado=14 or ll_estado=15 then
				dw_nuevo.Modify("gb_1.Height=648")
				dw_nuevo.Modify("glosa.Height=528")
				SELECT	"CD_ESTADO"."GLOSA"  
				INTO 		:ls_descrip  
				...
				if ll_estado=ll_ult_estado then
					...
					if ll_estado=11 and ll_ult_estado<>9 then
						...
						dw_nuevo.setfocus()
						dw_nuevo.setcolumn('estado')
					elseif (ll_estado=4 and ll_ult_estado<3 and ll_ult_estado>99) or (ll_estado=9 and ll_ult_estado<3 and ll_ult_estado>99) then
						...
						dw_nuevo.setfocus()
						dw_nuevo.setcolumn('estado')
					elseif ll_estado=12 and ll_ult_estado=5 then
						dw_nuevo.setitem(1,'glosa',ls_ult_glosa)
						dw_nuevo.accepttext()
					else
						if ll_estado=4 then
	...
	CASE 'cod_agente'
		...
		ldt_fecha_estado	= dw_nuevo.getitemdatetime(1,'fecha_estado')
		ls_glosa				= trim(dw_nuevo.getitemstring(1,'glosa'))
		if ls_cod_age<>ls_age_actual and not isnull(ls_cod_age) and not isnull(ls_age_actual) then
			ls_glosa		= trim(dw_nuevo.getitemstring(1,'glosa'))
