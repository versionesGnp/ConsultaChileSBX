forward
global type w_cd_memorandum from window
end type
type st_estado_asociado from statictext within w_cd_memorandum
end type
type st_5 from statictext within w_cd_memorandum
end type
type st_numero from statictext within w_cd_memorandum
end type
type st_4 from statictext within w_cd_memorandum
end type
type st_serie from statictext within w_cd_memorandum
end type
type st_3 from statictext within w_cd_memorandum
end type
type st_base from statictext within w_cd_memorandum
end type
type st_2 from statictext within w_cd_memorandum
end type
type em_referencia from editmask within w_cd_memorandum
end type
type st_1 from statictext within w_cd_memorandum
end type
type cb_grabar from commandbutton within w_cd_memorandum
end type
type cb_cerrar from commandbutton within w_cd_memorandum
end type
type gb_2 from groupbox within w_cd_memorandum
end type
type cbx_excepcion from checkbox within w_cd_memorandum
end type
type mle_memo from multilineedit within w_cd_memorandum
end type
type gb_1 from groupbox within w_cd_memorandum
end type
end forward
...
global w_cd_memorandum w_cd_memorandum
...
event clicked;Long	ll_pasa_grabar
...
if not isnull(is_memo) and not isnull(is_ref) then
	...
	if isvalid(w_validar_promesa_operaciones) then
		...
		if cbx_excepcion.checked=true then
			UPDATE	"CD_FOLIO"  
			SET 		"ULT_MEMO" = :is_memo,   
						"ULT_REFERENCIA_MEMO" = :is_ref,   
						"ULT_TIPO_EXCEPCION" = :il_tipo_opcion,   
						"ULT_ESTADO_EXCEPCION" = 'S',   
						"ULT_ESTADO_ASOCIADO_EXCEPCION" = :il_estado_anterior  
			...
			if sqlca.sqlcode=0 then
				commit;
				messagebox("Grabar","Grabación Exitosa")
				w_validar_promesa_operaciones.cb_memo.enabled	= false
				w_validar_promesa_operaciones.cb_limpiar.triggerevent(clicked!)
				w_validar_promesa_operaciones.setfocus()
				close(w_cd_memorandum)
			else
				rollback;
				messagebox("Error","Error al Grabar SQL: "+sqlca.sqlerrtext)
