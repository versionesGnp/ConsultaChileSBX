forward
global type w_tipo_servicio_detalle_bk from window
end type
type cb_1 from commandbutton within w_tipo_servicio_detalle_bk
end type
type dw_venta from datawindow within w_tipo_servicio_detalle_bk
end type
type dw_detalle from datawindow within w_tipo_servicio_detalle_bk
end type
type cb_ingreso_reg from commandbutton within w_tipo_servicio_detalle_bk
end type
type cb_detalle from commandbutton within w_tipo_servicio_detalle_bk
end type
type dw_lista from datawindow within w_tipo_servicio_detalle_bk
end type
type st_1 from statictext within w_tipo_servicio_detalle_bk
end type
type dw_tipo_servicio from datawindow within w_tipo_servicio_detalle_bk
end type
type cb_grabar from commandbutton within w_tipo_servicio_detalle_bk
end type
type cb_cerrar from commandbutton within w_tipo_servicio_detalle_bk
end type
end forward
...
global w_tipo_servicio_detalle_bk w_tipo_servicio_detalle_bk

type variables
string	is_base,is_serie,is_uso
long 	il_row, il_row_selec,il_row_vta,il_cod_parque,il_cod_serv
Double	il_numero
end variables
...
public subroutine wf_grabar (string as_moneda, long al_cod_serv, double adb_precio)
end prototypes

event ue_calcular();long	ll_tot_lista,ll_cod_ser,ll_res,ll_tot_reg,ll_indi,ll_cta,ll_cod_serv_tab

if not isnull(il_cod_serv) and il_cod_serv>0 and not isnull(il_cod_parque) and il_cod_parque>0 then
		...
		ll_tot_lista																		= dw_venta.retrieve(is_base,is_serie,il_numero)
		if ll_tot_lista>0 then
			ll_cod_ser																	= dw_venta.getitemnumber(1,'cod_servicio')
			if ll_cod_ser=il_cod_serv then
			...
			if ll_res=1 then
				...
				if ll_res=1 then
					if il_cod_serv=99 then
						...
						dw_tipo_servicio.object.cod_servicio.protect		= 1
						ll_tot_reg													= dw_lista.retrieve(il_cod_serv,is_uso)
						if ll_tot_reg>0 then
							for ll_indi=1 to ll_tot_reg
...
if as_tipo_carga='L' then
	...
	if ll_tot_reg>0 then
		for ll_indi=1 to ll_tot_reg
			...
			ldb_porc_pago									= dw_lista.getitemnumber(ll_indi,'porce_descuento')
			...
			ll_cod_det_tot									= 1
			ls_uso											= dw_lista.getitemstring(ll_indi,'uso')
			...
			dw_venta.setitem(ll_indi,'uso',ls_uso)
			dw_venta.accepttext()
		next
	end if
			...
			if ll_tot_reg2>0 then
				for ll_indi2=1 to ll_tot_reg2
					...
					if ll_cod_serv=ll_cod_serv2 and ll_cod_det=ll_cod_det2 then
						...
						dw_lista.setitem(ll_indi,'uso',ls_uso)
...
public subroutine wf_grabar (string as_moneda, long al_cod_serv, double adb_precio);string		ls_mon_orig,ls_forma_pag
long		ll_nro_cta,ll_gto_peso
double	ldb_pie,ldb_valor_uf,ldb_gtos_adm,ldb_int_uf,ldb_int_peso,ldb_gto_uf,ldb_interes,ldb_nulo
...
if dw_venta.update() = 1 then
	...
	if isvalid(w_ingresar_funeraria) then
		...
		w_ingresar_funeraria.dw_anexo_funeraria.setitem(1,'producto_pago_valor_cuota',ldb_nulo)
	...
	elseif isvalid(w_validar_promesa_operaciones) then
		...
		w_validar_promesa_operaciones.dw_revision.setitem(1,'valor_cuota',ldb_nulo)
...
on w_tipo_servicio_detalle_bk.create
...
on w_tipo_servicio_detalle_bk.destroy
destroy(this.cb_1)
...
il_cod_serv			= long(substr(1,1,message.stringparm))
...
is_uso				= substr(1,6,message.stringparm)
...
event clicked;if dw_venta.rowcount() >0 then 
	f_printdlg(dw_venta,gstr_print,w_tipo_servicio_detalle)
end if
end event
...
event clicked;string		ls_moneda,ls_gls_serv_c,ls_gls_det,ls_gls_det_final_c,ls_estado_c
long		ll_res,ll_cod_servicio,ll_serv_lista,ll_serv_vta,ll_cta,ll_tot_reg,&
			ll_cod_serv_c,ll_cod_det_c,ll_cod_det_final_c,ll_graba=0
...
if ll_tot_reg>0 then
	if ll_serv_lista=ll_serv_vta then
		...
		WHERE	( "PRODUCTO_VENTA_ASOCIADA"."BASE" = :is_base ) AND  
					...
					( "PRODUCTO_VENTA_ASOCIADA"."ESTADO" = 'V' )
		USING	sqlca;
		if sqlca.sqlcode=0 then
			if not isnull(ll_cta) and ll_cta>0 then
				messagebox("Advertencia","Existe Un Producto Vigente Asociado al Contrato: "+is_base+'-'+is_serie+'-'+string(il_numero))
				ll_res								= MessageBox("Grabar", 'Desea Grabar Modificaciones Servicio Funerario', Exclamation!, YesNo!, 2)
				if ll_res=1 then
					DECLARE x1 CURSOR FOR
					SELECT	"COD_SERVICIO",	"GLOSA_SERVICIO",	"COD_DETALLE",	"GLOSA_DETALLE",	"COD_DETALLE_FINAL",	"GLOSA_DETALLE_FINAL",	"FECHA_CREA",	"VALOR_PRODUCTO",	"PORCE_PAGO",	"ESTADO",		"FECHA_VIGENCIA"
					FROM		"PRODUCTO_VENTA_ASOCIADA"  
   					WHERE	( "PRODUCTO_VENTA_ASOCIADA"."BASE" = :is_base ) AND  
         						( "PRODUCTO_VENTA_ASOCIADA"."SERIE" = :is_serie ) AND  
					...
					if sqlca.sqlcode=0 then
						...
						fetch x1 INTO	:ll_cod_serv_c,:ls_gls_serv_c,:ll_cod_det_c,:ls_gls_det,:ll_cod_det_final_c,:ls_gls_det_final_c,:ldt_fec_cre_c,:ldb_valor_c,:ldb_por_pag_c,:ls_estado_c,:ldt_fec_vig_c;
							if not isnull(ll_cod_serv_c) and ll_cod_serv_c>0 and not isnull(ll_cod_det_c) and ll_cod_det_c>0 and not isnull(ll_cod_det_final_c) and ll_cod_det_final_c>0 then
								INSERT INTO "LOG_PRODUCTO_VENTA_ASOCIADA"  
												( "BASE",		"SERIE",	"NUMERO",	"COD_SERVICIO",	"GLOSA_SERVICIO",	"COD_DETALLE",	"GLOSA_DETALLE",	"COD_DETALLE_FINAL",	"GLOSA_DETALLE_FINAL",	"FECHA_CREA",	"VALOR_PRODUCTO",	"PORCE_PAGO",	"ESTADO",		"FECHA_VIGENCIA",	"FECHA_ELIMINA",	"USUARIO_ELIMINA" )  
								VALUES		( :is_base,	:is_serie,	:il_numero,	:ll_cod_serv_c,		:ls_gls_serv_c,			:ll_cod_det_c,		:ls_gls_det,				:ll_cod_det_final_c,		:ls_gls_det_final_c,			:ldt_fec_cre_c,		:ldb_valor_c,			:ldb_por_pag_c,	:ls_estado_c,	:ldt_fec_vig_c,			:gdt_fec_sistema,	:gs_user )
								USING	Trans_1;
							...
							setnull(ll_cod_serv_c);setnull(ls_gls_serv_c);setnull(ll_cod_det_c);setnull(ls_gls_det);setnull(ll_cod_det_final_c);setnull(ls_gls_det_final_c)
							setnull(ldb_valor_c);setnull(ldb_por_pag_c);setnull(ls_estado_c);setnull(ldt_fec_vig_c)
						LOOP
					end if
					close x1;
					if ll_graba=ll_cta then
						wf_grabar(ls_moneda,ll_cod_servicio,ldb_precio)
...
event clicked;close(w_tipo_servicio_detalle)
end event
