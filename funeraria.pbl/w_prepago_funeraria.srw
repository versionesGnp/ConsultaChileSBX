forward
global type w_prepago_funeraria from window
end type
type cb_pagare from commandbutton within w_prepago_funeraria
end type
type cb_3 from commandbutton within w_prepago_funeraria
end type
type dw_3 from datawindow within w_prepago_funeraria
end type
type dw_4 from datawindow within w_prepago_funeraria
end type
type dw_2 from datawindow within w_prepago_funeraria
end type
type dw_1 from datawindow within w_prepago_funeraria
end type
type cb_1 from commandbutton within w_prepago_funeraria
end type
type gb_1 from groupbox within w_prepago_funeraria
end type
end forward
...
global w_prepago_funeraria w_prepago_funeraria
...
on w_prepago_funeraria.destroy
...
gf_centrar(w_prepago_funeraria)
...
CHOOSE CASE gs_base
	...
	CASE "D"
		...
		dw_1.setitem(1,'pie',valor)
	CASE "F","G", "V"
		dw_2.DataObject	= 'd_datos_cuenta_ci_funeraria_repacta_1'
		dw_2.SetTransObject(SQLCA)
		dw_2.Retrieve(gs_serie,gi_numero)
		nro 					= dw_2.getitemnumber(1,'producto_pago_plazo')
		valor 				= dw_2.getitemnumber(1,'producto_pago_pie')
		dw_1.setitem(1,'nro_cuotas',nro)
		dw_1.setitem(1,'pie',valor)
END CHOOSE
...
//	select factor_gc into :factor from interes_cuotas where nro_cuotas = :cuotas;
	...
	moneda = dw_2.getitemstring(1,'producto_pago_moneda')
	if moneda = '2' then
...
ldb_precio						= double(dw_1.getitemnumber(1,'producto_pago_precio'))
ldb_pie							= double(dw_1.getitemnumber(1,'producto_pago_pie'))
ll_pie_pagado_peso			= double(dw_1.getitemnumber(1,'producto_pago_pie_pagado'))
ls_moneda						= dw_1.getitemstring(1,'producto_pago_moneda')
ll_plazo							= dw_1.getitemnumber(1,'producto_pago_plazo')
ll_ctas_mora					= dw_1.getitemnumber(1,'c_cuotas_mora')
ll_ctas_pag						= dw_1.getitemnumber(1,'producto_anexo_cuotas_pag')
ldb_valor_cuota				= round(double(dw_1.getitemnumber(1,'producto_pago_valor_cuota')),4)
ldb_tasa							= (double(dw_1.getitemnumber(1,'producto_pago_tasa')) / 100)
ldt_fecha_promesa			= dw_1.getitemdatetime(1,'producto_pago_fecha')
ldb_valor_cuotas_mora		= round((ll_ctas_mora * ldb_valor_cuota),4)
ll_valor_cuota_peso			= dw_1.getitemnumber(1,'c_valor_cuota_pesos')
ldb_gasto_adb					= dw_1.getitemnumber(1,'producto_pago_gasto_adm')
...
integer height = 376
boolean titlebar = true
string dataobject = "dw_imprime_prepago_funeraria_2"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
...
integer height = 460
integer taborder = 10
string dataobject = "d_datos_cuenta_ci_funeraria_repacta_1"
...
integer height = 964
integer taborder = 20
string dataobject = "d_encabezado_cuenta_cte_funeraria_pre"
boolean border = false
boolean livescroll = true
end type
...
event clicked;close(w_prepago_funeraria)
end event
