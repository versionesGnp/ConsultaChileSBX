forward
global type w_asigna_modif_reclamos_ugn from window
end type
type st_1 from statictext within w_asigna_modif_reclamos_ugn
end type
type dw_asig_sol_trabajo from datawindow within w_asigna_modif_reclamos_ugn
end type
type dw_asig_sol_nuevo from datawindow within w_asigna_modif_reclamos_ugn
end type
type dw_asig_sol_origen from datawindow within w_asigna_modif_reclamos_ugn
end type
type cb_gestionar from commandbutton within w_asigna_modif_reclamos_ugn
end type
type cb_cerrar from commandbutton within w_asigna_modif_reclamos_ugn
end type
type cb_limpiar from commandbutton within w_asigna_modif_reclamos_ugn
end type
type cb_1 from commandbutton within w_asigna_modif_reclamos_ugn
end type
type cb_grabar from commandbutton within w_asigna_modif_reclamos_ugn
end type
type gb_1 from groupbox within w_asigna_modif_reclamos_ugn
end type
end forward
...
string title = "Otras Gestiones UGN"
...
global w_asigna_modif_reclamos_ugn w_asigna_modif_reclamos_ugn

type variables
string		is_nivel,is_cod_solic,is_email
long		il_num_reclamo,il_cod_parque,il_cod_gestion,il_rut_contacto
...
public function long wf_graba_gestion (string as_user_crea, long al_cod_gestion, datetime adt_fec_gestion, string as_glosa_gestion, string as_user_asig, string as_depto_asig, string as_nivel, long al_num_reclamo);long	ll_graba=0
...
SET 		"USUARIO_CREA" = :as_user_crea,   
			"ULT_ESTADO_GESTION" = :al_cod_gestion,   
...
if sqlca.sqlcode=0 then
	commit;
	if al_cod_gestion=12 or al_cod_gestion=10 or al_cod_gestion=11 then
		UPDATE		"RECLAMO_SOLICITUD"  
...
setnull(ls_nulo)
ls_nom_contacto				= trim(dw_asig_sol_origen.getitemstring(1,'nombre_contacto'))
ls_ap_pat_contacto			= trim(dw_asig_sol_origen.getitemstring(1,'a_paterno_contacto'))
ls_ap_mat_contacto			= trim(dw_asig_sol_origen.getitemstring(1,'a_materno_contacto'))
ll_folio							= dw_asig_sol_origen.getitemnumber(1,'reclamo_solicitud_numero_reclamo')
ll_cod_reclamo					= dw_asig_sol_origen.getitemnumber(1,'codigo_reclamo')
...
if as_columna = 'codigo_gestion' then
	dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'usuario_asig',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'depto_asig',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'nivel',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'observacion',ls_nulo)
	ls_cod_solic									=	dw_asig_sol_nuevo.getitemstring(1,'codigo_solicitud')
	ll_cod_gestion								=	dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	...
	dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_fec_estado)
	dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_glosa_gestion)
	dw_asig_sol_nuevo.accepttext()
elseif as_columna = 'usuario_asig' then
	dw_asig_sol_nuevo.setitem(1,'depto_asig',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'nivel',ls_nulo)
	dw_asig_sol_nuevo.setitem(1,'observacion',ls_nulo)
	ls_user_asig										=	dw_asig_sol_nuevo.getitemstring(1,'usuario_asig')
	ls_cod_solic										=	dw_asig_sol_nuevo.getitemstring(1,'codigo_solicitud')
	ll_cod_gestion									=	dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	ldt_fec_estado									=	dw_asig_sol_nuevo.getitemdatetime(1,'fecha_gestion')
	ls_glosa_gestion								=	dw_asig_sol_nuevo.getitemstring(1,'glosa_gestion')
	if ll_cod_gestion <> 6 and not isnull(ll_cod_gestion) then
		if ll_cod_gestion<9 then
			ls_observ								=	dw_asig_sol_nuevo.getitemstring(1,'observacion')
		else
			ls_observ								=	dw_asig_sol_origen.getitemstring(1,'reclamo_solicitud_detalle_observacion')
		end if
		SELECT	"RECLAMO_CODIGO_TIPO"."DESCRIPCION"  
		INTO 		:ls_descrip  
		FROM 	"RECLAMO_CODIGO_TIPO"  
	...
	elseif ll_cod_gestion = 6 and not isnull(ll_cod_gestion) then
		...
		ls_texto_6									= 'ESTIMADO(A) '+ls_nom_contacto+' '+ls_ap_pat_contacto+' '+ls_ap_mat_contacto+'~r'+'~r'+'~r'
		...
		ls_texto_6									= ls_texto_6 +string(ll_folio,'###.###.###.###.##0')+', Y EL SIGUIENTE DETALLE.'+'~r'
		...
		ls_texto_6									= ls_texto_6 +'-  '+ls_descrip_reclamo+'~r'
		if ll_dias_solucion=1 then
			ls_texto_6								= ls_texto_6 +'SERA RESUELTO EN LAS PROXIMAS 24 HORAS.'+'~r'
		else
			ls_texto_6								= ls_texto_6 +'SERA RESUELTO EN LOS PROXIMOS '+string(ll_dias_solucion,'###,##0')+' DIAS.'+'~r'
		end if
		ls_texto_6									= ls_texto_6 +'ESTE EMAIL ES GENERADO DE MANERA AUTOMATICA, POR FAVOR NO RESPONDER.'+'~r'+'~r'
		ls_texto_6									= ls_texto_6 +'RESPONSABLE DE LA SOLICITUD:'+'~r'+'~r'
		ls_texto_6									= ls_texto_6 +'EJECUTIVO				'+ls_nom_encargado+'~r'
		ls_texto_6									= ls_texto_6 +'DEPARTAMENTO		'+ls_descrip_depto
		ls_observ									= ls_texto_6
	end if
	dw_asig_sol_nuevo.setitem(1,'codigo_gestion',ll_cod_gestion)
	dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_fec_estado)
	dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_observ) //ls_glosa_gestion)
	dw_asig_sol_nuevo.setitem(1,'depto_asig',ls_depto_asig)
	dw_asig_sol_nuevo.setitem(1,'nivel',ls_nivel)
	dw_asig_sol_nuevo.setitem(1,'observacion',ls_observ)
	dw_asig_sol_nuevo.accepttext()
	ll_tot_reg									= dw_asig_sol_trabajo.retrieve(ls_cod_solic,ls_user_asig)
	if ll_tot_reg>0 then
		...
		if sqlca.sqlcode=0 then
			if not isnull(ldt_fec_ini) and not isnull(ldt_fec_fin) then
				...
				dw_asig_sol_trabajo.setitem(1,'dia_compro',ll_dia_comp)
				dw_asig_sol_trabajo.setitem(1,'dia_actual',ll_dia_act)
				dw_asig_sol_trabajo.accepttext()
			end if
		end if
	end if
	dw_asig_sol_nuevo.accepttext()
	dw_asig_sol_nuevo.setcolumn('observacion')
elseif as_columna = 'observacion' then
	ll_cod_gestion								= dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	ldt_fec_estado								= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_gestion')
	ls_glosa_gestion							= dw_asig_sol_nuevo.getitemstring(1,'glosa_gestion')
	ls_user_asig									= dw_asig_sol_nuevo.getitemstring(1,'usuario_asig')
	ls_depto_asig								= dw_asig_sol_nuevo.getitemstring(1,'depto_asig')
	ls_nivel										= dw_asig_sol_nuevo.getitemstring(1,'nivel')
	ls_observ									= dw_asig_sol_nuevo.getitemstring(1,'observacion')
	dw_asig_sol_nuevo.setitem(1,'codigo_gestion',ll_cod_gestion)
	dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_fec_estado)
	dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_glosa_gestion)
...
elseif as_columna = 'fecha_compromiso' then
	ls_cod_solic									= dw_asig_sol_nuevo.getitemstring(1,'codigo_solicitud')
	ll_cod_gestion								= dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	ldt_fec_compr								= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_compromiso')
	if ll_cod_gestion <> 6 and not isnull(ll_cod_gestion) then
		SELECT	"RECLAMO_CODIGO_TIPO"."DESCRIPCION"  
		INTO 		:ls_descrip  
		FROM 	"RECLAMO_CODIGO_TIPO"  
...
on w_asigna_modif_reclamos_ugn.create
...
on w_asigna_modif_reclamos_ugn.destroy
destroy(this.st_1)
...
gf_centrar(w_asigna_modif_reclamos_ugn)
is_email																							= 'N'
dw_asig_sol_origen.dataobject 															= 'dw_asigna_modif_reclamo_origen'
dw_asig_sol_origen.settransobject(sqlca)
...
ll_tot_reg1																						= dw_asig_sol_origen.retrieve(il_num_reclamo,il_cod_gestion,idt_fec_gestion)
if ll_tot_reg1>0 then
	SELECT	MAX("RECLAMO_SOLICITUD_DETALLE"."FECHA_COMPROMISO")
	INTO 		:ldt_fec_comp  
	FROM 	"RECLAMO_SOLICITUD_DETALLE"  
	WHERE ( "RECLAMO_SOLICITUD_DETALLE"."NUMERO_RECLAMO" = :il_num_reclamo ) AND  
			  ( "RECLAMO_SOLICITUD_DETALLE"."CODIGO_GESTION" = 6 )  
	USING		sqlca;
	if sqlca.sqlcode=0 then
		dw_asig_sol_origen.setitem(1,'c_max_fec_comp',ldt_fec_comp)
	else
		dw_asig_sol_origen.setitem(1,'c_max_fec_comp',ldt_nulo)
	end if
end if
...
dw_asig_sol_nuevo.dataobject 									= 'dw_asigna_modif_reclamo_origen_ugn'
dw_asig_sol_nuevo.settransobject(sqlca)
dw_asig_sol_nuevo.getchild('codigo_gestion',idw_detalle5)
idw_detalle5.settransobject(sqlca)
if idw_detalle5.retrieve(gs_depto,is_nivel,ll_etapa)=0 then
	idw_detalle5.insertrow(0)
end if
dw_asig_sol_nuevo.getchild('usuario_asig',idw_detalle6)
idw_detalle6.settransobject(sqlca)
if idw_detalle6.retrieve(gs_depto)=0 then
	idw_detalle6.insertrow(0)
end if
dw_asig_sol_nuevo.getchild('codigo_solucion',idw_detalle8)
idw_detalle8.settransobject(sqlca)
if idw_detalle8.retrieve(gs_depto)=0 then
	idw_detalle8.insertrow(0)
end if
dw_asig_sol_trabajo.dataobject 									= 'dw_asigna_modif_reclamo_origen_trab'
dw_asig_sol_trabajo.settransobject(sqlca)
dw_asig_sol_trabajo.getchild('ult_estado_gestion',idw_detalle7)
idw_detalle7.settransobject(sqlca)
if idw_detalle7.retrieve(gs_depto)=0 then
	idw_detalle7.insertrow(0)
end if
ll_new																						= dw_asig_sol_nuevo.insertrow(0)
dw_asig_sol_nuevo.scrolltorow(ll_new)
dw_asig_sol_nuevo.setitem(ll_new,"numero_reclamo",il_num_reclamo)
dw_asig_sol_nuevo.setitem(ll_new,"cod_parque",il_cod_parque)
dw_asig_sol_nuevo.setitem(ll_new,"codigo_solicitud",is_cod_solic)
dw_asig_sol_nuevo.setitem(ll_new,"fecha_crea",idt_fec_crea)
dw_asig_sol_nuevo.setitem(ll_new,"rut_contacto",il_rut_contacto)
dw_asig_sol_nuevo.setitem(ll_new,"usuario_crea",gs_user)
dw_asig_sol_nuevo.setitem(ll_new,"codigo_notificacion",5)
dw_asig_sol_nuevo.accepttext()

dw_asig_sol_origen.accepttext()
...
integer taborder = 20
string title = "none"
string dataobject = "dw_asigna_modif_reclamo_origen_ugn"
boolean border = false
boolean livescroll = true
end type

event itemchanged;string		ls_columna,ls_user_asig,ls_pasa,ls_nulo,ls_depto_crea,ls_cod_solic_ant,ls_descrip,ls_ult_glosa
...
ls_columna = dwo.name
ll_cod_parque_ant																				= il_cod_parque
ls_cod_solic_ant																				= dw_asig_sol_origen.getitemstring(1,'codigo_solicitud')
ldt_fec_crea_ant																				= dw_asig_sol_origen.getitemdatetime(1,'fecha_crea')
ll_rut_contac_ant																				= dw_asig_sol_origen.getitemnumber(1,'rut_contacto')
ll_gestion_ant																					= dw_asig_sol_origen.getitemnumber(1,'ult_estado_gestion')
ll_cod_reclamo																					= dw_asig_sol_origen.getitemnumber(1,'codigo_reclamo')
...
if ls_columna = 'codigo_gestion' then
	ll_gestion_act																				= dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	if ll_gestion_act = 10 then
		dw_asig_sol_nuevo.setitem(1,'codigo_solucion',ll_nulo)
		dw_asig_sol_nuevo.accepttext()
	end if
	if ll_gestion_ant = 0 and ll_gestion_act <> 1 then
		messagebox("Advertencia","No Puede Ingresar Tipo de Gestión")
		dw_asig_sol_nuevo.setitem(1,'codigo_gestion',ll_nulo)
		dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_nulo)
		dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_nulo)
		dw_asig_sol_nuevo.accepttext()
		ls_pasa		= 'N'
	...
	elseif ll_gestion_ant = 5 and ll_gestion_act<>1 and ll_gestion_act <> 4 and ll_gestion_act <> 6 and ll_gestion_act <> 10 then
		...
		dw_asig_sol_nuevo.accepttext()
		ls_pasa		= 'N'	
	elseif ll_gestion_ant = 6 and ll_gestion_act <> 6 and ll_gestion_act <> 7 and ll_gestion_act <> 8 and ll_gestion_act <> 10 then
		messagebox("Advertencia","No Puede Ingresar Tipo de Gestión")
		dw_asig_sol_nuevo.setitem(1,'codigo_gestion',ll_nulo)
		dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_nulo)
		dw_asig_sol_nuevo.setitem(1,'glosa_gestion',ls_nulo)
	...
	elseif ll_gestion_ant = 12 and ll_gestion_act <> 0 then
		messagebox("Advertencia","No Puede Ingresar Tipo de Gestión")
		dw_asig_sol_nuevo.setitem(1,'usuario_asig',ls_user_asig)
		dw_asig_sol_nuevo.object.usuario_asig.protect			= 1
		dw_asig_sol_nuevo.accepttext()
		ls_pasa		= 'N'		
	else
		ls_pasa		= 'S'
	...
	elseif ls_pasa='S' then
		if ll_gestion_act=1 or ll_gestion_act=3 or ll_gestion_act=4 or ll_gestion_act=5 or ll_gestion_act=7 or ll_gestion_act=8 or ll_gestion_act=9 or ll_gestion_act=10 or ll_gestion_act=11 then
			...
			elseif ll_gestion_act=3 or ll_gestion_act=5 or ll_gestion_act=7 or ll_gestion_act=8 or ll_gestion_act=11 then
				ll_cod_parque_ant																= dw_asig_sol_origen.getitemnumber(1,'cod_parque')
				ls_cod_solic_ant																= dw_asig_sol_origen.getitemstring(1,'codigo_solicitud')
				ldt_fec_crea_ant																= dw_asig_sol_origen.getitemdatetime(1,'fecha_crea')
				ll_rut_contac_ant																= dw_asig_sol_origen.getitemnumber(1,'rut_contacto')
			...
			if ll_gestion_act =10 or  ll_gestion_act =7 then
				dw_asig_sol_nuevo.setitem(1,'codigo_notificacion',5)
			end if
			dw_asig_sol_nuevo.accepttext()
		...
		elseif ll_gestion_act=2 then
			...
			dw_asig_sol_nuevo.object.usuario_asig.protect			= 0
			dw_asig_sol_nuevo.accepttext()
		elseif ll_gestion_act=6 then
			...
			dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_fec_estado)
			dw_asig_sol_nuevo.setitem(1,'usuario_asig',ls_user_asig)
			dw_asig_sol_nuevo.setitem(1,'fecha_compromiso',ldt_fec_compr)
			dw_asig_sol_nuevo.accepttext()
			wf_modif_itemchanged('usuario_asig')
		...
		elseif ll_gestion_act=12 then
			ls_user_asig																				= gs_user
			ldt_fec_compr																			= dw_asig_sol_origen.getitemdatetime(1,'reclamo_solicitud_detalle_fecha_compromiso')
			dw_asig_sol_nuevo.setitem(1,'fecha_gestion',ldt_fec_estado)
			dw_asig_sol_nuevo.setitem(1,'usuario_asig',ls_user_asig)
			dw_asig_sol_nuevo.setitem(1,'fecha_compromiso',ldt_fec_compr)
			dw_asig_sol_nuevo.accepttext()
...
if ls_columna = 'p_fec_compro' then
	ldt_fecha											= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_compromiso')
	if f_valida_fecha(string(ldt_fecha,'dd/mm/yyyy'))=-1 then 
		dw_asig_sol_nuevo.setitem(1,'fecha_compromiso',today())
		dw_asig_sol_nuevo.setfocus()
		dw_asig_sol_nuevo.setcolumn('fecha_compromiso')
		return
	end if	
	...
	IF Message.StringParm <> ls_fecha THEN
		...
		dw_asig_sol_nuevo.setitem(1,'fecha_compromiso',ldt_fecha)
		wf_modif_itemchanged('fecha_compromiso')
		dw_asig_sol_nuevo.accepttext()
	end if
end if		
...
event clicked;string 	ls_cod_solicitud,ls_user_crea,ls_observ,ls_glosa_gestion,ls_user_asig,ls_depto_asig,ls_nivel,ls_pasa,ls_string
...
datetime	ldt_fec_crea,ldt_fec_gestion,ldt_fec_compro,ldt_fec_gestion_ant,ldt_fec_ini,ldt_fec_fin,ldt_fecha_comp,ldt_nulo
...
if is_email = 'N' then
	ls_pasa														= 'N'
	ll_num_reclamo											= dw_asig_sol_origen.getitemnumber(1,'reclamo_solicitud_numero_reclamo')
	ll_cod_gestion_ant											= dw_asig_sol_origen.getitemnumber(1,'ult_estado_gestion')
	ldt_fec_gestion_ant										= dw_asig_sol_origen.getitemdatetime(1,'ult_fecha_gestion')
	ls_cod_solicitud												= dw_asig_sol_nuevo.getitemstring(1,'codigo_solicitud')
	ldt_fec_crea													= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_crea')
	ll_rut_contac												= dw_asig_sol_nuevo.getitemnumber(1,'rut_contacto')
	ls_user_crea												= dw_asig_sol_nuevo.getitemstring(1,'usuario_crea')
	ll_cod_gestion												= dw_asig_sol_nuevo.getitemnumber(1,'codigo_gestion')
	ldt_fec_gestion												= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_gestion')
	ls_observ													= dw_asig_sol_nuevo.getitemstring(1,'observacion')
	ls_glosa_gestion											= dw_asig_sol_nuevo.getitemstring(1,'glosa_gestion')
	ll_cod_parque												= dw_asig_sol_nuevo.getitemnumber(1,'cod_parque')
	ls_user_asig													= dw_asig_sol_nuevo.getitemstring(1,'usuario_asig')
	ls_depto_asig												= dw_asig_sol_nuevo.getitemstring(1,'depto_asig')
	ls_nivel														= dw_asig_sol_nuevo.getitemstring(1,'nivel')
	ldt_fec_compro												= dw_asig_sol_nuevo.getitemdatetime(1,'fecha_compromiso')
	...
	if ls_pasa='S' then
		...
		if ll_res=1 then
			if dw_asig_sol_nuevo.update() = 1 then
				commit using sqlca;
				...
				if ll_graba>=1 then
					...
					close(w_asigna_modif_reclamos_ugn)
				end if
			else
...
event clicked;close(w_asigna_modif_reclamos_ugn)
end event
...
//		ls_parrafo_4									= ''
//		
//		select	EnviarMail('mirkom@nuestrosparques.cl','mirkom@nuestrosparques.cl',:ls_asunto,:ls_texto) 
//		INTO 		:ls_res
//		from 		DUAL;
//		if ls_res='OK' then
