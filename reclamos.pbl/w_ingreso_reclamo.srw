forward
global type w_ingreso_reclamo from window
end type
type cb_proceso from commandbutton within w_ingreso_reclamo
end type
type cb_ing_gestion from commandbutton within w_ingreso_reclamo
end type
type ddplb_contratos from dropdownpicturelistbox within w_ingreso_reclamo
end type
type cb_detalle_gestion from commandbutton within w_ingreso_reclamo
end type
type cb_fallecido from commandbutton within w_ingreso_reclamo
end type
type cb_ctacte from commandbutton within w_ingreso_reclamo
end type
type cb_print from commandbutton within w_ingreso_reclamo
end type
type cb_anular from commandbutton within w_ingreso_reclamo
end type
type cb_buscar from commandbutton within w_ingreso_reclamo
end type
type cb_limpiar from commandbutton within w_ingreso_reclamo
end type
type cb_cerrar from commandbutton within w_ingreso_reclamo
end type
type cb_grabar from commandbutton within w_ingreso_reclamo
end type
type dw_ingreso from datawindow within w_ingreso_reclamo
end type
end forward
...
global w_ingreso_reclamo w_ingreso_reclamo

type variables
datawindowchild			idw_detalle5,idw_detalle6,idw_detalle7,idw_detalle8
Long		il_veces, il_modif,il_rut,il_parque
string		is_dv,is_area,is_sector,is_sepultura,is_boton_reclamo
end variables
...
public function string wf_validar ();Long		ll_reclamo,ll_cod_parque,ll_rut_titular,ll_cod_reclamo,ll_cod_asig,ll_rut_contacto,ll_ult_cod_gestion,ll_cod_parque_usuario
String		ls_cod_solicitud	,ls_base,ls_serie,ls_area,ls_sector,ls_sepultura,ls_cod_gerencia,ls_usuario_asig,ls_depto_asig,ls_nom_contacto,&
			ls_ap_pat_contacto,ls_email,ls_fono_part,ls_fono_comer,ls_celular,ls_ult_glosa,ls_tipo_solicitud,ls_glosa,ls_pasa='S'
...
ls_tipo_solicitud		= dw_ingreso.getitemstring(1,'tipo_solicitud')
if isnull(ls_cod_solicitud) then
	messagebox("Advertencia","Debe Seleccionar Solicitud")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('codigo_solicitud')
	ls_pasa				= 'N'
elseif isnull(ls_tipo_solicitud) then
	messagebox("Advertencia","Debe Seleccionar Tipo Solicitud")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('tipo_solicitud')
	ls_pasa				= 'N'
elseif isnull(ll_cod_parque) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Seleccionar Parque")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('cod_parque')
	ls_pasa				= 'N'
elseif isnull(ls_base) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Seleccionar Base Contrato")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('base')
	ls_pasa				= 'N'
elseif isnull(ls_serie) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Seleccionar Serie Contrato")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('serie')
	ls_pasa				= 'N'
elseif isnull(ll_numero) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Ingresar Número Contrato")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('numero')
	ls_pasa				= 'N'
elseif isnull(ll_rut_titular) and ll_cod_parque_usuario <> 9999 then
...
elseif isnull(ll_cod_reclamo) then
	...
	ls_pasa				= 'N'
elseif isnull(ls_area) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Seleccionar Area Sepultura")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('area')
	ls_pasa				= 'N'
elseif isnull(ls_sector) and ll_cod_parque_usuario <> 9999 then
	messagebox("Advertencia","Debe Seleccionar Sector Sepultura")
	dw_ingreso.setfocus()
	dw_ingreso.setcolumn('sector')
	ls_pasa				= 'N'
elseif isnull(ls_sepultura) and ll_cod_parque_usuario <> 9999 then
...
event open;String		ls_estado,ls_descrip,ls_estado_titular,ls_dv
Long		ll_new,ll_cod_parque,ll_mora_cred,ll_mora_mant,ll_tipo_cliente

SELECT sysdate  INTO :gdt_fec_sistema FROM "TASA"  WHERE "TASA"."LOOK" = 1   ;
if gs_user='MIRKO' then
...
if isvalid(w_cuenta_corriente_oferta) then
	...
	il_parque				= Double(substr(1,6,Message.StringParm))
	is_area 				= substr(1,7,Message.StringParm)
	is_sector				= substr(1,8,Message.StringParm)
	is_sepultura			= substr(1,9,Message.StringParm)
	is_boton_reclamo	= substr(1,10,Message.StringParm)
	
	
	
	if is_boton_reclamo = 'S' then
		dw_ingreso.setitem(ll_new,'codigo_solicitud',3)
		dw_ingreso.setitem(ll_new,'tipo_solicitud','S')
		...
		dw_ingreso.setitem(ll_new,'area',is_area)
		dw_ingreso.setitem(ll_new,'sector',is_sector)
		dw_ingreso.setitem(ll_new,'rut_titular',il_rut)
		dw_ingreso.setitem(ll_new,'sepultura',is_sepultura)
		
		idw_detalle.retrieve(il_parque)
		idw_detalle2.retrieve(il_parque)
		idw_detalle3.retrieve(il_parque,is_sector)
		idw_detalle6.retrieve(gs_base)
		dw_ingreso.setitem(ll_new,'area',is_area)
		dw_ingreso.setitem(ll_new,'sector',is_sector)
		dw_ingreso.setitem(ll_new,'sepultura',is_sepultura)	
		...
		SELECT 		"CLIENTE"."ESTADO_TITULAR","CLIENTE"."TIPO_CLIENTE","CLIENTE"."DV"    
		INTO 			:ls_estado_titular, :ll_tipo_cliente, :ls_dv  
		FROM 		"CLIENTE"  
		WHERE 		"CLIENTE"."RUT" = :il_rut  
		USING		sqlca;
		//--
		dw_ingreso.setitem(1,'rut_titular',il_rut)
		dw_ingreso.setitem(ll_new,'c_dv',ls_dv)
		if not isnull(ll_tipo_cliente) then
			dw_ingreso.setitem(1,'cliente_estado_titular',ls_estado_titular)
		end if
		if not isnull(ll_tipo_cliente) then
			dw_ingreso.setitem(1,'cliente_tipo_cliente',ll_tipo_cliente)
		end if	
	end if	
end if	
//--
...
event clicked;long	ll_cod_reclamo,ll_cod_parque,ll_dias_faltantes, ll_dias_alerta, ll_contar_reclamos
String	ls_depto_asig, ls_email

DECLARE x1 CURSOR FOR  
SELECT 	"VISTA_RECLAMO_LISTA_CONTEO"."CODIGO_RECLAMO",   
			"VISTA_RECLAMO_LISTA_CONTEO"."COD_PARQUE",   
			"VISTA_RECLAMO_LISTA_CONTEO"."DEPTO_ASIG",   
			"VISTA_RECLAMO_LISTA_CONTEO"."EMAIL",   
			"VISTA_RECLAMO_LISTA_CONTEO"."DIAS_FALTANTES",   
			"VISTA_RECLAMO_LISTA_CONTEO"."DIAS_ALERTA",   
			"VISTA_RECLAMO_LISTA_CONTEO"."CONTAR_RECLAMOS"  
FROM 	"VISTA_RECLAMO_LISTA_CONTEO"
WHERE	"VISTA_RECLAMO_LISTA_CONTEO"."DIAS_FALTANTES" = "VISTA_RECLAMO_LISTA_CONTEO"."DIAS_ALERTA"
USING	sqlca;
open x1;
if sqlca.sqlcode=0 then
	do while sqlca.sqlcode=0
		fetch x1 into :ll_cod_reclamo, :ll_cod_parque, :ls_depto_asig, :ls_email, :ll_dias_faltantes, :ll_dias_alerta, :ll_contar_reclamos;
...
event selectionchanged;String		ls_string,ls_base,ls_serie,ls_estado,ls_sector,ls_descrip,ls_depto,ls_area,ls_sepultura,ls_cod_gerencia_select,ls_codigo_sol,ls_tipo_solicitud,ls_cod_reclamo,&
			ls_cod_gerencia
Long		ll_rut,ll_cod_parque,ll_mora_cred,ll_mora_mant,ll_largo,ll_nulo
Double	ll_numero
...
if not isnull(ls_string) then
	...
	if not isnull(ls_base) and not isnull(ls_serie) and ll_numero > 0 then
		...
		if sqlca.sqlcode= 0 then
			...
			ls_cod_reclamo		= dw_ingreso.getitemstring(1,'codigo_solicitud')
...
//				FROM 	"ENCARGADOS"  
//				WHERE 	"ENCARGADOS"."DEPTO" = :ls_depto and length("ENCARGADOS"."RECLAMO_COG_GEREN") > 0 ;
			end if
...
dw_ingreso.getchild('usuario_asignado',idw_detalle4)
idw_detalle4.settransobject(sqlca)
idw_detalle4.retrieve(gs_user,9999,'GP')
...
event clicked;Long		ll_reclamo,ll_cod_parque,ll_rut_titular,ll_cod_reclamo,ll_cod_asig,ll_rut_contacto,ll_ult_cod_gestion,ll_max,ll_res,ll_cod_parque_usuario,ll_dias_compr,ll_count
String		ls_cod_solicitud	,ls_usuario_asig,ls_depto_asig,	ls_ult_glosa,ls_glosa,ls_pasa,ls_nom_contacto,ls_ap_pat_contacto,ls_email,ls_fono_part,&
			...
			ls_nivel,ls_cod_reg,ls_tipo_solicitud,ls_estado,ls_email_para,ls_descrip,ls_glosa_gestion,ls_asunto,ls_glosa_email,ls_nombre_usuario,ls_email_de,ls_res,ls_cierre_auto
Datetime	ldt_nulo,ldt_fec_compr
Double	ll_numero,ll_bandera

ls_cod_reg							= dw_ingreso.getitemstring(1,'cod_estado_reg')
if ls_cod_reg = 'A' then
	messagebox("Advertencia","No es posible Actualizar Requerimiento - Estado ANULADO")
else
	SELECT sysdate  INTO :gdt_fec_sistema FROM "TASA"  WHERE "TASA"."LOOK" = 1   ;
	Setnull(ldt_nulo)
	SELECT	"ENCARGADOS"."COD_PARQUE"  
	...
	if ll_reclamo > 0 then
		...
		if ls_pasa='S' then
			...
			ll_ult_cod_gestion			= dw_ingreso.getitemnumber(1,'ult_estado_gestion')
			ls_tipo_solicitud			= dw_ingreso.getitemstring(1,'tipo_solicitud')
			ls_cod_solicitud				= dw_ingreso.getitemstring(1,'codigo_solicitud') //nuevo
			ll_cod_reclamo				= dw_ingreso.getitemnumber(1,'codigo_reclamo') // nuevo
			
			SELECT 	"RECLAMO_CODIGO_RECLAMO_AUTO"."ESTADO_CIERRE"  
			INTO 		:ls_cierre_auto
			FROM 	"RECLAMO_CODIGO_RECLAMO_AUTO"
			WHERE 	( "RECLAMO_CODIGO_RECLAMO_AUTO"."CODIGO" = :ll_cod_reclamo ) AND
						( "RECLAMO_CODIGO_RECLAMO_AUTO"."CORRELATIVO"  = (SELECT MAX("RECLAMO_CODIGO_RECLAMO_AUTO"."CORRELATIVO") 
																									 FROM "RECLAMO_CODIGO_RECLAMO_AUTO" 
																									 WHERE "RECLAMO_CODIGO_RECLAMO_AUTO"."CODIGO" = :ll_cod_reclamo));
																									 
			if isnull(ls_cierre_auto) then ls_cierre_auto='N'
			if ls_tipo_solicitud	= 'F' or ls_tipo_solicitud	= 'T' or ls_tipo_solicitud = 'P' or ls_cod_solicitud	= '7' or ls_cierre_auto = 'S' or (ls_usuario_asig = gs_user and ls_tipo_solicitud = 'R') then
				if ls_cod_solicitud	= '7' and ls_usuario_asig = gs_user then ll_ult_cod_gestion	 = 10
				if ls_cod_solicitud	= '3' and ls_usuario_asig = gs_user and ls_tipo_solicitud = 'R' then ll_ult_cod_gestion	 = 17
				if ls_tipo_solicitud	= 'F' then ll_ult_cod_gestion	 = 14
				if ls_tipo_solicitud	= 'T' then ll_ult_cod_gestion = 15
				if ls_tipo_solicitud	= 'P' then ll_ult_cod_gestion = 16
				if ls_cierre_auto	= 'S' then ll_ult_cod_gestion = 10
				dw_ingreso.setitem(1,'ult_estado_gestion',ll_ult_cod_gestion)
				ls_cod_reg			= 'G'
				dw_ingreso.setitem(1,'cod_estado_reg',ls_cod_reg)
				dw_ingreso.accepttext()
			end if	
			...
			if len(ls_email) < 2  and len(ls_fono_part) < 2  and len(ls_fono_comer) < 2 and len(ls_celular) < 2  then
				...
				if dw_ingreso.update()=1 then
					...
					if ll_cod_parque_usuario = 9999 then
						ll_res						= messagebox("Grabar","Grabación Exitosa Requerimiento N° "+String(ll_reclamo,'###,###,###,###,##0')+", desea Ingresar Otro a Departamento Seleccionado",Exclamation!,YesNo!,2)
					else
						ll_res						= messagebox("Grabar","Grabación Exitosa Requerimiento N° "+String(ll_reclamo,'###,###,###,###,##0')+", desea Ingresar Nuevo Requerimiento a Sepultura "+ls_sector+'-'+ls_sepultura,Exclamation!,YesNo!,2)
					end if
					if ll_res=1 then
						ls_cod_solicitud			= dw_ingreso.getitemstring(1,'codigo_solicitud')
						ls_tipo_solicitud		= dw_ingreso.getitemstring(1,'tipo_solicitud')
						...
						dw_ingreso.setitem(1,'tipo_solicitud',ls_tipo_solicitud)
						dw_ingreso.setitem(1,'cod_parque',ll_cod_parque)
						dw_ingreso.setitem(1,'base',ls_base)
						dw_ingreso.setitem(1,'serie',ls_serie)
						dw_ingreso.setitem(1,'numero',ll_numero)
		...
		if ls_pasa='S' then
			...
			if len(ls_email) < 2  and len(ls_fono_part) < 2  and len(ls_fono_comer) < 2 and len(ls_celular) < 2  then
				...
				if dw_ingreso.update()=1 then
					...
					ldt_fec_compr			= f_retorna_fecha_vcto_requerimiento(gdt_fec_sistema,ll_dias_compr)
					...
					if ls_usuario_asig = gs_user and ll_ult_cod_gestion=4  then
						...
						VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				2,								:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
						USING		sqlca;
						if sqlca.sqlcode=0 then
							commit;
						else
					...
					//- autogestion para reclamos seleccionados rapidos 
					if ls_cod_solicitud = '5' or ls_cod_solicitud = '7' or ls_cod_solicitud = '3' and ls_usuario_asig = gs_user and ls_cierre_auto <> 'S' then
						SELECT 	count("RECLAMO_CODIGO_RECLAMO_AUTO"."CODIGO")
						INTO 		:ll_count  
						FROM 	"RECLAMO_CODIGO_RECLAMO_AUTO"  
						WHERE 	"RECLAMO_CODIGO_RECLAMO_AUTO"."CODIGO" = :ll_cod_reclamo;
						if ll_count = 1 then
							INSERT INTO 	  "RECLAMO_SOLICITUD_DETALLE"  
												( "NUMERO_RECLAMO",		"CODIGO_SOLICITUD",	"FECHA_CREA",	"USUARIO_CREA",		"CODIGO_GESTION",		"FECHA_GESTION",	"OBSERVACION",	"GLOSA_GESTION",	"RUT_CONTACTO",	"COD_PARQUE",	"USUARIO_ASIG",		"DEPTO_ASIG",		"NIVEL",		"FECHA_COMPROMISO" )  
							VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				17,								:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
							USING		sqlca;
							if sqlca.sqlcode=0 then
								commit;
								ll_ult_cod_gestion	= 10
								ls_cod_reg			= 'G'
								UPDATE 		"RECLAMO_SOLICITUD"  
								SET 			"COD_ESTADO_REG" = :ls_cod_reg  
								WHERE 		"RECLAMO_SOLICITUD"."NUMERO_RECLAMO" = :ll_max
								USING		sqlca;
							else
								rollback;
							end if
					...
					if ls_cod_solicitud = '5' or ls_cod_solicitud = '7' or ls_cod_solicitud = '3' and (ls_usuario_asig = gs_user and ls_tipo_solicitud = 'R') then
							UPDATE 		"RECLAMO_SOLICITUD_DETALLE"  
							SET 			"CODIGO_GESTION" = 17
							WHERE 		"RECLAMO_SOLICITUD_DETALLE"."NUMERO_RECLAMO" = :ll_max
							USING		sqlca;
							if sqlca.sqlcode=0 then
								commit;
							else
					...
					//- fin
					//-- auto gestion solicitud interna 01/06/2019
					if ls_usuario_asig = gs_user and ll_ult_cod_gestion=10 and  ls_cod_solicitud = '7'   then
						INSERT INTO 	  "RECLAMO_SOLICITUD_DETALLE"  
											( "NUMERO_RECLAMO",		"CODIGO_SOLICITUD",	"FECHA_CREA",	"USUARIO_CREA",		"CODIGO_GESTION",		"FECHA_GESTION",	"OBSERVACION",	"GLOSA_GESTION",	"RUT_CONTACTO",	"COD_PARQUE",	"USUARIO_ASIG",		"DEPTO_ASIG",		"NIVEL",		"FECHA_COMPROMISO" )  
						VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				2,								:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
						USING		sqlca;
						if sqlca.sqlcode=0 then
							...
							INSERT INTO 	  "RECLAMO_SOLICITUD_DETALLE"  
												( "NUMERO_RECLAMO",		"CODIGO_SOLICITUD",	"FECHA_CREA",	"USUARIO_CREA",		"CODIGO_GESTION",		"FECHA_GESTION",	"OBSERVACION",	"GLOSA_GESTION",	"RUT_CONTACTO",	"COD_PARQUE",	"USUARIO_ASIG",		"DEPTO_ASIG",		"NIVEL",		"FECHA_COMPROMISO" )  
							VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				4,								:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
							USING		sqlca;
							if sqlca.sqlcode=0 then
								commit;
							else
					...
					if ls_usuario_asig = gs_user and  ls_cod_solicitud = '3' and ls_tipo_solicitud = 'R' and ll_ult_cod_gestion = 17 then
						INSERT INTO 	  "RECLAMO_SOLICITUD_DETALLE"  
											( "NUMERO_RECLAMO",		"CODIGO_SOLICITUD",	"FECHA_CREA",	"USUARIO_CREA",		"CODIGO_GESTION",		"FECHA_GESTION",	"OBSERVACION",	"GLOSA_GESTION",	"RUT_CONTACTO",	"COD_PARQUE",	"USUARIO_ASIG",		"DEPTO_ASIG",		"NIVEL",		"FECHA_COMPROMISO" )  
						VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				17,								:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
						USING		sqlca;
					...
					//--fin				
					if ll_bandera <> 1 then
						INSERT INTO 	  "RECLAMO_SOLICITUD_DETALLE"  
											( "NUMERO_RECLAMO",		"CODIGO_SOLICITUD",	"FECHA_CREA",	"USUARIO_CREA",		"CODIGO_GESTION",		"FECHA_GESTION",	"OBSERVACION",	"GLOSA_GESTION",	"RUT_CONTACTO",	"COD_PARQUE",	"USUARIO_ASIG",		"DEPTO_ASIG",		"NIVEL",		"FECHA_COMPROMISO" )  
						VALUES 			( :ll_max, 						:ls_cod_solicitud, 			:gdt_fec_sistema,	:gs_user,				:ll_ult_cod_gestion,		:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto, 		:ll_cod_parque, 	:ls_usuario_asig, 		:ls_depto_asig, 	'2',				:ldt_fec_compr)  
						USING		sqlca;
						if sqlca.sqlcode=0 then
							commit;
						else
...
//					if ls_usuario_asig = gs_user and gs_depto='P' then
					if ll_numero > 0 and not isnull(ls_base) and not isnull(ls_serie) then
						SELECT 	"RECLAMO_CODIGO_RECLAMO"."CODIGO_ESTADO_ATC"  
						INTO 		:ls_estado  
						FROM 	"RECLAMO_CODIGO_RECLAMO"  
						WHERE 	"RECLAMO_CODIGO_RECLAMO"."CODIGO" = :ll_cod_reclamo 
						USING	sqlca;
						if not isnull(ls_estado) and len(ls_estado) > 0 then
							INSERT INTO "ATENCION_LOG"  
											( "RUT"  ,		"ESTADO"  ,"FECHA"  ,			"GLOSA"    ,		"USUARIO" ,	"BASE"  ,	"SERIE"  ,	"NUMERO" )  
							VALUES 		( :ll_rut_titular,	:ls_estado,	:gdt_fec_sistema,	:ls_ult_glosa,	:gs_user,	:ls_base,:ls_serie,		:ll_numero)  ;
							if sqlca.sqlcode <> 0 then
					...
					if isnull(ls_email_para) or ls_email_para='-' or ls_email_para='' then
						...
						FROM		"RECLAMO_ESTADO"  
						WHERE 	( "RECLAMO_ESTADO"."CODIGO_ESTADO" = :ll_ult_cod_gestion )
						USING	sqlca;
						if sqlca.sqlcode=0 then
						...
						ls_asunto										= ls_glosa_gestion+' A '+ls_descrip + ' Nº : '+string(ll_max,'###,###,###,###')
						ls_glosa_email									= 'ESTIMADO(A)      SISTEMA MODULO REQUERIMIENTO INFORMA QUE ' + ls_glosa_gestion + ' Nº : '+string(ll_max,'###,###,###,###') + ', EL MOTIVO ES '+ls_ult_glosa+'.    FAVOR GESTIONAR SOLICITUD DE CLIENTE '+ls_nom_contacto+ ' '+ls_ap_pat_contacto+'.'
						SELECT	"NOMBRE",				"EMAIL" 
						...
						if ls_res='OK' then
							...
							VALUES		( :ll_max,					:ls_cod_solicitud,			:gdt_fec_sistema,	:gs_user,			99,						:gdt_fec_sistema,		:ls_ult_glosa,		:ls_ult_glosa,			:ll_rut_contacto,		:ll_cod_parque,		:ls_usuario_asig,	:ls_depto_asig,		'2')
							USING	sqlca;
							if sqlca.sqlcode = 0 then
								commit using sqlca;
							else
...
//	else
//		dw_ingreso.setitem(1,'ult_estado_gestion',1)
//	end if
	...
	//elseif (ls_tipo_solicitud = 'F'	 or ls_tipo_solicitud = 'T') then
		//dw_ingreso.setitem(1,'ult_estado_gestion',10)
	elseif ls_tipo_solicitud = 'F' then
		dw_ingreso.setitem(1,'ult_estado_gestion',14)
	elseif ls_tipo_solicitud = 'T' then
		dw_ingreso.setitem(1,'ult_estado_gestion',15)
	else
		dw_ingreso.setitem(1,'ult_estado_gestion',1)
	end if
	dw_ingreso.setitem(1,'codigo_reclamo',ll_nulo)
...
elseif ls_columna='numero_reclamo_original' then
	ll_folio_asoc		= dw_ingreso.getitemnumber(1,'numero_reclamo_original')
	if ll_folio_asoc > 0 then
		...
		if ll_count_req = 0 or isnull(ll_count_req) then
			...
			dw_ingreso.setitem(1,'numero_reclamo_original',ll_nulo)
		end if
	end if
...
elseif ls_columna='codigo_solicitud' then
	...
	ls_depto				= dw_ingreso.getitemstring(1,'depto_asig')
	ll_cod_parque		= dw_ingreso.getitemnumber(1,'cod_parque')
	ls_usu_asignado	= dw_ingreso.getitemstring(1,'usuario_asignado')
	ll_numero			= dw_ingreso.getitemnumber(1,'numero')
	ls_tipo_solicitud	= dw_ingreso.getitemstring(1,'tipo_solicitud')
	if not isnull(ls_usu_asignado) or ll_numero > 0 then
		...
		dw_ingreso.setitem(1,'codigo_solicitud',ls_codigo_sol)
		dw_ingreso.accepttext()
	end if
	SELECT	"RECLAMO_CODIGO_TIPO"."CODIGO_GERENCIA"  
	INTO 		:ls_cod_gerencia_select  
	...
	if ll_cod_parque_usuario = 9999 and ls_cod_gerencia_select <> 'GQ' then 
		idw_detalle4.retrieve(gs_user,ll_cod_parque_usuario,ls_cod_gerencia_select)
		dw_ingreso.setitem(1,'cod_parque',ll_cod_parque_usuario)
		dw_ingreso.accepttext()
	else
		idw_detalle4.retrieve(gs_user,ll_cod_parque,ls_cod_gerencia_select)
	end if
	if ls_codigo_sol='4' or ls_codigo_sol='5' then
...
Long		ll_estado,ll_nulo,ll_rut,ll_count_reg,ll_max_reclamo,ll_res,ll_cod_parque_usuario,ll_folio_asoc,ll_count_req
