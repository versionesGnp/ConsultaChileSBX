forward
global type w_servipag_bkp from window
end type
type dw_baestado from datawindow within w_servipag_bkp
end type
type dw_pagos_stgo from datawindow within w_servipag_bkp
end type
type dw_pagos from datawindow within w_servipag_bkp
end type
type dw_no_existe from datawindow within w_servipag_bkp
end type
type dw_lista_cupon_existe from datawindow within w_servipag_bkp
end type
type tab_1 from tab within w_servipag_bkp
end type
...
type dw_servipag from datawindow within w_servipag_bkp
end type
type st_1 from statictext within w_servipag_bkp
end type
type cb_cerrar from commandbutton within w_servipag_bkp
end type
end forward
...
string title = "Generar Archivo Envio SERVIPAG"
...
global w_servipag_bkp w_servipag_bkp
...
string title = "none"
string dataobject = "dwe_recuperar_servipag_stgo"
...
String	ls_fecha_hoy,ls_string,ls_monto_total,ls_numero_cliente,ls_rut,ls_referencia,&
			ls_tipo_documento,ls_numero_documento,ls_fecha_emision,ls_saldo_anterior,&
			ls_fecha_vencimiento,ls_interes,ls_cobranza_externa,ls_cuota,ls_base,ls_serie
Long		ll_tot_reg,ll_rut,ll_cant_reg,ll_largo,ll_blanco,ll_new,ll_indi
DateTime	ldt_fecha_hoy
Double	ldb_uf_dia,ldb_tasa_peso,ldb_tasa_uf,id_factor_gc,ldb_interes_gc,ll_numero

SetPointer(HourGlass!)
st_ultimo.text							= ''
dw_servipag.dataobject				= 'dwe_enviar_servipag'
dw_baestado.dataobject				= 'dwe_enviar_servipag'
...
if sqlca.sqlcode = 0 then
   ...
   WHERE   "TASA"."LOOK" = 1  ;
	...
	DECLARE sp_proc_cadena_mora_servipag PROCEDURE FOR sp_cadena_mora_servipag(:ldt_fecha_hoy);
...
event clicked;Long		ll_indi,ll_tot_reg,ll_max_corr,ll_count_reg,ll_numero_cupon,ll_monto_cuota,&
			ll_interes,ll_gasto_cob,ll_rut,ll_ctas_pag_s,ll_ctas_pag_m,&
			ll_tot_pagado_s,ll_tot_pagado_m,ll_nro_cuota
String	ls_fecha_archivo,ls_linea,ls_servipag,ls_tipo_mov,ls_serie,ls_base,ls_caja,ls_nombre_archivo
...
if ll_tot_reg > 0 then
				...
				if ll_count_reg>0 then
					...
					if sqlca.sqlcode=0 then
						...
						for ll_indi=1 to ll_tot_reg
							...
							VALUES 	 ( :ll_numero_cupon,	'CU',			:ld_fecha_pago,	:ls_tipo_mov,	:ll_monto_cuota,	1,					:ls_serie,	:ll_numero,	:ls_caja,	' ',			:ldb_uf_dia,	'A',				:ls_base,	:ll_rut,	:gs_user,	null,			null,				null,					null,					null,			null,				null,			:ld_fecha_pago,	'0',				'N',				null )  
							USING		sqlca;
							if sqlca.sqlcode=0 then
								commit;
								CHOOSE CASE ls_base
								...
								if ll_interes > 0 then
									INSERT INTO "INGRESO"  
												 ( "FOLIO",				"TIPO_COB",	"FECHA_PAGO",		"TIPO_MOV",		"MONTO",				"CUOTAS_PAG",	"SERIE",		"CONTRATO",	"COD_CAJA",	"REZAGO",	"VALOR_UF",		"PAGO_HIST",	"BASE",		"RUT",	"USUARIO",	"COD_AGE",	"FECHA_VENC",	"PAGO_EXTERNO",	"LUGAR_EXTERNO",	"VIENE_DE",	"FECHA_ANT",	"FCHA_ANT",	"SERVIPAG",			"CODIGO_OTRO",	"TRASPASO_E",	"FECHA_TRASPASO_REZAGO" )
									VALUES 	 ( :ll_numero_cupon,	'CU',			:ld_fecha_pago,	'G',				:ll_interes,		0,					:ls_serie,	:ll_numero,	:ls_caja,	' ',			:ldb_uf_dia,	'A',				:ls_base,	:ll_rut,	:gs_user,	null,			null,				null,					null,					null,			null,				null,			:ld_fecha_pago,	'0',				'N',				null )  
									USING		sqlca;
									if sqlca.sqlcode=0 then
										commit;
									else
								...
								if ll_gasto_cob > 0 then
									INSERT INTO "INGRESO"  
												 ( "FOLIO",				"TIPO_COB",	"FECHA_PAGO",		"TIPO_MOV",		"MONTO",				"CUOTAS_PAG",	"SERIE",		"CONTRATO",	"COD_CAJA",	"REZAGO",	"VALOR_UF",		"PAGO_HIST",	"BASE",		"RUT",	"USUARIO",	"COD_AGE",	"FECHA_VENC",	"PAGO_EXTERNO",	"LUGAR_EXTERNO",	"VIENE_DE",	"FECHA_ANT",	"FCHA_ANT",	"SERVIPAG",			"CODIGO_OTRO",	"TRASPASO_E",	"FECHA_TRASPASO_REZAGO" )
									VALUES 	 ( :ll_numero_cupon,	'CU',			:ld_fecha_pago,	'J',				:ll_gasto_cob,		0,					:ls_serie,	:ll_numero,	:ls_caja,	' ',			:ldb_uf_dia,	'A',				:ls_base,	:ll_rut,	:gs_user,	null,			null,				null,					null,					null,			null,				null,			:ld_fecha_pago,	'0',				'N',				null )  
									USING		sqlca;
									if sqlca.sqlcode=0 then
										commit;
									else
						...
						if dw_cargar.update()=1 then
							...
							rollback;
							messagebox("Error Grabar","Error al Carga SERVIPAG SQL: "+sqlca.sqlerrtext)
						end if
	...
	elseif gs_conexion = "Parque Santiago" then
		if dw_cargar.getitemnumber(1,'numero') > 0 then
			ld_fecha_archivo	= date(dw_cargar.getitemdatetime(1,'fecha_pago'))
			SELECT	"TAB_UF"."VALOR_UF"  
			INTO 		:ldb_uf_dia  
			FROM 		"TAB_UF"  
			WHERE 	"TAB_UF"."FECHA_UF" = :ld_fecha_archivo   ;
				...
				if sqlca.sqlcode=0 then
					...
					for ll_indi=1 to ll_tot_reg
						...
						if sqlca.sqlcode=0 then
							...
							END CHOOSE
							DECLARE sp_nuevo_cadena_mora1 PROCEDURE FOR sp_nuevo_cadena_mora(:ls_base,:ls_serie,:ll_numero,'M');
					...
					if dw_lista_cupon_existe.rowcount()>0 then
						...
						if dw_lista_cupon_existe.update()=1 then
							commit;
							if messagebox("Advertencia","Desea Imprimir Pagos NO Cargado(s)",Exclamation!,YesNo!,2)=1 then
								cb_print_cupones.triggerevent(clicked!)
							end if
						else
							rollback;
							messagebox("Error Grabar","Error al Grabar Cupones Existentes SQL: "+sqlca.sqlerrtext)
					...
					rollback;
					messagebox("Error Grabar","Error al Grabar LOG_SERVIPAG SQL: "+sqlca.sqlerrtext)
				end if
			end if
		else
			messagebox("Advertencia","Debe Verificar Nº Contrato")
...
Long		ll_indi,ll_tot_reg,ll_numero_cupon,ll_rut,ll_interes,ll_cobranza_externa,&
			ll_cuota,ll_monto_cuota,ll_new,ll_cod_parque,ll_existe,&
			ll_ctas_pag_m,ll_nro_cuota
String	docname,ls_linea,ls_rut,ls_dv,ls_tipo_documento,ls_fecha_archivo,&
			ls_numero_cupon,ls_interes,ls_cobranza_externa,ls_cuota,ls_fecha_pago,&
			ls_servipag,ls_tipo_mov,ls_monto_cuota,ls_base,ls_serie,ls_forma_pago,&
			ls_cod_caja,ls_caja,ls_numero_pagare,ls_nom_arch
...
event clicked;if dw_problemas.rowcount() > 0 then f_Print( dw_problemas )		//f_printdlg(dw_problemas,gstr_print,w_servipag)
...
if dw_problemas.rowcount() > 0 then f_DWToExcel( dw_paso )
end event
