forward
global type w_ingresar_nota_de_credito_2 from window
end type
type dw_print_nc from datawindow within w_ingresar_nota_de_credito_2
end type
type st_7 from statictext within w_ingresar_nota_de_credito_2
end type
type ddplb_motivo from dropdownpicturelistbox within w_ingresar_nota_de_credito_2
end type
type dw_detalle_factura from datawindow within w_ingresar_nota_de_credito_2
end type
type sle_1 from uo_convierte_numero within w_ingresar_nota_de_credito_2
end type
type dw_factura from datawindow within w_ingresar_nota_de_credito_2
end type
type cb_ctacte from commandbutton within w_ingresar_nota_de_credito_2
end type
type cb_limpiar from commandbutton within w_ingresar_nota_de_credito_2
end type
type cb_grabar from commandbutton within w_ingresar_nota_de_credito_2
end type
type st_6 from statictext within w_ingresar_nota_de_credito_2
end type
type dw_nota_credito from datawindow within w_ingresar_nota_de_credito_2
end type
type cb_generar from commandbutton within w_ingresar_nota_de_credito_2
end type
type pb_eliminar from picturebutton within w_ingresar_nota_de_credito_2
end type
type pb_insertar from picturebutton within w_ingresar_nota_de_credito_2
end type
type dw_destino from datawindow within w_ingresar_nota_de_credito_2
end type
type rb_comprobante from radiobutton within w_ingresar_nota_de_credito_2
end type
type rb_contrato from radiobutton within w_ingresar_nota_de_credito_2
end type
type st_5 from statictext within w_ingresar_nota_de_credito_2
end type
type st_1 from statictext within w_ingresar_nota_de_credito_2
end type
type em_folio from editmask within w_ingresar_nota_de_credito_2
end type
type ddlb_tipo_cob from dropdownlistbox within w_ingresar_nota_de_credito_2
end type
type cb_cerrar from commandbutton within w_ingresar_nota_de_credito_2
end type
type dw_lista from datawindow within w_ingresar_nota_de_credito_2
end type
type pb_procesar from picturebutton within w_ingresar_nota_de_credito_2
end type
type em_numero from editmask within w_ingresar_nota_de_credito_2
end type
type st_4 from statictext within w_ingresar_nota_de_credito_2
end type
type st_3 from statictext within w_ingresar_nota_de_credito_2
end type
type st_2 from statictext within w_ingresar_nota_de_credito_2
end type
type ddlb_base from dropdownlistbox within w_ingresar_nota_de_credito_2
end type
type gb_1 from groupbox within w_ingresar_nota_de_credito_2
end type
type ddlb_serie from dropdownlistbox within w_ingresar_nota_de_credito_2
end type
type gb_2 from groupbox within w_ingresar_nota_de_credito_2
end type
type gb_3 from groupbox within w_ingresar_nota_de_credito_2
end type
end forward
...
global w_ingresar_nota_de_credito_2 w_ingresar_nota_de_credito_2

type variables
string		is_opcion,is_nota_credito
Long		il_row,il_row_destino,il_max_folio_new,il_ok_boleta
...
//	end if
	...
	if ls_tipo_cob_ori = 'BA' or ls_tipo_cob_ori ='FA' then
		ll_new_reg			= dw_factura.insertrow(0)
		ls_string_boleta	= 'Tasa IVA'+Space(22)+' | '+string(int((idb_iva - 1) * 100))
		dw_factura.setitem(ll_new_reg,'linea_factura',ls_string_boleta)
	end if
	ll_new_reg			= dw_factura.insertrow(0)
...
if al_numero > 0 then
	...
	WHERE ( "CADENA"."CODIGO" = "REPACTA_MANTENCION"."BASE" ) and  
				...
				( "CADENA"."NUMERO" = :al_numero ) )   
	UNION   
	SELECT 	"PRODUCTO_PAGO"."PLAZO",   	"CADENA"."RUT",   	"CLIENTE"."DV",   	"PRODUCTO_PAGO"."VALOR_CUOTA",   	0 as valor_cuota_m,   	"CADENA"."COD_PARQUE",   	"PRODUCTO_PAGO"."FECHA_PRIM",   	"CADENA_MORA"."FECHA_VENC_MANT",   	"PRODUCTO_PAGO"."MONEDA"  
	FROM 	"PRODUCTO_ANEXO",   "CADENA",   "CADENA_MORA",   "CLIENTE",   "PRODUCTO_PAGO"  
...
on w_ingresar_nota_de_credito_2.create
...
on w_ingresar_nota_de_credito_2.destroy
...
datetime	ldt_fecha_pago,ldt_nulo,ld_fecha_hoy
...
ddlb_tipo_cob.reset()
//ddplb_motivo.reset()
if il_max_folio_new > 0 and il_ok_boleta = 0 then
	ldt_fecha_pago		= datetime(date(gdt_fec_sistema),time('00:00:00'))
	DELETE FROM "INGRESO_NEWSING"  
	WHERE 	( "INGRESO_NEWSING"."FOLIO" = :il_max_folio_new ) AND  
				( "INGRESO_NEWSING"."TIPO_COB" = :gs_tipo_cobro ) AND  
				( "INGRESO_NEWSING"."FECHA_PAGO" = :ldt_fecha_pago ) AND  
	...
	WHERE 	( "INGRESO_NEWSING_CABECERA"."TIPO_COB" = :gs_tipo_cobro ) AND  
				( "INGRESO_NEWSING_CABECERA"."FOLIO" = :il_max_folio_new ) AND  
				( "INGRESO_NEWSING_CABECERA"."FECHA_PAGO" = :ldt_fecha_pago ) AND  
	...
	WHERE 	( "DOCUMENTOS_NEWSING"."FOLIO" = :il_max_folio_new ) AND  
				( "DOCUMENTOS_NEWSING"."TIPO_COB" = :gs_tipo_cobro ) AND  
				( "DOCUMENTOS_NEWSING"."FECHA_PAGO" = :ldt_fecha_pago ) AND  
...
event clicked;Long		ll_folio,ll_monto,ll_ctas_pag,ll_sum,ll_graba1=0,ll_graba2=0,ll_graba3=0,ll_graba4=0,ll_graba5=0,ll_suma_total,ll_tot_reg,ll_indi,ll_folio_mod,ll_monto_mod,ll_cuotas_pag,ll_numero,ll_count_reg_pie,&
			...
			ll_suma_total_nc,ll_ok_boleta,ll_fila_baf,ll_fila_bae
Datetime	ldt_fecha_pago,ldt_fecha_pago_mod,ldt_pago
String		ls_tipo_cob,ls_tipo_mov,ls_base,ls_serie,ls_tipo_nc,ls_tipo_motivo,ls_rut_empresa,ls_rezago,ls_monto_palabra,email_de,ls_email_para,ls_asunto,ls_texto,ls_ok_boleta,ls_res
...
//		ddplb_motivo.setfocus()
		...
		if isnull(dw_nota_credito.getitemstring(1,'codigo_otro')) then
			...
			if ls_tipo_cob='BA' or ls_tipo_cob='FA' or ls_tipo_cob='FE' then
				if ls_tipo_cob='BA' then
					ll_fila_baf		 			= dw_destino.Find("tipo_mov = 'F'", 1, dw_destino.RowCount())
					ll_fila_bae		 			= dw_destino.Find("(tipo_mov = 'E' or tipo_mov = 'A') and base='L' ", 1, dw_destino.RowCount())
					if ll_fila_baf > 0 or ll_fila_bae > 0 then
						dw_nota_credito.setitem(1,'codigo_otro','0')
					else
		...
		if gs_tipo_cobro = 'NE' then
			DECLARE sp_proc_be PROCEDURE FOR F_XML_NE_ADEXUS(:gs_tipo_cobro, :il_max_folio_new, :gd_hoy, :gs_caja, :gi_rut, :ls_tipo_cob, :ll_folio_mod, :ldt_fecha_pago_mod,'1.0');
			EXECUTE sp_proc_be;
			FETCH sp_proc_be into :ll_ok_boleta ;
			ls_ok_boleta				= string(ll_ok_boleta)
			if ll_ok_boleta > 0 then
				il_ok_boleta			= ll_ok_boleta
				dw_nota_credito.setitem(1,'folio',ll_ok_boleta)
				dw_nota_credito.accepttext()
				UPDATE 	"INGRESO_NEWSING_CABECERA"  
				SET 		"FOLIO_BE" = :ll_ok_boleta  
				WHERE 	( "INGRESO_NEWSING_CABECERA"."TIPO_COB" = :gs_tipo_cobro ) AND  
							( "INGRESO_NEWSING_CABECERA"."FOLIO" = :il_max_folio_new ) AND  
				...
				SET 			"FOLIO_BE_ING" = :ll_ok_boleta  
				WHERE 	( "INGRESO_NEWSING"."FOLIO" = :il_max_folio_new ) AND  
							( "INGRESO_NEWSING"."TIPO_COB" = :gs_tipo_cobro ) AND  
							( "INGRESO_NEWSING"."FECHA_PAGO" = :gd_hoy ) AND  
							( "INGRESO_NEWSING"."COD_CAJA" = :gs_caja ) AND  
							( "INGRESO_NEWSING"."RUT" = :gi_rut )   
				USING	sqlca;
				if sqlca.sqlcode=0 then
				...
				UPDATE 		"DOCUMENTOS_NEWSING"  
				SET 			"FOLIO_BE" = :ll_ok_boleta  
				WHERE 	( "DOCUMENTOS_NEWSING"."TIPO_COB" = :gs_tipo_cobro ) AND  
							( "DOCUMENTOS_NEWSING"."FOLIO" = :il_max_folio_new ) AND  
							( "DOCUMENTOS_NEWSING"."FECHA_PAGO" = :gd_hoy ) AND  
							( "DOCUMENTOS_NEWSING"."COD_CAJA" = :gs_caja )   
				USING	sqlca;
				if sqlca.sqlcode=0 then
					commit;
				...
				messagebox("Error",string(ll_ok_boleta))
				
				email_de			= 'mirkom@nuestrosparques.cl'
				ls_email_para	= 'mirkom@nuestrosparques.cl'
				ls_asunto		= 'Error XML Nota de Crédito Eletronica '+gs_tipo_cobro+string(il_max_folio_new)
...
//							f_printdlg(dw_print_nc,gstr_print,w_ingresar_nota_de_credito_2)
	...
	//			
				...
				SET 		   "FOLIO_COMPROBANTE" = 0  
				WHERE 	( "FICHA_SEPULTACION"."NUMERO" = :ll_numero ) AND  
							( "FICHA_SEPULTACION"."FOLIO_COMPROBANTE" = :ll_folio_mod )   
				USING		sqlca;
				if sqlca.sqlcode=0 then
					commit;
				else
				...
				SET 		  "FOLIO_BOLETA" = 0  
				WHERE 	( "FICHA_SEPULTACION_PAGO"."NUMERO" = :ll_numero ) AND  
							( "FICHA_SEPULTACION_PAGO"."TIPO_COB" = :ls_tipo_cob ) AND  
							( "FICHA_SEPULTACION_PAGO"."FOLIO_BOLETA" = :ll_folio_mod )   
				USING	sqlca;
				if sqlca.sqlcode=0 then
					commit;
				else
				...
				messagebox("Grabar","Grabación Exitosa Nota de Credito "+string(ll_ok_boleta))
...
event clicked;Long		ll_new,ll_total,ll_suma_cuotas,ll_min_boleta,ll_grupo,ll_folio,ll_fila_e,ll_cod_parque,ll_ctas_pag_s_sap,ll_folio_original,ll_exento,ll_neto,	ll_iva,ll_cod_parque_cab,&
			ll_fila_baf,ll_fila_bae,ll_largo,ll_tot_reg_des,ll_indi,ll_fila_4,ll_fila_6,ll_monto_4=0,ll_monto_6=0,ll_fila_f,ll_fila_d,ll_monto_f,ll_dscto_f,ll_fila_dscto,ll_cod_parque_ori
Date		ld_fecha
String		ls_rut_empresa,ls_nombre_comp,ls_ap_pat,ls_ap_mat,ls_tipo_cob,ls_nom,ls_estado_contrato,ls_tipo_cob_ori,ls_rezago,ls_tipo_mov,ls_codigo_otro,ls_traspaso,&
			ls_moneda_sap,ls_tipocob_original,ls_nro_contrato,ls_fecha_pago_palabra,ls_monto_total_palabra,ls_cod_mov
datetime	ldt_fecha_pago,ldt_pago,ldt_pago_original
Double	ldb_monto_sap,ll_monto_iva=0,ll_sum_iva_inmob=0
...
//		end if
		...
		dw_nota_credito.setitem(ll_new,'cuotas_pag',ll_suma_cuotas)
		dw_nota_credito.setitem(ll_new,'fecha_pago',ld_fecha)
		dw_nota_credito.setitem(ll_new,'moneda_sap','1')
		dw_nota_credito.setitem(ll_new,'monto_uf',ll_total)
		...
		dw_nota_credito.setitem(ll_new,'monto_uf',ll_total)
		if ls_rezago=' ' then
			dw_nota_credito.setitem(ll_new,'rezago_sap','N')
		else
			dw_nota_credito.setitem(ll_new,'rezago_sap',ls_rezago)
		end if
		dw_nota_credito.setitem(ll_new,'tipo_mov','Z')
...
//	gs_tipo_cobro					= "NE"
	...
	if gs_base <> 'B' and ls_estado_contrato='A' and ll_fila_e > 0 then
		...
		ll_tot_reg_des					= dw_destino.rowcount()
		for ll_indi=1 to ll_tot_reg_des
			ls_cod_mov					= dw_destino.getitemstring(ll_indi,'tipo_mov')
			if ls_cod_mov='D' or ls_cod_mov='X' or ls_cod_mov='V' then
				ll_monto_iva			= dw_destino.getitemnumber(ll_indi,'monto')
				ll_sum_iva_inmob		= ll_sum_iva_inmob + ll_monto_iva
			end if
		next
		if ls_tipo_cob_ori = 'BA' or ls_tipo_cob_ori = 'FA' then 
			ll_fila_baf		 			= dw_destino.Find("tipo_mov = 'F' or tipo_mov='M'", 1, dw_destino.RowCount())
			ll_fila_bae		 			= dw_destino.Find("(tipo_mov = 'E' or tipo_mov = 'A') and base='L' ", 1, dw_destino.RowCount())
			if ll_fila_baf > 0 or ll_fila_bae > 0 or ll_sum_iva_inmob > 0 then
				ll_cod_parque 			= gl_cod_parque
				if ll_cod_parque=142 and gs_base='G' and gs_serie='I' then
					ll_cod_parque		= 150
				end if
			else
				if gs_base='G' or gs_base='F' then
					if gs_base='G' and (gs_serie='I') then
						ll_cod_parque	= 150
					else
						ll_cod_parque 	= 99
					end if
					...
					ll_cod_parque 		= gl_cod_parque
					SELECT 	"INGRESO_NEWSING_CABECERA"."COD_PARQUE"  
					INTO 		:ll_cod_parque_ori  
					FROM 	"INGRESO_NEWSING_CABECERA"  
					WHERE ( "INGRESO_NEWSING_CABECERA"."FOLIO_BE" = :ll_folio_original ) AND  
							  ( "INGRESO_NEWSING_CABECERA"."TIPO_COB" = :ls_tipocob_original ) AND  
							  ( "INGRESO_NEWSING_CABECERA"."FECHA_PAGO" = :ldt_pago_original )   ;
					if ll_cod_parque_ori <> gl_cod_parque then
						ll_cod_parque		= ll_cod_parque_ori
					end if
				end if
			end if
			if ll_fila_bae > 0 then
				ll_fila_4				= dw_destino.Find("codigo_otro = '4' and base='L' ", 1, dw_destino.RowCount())
				ll_fila_6				= dw_destino.Find("codigo_otro = '6' and base='L' ", 1, dw_destino.RowCount())
				if ll_fila_4 > 0 then
					ll_monto_4		= dw_destino.getitemnumber(ll_fila_4,'monto')
				end if
				if ll_fila_6 > 0 then
					ll_monto_6		= dw_destino.getitemnumber(ll_fila_6,'monto')
				end if
			end if
		else
			ll_cod_parque 		= gl_cod_parque
		...
		if gl_cod_parque > 0 and gs_tipo_cobro	= "NC" then
			if gl_cod_parque=142 or gl_cod_parque=150 then
		...
		elseif gl_cod_parque > 0 and gs_tipo_cobro	= "NE" then
			SELECT DISTINCT	MAX("INGRESO_NEWSING_CABECERA"."FOLIO")
			...
			if il_max_folio_new > 0 then
				...
				ls_monto_total_palabra				= trim(ls_monto_total_palabra)
				ll_largo									= len(ls_monto_total_palabra)
				if mid(ls_monto_total_palabra,ll_largo,1) = ',' then 
					ls_monto_total_palabra 			= mid(ls_monto_total_palabra,1,(ll_largo - 1))
				end if
				
				gl_valor_iva								= ll_sum_iva_inmob
				if ls_tipocob_original = 'BA' or ls_tipocob_original = 'FA' then
...
//						ll_total							= long(st_total.text)
						...
						ll_tot_reg_des					= dw_destino.rowcount()
						if ll_tot_reg_des > 0 then
							ll_fila_f 						= dw_destino.Find( "tipo_mov = 'F'", 1,  ll_tot_reg_des)
							if ll_fila_f > 0 then
								ll_fila_d 					= dw_destino.Find( "tipo_mov = 'D'", 1,  ll_tot_reg_des)
								if ll_fila_d = 0 then
									ll_monto_f			= dw_destino.getitemnumber(ll_fila_f,'monto')
									ll_fila_dscto			= dw_destino.Find( "tipo_mov = 'S' and codigo_otro='132'", 1,  ll_tot_reg_des)
									ll_exento				= ll_monto_f
									ll_neto				= round((ll_total - gl_valor_iva) - ll_exento  ,0)
									if ll_fila_dscto > 0 then
										ll_dscto_f		= dw_destino.getitemnumber(ll_fila_dscto,'monto')
										ll_exento			= ll_monto_f + ll_dscto_f
										ll_neto			= round((ll_total - gl_valor_iva) - ll_exento  ,0)
										if ll_neto < 0 then
							...
							ll_fila_f 						= dw_destino.Find( "tipo_mov = 'E'", 1,  ll_tot_reg_des)
							if ll_fila_f > 0 then
								ll_fila_4 					= dw_destino.Find( "tipo_mov = '4'", 1,  ll_tot_reg_des)
								ll_fila_6 					= dw_destino.Find( "tipo_mov = '6'", 1,  ll_tot_reg_des)
								ll_fila_d 					= dw_destino.Find( "tipo_mov = 'D'", 1,  ll_tot_reg_des)
								if ll_fila_d = 0 then
									ll_monto_f			= dw_destino.getitemnumber(ll_fila_f,'monto')
									ll_fila_dscto			= dw_destino.Find( "tipo_mov = 'S' and codigo_otro='131'", 1, ll_tot_reg_des)
									if ll_fila_4 > 0 then
										ll_monto_4		= dw_destino.getitemnumber(ll_fila_4,'monto')
									end if
									if ll_fila_6 > 0 then
									...
									ll_exento				= ll_monto_f + ll_monto_4 + ll_monto_6
									ll_neto				= round((ll_total - gl_valor_iva) - ll_exento  ,0)
									if ll_fila_dscto > 0 then
										ll_dscto_f		= dw_destino.getitemnumber(ll_fila_dscto,'monto')
										ll_exento			= ll_monto_f + ll_dscto_f + ll_monto_4 + ll_monto_6
										ll_neto			= round((ll_total - gl_valor_iva) - ll_exento  ,0)
										if ll_neto < 0 then
											ll_exento		= ll_exento + ll_neto
											ll_neto		= 0
...
//				if ls_tipocob_original = 'BA' or ls_tipocob_original = 'FA' then
//					if ll_sum_iva_inmob > 0 then
//						ll_exento							= ll_monto_4 + ll_monto_6
//						ll_iva								= ll_sum_iva_inmob
//						ll_neto							= ((round(ll_total - ll_iva,0) ) - ( ll_monto_4 + ll_monto_6))
//						ll_total							= ll_neto + ll_iva
//					else
//						ll_exento							= 0
//						ll_neto							= round(ll_total / idb_iva,0)
//						ll_iva								= round(ll_total - ll_neto,0)
////						ll_iva								= round((ll_total * idb_tasa_iva) / 100,0)
////						ll_neto							= round(ll_total - ll_iva,0)
//						ll_total							= ll_neto + ll_iva
//					end if
//				else
//					ll_exento								= ll_total
...
//				is_proximo_pago				= f_retorna_prox_pago_mant( gs_base, gs_serie, gi_numero, ai_cuotas_pag,as_tipo_mov )
				...
				if sqlca.sqlcode=0 then
					...
					SELECT 	"INGRESO_NEWSING_CABECERA"."COD_PARQUE"  
					INTO 		:ll_cod_parque_cab  
					FROM 	"INGRESO_NEWSING_CABECERA"  
					WHERE 	( "INGRESO_NEWSING_CABECERA"."NRO_CONTRATO" = :ls_nro_contrato ) AND  
								( "INGRESO_NEWSING_CABECERA"."TIPO_COB" = :ls_tipocob_original ) AND  
								( "INGRESO_NEWSING_CABECERA"."FOLIO_BE" = :ll_folio_original ) AND  
								( "INGRESO_NEWSING_CABECERA"."FECHA_PAGO" = :ldt_pago_original )   ;
					...
					INSERT INTO "INGRESO_NEWSING_CABECERA"  
  					...
  					VALUES 		( :gs_tipo_cobro,	:il_max_folio_new,	:ldt_fecha_pago,	:ls_fecha_pago_palabra,		:ll_cod_parque_cab,		:ls_nro_contrato,		:gd_uf_dia,	:gs_caja,			:gs_user,		null,				null,			null,						:ls_monto_total_palabra,			null,				null,				null,				null,				null,				:gi_rut,				:ls_rezago,	:ll_neto,		:ll_exento,	:ll_iva,	:ll_total,		:gs_tcp_ip_print,	:gs_tcp_ip,	:gs_empresa,	null,								:ls_tipocob_original,			:ll_folio_original,			:ldt_pago_original				) 
					USING	sqlca;
					if sqlca.sqlcode=0 then
						commit;
					else
