forward
global type w_categorias_sup from window
end type
type st_1 from statictext within w_categorias_sup
end type
type dw_1 from uo_datawindow within w_categorias_sup
end type
type ddlb_1 from dropdownlistbox within w_categorias_sup
end type
type cb_2 from commandbutton within w_categorias_sup
end type
type dw_3 from datawindow within w_categorias_sup
end type
type dw_2 from datawindow within w_categorias_sup
end type
type cb_1 from commandbutton within w_categorias_sup
end type
end forward
...
global w_categorias_sup w_categorias_sup
...
WHERE ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
	...
	( "PAGO_OFERTA"."MONEDA" = '1' ) )  and ("OFERTA_V"."COD_SUP" = :cod);
...
WHERE ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
	...
	( ( "OFERTA_V"."CIERRE_VENTA" = (  SELECT DISTINCT max( "CIERRE_VENTA"."CIERRE_VENTA")  
													 FROM "CIERRE_VENTA" )) AND  
	( "PAGO_OFERTA"."MONEDA" = '2' ) )  and ("OFERTA_V"."COD_SUP" = :cod);
...
WHERE ( "ANEXO_LIBERADOR"."SERIE_M" = "PAGO_LIBERADOR"."SERIE_M" ) and  
	...
	( ( "ANEXO_LIBERADOR"."CIERRE_VENTA" = (  SELECT DISTINCT MAX("CIERRE_VENTA"."CIERRE_VENTA")  
															  FROM "CIERRE_VENTA" )) AND  
	( "PAGO_LIBERADOR"."MONEDA" = '2' ) AND  	( "ANEXO_LIBERADOR"."COD_SUPER" = :cod ) )   ;
...
f_ddlb_all_clasificacion(ddlb_1,"SELECT JEFE_VENTAS,NOMBRE ,A_PATERNO FROM JEFE_VENTAS WHERE ESTADO = 'A'" ,0)
//                   "SELECT cod_sup    , nombre,a_paterno FROM supervisor where estado = 'A'"
if gs_depto='B' then
	ddlb_1.text = gs_user 
	ddlb_1.enabled = false
	ddlb_1.triggerevent(selectionchanged!)
elseif gs_depto='R' or gs_depto='K' or gs_depto='M' then
	messagebox("Advertencia","No Tiene Acceso")
	close(w_categorias_sup)
else
end if
...
event doubleclicked;call super::doubleclicked;string cod_age,age,nombre,tipo_clas,mes1,mes2,mes3,mes4,mes5,mes6,mes7,mes8,mes9,mes10,mes11,mes12,&
...
dw_2.dataobject='w_clas_fecha_agente'
dw_3.dataobject='dw_agetentes_categoria'

dw_2.settransobject(sqlca)
dw_3.settransobject(sqlca)
if row > 0 then
	cod_sup = getitemstring(row,'codigo')
	dw_1.reset()
	if dw_3.retrieve(cod_sup) > 0 then
		...
		dw_1.object.t_2.text = 'Supervisor: '+cod_sup+' - '+nombre_sup+' (Canal: '+canal_sup+')'
		...
		dw_1.object.t_6.text = 'Jefe de Venta : '+ls_cod_jefe+' - '+nombre_jefe
		
	//	dw_1.setfilter("( mes12='A' and   mes11='A' and    mes10='A' and    mes9 ='A' and   mes8 ='A' and   mes7 ='A') and  ( prod > meta )")
	//	dw_1.filter()
		
	else
		messagebox('Volver','Volvera a listar supervisores', information!)
		dw_1.reset()
		dw_1.object.t_1.text = ''
		dw_1.object.t_2.text = ''
		dw_1.object.t_6.text = ''
		dw_3.dataobject = 'dw_sup_categoria'
		dw_2.dataobject = 'w_clas_fecha_sup'
		ddlb_1.triggerevent (selectionchanged!)
	end if
end if
end event

event clicked;call super::clicked;if dwo.name ='volver' then
	dw_1.reset()
	dw_1.object.t_1.text = ''
	dw_1.object.t_2.text = ''
	dw_1.object.t_6.text = ''
...
event selectionchanged;call super::selectionchanged;string cod_age,age,nombre,tipo_clas,mes1,mes2,mes3,mes4,mes5,mes6,mes7,mes8,mes9,mes10,mes11,mes12,&
		 clasifica,cod_sup,nombre_sup,clas_sup,canal_sup
date fecha_ini,fecha_clas,fecha
double i,j,filas,contratos,mora,porcentaje,vsp,vsp_clas,vsp_sup,meta,prod
...
if dw_3.retrieve(mid(this.text,1,3)) > 0 then
	for i = 1 to dw_3.rowcount()
		...
		if dw_2.retrieve(cod_age) > 0 then
			...
			dw_1.insertrow(0)
			SELECT CONTRATOS_CLASIFICA_SUP,MORA_CLASIFICA_SUP,round(PORCE_CLASIFICA_SUP,2),VSP      ,TIPO_CLASIFICA_SUP   
			into   :contratos             ,:mora             ,:porcentaje        ,:vsp_clas,:clasifica
			FROM CLASIFICA_HIST_SUP  
			WHERE ( COD_SUP = :age ) AND ( FECHA_CLASIFICA_SUP = (  SELECT max(FECHA_CLASIFICA_SUP) FROM CLASIFICA_HIST_SUP ))   ;
			dw_1.setitem(dw_1.rowcount(),'mes1',mes1)
			dw_1.setitem(dw_1.rowcount(),'mes2',mes2)
			dw_1.setitem(dw_1.rowcount(),'mes3',mes3)
			dw_1.setitem(dw_1.rowcount(),'mes4',mes4)
	...
	SELECT "JEFE_VENTAS"."NOMBRE"||' '||"JEFE_VENTAS"."A_PATERNO"||' '||"JEFE_VENTAS"."A_MATERNO"  as nombre 
	INTO :nombre_sup  
	FROM "JEFE_VENTAS"  
	WHERE "JEFE_VENTAS"."JEFE_VENTAS" = :cod_sup   ;
	

	dw_1.object.t_2.text = 'Jefe de Ventas: '+mid(this.text,1,3)+' - '+nombre_sup
	dw_1.object.t_5.text = gs_conexion
	dw_1.object.t_6.text = ''
...
event clicked;if dw_1.rowcount() > 0 then
	dw_1.object.volver.visible = false
	f_Print( dw_1 )
//	f_printdlg(dw_1,gstr_print,parent)
	dw_1.object.volver.visible = true
end if
end event
...
integer taborder = 30
string title = "none"
string dataobject = "dw_sup_categoria"
...
boolean titlebar = true
string title = "none"
string dataobject = "w_clas_fecha_sup"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
...
event clicked;close(w_categorias_sup)
end event
