forward
global type w_lista_crematorio from window
end type
type cb_inactivar from commandbutton within w_lista_crematorio
end type
type st_16 from statictext within w_lista_crematorio
end type
type dw_parque from datawindow within w_lista_crematorio
end type
type st_1 from statictext within w_lista_crematorio
end type
type pb_ok from picturebutton within w_lista_crematorio
end type
type dp_fecha from datepicker within w_lista_crematorio
end type
type cb_exportar from commandbutton within w_lista_crematorio
end type
type cb_imprimir from commandbutton within w_lista_crematorio
end type
type cb_agregar from commandbutton within w_lista_crematorio
end type
type cb_cerrar from commandbutton within w_lista_crematorio
end type
type dw_lista from datawindow within w_lista_crematorio
end type
type gb_1 from groupbox within w_lista_crematorio
end type
end forward
...
global w_lista_crematorio w_lista_crematorio
...
//		this.title							= 'Resumen de Uso Crematorio día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
//		dw_lista.object.t_titulo.text	= 'Resumen de Uso Crematorio día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
//		
...
//		ll_duracion_crematorio			= 90 / il_intervalo_templo
	...
	gs_ventana	= 'w_lista_crematorio'
	f_valida_objeto2()
else
	messagebox("Advertencia","Debe Seleccionar Parque")
//	close(w_lista_crematorio)
end if
end event
...
if ll_fila > 0 then
	...
	if ls_ag_externa='S' then
		...
		if ll_res=1 then
			...
			WHERE ( "FC_AGENDA_CREMATORIO"."RUT_FALLECIDO" = :ll_rut_falle ) AND  
...
if isnull(ldt_fec_ini) then
	...
	if il_cod_parque>0 then
		if is_tipo = 'C' then	// Resumen Crematorio
			dw_lista.setredraw(false)
			w_lista_crematorio.title		= 'Resumen de Uso Crematorio día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
			dw_lista.object.t_titulo.text	= 'Resumen de Uso Crematorio día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
			...
			if isvalid(w_ficha_crematorio) or isnull(is_nombre) or is_nombre='' then
				...
				SELECT 	"FC_AGENDA_CREMATORIO"."FOLIO_CREMATORIO", 	"FC_AGENDA_CREMATORIO"."FECHA_CREMACION",	"FC_AGENDA_CREMATORIO"."HORA_CINERACION", 	"FC_AGENDA_CREMATORIO"."MINUTO_CINERACION", 	"FC_AGENDA_CREMATORIO"."RUT_TITULAR", 	"FC_AGENDA_CREMATORIO"."RUT_FALLECIDO", 	"FC_AGENDA_CREMATORIO"."NOMBRE_FALLECIDO",	"FC_AGENDA_CREMATORIO"."AP_PATERNO_FALLECIDO",	"FC_AGENDA_CREMATORIO"."AP_MATERNO_FALLECIDO",	"FC_AGENDA_CREMATORIO"."FECHA_DEFUNCION",	"FC_AGENDA_CREMATORIO"."ESTADO_GESTION", 	"FC_AGENDA_CREMATORIO"."EJECUTIVO",	"FC_AGENDA_CREMATORIO"."OBSERVACION", 	"FC_AGENDA_CREMATORIO"."COD_PARQUE",	"FC_AGENDA_CREMATORIO"."CODIGO_RELIGION",	"FC_AGENDA_CREMATORIO"."SW_ORADOR",	"FC_AGENDA_CREMATORIO"."SW_COVID",	"FC_AGENDA_CREMATORIO"."SW_PROTOCOLO",	"FC_AGENDA_CREMATORIO"."BASE",	"FC_AGENDA_CREMATORIO"."SERIE",	"FC_AGENDA_CREMATORIO"."NUMERO",	"FC_AGENDA_CREMATORIO"."HORA_CINERACION_FIN",	"FC_AGENDA_CREMATORIO"."MINUTO_CINERACION_FIN",	"FC_AGENDA_CREMATORIO"."AGENDA_PERGOLA",	"FC_AGENDA_CREMATORIO"."FECHA_PERGOLA",	"FC_AGENDA_CREMATORIO"."HORA_INI_PERGOLA",	"FC_AGENDA_CREMATORIO"."MINUTO_INI_PERGOLA",	"FC_AGENDA_CREMATORIO"."HORA_FIN_PERGOLA",	"FC_AGENDA_CREMATORIO"."MINUTO_FIN_PERGOLA",	"FC_AGENDA_CREMATORIO"."AGENDA_CAPILLA",	"FC_AGENDA_CREMATORIO"."FECHA_CAPILLA",	"FC_AGENDA_CREMATORIO"."HORA_INI_CAPILLA",	"FC_AGENDA_CREMATORIO"."MINUTO_INI_CAPILLA",	"FC_AGENDA_CREMATORIO"."HORA_FIN_CAPILLA",	"FC_AGENDA_CREMATORIO"."MINUTO_FIN_CAPILLA",	"FC_AGENDA_CREMATORIO"."AGENDA_VISUALIZA",	"FC_AGENDA_CREMATORIO"."FECHA_VISUALIZA",	"FC_AGENDA_CREMATORIO"."HORA_INI_VISUALIZA",	"FC_AGENDA_CREMATORIO"."MINUTO_INI_VISUALIZA",	"FC_AGENDA_CREMATORIO"."HORA_FIN_VISUALIZA",	"FC_AGENDA_CREMATORIO"."MINUTO_FIN_VISUALIZA",	"FC_AGENDA_CREMATORIO"."AGENDA_EXTERNA" 
				FROM 	"FC_AGENDA_CREMATORIO"  
				WHERE ( "FC_AGENDA_CREMATORIO"."FECHA_CREMACION" = :idt_fecha_cremacion ) AND  
//						  ( "FC_AGENDA_CREMATORIO"."FOLIO_CREMATORIO" <> :il_folio )  AND
						  ( "FC_AGENDA_CREMATORIO"."ESTADO_REG" <> 'I' )
				ORDER BY 	"FC_AGENDA_CREMATORIO"."HORA_CINERACION" ASC,
								"FC_AGENDA_CREMATORIO"."MINUTO_CINERACION" ASC,
								"FC_AGENDA_CREMATORIO"."FOLIO_CREMATORIO" ASC
				USING	Trans_1;
				open x0;
					...
					DO WHILE Trans_1.sqlcode=0
						...
						if not isnull(ls_nombre_fall) and ll_folio_c > 0 then
							...
							if ll_reg > 0  then
								...
								dw_lista.setitem(ll_reg,'dv_titular','')
								dw_lista.setitem(ll_reg,'rut_fallecido',ll_rut_fall_c)
								dw_lista.setitem(ll_reg,'dv_fallecido','')
...
//									dw_lista.setitem(ll_reg,'observacion',is_obs)
				...
				WHERE 	( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_CEREMONIA"."FOLIO_CREMATORIO" ) and  
							...
							( "FC_CABECERA"."FECHA_INGRESO_CINERARIO" = :idt_fecha_cremacion )  AND
...
datetime value = DateTime(Date("2022-06-27"), Time("22:06:41.000000"))
integer textsize = -8
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
event clicked;String		ls_string,ls_nom,ls_nombre_fall,ls_pat_fall,ls_mat_fall,ls_obs_ceremonia,ls_base,ls_serie,ls_dv_titular,ls_dv_fall,ls_agenda_pergola,ls_agenda_capilla,	ls_agenda_visualiza,ls_nom1,ls_nom2,ls_nom3,ls_dv_tit,&
			ls_nombre_tit,ls_ap_pat_tit,ls_ap_mat_tit,ls_tipo_origen,ls_tipo_sol_origen,ls_nom_fall,ls_ap_pat_fall,ls_ap_mat_fall,ls_visual_serv,ls_dv_respon,ls_nom_respon,ls_ap_pat_respon,ls_ap_mat_respon,&
			ls_responzable_retiro,ls_fono1,ls_fono2,ls_descrip_origen,email_de,ls_email_para,ls_asunto,ls_texto,ls_res,ls_codigo_otro,ls_descrip_otro
Long		ll_reg,ll_reg_hora,ll_tot_reg,ll_new,ll_min_new,ll_reg_hora_otra,ll_tot_reg_ant,ll_indi,ll_hora_sel,ll_min_sel,ll_hora_cineracion,ll_minuto_cineracion,ll_cod_religion,ll_sw_orador,ll_estado_gestion,ll_hora_ing_cinera,&
			ll_min_ing_cinera,ll_numero,ll_protocolo_covid,ll_sw_covid19,ll_rut_titular,ll_rut_fall,ll_count_agenda,ll_count_capilla,ll_hora_cineracion_fin,ll_min_cineracion_fin,ll_count_pergola,ll_hora_ini_capilla,ll_min_ini_capilla,&
			ll_hora_fin_capilla,	ll_min_fin_capilla,ll_hora_ini_pergola,ll_minuto_ini_pergola,ll_hora_fin_pergola,ll_minuto_fin_pergola,ll_minuto_ini_capilla,ll_minuto_fin_capilla,ll_hora_ini_visual,ll_min_ini_visual,ll_hora_fin_visual,&
			ll_min_fin_visual,ll_hora_ini_visualiza,ll_min_ini_visualiza,ll_hora_fin_visualiza,ll_min_fin_visualiza,ll_fila,ll_hora_ini,ll_min_ini,ll_hora_fin,ll_min_fin,ll_rut_tit,ll_hora_cine,ll_minuto_cine,ll_rut_respon,ll_hora_retiro_ceniza,&
			ll_minuto_retiro_ceniza,ll_sw_participa,ll_destino_cuerpo,ll_res_correo
datetime	ldt_fecha_ing_cinera,ldt_fecha_defuncion,ldt_fecha_pergola,ldt_fecha_capilla,ldt_fecha_visualiza,ldt_fec_ini,ldt_fecha_cine,ldt_fec_def,ldt_fecha_retiro_ceniza
...
//			ls_agenda_visualiza			= is_uso_visualiza
			...
			if ll_count_agenda=0 or isnull(ll_count_agenda) then	
				...
				if sqlca.sqlcode=0 then
					...
					SELECT 		"FC_ARANCEL"."CODIGO_OTRO",	"FC_CABECERA"."BASE",	"FC_CABECERA"."SERIE",	"FC_CABECERA"."NUMERO",	"FC_TITULAR"."RUT_TITULAR",	"FC_TITULAR"."DV_TITULAR",		"FC_TITULAR"."NOMBRES",	"FC_TITULAR"."APELLIDO_PATERNO",	"FC_TITULAR"."APELLIDO_MATERNO",	"FC_CABECERA"."FECHA_INGRESO_CINERARIO",	"FC_CABECERA"."HORA_INGRESO_CINERARIO",	"FC_CABECERA"."MINUTO_INGRESO_CINERARIO",	"FC_CABECERA"."TIPO_ORIGEN",	"FC_CABECERA"."TIPO_SOLICITUD_ORIGEN",	"FC_FALLECIDO"."RUT_FALLECIDO",	"FC_FALLECIDO"."DV_FALLECIDO",	"FC_FALLECIDO"."NOMBRES",	"FC_FALLECIDO"."APELLIDO_PATERNO",	"FC_FALLECIDO"."APELLIDO_MATERNO",		"FC_FALLECIDO"."FECHA_DEFUNCION"
					...
					ll_res_correo				= messagebox("Envio Email","Se Creó Agenda Cremación, desea Enviar Correo de Aviso Administrador Crematorio",Exclamation!,YesNo!,2)
...
//						ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
						ls_asunto					= '<< PRUEBA >> Se Agenda Cremación Ficha N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
						ls_texto						= '<< PRUEBA >> Se Agenda Cremación Servicio Ficha Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
				...
				if ll_count_agenda=0 or isnull(ll_count_agenda) then	
					...
					if sqlca.sqlcode=0 then
						...
						ll_res_correo				= messagebox("Envio Email","Se Actualiza Agenda Cremación, desea Enviar Correo de Aviso Administrador Crematorio",Exclamation!,YesNo!,2)
						if ll_res_correo=1 then
							SELECT 	"FC_TIPO_SOLICITUD_ORIGEN"."DESCRIPCION"  
							INTO 		:ls_descrip_origen  
							FROM 	"FC_TIPO_SOLICITUD_ORIGEN"  
	...
	//						email_de						= 'consulta@nuestrosparques.cl'
	//						ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
							ls_asunto					= '<< PRUEBA >> Se Actualiza Agenda Cremación Ficha N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
							ls_texto						= '<< PRUEBA >> Se Actualiza Agenda Cremación Servicio Ficha Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
																'Fecha y hora de Recepción del Fallecido: '+string(ldt_fecha_cine,'dd/mm/yyyy')+'   '+string(ll_hora_cine,'00')+':'+string(ll_minuto_cine,'00')+&
																'      Rut y Nombre Fallecido: '+string(ll_rut_fall)+'-'+ls_dv_fall+'    '+ls_nom_fall+' '+ls_ap_pat_fall+' '+ls_ap_mat_fall+'     Fecha Defunción: '+string(ldt_fec_def,'dd/mm/yyyy')+&
																'      Rut y Nombre Titular: '+string(ll_rut_tit)+'-'+ls_dv_tit+'    '+ls_nombre_tit+' '+ls_ap_pat_tit+' '+ls_ap_mat_tit+&
																'      Contrato N°: '+ls_base+'-'+ls_serie+string(ll_numero)+'      Tipo Servicio: '+ls_codigo_otro+'-'+ls_descrip_otro
			...
			if isvalid(w_ficha_crematorio) then
				w_ficha_crematorio.tab_crematorio.tabpage_1.dw_ficha.setitem(1,'fecha_ingreso_cinerario',idt_fecha_cremacion)
...
//				w_ficha_crematorio.tab_crematorio.tabpage_1.dw_ficha.setitem(1,'minuto_cineracion',ll_min_sel)
	...
	if isnull(ls_nom1) and isnull(ls_nom2) and isnull(ls_nom3) then
		...
		ls_string								= string(ldt_fec_ini)+'~t'+string(ll_hora_ini)+'~t'+string(ll_min_ini)+'~t'+string(ll_hora_fin)+'~t'+string(ll_min_fin)+'~t'+string(ll_rut_fall)+'~t'+'C'
...
event clicked;close(w_lista_crematorio)
end event
