forward
global type w_fc_ficha_traslado from window
end type
type dw_print from datawindow within w_fc_ficha_traslado
end type
type cb_limpiar from commandbutton within w_fc_ficha_traslado
end type
type cb_nuevo from commandbutton within w_fc_ficha_traslado
end type
type cb_buscar from commandbutton within w_fc_ficha_traslado
end type
type pb_aceptar from picturebutton within w_fc_ficha_traslado
end type
type em_folio from editmask within w_fc_ficha_traslado
end type
type st_1 from statictext within w_fc_ficha_traslado
end type
type cb_imprimir from commandbutton within w_fc_ficha_traslado
end type
type cb_eliminar from commandbutton within w_fc_ficha_traslado
end type
type cb_cerrar from commandbutton within w_fc_ficha_traslado
end type
type cb_grabar from commandbutton within w_fc_ficha_traslado
end type
type tab_1 from tab within w_fc_ficha_traslado
end type
...
global w_fc_ficha_traslado w_fc_ficha_traslado
...
on w_fc_ficha_traslado.destroy
...
//		tab_1.tabpage_2.dw_fallecido.retrieve(ll_folio)
		...
		if is_opcion='N' then
			...
			if sqlca.sqlcode=0 then
				...
				INTO 		:ls_nivel  
				FROM 	"FALLECIDOS"  
				WHERE 	"FALLECIDOS"."RUT" = :il_rut_fallecido  AND
					...
					if ll_folio_ci > 0 then
						...
						if sqlca.sqlcode=0 then
							...
							if sqlca.sqlcode=0 then
								...
								VALUES 	 ( :ll_folio_ci, 	'CI',				999, 				999 , 					1, 				:ldt_fecha_pago, 	'EF', 				999 , 					' ', 			:ldt_fecha_pago, 	'CC',				'C',										Null,										Null,													0,										0,									'0',											0,										0,									'0',											0,								Null,						1,												:ll_hora_pago,		:ll_minuto_pago,	Null,							'BA',						:ldt_fecha_pago,		1,					'N')
								USING		sqlca ;
								if sqlca.sqlcode=0 then
									commit;
								else
