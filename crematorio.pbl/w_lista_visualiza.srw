forward
global type w_lista_visualiza from window
end type
type cb_inactivar from commandbutton within w_lista_visualiza
end type
type st_16 from statictext within w_lista_visualiza
end type
type dw_parque from datawindow within w_lista_visualiza
end type
type st_1 from statictext within w_lista_visualiza
end type
type pb_ok from picturebutton within w_lista_visualiza
end type
type dp_fecha from datepicker within w_lista_visualiza
end type
type cb_exportar from commandbutton within w_lista_visualiza
end type
type cb_imprimir from commandbutton within w_lista_visualiza
end type
type cb_agregar from commandbutton within w_lista_visualiza
end type
type cb_cerrar from commandbutton within w_lista_visualiza
end type
type dw_lista from datawindow within w_lista_visualiza
end type
type gb_1 from groupbox within w_lista_visualiza
end type
end forward
...
string title = "Calendario Capilla"
...
global w_lista_visualiza w_lista_visualiza
...
if il_cod_parque > 0 then
	if is_tipo = 'V' or is_tipo='R' then	// Resumen Capilla
		pb_ok.triggerevent(clicked!)
	end if
	gs_ventana	= 'w_lista_visualiza'
	f_valida_objeto2()
else
...
if ll_fila > 0 then
	...
	if ls_ag_externa='S' then
		...
		if ll_res=1 then
			...
			WHERE ( "FC_AGENDA_VISUALIZA"."RUT_FALLECIDO" = :ll_rut_falle ) AND  
					  ( "FC_AGENDA_VISUALIZA"."ESTADO_REG" = 'P' )  
			USING	sqlca;
...
if isnull(ldt_fec_ini) then
	...
	if il_cod_parque>0 then
		if is_tipo = 'V' or is_tipo='R' then	// Resumen Capilla
			...
			w_lista_visualiza.title			= 'Resumen de Uso Visualización / Retiro Cenizas día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
			dw_lista.object.t_titulo.text	= 'Resumen de Uso Visualización / Retiro Cenizas día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
			...
			if isvalid(w_ficha_crematorio) or isnull(is_nombre) or is_nombre='' then
				...
				SELECT 	"FC_AGENDA_VISUALIZA"."FOLIO_CREMATORIO", 	"FC_AGENDA_VISUALIZA"."FECHA_CREMACION",	"FC_AGENDA_VISUALIZA"."HORA_CINERACION", 	"FC_AGENDA_VISUALIZA"."MINUTO_CINERACION", 	"FC_AGENDA_VISUALIZA"."RUT_TITULAR", 	"FC_AGENDA_VISUALIZA"."RUT_FALLECIDO", 	"FC_AGENDA_VISUALIZA"."NOMBRE_FALLECIDO",	"FC_AGENDA_VISUALIZA"."AP_PATERNO_FALLECIDO",	"FC_AGENDA_VISUALIZA"."AP_MATERNO_FALLECIDO",	"FC_AGENDA_VISUALIZA"."FECHA_DEFUNCION",	"FC_AGENDA_VISUALIZA"."ESTADO_GESTION", 	"FC_AGENDA_VISUALIZA"."EJECUTIVO",	"FC_AGENDA_VISUALIZA"."OBSERVACION", 	"FC_AGENDA_VISUALIZA"."COD_PARQUE",	"FC_AGENDA_VISUALIZA"."CODIGO_RELIGION",	"FC_AGENDA_VISUALIZA"."SW_ORADOR",	"FC_AGENDA_VISUALIZA"."SW_COVID",	"FC_AGENDA_VISUALIZA"."SW_PROTOCOLO",	"FC_AGENDA_VISUALIZA"."BASE",	"FC_AGENDA_VISUALIZA"."SERIE",	"FC_AGENDA_VISUALIZA"."NUMERO",	"FC_AGENDA_VISUALIZA"."HORA_CINERACION_FIN",	"FC_AGENDA_VISUALIZA"."MINUTO_CINERACION_FIN",	"FC_AGENDA_VISUALIZA"."AGENDA_PERGOLA",	"FC_AGENDA_VISUALIZA"."FECHA_PERGOLA",	"FC_AGENDA_VISUALIZA"."HORA_INI_PERGOLA",	"FC_AGENDA_VISUALIZA"."MINUTO_INI_PERGOLA",	"FC_AGENDA_VISUALIZA"."HORA_FIN_PERGOLA",	"FC_AGENDA_VISUALIZA"."MINUTO_FIN_PERGOLA",	"FC_AGENDA_VISUALIZA"."AGENDA_CAPILLA",	"FC_AGENDA_VISUALIZA"."FECHA_CAPILLA",	"FC_AGENDA_VISUALIZA"."HORA_INI_CAPILLA",	"FC_AGENDA_VISUALIZA"."MINUTO_INI_CAPILLA",	"FC_AGENDA_VISUALIZA"."HORA_FIN_CAPILLA",	"FC_AGENDA_VISUALIZA"."MINUTO_FIN_CAPILLA",	"FC_AGENDA_VISUALIZA"."AGENDA_VISUALIZA",	"FC_AGENDA_VISUALIZA"."FECHA_VISUALIZA",	"FC_AGENDA_VISUALIZA"."HORA_INI_VISUALIZA",	"FC_AGENDA_VISUALIZA"."MINUTO_INI_VISUALIZA",	"FC_AGENDA_VISUALIZA"."HORA_FIN_VISUALIZA",	"FC_AGENDA_VISUALIZA"."MINUTO_FIN_VISUALIZA",	"FC_AGENDA_VISUALIZA"."AGENDA_EXTERNA"  
				FROM 	"FC_AGENDA_VISUALIZA"  
				WHERE ( "FC_AGENDA_VISUALIZA"."FECHA_CREMACION" = :idt_fecha_cremacion ) AND  
//						  ( "FC_AGENDA_VISUALIZA"."FOLIO_CREMATORIO" <> :il_folio )  AND
						  ( "FC_AGENDA_VISUALIZA"."ESTADO_REG" <> 'I' )
				ORDER BY 	"FC_AGENDA_VISUALIZA"."HORA_CINERACION" ASC,
								"FC_AGENDA_VISUALIZA"."MINUTO_CINERACION" ASC,
								"FC_AGENDA_VISUALIZA"."FOLIO_CREMATORIO" ASC
				USING	Trans_1;
				open x0;
						...
						if not isnull(ls_nombre_fall) then
							...
							if ll_reg > 0  then
								...
								if ls_uso_visualiza='S' then
									...
									elseif is_tipo='R' then
										ls_obs_ceremonia	= ls_obs_ceremonia + ', USO SALA RETIRO CENIZAS '
									end if
								end if
...
datetime value = DateTime(Date("2022-07-08"), Time("09:12:27.000000"))
integer textsize = -8
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
event clicked;String		ls_string,ls_nom,ls_nombre_fall,ls_pat_fall,ls_mat_fall,ls_obs_ceremonia,ls_base,ls_serie,ls_dv_titular,ls_dv_fall,ls_agenda_pergola,ls_agenda_capilla,ls_nom_fall,ls_ap_pat_fall,ls_ap_mat_fall,&
			ls_agenda_visualiza,ls_nom1,ls_nom2,ls_nom3,ls_visual_serv,ls_dv_respon,ls_nom_respon,ls_ap_pat_respon,ls_ap_mat_respon,ls_responzable_retiro,ls_fono1,ls_fono2,email_de,ls_email_para,&
			ls_asunto,ls_texto,ls_tipo_origen,ls_tipo_sol_origen,ls_descrip_origen,ls_res,ls_dv_tit,ls_nombre_tit,ls_ap_pat_tit,ls_ap_mat_tit,ls_descrip_paren,ls_codigo_otro,ls_descrip_otro
Long		ll_reg,ll_reg_hora,ll_tot_reg,ll_new,ll_min_new,ll_reg_hora_otra,ll_tot_reg_ant,ll_indi,ll_hora_sel,ll_min_sel,ll_hora_cineracion,ll_minuto_cineracion,ll_cod_religion,ll_sw_orador,ll_estado_gestion,&
			ll_hora_ing_cinera,ll_min_ing_cinera,ll_numero,ll_protocolo_covid,ll_sw_covid19,ll_rut_titular,ll_rut_fall,	ll_count_agenda,ll_count_capilla,ll_hora_cineracion_fin,ll_min_cineracion_fin,ll_count_pergola,&
			ll_hora_ini_capilla,ll_min_ini_capilla,ll_hora_fin_capilla,ll_min_fin_capilla,ll_hora_ini_pergola,ll_minuto_ini_pergola,ll_hora_fin_pergola,ll_minuto_fin_pergola,ll_minuto_ini_capilla,ll_minuto_fin_capilla,&
			ll_hora_ini_visual,ll_min_ini_visual,ll_hora_fin_visual,ll_min_fin_visual,ll_hora_ini_visualiza,ll_min_ini_visualiza,ll_hora_fin_visualiza,ll_min_fin_visualiza,ll_fila,ll_hora_ini,ll_min_ini,ll_hora_fin,ll_min_fin,&
			ll_rut_respon,ll_hora_retiro_ceniza,ll_minuto_retiro_ceniza,ll_sw_participa,ll_destino_cuerpo,ll_res_correo,ll_hora_cine,ll_minuto_cine,ll_rut_tit
datetime	ldt_fecha_ing_cinera,ldt_fecha_defuncion,ldt_fecha_pergola,ldt_fecha_capilla,ldt_fecha_visualiza,ldt_fec_ini,ldt_fec_def,ldt_fecha_retiro_ceniza,ldt_fecha_cine
...
//			ls_agenda_visualiza			= is_uso_visualiza
			...
			SELECT 	COUNT("FC_AGENDA_VISUALIZA"."EJECUTIVO")  
			INTO 		:ll_count_capilla  
			FROM 	"FC_AGENDA_VISUALIZA" 
			WHERE ( "FC_AGENDA_VISUALIZA"."FECHA_CREMACION" = :ldt_fecha_ing_cinera ) AND  
					  ( "FC_AGENDA_VISUALIZA"."HORA_CINERACION" >= :ll_hora_cineracion ) AND  
					  ( "FC_AGENDA_VISUALIZA"."MINUTO_CINERACION" >= :ll_minuto_cineracion ) AND  
					  ( "FC_AGENDA_VISUALIZA"."HORA_CINERACION_FIN" <= :ll_hora_cineracion_fin ) AND  
					  ( "FC_AGENDA_VISUALIZA"."MINUTO_CINERACION_FIN" <= :ll_min_cineracion_fin ) AND
					  ( "FC_AGENDA_VISUALIZA"."RUT_TITULAR" <> :ll_rut_titular)  AND
					  ( "FC_AGENDA_VISUALIZA"."RUT_FALLECIDO" <> :ll_rut_fall) and 
					  ( "FC_AGENDA_VISUALIZA"."TIPO_FICHA" = :is_tipo)
			USING	sqlca;
			...
			FROM 	"FC_AGENDA_CREMATORIO",	 "FC_AGENDA_VISUALIZA" 
			WHERE 	"FC_AGENDA_CREMATORIO"."FOLIO_CREMATORIO" = "FC_AGENDA_VISUALIZA"."FOLIO_CREMATORIO" AND
						"FC_AGENDA_VISUALIZA"."TIPO_FICHA" = :is_tipo AND
						"FC_AGENDA_CREMATORIO"."FOLIO_CREMATORIO" = :il_folio   ;
			if ll_count_agenda=0 or isnull(ll_count_agenda) then	
				if is_uso_visualiza='S' then
					INSERT INTO "FC_AGENDA_VISUALIZA"  
								( "FOLIO_CREMATORIO",  	"FECHA_CREMACION",	"HORA_CINERACION", 	"MINUTO_CINERACION", 	"RUT_TITULAR", 	"RUT_FALLECIDO", 	"NOMBRE_FALLECIDO", 	"AP_PATERNO_FALLECIDO", 	"AP_MATERNO_FALLECIDO",	"FECHA_DEFUNCION", 	"ESTADO_GESTION", 		"EJECUTIVO", 	"OBSERVACION", 		"COD_PARQUE", 	"CODIGO_RELIGION", 		"SW_ORADOR", 	"SW_COVID", 	"SW_PROTOCOLO", 	"BASE", 	"SERIE", 		"NUMERO", 		"HORA_CINERACION_FIN", 		"MINUTO_CINERACION_FIN", 	"AGENDA_PERGOLA", 		"FECHA_PERGOLA", 		"HORA_INI_PERGOLA", 	"MINUTO_INI_PERGOLA", 	"HORA_FIN_PERGOLA", 	"MINUTO_FIN_PERGOLA", 	"AGENDA_CAPILLA", 	"FECHA_CAPILLA", 	"HORA_INI_CAPILLA", 	"MINUTO_INI_CAPILLA", 		"HORA_FIN_CAPILLA", 	"MINUTO_FIN_CAPILLA",		"AGENDA_VISUALIZA", 	"FECHA_VISUALIZA", 	"HORA_INI_VISUALIZA", 	"MINUTO_INI_VISUALIZA", 		"HORA_FIN_VISUALIZA", 	"MINUTO_FIN_VISUALIZA",	"AGENDA_EXTERNA",		"ESTADO_REG",	"TIPO_FICHA"  )  
					VALUES 	( :il_folio,   						:ldt_fecha_ing_cinera,	:ll_hora_cineracion,		:ll_minuto_cineracion,		:ll_rut_titular,		:ll_rut_fall,				:ls_nombre_fall,			:ls_pat_fall,							:ls_mat_fall,							:ldt_fecha_defuncion,		:ll_estado_gestion,		:gs_user,		:ls_obs_ceremonia,	:il_cod_parque,		:ll_cod_religion,			:ll_sw_orador,		:ll_sw_covid19,	:ll_protocolo_covid,	:ls_base,	:ls_serie,		:ll_numero,		:ll_hora_cineracion_fin,			:ll_min_cineracion_fin,			:ls_agenda_pergola,		:ldt_fecha_pergola,		:ll_hora_ini_pergola,		:ll_minuto_ini_pergola,		:ll_hora_fin_pergola,		:ll_minuto_fin_pergola,		:ls_agenda_capilla,	:ldt_fecha_capilla,		:ll_hora_ini_capilla,		:ll_minuto_ini_capilla,			:ll_hora_fin_capilla,		:ll_minuto_fin_capilla,			:ls_agenda_visualiza,		:ldt_fecha_visualiza,	:ll_hora_ini_visualiza,		:ll_min_ini_visualiza,				:ll_hora_fin_visualiza,			:ll_min_fin_visualiza,			'N',							'P',						:is_tipo	)
					USING		sqlca;
					if sqlca.sqlcode=0 then
						commit;
						if is_tipo = 'V' then
							SELECT 		"FC_TITULAR"."RUT_TITULAR",	"FC_TITULAR"."DV_TITULAR",		"FC_TITULAR"."NOMBRES",	"FC_TITULAR"."APELLIDO_PATERNO",	"FC_TITULAR"."APELLIDO_MATERNO",	"FC_CABECERA"."FECHA_CINERACION",	"FC_CABECERA"."HORA_CINERACION",	"FC_CABECERA"."MINUTO_CINERACION",	"FC_CABECERA"."TIPO_ORIGEN",	"FC_CABECERA"."TIPO_SOLICITUD_ORIGEN",	"FC_FALLECIDO"."RUT_FALLECIDO",	"FC_FALLECIDO"."DV_FALLECIDO",	"FC_FALLECIDO"."NOMBRES",	"FC_FALLECIDO"."APELLIDO_PATERNO",	"FC_FALLECIDO"."APELLIDO_MATERNO",		"FC_FALLECIDO"."FECHA_DEFUNCION",	"FC_RESPONSABLE_RETIRO"."VISUALIZAR_SERVICIO",	"FC_RESPONSABLE_RETIRO"."RUT_RESPONSABLE",	"FC_RESPONSABLE_RETIRO"."DV_RESPONSABLE",	"FC_RESPONSABLE_RETIRO"."NOMBRE",		"FC_RESPONSABLE_RETIRO"."APELLIDO_PATERNO",	"FC_RESPONSABLE_RETIRO"."APELLIDO_MATERNO",		"FC_RESPONSABLE_RETIRO"."FECHA_RETIRO_CENIZA",		"FC_RESPONSABLE_RETIRO"."HORA_RETIRO_CENIZA",	"FC_RESPONSABLE_RETIRO"."MINUTO_RETIRO_CENIZA",	"FC_RESPONSABLE_RETIRO"."RESPONSABLE_RETIRO",	"FC_RESPONSABLE_RETIRO"."SW_PARTICIPA_CREMACION",	"FC_RESPONSABLE_RETIRO"."FONO_1",		"FC_RESPONSABLE_RETIRO"."FONO_2",		"FC_RESPONSABLE_RETIRO"."DESTINO_CUERPO"  
							INTO 			:ll_rut_tit,								:ls_dv_tit,								:ls_nombre_tit,					:ls_ap_pat_tit,									:ls_ap_mat_tit,									:ldt_fecha_cine,								:ll_hora_cine,									:ll_minuto_cine,									:ls_tipo_origen,							:ls_tipo_sol_origen,									:ll_rut_fall,									:ls_dv_fall,   								:ls_nom_fall,						:ls_ap_pat_fall,									:ls_ap_mat_fall,									:ldt_fec_def,									:ls_visual_serv,														:ll_rut_respon,													:ls_dv_respon,													:ls_nom_respon,									:ls_ap_pat_respon,											:ls_ap_mat_respon,												:ldt_fecha_retiro_ceniza,												:ll_hora_retiro_ceniza,											:ll_minuto_retiro_ceniza,												:ls_responzable_retiro,											:ll_sw_participa,															:ls_fono1,											:ls_fono2,											:ll_destino_cuerpo
							FROM			"FC_CABECERA",	"FC_RESPONSABLE_RETIRO",	"FC_FALLECIDO",	"FC_TITULAR"  
							WHERE 	( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_RESPONSABLE_RETIRO"."FOLIO_CREMATORIO" ) and  
										( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_FALLECIDO"."FOLIO_CREMATORIO" ) and  
										( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_TITULAR"."FOLIO_CREMATORIO" ) and  
										( "FC_CABECERA"."FOLIO_CREMATORIO" = :il_folio )    ;
										
							ll_res_correo				= messagebox("Envio Email","Se Creó Agenda Visualización, desea Enviar Correo de Aviso Administrador Crematorio",Exclamation!,YesNo!,2)
							if ll_res_correo=1 then
								...
								SELECT DISTINCT "PARENTESCO"."PARENTESCO"  
								INTO 		:ls_descrip_paren  
								FROM 	"PARENTESCO"  
								WHERE 	"PARENTESCO"."CODIGO" = :ls_responzable_retiro   ;
...
//								ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
								ls_asunto					= '<< PRUEBA >> Se Crea Agenda Visualización Ficha de Cremación N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
								ls_texto						= '<< PRUEBA >> Se Crea Agenda Visualización Servicio Ficha de Cremación Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
																	'Fecha y hora de Recepción del Fallecido: '+string(ldt_fecha_cine,'dd/mm/yyyy')+'   '+string(ll_hora_cine,'00')+':'+string(ll_minuto_cine,'00')+&
																	'      Rut y Nombre Fallecido: '+string(ll_rut_fall)+'-'+ls_dv_fall+'    '+ls_nom_fall+' '+ls_ap_pat_fall+' '+ls_ap_mat_fall+'     Fecha Defunción: '+string(ldt_fec_def,'dd/mm/yyyy')+&
																	'      Rut y Nombre Titular: '+string(ll_rut_tit)+'-'+ls_dv_tit+'    '+ls_nombre_tit+' '+ls_ap_pat_tit+' '+ls_ap_mat_tit+ &
																	'      Rut y Nombre Responsable: '+string(ll_rut_respon)+'-'+ls_dv_respon+'    '+ls_nom_respon+' '+ls_ap_pat_respon+' '+ls_ap_mat_respon+&
																	'      Fecha y Hora Retiro: '+string(ldt_fecha_retiro_ceniza,'dd/mm/yyyy')+'  '+string(ll_hora_retiro_ceniza,'00')+':'+string(ll_minuto_retiro_ceniza,'00')+ &
																	'      Parentesco: '+ls_descrip_paren+'    Fono1: '+ls_fono1+'    Fono2: '+ls_fono2
						...
						elseif is_tipo='R' then
							...
							INTO 			:ls_codigo_otro,						:ls_base,						:ls_serie,							:ll_numero,							:ll_rut_tit,								:ls_dv_tit,								:ls_nombre_tit,					:ls_ap_pat_tit,									:ls_ap_mat_tit,									:ldt_fecha_cine,								:ll_hora_cine,									:ll_minuto_cine,									:ls_tipo_origen,							:ls_tipo_sol_origen,									:ll_rut_fall,									:ls_dv_fall,   								:ls_nom_fall,						:ls_ap_pat_fall,									:ls_ap_mat_fall,									:ldt_fec_def,									:ls_visual_serv,														:ll_rut_respon,													:ls_dv_respon,													:ls_nom_respon,									:ls_ap_pat_respon,											:ls_ap_mat_respon,												:ldt_fecha_retiro_ceniza,												:ll_hora_retiro_ceniza,											:ll_minuto_retiro_ceniza,												:ls_responzable_retiro,											:ll_sw_participa,															:ls_fono1,											:ls_fono2,											:ll_destino_cuerpo
							FROM			"FC_CABECERA",	"FC_RESPONSABLE_RETIRO",	"FC_FALLECIDO",	"FC_TITULAR"  ,	"FC_ARANCEL"
							WHERE 	( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_RESPONSABLE_RETIRO"."FOLIO_CREMATORIO" ) and  
										( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_FALLECIDO"."FOLIO_CREMATORIO" ) and  
							...
							ll_res_correo				= messagebox("Envio Email","Se Creó Agenda Retiro de Cenizas, desea Enviar Correo de Aviso Administrador Crematorio",Exclamation!,YesNo!,2)
							if ll_res_correo=1 then
								SELECT 	"FC_TIPO_SOLICITUD_ORIGEN"."DESCRIPCION"  
								INTO 		:ls_descrip_origen  
								FROM 	"FC_TIPO_SOLICITUD_ORIGEN"  
...
//								email_de						= 'consulta@nuestrosparques.cl'
//								ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
								ls_asunto					= '<< PRUEBA >> Se Crea Agenda Retiro de Cenizas Ficha de Cremación N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
								ls_texto						= '<< PRUEBA >> Se Crea Agenda Retiro de Cenizas Servicio Ficha de Cremación Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
																	'Fecha y hora de Recepción del Fallecido: '+string(ldt_fecha_cine,'dd/mm/yyyy')+'   '+string(ll_hora_cine,'00')+':'+string(ll_minuto_cine,'00')+&
																	'      Rut y Nombre Fallecido: '+string(ll_rut_fall)+'-'+ls_dv_fall+'    '+ls_nom_fall+' '+ls_ap_pat_fall+' '+ls_ap_mat_fall+'     Fecha Defunción: '+string(ldt_fec_def,'dd/mm/yyyy')+&
																	'      Rut y Nombre Titular: '+string(ll_rut_tit)+'-'+ls_dv_tit+'    '+ls_nombre_tit+' '+ls_ap_pat_tit+' '+ls_ap_mat_tit+ &
																	'      Rut y Nombre Responsable: '+string(ll_rut_respon)+'-'+ls_dv_respon+'    '+ls_nom_respon+' '+ls_ap_pat_respon+' '+ls_ap_mat_respon+&
																	'      Fecha y Hora Retiro: '+string(ldt_fecha_retiro_ceniza,'dd/mm/yyyy')+'  '+string(ll_hora_retiro_ceniza,'00')+':'+string(ll_minuto_retiro_ceniza,'00')+ &
																	'      Parentesco: '+ls_descrip_paren+'    Fono1: '+ls_fono1+'    Fono2: '+ls_fono2+&
																	'      Contrato N°: '+ls_base+'-'+ls_serie+string(ll_numero)+'      Tipo Servicio: '+ls_codigo_otro+'-'+ls_descrip_otro
				...
				Messagebox("Grabar","Grabación Exitosa Agenda Ficha cremación Folio N° "+string(il_folio))
			else
...
//									email_de						= 'consulta@nuestrosparques.cl'
//									ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
									ls_asunto					= '<< PRUEBA >> Se Agenda Visualización Ficha de Cremación N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
									ls_texto						= '<< PRUEBA >> Se Agenda Visualización Servicio Ficha de Cremación Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
																		'Fecha y hora de Recepción del Fallecido: '+string(ldt_fecha_cine,'dd/mm/yyyy')+'   '+string(ll_hora_cine,'00')+':'+string(ll_minuto_cine,'00')+&
																		'      Rut y Nombre Fallecido: '+string(ll_rut_fall)+'-'+ls_dv_fall+'    '+ls_nom_fall+' '+ls_ap_pat_fall+' '+ls_ap_mat_fall+'     Fecha Defunción: '+string(ldt_fec_def,'dd/mm/yyyy')+&
																		'      Rut y Nombre Titular: '+string(ll_rut_tit)+'-'+ls_dv_tit+'    '+ls_nombre_tit+' '+ls_ap_pat_tit+' '+ls_ap_mat_tit+ &
																		'      Rut y Nombre Responsable: '+string(ll_rut_respon)+'-'+ls_dv_respon+'    '+ls_nom_respon+' '+ls_ap_pat_respon+' '+ls_ap_mat_respon+&
							...
							elseif is_tipo = 'R' then
								...
								ll_res_correo				= messagebox("Envio Email","Se Actualiza Agenda Retiro de Cenizas, desea Enviar Correo de Aviso Administrador Crematorio",Exclamation!,YesNo!,2)
								if ll_res_correo=1 then
									SELECT 	"FC_TIPO_SOLICITUD_ORIGEN"."DESCRIPCION"  
									INTO 		:ls_descrip_origen  
									FROM 	"FC_TIPO_SOLICITUD_ORIGEN"  
...
//									email_de						= 'consulta@nuestrosparques.cl'
//									ls_email_para				= 'mirkom@nuestrosparques.cl'		//'lvasquez@nuestrosparques.cl'		//'CoordinadorClientescrematorio@nuestrosparques.cl'
									ls_asunto					= '<< PRUEBA >> Se Actualiza Agenda Retiro de Cenizas Ficha de Cremación N° '+string(il_folio)+' Contrato N° '+ls_base+'-'+ls_serie+'-'+string(ll_numero)
									ls_texto						= '<< PRUEBA >> Se Actualiza Agenda Retiro de Cenizas Servicio Ficha de Cremación Número: '+string(il_folio)+ ', Tipo Origen: '+ls_tipo_origen+' Tipo Solicitud: '+ls_tipo_sol_origen+'-'+ls_descrip_origen+'     Se adjunta Antecedentes  '+&
																		'Fecha y hora de Recepción del Fallecido: '+string(ldt_fecha_cine,'dd/mm/yyyy')+'   '+string(ll_hora_cine,'00')+':'+string(ll_minuto_cine,'00')+&
																		'      Rut y Nombre Fallecido: '+string(ll_rut_fall)+'-'+ls_dv_fall+'    '+ls_nom_fall+' '+ls_ap_pat_fall+' '+ls_ap_mat_fall+'     Fecha Defunción: '+string(ldt_fec_def,'dd/mm/yyyy')+&
																		'      Rut y Nombre Titular: '+string(ll_rut_tit)+'-'+ls_dv_tit+'    '+ls_nombre_tit+' '+ls_ap_pat_tit+' '+ls_ap_mat_tit+ &
																		'      Rut y Nombre Responsable: '+string(ll_rut_respon)+'-'+ls_dv_respon+'    '+ls_nom_respon+' '+ls_ap_pat_respon+' '+ls_ap_mat_respon+&
					...
					Messagebox("Actualizar","Grabación Exitosa Agenda Visualización Ficha Cremación Folio N° "+string(il_folio))
...
//				end if;
			...
			if isvalid(w_ficha_crematorio) then
				if is_tipo='V' then
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.setitem(1,'visualizar_servicio','S')
//					w_ficha_crematorio.tab_crematorio.tabpage_1.dw_ficha.setitem(1,'hora_ingreso_cinerario',ll_hora_sel)
//					w_ficha_crematorio.tab_crematorio.tabpage_1.dw_ficha.setitem(1,'minuto_ingreso_cinerario',ll_min_sel)
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.accepttext()
				elseif is_tipo='R' then
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.setitem(1,'fecha_retiro_ceniza',ldt_fecha_visualiza)
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.setitem(1,'hora_retiro_ceniza',ll_hora_sel)
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.setitem(1,'minuto_retiro_ceniza',ll_min_sel)
					w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.accepttext()
				end if
...
//				w_ficha_crematorio.tab_crematorio.tabpage_8.dw_cenizas.setitem(1,'minuto_uso_pergola',ll_minuto_ini_pergola)
	...
	if isnull(ls_nom1) and isnull(ls_nom2) and isnull(ls_nom3) then
		...
		ls_string								= string(ldt_fec_ini)+'~t'+string(ll_hora_ini)+'~t'+string(ll_min_ini)+'~t'+string(ll_hora_fin)+'~t'+string(ll_min_fin)+'~t'+string(ll_rut_fall)+'~t'+'V'
...
event clicked;close(w_lista_visualiza)
end event
