forward
global type w_lista_ingreso_parque from window
end type
type cb_inactivar from commandbutton within w_lista_ingreso_parque
end type
type st_16 from statictext within w_lista_ingreso_parque
end type
type dw_parque from datawindow within w_lista_ingreso_parque
end type
type st_1 from statictext within w_lista_ingreso_parque
end type
type pb_ok from picturebutton within w_lista_ingreso_parque
end type
type dp_fecha from datepicker within w_lista_ingreso_parque
end type
type cb_exportar from commandbutton within w_lista_ingreso_parque
end type
type cb_imprimir from commandbutton within w_lista_ingreso_parque
end type
type cb_agregar from commandbutton within w_lista_ingreso_parque
end type
type cb_cerrar from commandbutton within w_lista_ingreso_parque
end type
type dw_lista from datawindow within w_lista_ingreso_parque
end type
type gb_1 from groupbox within w_lista_ingreso_parque
end type
end forward
...
global w_lista_ingreso_parque w_lista_ingreso_parque
...
String		is_nombre,is_ap_pat,is_ap_mat,is_ejecutivo,is_obs,is_tipo,is_uso_visualiza,is_uso_capilla,is_uso_pergola,is_ingreso_parque,is_grabo='N'
datetime	idt_fecha_cremacion
end variables

on w_lista_ingreso_parque.create
...
on w_lista_ingreso_parque.destroy
...
gf_centrar(w_lista_ingreso_parque)
Setnull(ll_nulo)
...
//is_ingreso_parque						= substr(1,14,Message.StringParm)
ll_hora_cineracion						= il_hora
ll_minuto_cineracion					= il_min

w_lista_ingreso_parque.title			= 'Resumen de Ingreso Parque día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
dw_lista.object.t_titulo.text			= 'Resumen de Ingreso Parque día '+string(idt_fecha_cremacion,'dd/mm/yyyy')
...
if il_cod_parque > 0 then
	if is_tipo = 'C' then	// Resumen Capilla
		pb_ok.triggerevent(clicked!)
	end if
	gs_ventana	= 'w_lista_ingreso_parque'
	f_valida_objeto2()
else
...
if ll_fila > 0 then
	...
	if ls_ag_externa='S' then
		...
		if ll_res=1 then
			...
			WHERE ( "FC_AGENDA_CAPILLA"."RUT_FALLECIDO" = :ll_rut_falle ) AND  
...
String		ls_fecha,ls_parque,ls_base,ls_serie,ls_dv_titular,ls_nom_titular,ls_pat_titular,ls_mat_titular,ls_email,ls_celular,ls_obs_ceremonia,ls_cod_maestro,ls_dv_fall,&
			...
			ls_ag_externa,ls_tipo_origen,ls_ing_parque='S'
...
datetime	ldt_fecha_defuncion,ldt_fecha_cineracion,ldt_fecha_ing_cinera,ldt_fecha_c,ldt_fec_defun_c,ldt_fecha_pergola_c,ldt_fecha_capilla_c,ldt_fecha_visual_c,&
			ldt_fec_ini,ldt_fecha_cabecera,ldt_fecha_uso_capilla
...
//	is_ingreso_parque						= substr(1,14,Message.StringParm)
	ll_hora_cineracion						= il_hora
	ll_minuto_cineracion					= il_min
//	ls_ing_parque							= is_ingreso_parque
...
//	if is_ingreso_parque='N' then
//		SELECT 	"FC_CABECERA"."FECHA_INGRESO_CINERARIO",	"FC_CABECERA"."TIPO_ORIGEN",	"FC_RESPONSABLE_RETIRO"."FECHA_USO_CAPILLA"  
//		INTO 		:ldt_fecha_cabecera,										:ls_tipo_origen,							:ldt_fecha_uso_capilla
//		FROM 	"FC_CABECERA",	"FC_RESPONSABLE_RETIRO"    
...
//			SELECT 	"FC_CABECERA"."FECHA_CINERACION"
//			INTO 		:ldt_fecha_cabecera
//			FROM 	"FC_CABECERA"  
//			WHERE 	"FC_CABECERA"."FOLIO_CREMATORIO" = :il_folio   ;
//		end if
//		if not isnull(ldt_fecha_uso_capilla) then ls_ing_parque='N'
//	else
	if il_folio > 0 then
		SELECT 	"FC_CABECERA"."FECHA_CINERACION"  
		INTO 		:ldt_fecha_cabecera  
	...
	if il_cod_parque>0 then
		if is_tipo = 'C' then	// Resumen Capilla
			...
			if isvalid(w_ficha_crematorio) or isnull(is_nombre) or is_nombre='' then
				...
				SELECT 	"FC_AGENDA_ING_PARQUE"."FOLIO_CREMATORIO", 	"FC_AGENDA_ING_PARQUE"."FECHA_CREMACION",	"FC_AGENDA_ING_PARQUE"."HORA_CINERACION", 	"FC_AGENDA_ING_PARQUE"."MINUTO_CINERACION", 	"FC_AGENDA_ING_PARQUE"."RUT_TITULAR", 	"FC_AGENDA_ING_PARQUE"."RUT_FALLECIDO", 	"FC_AGENDA_ING_PARQUE"."NOMBRE_FALLECIDO",	"FC_AGENDA_ING_PARQUE"."AP_PATERNO_FALLECIDO",	"FC_AGENDA_ING_PARQUE"."AP_MATERNO_FALLECIDO",	"FC_AGENDA_ING_PARQUE"."FECHA_DEFUNCION",	"FC_AGENDA_ING_PARQUE"."ESTADO_GESTION", 	"FC_AGENDA_ING_PARQUE"."EJECUTIVO",	"FC_AGENDA_ING_PARQUE"."OBSERVACION", 	"FC_AGENDA_ING_PARQUE"."COD_PARQUE",	"FC_AGENDA_ING_PARQUE"."CODIGO_RELIGION",	"FC_AGENDA_ING_PARQUE"."SW_ORADOR",	"FC_AGENDA_ING_PARQUE"."SW_COVID",	"FC_AGENDA_ING_PARQUE"."SW_PROTOCOLO",	"FC_AGENDA_ING_PARQUE"."BASE",	"FC_AGENDA_ING_PARQUE"."SERIE",	"FC_AGENDA_ING_PARQUE"."NUMERO",	"FC_AGENDA_ING_PARQUE"."HORA_CINERACION_FIN",	"FC_AGENDA_ING_PARQUE"."MINUTO_CINERACION_FIN",	"FC_AGENDA_ING_PARQUE"."AGENDA_PERGOLA",	"FC_AGENDA_ING_PARQUE"."FECHA_PERGOLA",	"FC_AGENDA_ING_PARQUE"."HORA_INI_PERGOLA",	"FC_AGENDA_ING_PARQUE"."MINUTO_INI_PERGOLA",	"FC_AGENDA_ING_PARQUE"."HORA_FIN_PERGOLA",	"FC_AGENDA_ING_PARQUE"."MINUTO_FIN_PERGOLA",	"FC_AGENDA_ING_PARQUE"."AGENDA_CAPILLA",	"FC_AGENDA_ING_PARQUE"."FECHA_CAPILLA",	"FC_AGENDA_ING_PARQUE"."HORA_INI_CAPILLA",	"FC_AGENDA_ING_PARQUE"."MINUTO_INI_CAPILLA",	"FC_AGENDA_ING_PARQUE"."HORA_FIN_CAPILLA",	"FC_AGENDA_ING_PARQUE"."MINUTO_FIN_CAPILLA",	"FC_AGENDA_ING_PARQUE"."AGENDA_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."FECHA_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."HORA_INI_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."MINUTO_INI_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."HORA_FIN_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."MINUTO_FIN_VISUALIZA",	"FC_AGENDA_ING_PARQUE"."AGENDA_EXTERNA"  
				FROM 	"FC_AGENDA_ING_PARQUE"  
				WHERE ( "FC_AGENDA_ING_PARQUE"."FECHA_CREMACION" = :idt_fecha_cremacion ) AND  
//						  ( "FC_AGENDA_ING_PARQUE"."FOLIO_CREMATORIO" <> :il_folio )  AND
						  ( "FC_AGENDA_ING_PARQUE"."ESTADO_REG" <> 'I' ) 
				ORDER BY 	"FC_AGENDA_ING_PARQUE"."HORA_CINERACION" ASC,
								"FC_AGENDA_ING_PARQUE"."MINUTO_CINERACION" ASC,
								"FC_AGENDA_ING_PARQUE"."FOLIO_CREMATORIO" ASC
				USING	Trans_1;
				open x0;
...
//								end if
				...
				if il_folio > 0 then
					...
					WHERE 	( "FC_CABECERA"."FOLIO_CREMATORIO" = "FC_CEREMONIA"."FOLIO_CREMATORIO" ) and  
								...
								( "FC_CABECERA"."FECHA_CINERACION" = :ldt_fecha_cabecera )  AND
						...
						DO WHILE sqlca.sqlcode=0
							...
							if not isnull(ls_nombre_fall) and ls_nombre_fall<> '' then
								...
								if ll_reg > 0  then
									...
									ls_obs_ceremonia			= ls_obs_ceremonia + '- INGRESO PARQUE '
...
datetime value = DateTime(Date("2022-06-27"), Time("22:06:25.000000"))
integer textsize = -8
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
event clicked;if dw_lista.rowcount() > 0 then
	...
	f_printdlg(dw_lista,gstr_print,w_lista_ingreso_parque)
...
Long		ll_reg,ll_reg_hora,ll_tot_reg,ll_new,ll_min_new,ll_reg_hora_otra,ll_tot_reg_ant,ll_indi,ll_hora_sel,ll_min_sel,ll_hora_cineracion,ll_minuto_cineracion,&
			...
			ll_fila,ll_hora_ini,ll_min_ini,ll_hora_fin,ll_min_fin,ll_count_ingreso
...
//		dw_lista.setitem(il_row,'sepultura',is_sepultura)
	...
	//		dw_lista.setitem(ll_reg,'ejecutivo',ls_usuario_crea)
			...
			ldt_fecha_ing_cinera			= idt_fecha_cremacion	//dw_lista.getitemdatetime(il_row,'fecha_dia')
...
//			ls_agenda_visualiza			= is_uso_visualiza
			...
			FROM 	"FC_AGENDA_ING_PARQUE" 
			WHERE ( "FC_AGENDA_ING_PARQUE"."FECHA_CREMACION" = :ldt_fecha_ing_cinera ) AND  
					  ...
					  ( "FC_AGENDA_ING_PARQUE"."HORA_CINERACION_FIN" <= :ll_hora_cineracion_fin ) AND  
					  ( "FC_AGENDA_ING_PARQUE"."MINUTO_CINERACION_FIN" <= :ll_min_cineracion_fin ) AND
					  ( "FC_AGENDA_ING_PARQUE"."RUT_TITULAR" <> :ll_rut_titular)  AND
					  ( "FC_AGENDA_ING_PARQUE"."RUT_FALLECIDO" <> :ll_rut_fall) 
			USING	sqlca;
			...
			if ll_count_agenda=0 or isnull(ll_count_agenda) then	
				...
				INTO 		:ll_count_ingreso  
				FROM 	"FC_AGENDA_ING_PARQUE"  
				WHERE 	( "FC_AGENDA_ING_PARQUE"."FOLIO_CREMATORIO" = :il_folio )  ;
				if ll_count_ingreso = 0 then
					INSERT INTO "FC_AGENDA_ING_PARQUE"  
					...
					if sqlca.sqlcode=0 then
						commit;
						Messagebox("Grabar","Grabación Exitosa Agenda Ingreso Parque Folio N° "+string(il_folio))
					else
						rollback;
						Messagebox("Error Grabar","Error Grabar Ficha Cremación N° "+string(il_folio)+" SQL: "+sqlca.sqlerrtext)
					end if
				elseif ll_count_ingreso > 0 then
					...
					if sqlca.sqlcode=0 then
						commit;
						Messagebox("Actualización","Grabación Exitosa Agenda Ingreso Parque Folio N° "+string(il_folio))
					else
						rollback;
						Messagebox("Error Actualizar","Error Grabar Ficha Cremación N° "+string(il_folio)+" SQL: "+sqlca.sqlerrtext)
					end if;
				end if
			else
				SELECT 	Count("FC_AGENDA_ING_PARQUE"."NOMBRE_FALLECIDO")  
	...
	if isnull(ls_nom1) and isnull(ls_nom2) and isnull(ls_nom3) then
		...
		ls_string								= string(ldt_fec_ini)+'~t'+string(ll_hora_ini)+'~t'+string(ll_min_ini)+'~t'+string(ll_hora_fin)+'~t'+string(ll_min_fin)+'~t'+string(ll_rut_fall)+'~t'+'I'
...
event clicked;close(w_lista_ingreso_parque)
...
if row>0 then
	...
	if cb_agregar.enabled=true then
