forward
global type w_mantenedor_fosa_comun from window
end type
type p_1 from picture within w_mantenedor_fosa_comun
end type
type dw_print from datawindow within w_mantenedor_fosa_comun
end type
type cb_print from commandbutton within w_mantenedor_fosa_comun
end type
type st_help from statictext within w_mantenedor_fosa_comun
end type
type cb_ctacte from commandbutton within w_mantenedor_fosa_comun
end type
type cb_limpiar from commandbutton within w_mantenedor_fosa_comun
end type
type cb_desocupar from commandbutton within w_mantenedor_fosa_comun
end type
type st_6 from statictext within w_mantenedor_fosa_comun
end type
type st_5 from statictext within w_mantenedor_fosa_comun
end type
type st_4 from statictext within w_mantenedor_fosa_comun
end type
type st_3 from statictext within w_mantenedor_fosa_comun
end type
type st_2 from statictext within w_mantenedor_fosa_comun
end type
type st_1 from statictext within w_mantenedor_fosa_comun
end type
type cb_asignar from commandbutton within w_mantenedor_fosa_comun
end type
type pb_ok from picturebutton within w_mantenedor_fosa_comun
end type
type st_16 from statictext within w_mantenedor_fosa_comun
end type
type dw_parque from datawindow within w_mantenedor_fosa_comun
end type
type cb_cerrar from commandbutton within w_mantenedor_fosa_comun
end type
type dw_lista from datawindow within w_mantenedor_fosa_comun
end type
type st_titulo from statictext within w_mantenedor_fosa_comun
end type
type dw_detalle from datawindow within w_mantenedor_fosa_comun
end type
end forward
...
global w_mantenedor_fosa_comun w_mantenedor_fosa_comun

type variables
Long	il_fila,il_primer
end variables

on w_mantenedor_fosa_comun.create
this.p_1=create p_1
...
on w_mantenedor_fosa_comun.destroy
destroy(this.p_1)
...
Double	ll_numero,ll_numero_fosa
...
if il_fila>0 then
	...
	ll_llave						= dw_detalle.getitemnumber(il_fila,'llave_fallecido')
	gi_llave						= ll_llave
	if ls_estado_reg = 'O' then
		...
		if ll_res=1 then
			if il_fila > 0 then
				ls_base			= dw_detalle.getitemstring(il_fila,'base')
				ls_serie			= dw_detalle.getitemstring(il_fila,'serie')
				ll_numero		= dw_detalle.getitemnumber(il_fila,'numero')
				ls_nivel			= dw_detalle.getitemstring(il_fila,'niveles')
				ll_corr_fosa	= dw_detalle.getitemnumber(il_fila,'correlativo')
				if ls_base = ls_base_fosa and ls_serie = ls_serie_fosa and ll_numero = ll_numero_fosa then
					...
					if sqlca.sqlcode=0 then
						...
						dw_detalle.setitem(il_fila,'base',ls_nulo)
						dw_detalle.setitem(il_fila,'serie',ls_nulo)
						dw_detalle.setitem(il_fila,'numero',ll_nulo)
						dw_detalle.accepttext()
						commit;
						UPDATE	"CADENA"  
						SET 		"ESTADO" = 'R'
						...
						if sqlca.sqlcode=0 then
							commit;
							UPDATE	"FALLECIDOS"  
							SET 		"ESTADO_CONTRATO" = 'R'  
							WHERE  ( "FALLECIDOS"."LLAVE" = :ll_llave ) AND  
									 	( "FALLECIDOS"."BASE" = :ls_base ) AND  
							...
							if sqlca.sqlcode=0 then
								...
								rollback;
								messagebox("Error Actualizar","Error al Actualizar Estado Contrato FALLECIDO SQL: "+sqlca.sqlerrtext)
							end if
						else
							rollback;
							messagebox("Error Actualizar","Error al Actualizar Estado CADENA SQL: "+sqlca.sqlerrtext)
						end if
			...
			if ll_suma = ll_suma_g then
				if isvalid(w_ingreso_fallecimiento) then close(w_ingreso_fallecimiento)
...
Long		ll_suma_a,ll_res,ll_nulo,ll_tot_reg,ll_parque_fosa,ll_suma,ll_suma_g,ll_indi,&
			ll_corr_fosa,ll_row,ll_cod_parque,ll_tot_reg_det,ll_fila_det
Datetime	ldt_nulo
...
if il_fila > 0 and (ls_columna='niveles' or ls_columna='base' or ls_columna='serie' or ls_columna='numero') then
	...
	if ls_estado_reg = 'O' then
		messagebox("Advertencia","Nivel Ya Utilizado, Recuerde si desea Liberar Sepultura debe Presionar Botón DESOCUPAR FOSA")
