forward
global type w_lista_fosa_comun_para_fs from window
end type
type cb_imprimir from commandbutton within w_lista_fosa_comun_para_fs
end type
type cb_3 from commandbutton within w_lista_fosa_comun_para_fs
end type
type cb_2 from commandbutton within w_lista_fosa_comun_para_fs
end type
type cb_1 from commandbutton within w_lista_fosa_comun_para_fs
end type
type pb_buscar from picturebutton within w_lista_fosa_comun_para_fs
end type
type em_ap_pat from editmask within w_lista_fosa_comun_para_fs
end type
type st_2 from statictext within w_lista_fosa_comun_para_fs
end type
type st_1 from statictext within w_lista_fosa_comun_para_fs
end type
type cb_cerrar from commandbutton within w_lista_fosa_comun_para_fs
end type
type cb_aceptar from commandbutton within w_lista_fosa_comun_para_fs
end type
type dw_lista from datawindow within w_lista_fosa_comun_para_fs
end type
type gb_1 from groupbox within w_lista_fosa_comun_para_fs
end type
end forward
...
global w_lista_fosa_comun_para_fs w_lista_fosa_comun_para_fs
...
on w_lista_fosa_comun_para_fs.create
...
on w_lista_fosa_comun_para_fs.destroy
...
event open;gf_centrar(w_lista_fosa_comun_para_fs)
dw_lista.settransobject(sqlca)
il_parque				= Long(trim(substr(1,1,Message.StringParm)))
is_sector				= trim(substr(1,2,Message.StringParm))
is_sepultura			= trim(substr(1,3,Message.StringParm))
if dw_lista.retrieve(il_parque,is_sector,is_sepultura) = 0 then		//,is_sepultura
	messagebox("Advertencia","No Registra Datos Fosa Común Sector "+is_sector+ " Sepultura "+is_sepultura)	// 
	close(w_lista_fosa_comun_para_fs)
end if
end event
...
event clicked;close(w_lista_fosa_comun_para_fs)
end event
...
event clicked;Long		ll_fila,ll_llave_fallecido,ll_cod_parque_origen,ll_capacidad,ll_edad
...
Datetime	ldt_fecha_traslado,ldt_fecha_resolucion,ldt_fecha_ctto,ldt_fec_nac,ldt_fec_fall,ldt_null
...
if isvalid(w_ingreso_ficha_traslado) then
	...
	if ll_fila > 0 then
		...
		if sqlca.sqlcode=0 then
			...
			w_ingreso_ficha_traslado.tab_1.tabpage_3.dw_ficha_sepult.setitem(1,'estado_ctto_fosa','F')
			...
			w_ingreso_ficha_traslado.tab_1.tabpage_1.dw_ant_fall.setitem(1,'estado_ctto_fosa','F')
			w_ingreso_ficha_traslado.tab_1.tabpage_3.st_3.text			= 'Ficha de Traslado desde Fosa Común'
...
integer taborder = 10
string title = "none"
string dataobject = "dw_lista_fosa_comun_para_ficha"
