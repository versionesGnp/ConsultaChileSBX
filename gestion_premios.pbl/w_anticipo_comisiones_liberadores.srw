forward
global type w_anticipo_comisiones_liberadores from window
end type
type pb_1 from picturebutton within w_anticipo_comisiones_liberadores
end type
type cb_filtrar_a_pago from commandbutton within w_anticipo_comisiones_liberadores
end type
type cbx_vigente from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_resciliado from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_pendiente from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_con_saldo from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_castigada from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_pagada from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_todos_prom from checkbox within w_anticipo_comisiones_liberadores
end type
type cbx_todos_rescomi from checkbox within w_anticipo_comisiones_liberadores
end type
type pb_aceptar from picturebutton within w_anticipo_comisiones_liberadores
end type
type pb_filtrar from picturebutton within w_anticipo_comisiones_liberadores
end type
type pb_ordenar from picturebutton within w_anticipo_comisiones_liberadores
end type
type pb_imprimir from picturebutton within w_anticipo_comisiones_liberadores
end type
type pb_exportar from picturebutton within w_anticipo_comisiones_liberadores
end type
type pb_cerrar from picturebutton within w_anticipo_comisiones_liberadores
end type
type dw_lista from datawindow within w_anticipo_comisiones_liberadores
end type
type gb_1 from groupbox within w_anticipo_comisiones_liberadores
end type
type gb_2 from groupbox within w_anticipo_comisiones_liberadores
end type
end forward
...
string title = "Consulta Anticipo Comisiones Liberadores"
...
global w_anticipo_comisiones_liberadores w_anticipo_comisiones_liberadores
...
on w_anticipo_comisiones_liberadores.create
...
on w_anticipo_comisiones_liberadores.destroy
destroy(this.pb_1)
...
string	ls_codigo, ls_serie,ls_estado
...
gf_centrar(w_anticipo_comisiones_liberadores)
dw_lista.settransobject(sqlca)
this.title	= 'Consulta Anticipo Comisiones Liberadores'
w_contar_registros.st_titulo.text	= 'Comisiones Liberadores'
...
for ll_indi = 1 to ll_tot_reg
	...
	ls_cod_age	= dw_lista.getitemstring(ll_indi,'anexo_liberador_cod_agente')
	fecha_prim	= dw_lista.getitemdatetime(ll_indi,'pago_liberador_fecha_prim')
	cta_pag_s	= dw_lista.getitemnumber(ll_indi,'anexo_liberador_cta_pag_m')
	nro_cuotas	= dw_lista.getitemnumber(ll_indi,'pago_liberador_nro_cuotas')
	...
	ll_count_porc	= f_buscar_porc_res_comi( ls_cod_age , serie , numero, "L" )
	dw_lista.setitem(ll_indi,'por_res_comi',ll_count_porc)
	ll_count_ingreso	= f_buscar_ingreso_res_comi( ls_cod_age , serie , numero, "L" )
...
if ls_op_recep = "2" then
	...
	ls_filtter	= "(comision_promesa_c_pie + comision_promesa_c_primera + comision_promesa_c_segunda + comision_promesa_c_tercera + comision_promesa_c_cuarta + comision_promesa_c_quinta + comision_promesa_c_sexta) = 0 and anexo_liberador_estado_comi<>'P'"
	dw_lista.SetFilter(ls_filtter)
	dw_lista.Filter( )
end if
if dw_lista.rowcount() = 0 then
	messagebox("Advertencia","No registra dato")
	close(w_anticipo_comisiones_liberadores)
else
	dw_lista.sort()
end if
close(w_contar_registros)
gs_ventana	= 'w_anticipo_comisiones_liberadores'
f_valida_objeto_1()
SetPointer(Arrow!)
end event
...
if cbx_pendiente.checked=true then
	if not isnull(ls_filtter_rescomi) then
		ls_filtter_rescomi	= ls_filtter_rescomi+"anexo_liberador_estado_comi='"+"N"+"'"
	else
		ls_filtter_rescomi	= "anexo_liberador_estado_comi='"+"N"+"'"
	end if
end if
if cbx_castigada.checked=true then
	if not isnull(ls_filtter_rescomi) then
		ls_filtter_rescomi	= ls_filtter_rescomi+" or anexo_liberador_estado_comi='"+"C"+"'"
	else
		ls_filtter_rescomi	= "anexo_liberador_estado_comi='"+"C"+"'"
	end if
end if
if cbx_con_saldo.checked=true then
	if not isnull(ls_filtter_rescomi) then
		ls_filtter_rescomi	= ls_filtter_rescomi+" or anexo_liberador_estado_comi='"+"S"+"'"
	else
		ls_filtter_rescomi	= "anexo_liberador_estado_comi='"+"S"+"'"
	end if
end if
if cbx_pagada.checked=true then
	if not isnull(ls_filtter_rescomi) then
		ls_filtter_rescomi	= ls_filtter_rescomi+" or anexo_liberador_estado_comi='"+"P"+"'"
	else
		ls_filtter_rescomi	= "anexo_liberador_estado_comi='"+"P"+"'"
	end if
end if
...
event clicked;close(w_anticipo_comisiones_liberadores)
end event
...
integer height = 1760
integer taborder = 10
string dataobject = "dw_lista_contratos_sin_agrupar_liberador"
