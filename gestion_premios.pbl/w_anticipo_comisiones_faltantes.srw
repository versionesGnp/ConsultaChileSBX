forward
global type w_anticipo_comisiones_faltantes from window
end type
type dw_detalle from datawindow within w_anticipo_comisiones_faltantes
end type
type pb_filtrar from picturebutton within w_anticipo_comisiones_faltantes
end type
type pb_ordenar from picturebutton within w_anticipo_comisiones_faltantes
end type
type pb_imprimir from picturebutton within w_anticipo_comisiones_faltantes
end type
type pb_exportar from picturebutton within w_anticipo_comisiones_faltantes
end type
type pb_cerrar from picturebutton within w_anticipo_comisiones_faltantes
end type
type dw_lista from datawindow within w_anticipo_comisiones_faltantes
end type
end forward
...
global w_anticipo_comisiones_faltantes w_anticipo_comisiones_faltantes

on w_anticipo_comisiones_faltantes.create
...
on w_anticipo_comisiones_faltantes.destroy
...
event open;long		ll_tot_reg, ll_indi, cta_pag_s, nro_cuotas, cta_pag_m,RET, ret1, ll_row, ll_new,cuenta, ret11, dias
datetime fecha_prim, fecha_man, fecha_vto
string 	base, serie, ls_cod_age,ls_base_aux, ls_serie_aux,ls_codigo, ls_serie, ls_string
Double	ll_c_pie_aux, ll_c_primera_aux, ll_c_segunda_aux, ll_c_tercera_aux,ll_c_cuarta_aux ,ll_c_quinta_aux, ll_c_sexta_aux,&
			ll_numero_aux,ll_contrato,numero,ll_count_porc
SetPointer(HourGlass!)
gf_centrar(w_anticipo_comisiones_faltantes)
gs_ventana	= 'w_anticipo_comisiones_faltantes'
f_valida_objeto_1()
...
for ll_indi = 1 to ll_tot_reg
	ls_cod_age	= dw_detalle.getitemstring(ll_indi,'oferta_v_cod_age')
	fecha_prim	= dw_detalle.getitemdatetime(ll_indi,'pago_oferta_fecha_prim')
	cta_pag_s	= dw_detalle.getitemnumber(ll_indi,'oferta_v_cta_pag_s')
	nro_cuotas	= dw_detalle.getitemnumber(ll_indi,'pago_oferta_nro_cuotas')
...
//	ll_count_porc	= f_buscar_porc_res_comi( ls_cod_age , serie , numero )
//	dw_detalle.setitem(ll_indi,'por_res_comi',ll_count_porc)
  	SELECT "COMISION_PROMESA"."BASE"  
   INTO :ls_base_aux  
   FROM "COMISION_PROMESA"  
   WHERE ( "COMISION_PROMESA"."BASE" = 'O' ) AND  
	...
	if sqlca.sqlcode = 100 then
		...
		dw_lista.setitem(ll_new,'base','O')
...
if dw_lista.rowcount() = 0 then
	...
	close(w_anticipo_comisiones_faltantes)
else
...
event clicked;close(w_anticipo_comisiones_faltantes)
end event
...
integer taborder = 10
string title = "none"
string dataobject = "dwe_lista_contratos_faltantes"
