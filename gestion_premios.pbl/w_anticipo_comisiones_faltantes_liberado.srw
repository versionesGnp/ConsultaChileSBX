forward
global type w_anticipo_comisiones_faltantes_liberado from window
end type
type dw_detalle from datawindow within w_anticipo_comisiones_faltantes_liberado
end type
type pb_filtrar from picturebutton within w_anticipo_comisiones_faltantes_liberado
end type
type pb_ordenar from picturebutton within w_anticipo_comisiones_faltantes_liberado
end type
type pb_imprimir from picturebutton within w_anticipo_comisiones_faltantes_liberado
end type
type pb_exportar from picturebutton within w_anticipo_comisiones_faltantes_liberado
end type
type pb_cerrar from picturebutton within w_anticipo_comisiones_faltantes_liberado
end type
type dw_lista from datawindow within w_anticipo_comisiones_faltantes_liberado
end type
end forward
...
global w_anticipo_comisiones_faltantes_liberado w_anticipo_comisiones_faltantes_liberado

on w_anticipo_comisiones_faltantes_liberado.create
...
on w_anticipo_comisiones_faltantes_liberado.destroy
...
string		ls_codigo, ls_serie, ls_string,base, serie, ls_cod_age,ls_base_aux, ls_serie_aux
Long		RET, ret1, ll_row, ll_new,cuenta, ret11, dias
datetime fecha_prim, fecha_man, fecha_vto
Double	ll_numero_aux,numero,ll_count_porc,ll_contrato,ll_c_pie_aux, ll_c_primera_aux, ll_c_segunda_aux, ll_c_tercera_aux,&
			ll_c_cuarta_aux ,ll_c_quinta_aux, ll_c_sexta_aux
SetPointer(HourGlass!)
gf_centrar(w_anticipo_comisiones_faltantes_liberado)
gs_ventana	= 'w_anticipo_comisiones_faltantes_liberado'
f_valida_objeto_1()
...
for ll_indi = 1 to ll_tot_reg
	ls_cod_age	= dw_detalle.getitemstring(ll_indi,'anexo_liberador_cod_agente')
	fecha_prim	= dw_detalle.getitemdatetime(ll_indi,'pago_liberador_fecha_prim')
	cta_pag_s	= dw_detalle.getitemnumber(ll_indi,'anexo_liberador_cta_pag_m')
	nro_cuotas	= dw_detalle.getitemnumber(ll_indi,'pago_liberador_nro_cuotas')
   ...
   WHERE ( "COMISION_PROMESA"."BASE" = 'L' ) AND  
...
if dw_lista.rowcount() = 0 then
	...
	close(w_anticipo_comisiones_faltantes_liberado)
else
...
integer height = 200
integer taborder = 60
string dataobject = "dw_lista_contratos_sin_agrupar_falta_lib"
...
event clicked;close(w_anticipo_comisiones_faltantes_liberado)
end event
...
integer taborder = 10
string title = "none"
string dataobject = "dwe_lista_contratos_faltantes_liberador"
