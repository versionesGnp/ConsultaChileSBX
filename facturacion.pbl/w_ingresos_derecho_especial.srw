forward
global type w_ingresos_derecho_especial from window
end type
type dw_1 from datawindow within w_ingresos_derecho_especial
end type
type p_1 from picture within w_ingresos_derecho_especial
end type
type em_fecha from editmask within w_ingresos_derecho_especial
end type
type st_3 from statictext within w_ingresos_derecho_especial
end type
type cb_actualiza from commandbutton within w_ingresos_derecho_especial
end type
type st_2 from statictext within w_ingresos_derecho_especial
end type
type st_1 from statictext within w_ingresos_derecho_especial
end type
type sle_clave from singlelineedit within w_ingresos_derecho_especial
end type
type sle_usuario from singlelineedit within w_ingresos_derecho_especial
end type
type cb_aceptar from commandbutton within w_ingresos_derecho_especial
end type
type cb_cancelar from commandbutton within w_ingresos_derecho_especial
end type
type ln_1 from line within w_ingresos_derecho_especial
end type
type ln_2 from line within w_ingresos_derecho_especial
end type
type gb_1 from groupbox within w_ingresos_derecho_especial
end type
end forward
...
global w_ingresos_derecho_especial w_ingresos_derecho_especial
...
on w_ingresos_derecho_especial.create
...
on w_ingresos_derecho_especial.destroy
destroy(this.dw_1)
destroy(this.p_1)
...
this.title = gs_app_name+' - ( Ingresos Liberadores )'
...
string facename = "Arial"
boolean enabled = false
string text = "~'Ingresos &Liberadores~'"
end type

event clicked;integer ret
string fecha1

ret = messagebox(gs_app_name,'          ¿ Esta seguro que desea Actualizar los Ingresos de Liberadores ?',Question!,YesNoCancel! ,3)
SetPointer ( HourGlass! )
if ret = 1 then
	fecha1 = em_fecha.text
	DELETE FROM "FACTURA_OFERTA_INGRESO" WHERE "FACTURA_OFERTA_INGRESO"."BASE" = 'L' ;
	if sqlca.sqlcode = 0 then
		COMMIT using sqlca;
	else
		rollback;
	end if
 	DECLARE x1 PROCEDURE FOR FAC_OFERTA_INGRE_LIB_DOS (to_date(:fecha1,'dd-mm-yyyy')) ;
	execute x1;
	if sqlca.sqlcode < 0 then
...
IF is_clave = Upper(sle_clave.text) and sle_clave.text<>'' THEN
	...
	END IF
	w_ingresos_derecho_especial.height=1009
...
event clicked;close(w_ingresos_derecho_especial)
end event
...
long textcolor = 255
long backcolor = 79741120
string text = "Ingresos de Lib. ( Factura_Ofertas_Ingreso )"
end type
