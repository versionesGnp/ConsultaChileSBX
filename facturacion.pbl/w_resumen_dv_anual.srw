forward
global type w_resumen_dv_anual from window
end type
type dw_5 from datawindow within w_resumen_dv_anual
end type
type hpb_1 from hprogressbar within w_resumen_dv_anual
end type
type dw_4 from datawindow within w_resumen_dv_anual
end type
type cb_3 from commandbutton within w_resumen_dv_anual
end type
type p_1 from picture within w_resumen_dv_anual
end type
type st_1 from statictext within w_resumen_dv_anual
end type
type st_2 from statictext within w_resumen_dv_anual
end type
type st_3 from statictext within w_resumen_dv_anual
end type
type em_fecha_cierre from editmask within w_resumen_dv_anual
end type
type dw_3 from datawindow within w_resumen_dv_anual
end type
type dw_2 from datawindow within w_resumen_dv_anual
end type
type dw_1 from datawindow within w_resumen_dv_anual
end type
type cb_2 from commandbutton within w_resumen_dv_anual
end type
type cb_1 from commandbutton within w_resumen_dv_anual
end type
type gb_1 from groupbox within w_resumen_dv_anual
end type
end forward
...
global w_resumen_dv_anual w_resumen_dv_anual
...
forward prototypes
public subroutine wf_inserta_dw_devngadas_11 (date fecha_dv, long ctas, long ingre, string base)
...
public subroutine wf_inserta_dw_devngadas_11 (date fecha_dv, long ctas, long ingre, string base);long c,i

dw_4.setitem(1,'fecha_cierre',fecha_dv)

c = dw_4.getitemnumber(1,'ctas_devengadas')
if isnull(c) then c = 0
dw_4.setitem(1,'ctas_devengadas',c + ctas)

i = dw_4.getitemnumber(1,'ingresos')
if isnull(i) then i = 0
dw_4.setitem(1,'ingresos',i + ingre)

dw_4.setitem(1,'base',base)
dw_4.accepttext ()
end subroutine
...
public subroutine wf_graba_dv (date fecha_cierre, long ctas_dv, long ingresos, integer parque);INSERT INTO "CUOTAS_DEVENGADAS"  
  ...
  VALUES  (:fecha_cierre  ,:ctas_dv         ,:ingresos  ,'O',   :parque)  ;
if sqlca.sqlcode <> 0 then
   ...
   WHERE ( "CUOTAS_DEVENGADAS"."FECHA_CIERRE" = :fecha_cierre ) AND 
			( "CUOTAS_DEVENGADAS"."BASE" = 'O') and ("COD_PARQUE" = :parque)  ;
	if sqlca.sqlcode <> 0 then		
...
integer taborder = 31
boolean titlebar = true
string dataobject = "dw_devngadas_11"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
...
integer height = 504
boolean titlebar = true
string dataobject = "dw_resumen_dv_anual"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;close(w_resumen_dv_anual)
end event
...
long textcolor = 255
long backcolor = 67108864
string text = "Ctas. Mensuales Devengadas"
end type
