forward
global type w_calculo_aumento_dv from window
end type
type hpb_1 from hprogressbar within w_calculo_aumento_dv
end type
type dw_2 from datawindow within w_calculo_aumento_dv
end type
type st_3 from statictext within w_calculo_aumento_dv
end type
type st_2 from statictext within w_calculo_aumento_dv
end type
type dw_1 from datawindow within w_calculo_aumento_dv
end type
type cb_3 from commandbutton within w_calculo_aumento_dv
end type
type st_1 from statictext within w_calculo_aumento_dv
end type
type cb_2 from commandbutton within w_calculo_aumento_dv
end type
type cb_1 from commandbutton within w_calculo_aumento_dv
end type
type p_1 from picture within w_calculo_aumento_dv
end type
type em_fecha_cierre from editmask within w_calculo_aumento_dv
end type
type gb_1 from groupbox within w_calculo_aumento_dv
end type
end forward
...
global w_calculo_aumento_dv w_calculo_aumento_dv
...
on w_calculo_aumento_dv.create
...
on w_calculo_aumento_dv.destroy
destroy(this.hpb_1)
destroy(this.dw_2)
destroy(this.st_3)
destroy(this.st_2)
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_aumento_p"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;OpenWithParm(w_imprime_saldo_aumento_dv, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_aumento_dv)
end event
...
date		fecha
string   serie,moneda,base,fecha_of,estado_comi,age
integer  ret
...
WHERE ( "FACTURA_OFERTA_DV"."FECHA_CIERRE" = :fecha_cierre ) AND  
      ( "FACTURA_OFERTA_DV"."BASE" = 'A' ) AND  
...
WHERE ( "FACTURA_OFERTA_DV_M"."FECHA_CIERRE" = :fecha_cierre ) AND  
      ( "FACTURA_OFERTA_DV_M"."BASE" = 'A' ) AND  
...
long textcolor = 255
long backcolor = 67108864
string text = "Liberadores Devengados."
end type
