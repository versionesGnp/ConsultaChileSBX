forward
global type w_calculo_aumento from window
end type
type hpb_1 from hprogressbar within w_calculo_aumento
end type
type dw_2 from datawindow within w_calculo_aumento
end type
type st_3 from statictext within w_calculo_aumento
end type
type st_2 from statictext within w_calculo_aumento
end type
type dw_1 from datawindow within w_calculo_aumento
end type
type cb_3 from commandbutton within w_calculo_aumento
end type
type st_1 from statictext within w_calculo_aumento
end type
type cb_2 from commandbutton within w_calculo_aumento
end type
type cb_1 from commandbutton within w_calculo_aumento
end type
type p_1 from picture within w_calculo_aumento
end type
type em_fecha_cierre from editmask within w_calculo_aumento
end type
type gb_1 from groupbox within w_calculo_aumento
end type
end forward
...
global w_calculo_aumento w_calculo_aumento
...
forward prototypes
public subroutine wf_inicial (ref double capital_inicial, double interes_inicial, double gc_inicial, string base, string serie, long numero, string moneda)
public function integer wf_ingreso (date fecha, string serie, long numero, ref double monto, ref double cuota, ref double pie)
end prototypes

public subroutine wf_inicial (ref double capital_inicial, double interes_inicial, double gc_inicial, string base, string serie, long numero, string moneda);datetime fecha_n
double uf_n
...
WHERE ( "FACTURA_OFERTA_INGRESO"."BASE" ='A'  ) AND  
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_aumento"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;OpenWithParm(w_imprime_saldo_aumento, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_aumento)
end event
...
datetime ldt_fecha_prim,fecha_ult
datetime fecha_cierre,fecha_vto,fecha_oferta
date		fecha
string   serie,moneda,base,fecha_of,estado_comi,age,sup,jefe,estado_age,estado_sup,estado_jefe,estado_comi_sup
integer  ret
...
if dw_1.rowcount() > 0 then
	...
	for i = 1 to dw_1.rowcount()
		setnull(facturado)
		serie       					= dw_1.getitemstring(i,'serie_m')
		nro_oferta  					= dw_1.getitemnumber(i,'nro_aumento')
		wf_ingreso(date(fecha_cierre),serie,nro_oferta,monto,cuota,pie)
		pie_pag 						= pie
		//----- calculo pie parcelado ---//
		SELECT 	"PAGO_AUMENTO"."CTAS_PACTADAS_PIE"  
		INTO 		:ll_ctas_pie_parc  
		FROM 	"PAGO_AUMENTO"  
		WHERE 	"PAGO_AUMENTO"."SERIE_M" = :serie AND  
					"PAGO_AUMENTO"."NRO_AUMENTO" = :nro_oferta;
		if isnull(ll_ctas_pie_parc) or ll_ctas_pie_parc = 0 then
			ldb_saldo_pie			= 0
		else
			ldb_saldo_pie			= f_calculo_pie_parce_fac_aum('A', serie,nro_oferta,date(fecha_cierre))
		end if	
		//----//
		if not isnull(cuota) then 
			...
			ldt_fecha_prim     		= dw_1.getitemdatetime(i,'fecha_prim')
			fecha_ult      			= dw_1.getitemdatetime(i,'fecha_ult')
			ctas_pag_s     			= cuota
			nro_cuotas     			= dw_1.getitemnumber(i,'nro_cuotas')
			cuotas_mora    		= funcion_mora_fac( ldt_fecha_prim,ctas_pag_s,nro_cuotas,fecha )
...
//			if gastos_adm  = 0 then  gastos_adm  = 0.001 
...
//--------------------------------------------------------------------------------------------------------			
			...
			if nro_cuotas <=  ctas_pag_s then
				...
				fecha_vto   			= datetime(funcion_venc( ldt_fecha_prim , ctas_pag_s ))
			end if
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
		if  tasa = 0 then
			...
			if moneda = '2' then						
				...
				capital_inicial 	= ( ( f_vp( nro_cuotas, tasa, valor_cuota )  ) - gc_inicial + pie_pag )
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
			...
			INTO :age  
			FROM "ANEXO_AUMENTO"  
			WHERE ( "ANEXO_AUMENTO"."SERIE_M" = :serie ) AND ( "ANEXO_AUMENTO"."NRO_AUMENTO" = :nro_oferta ) ;
			if age = 'C' then
				comision		    = 0
			else
			...
			INSERT INTO "FACTURA_OFERTA"  
					 ( "BASE","SERIE","NUMERO"   ,"FEC_OFE"    ,"MORA"      ,"SALDO_TOTAL" ,"SALDO_CP" ,"SALDO_LP"   ,"CAPITAL" ,"CAPITAL_CP" ,"CAPITAL_LP" ,"INTERES" ,"INTERES_CP" ,"INTERES_LP" ,"GC"       ,"GC_CP" ,"GC_LP" ,"FECHA_PRIM" ,"CTAS_PAG" ,"PLAZO"    ,"MONTO_MORA" ,"PIE"     ,"VALOR_CTA" ,"FACTURADO" ,"FECHA_UF","FECHA_CIERRE","MONEDA","VALOR_UF","ESTADO","FECHA_PROCESO"     ,"MORA_PIE","PIE_PAGADO","COMISIONES","TOTAL_PAGADO","COMPRUEBA_SALDO","ESTADO_COMI","ESTADO_AGE","ESTADO_SUP","ESTADO_JEFE","PORCE_VENTA","PORCE_SUP","ESTADO_COMI_SUP" ,"CAPITAL_INICIAL","INTERES_INICIAL","GC_INICIAL" ) 
			VALUES ( 'A'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:ldt_fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'S'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,:estado_age ,:estado_sup ,:estado_jefe ,:porce_venta ,:porce_sup ,:estado_comi_sup  ,:capital_inicial ,:interes_inicial ,:gc_inicial  ) ;
