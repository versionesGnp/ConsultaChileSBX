forward
global type w_calculo_liberadores_dv from window
end type
type hpb_1 from hprogressbar within w_calculo_liberadores_dv
end type
type dw_2 from datawindow within w_calculo_liberadores_dv
end type
type st_3 from statictext within w_calculo_liberadores_dv
end type
type st_2 from statictext within w_calculo_liberadores_dv
end type
type dw_1 from datawindow within w_calculo_liberadores_dv
end type
type cb_3 from commandbutton within w_calculo_liberadores_dv
end type
type st_1 from statictext within w_calculo_liberadores_dv
end type
type cb_2 from commandbutton within w_calculo_liberadores_dv
end type
type cb_1 from commandbutton within w_calculo_liberadores_dv
end type
type p_1 from picture within w_calculo_liberadores_dv
end type
type em_fecha_cierre from editmask within w_calculo_liberadores_dv
end type
type gb_1 from groupbox within w_calculo_liberadores_dv
end type
end forward
...
global w_calculo_liberadores_dv w_calculo_liberadores_dv
...
on w_calculo_liberadores_dv.create
...
on w_calculo_liberadores_dv.destroy
destroy(this.hpb_1)
destroy(this.dw_2)
destroy(this.st_3)
destroy(this.st_2)
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_liberador_p"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;OpenWithParm(w_imprime_saldo_liberadores_dv, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_liberadores_dv)
end event
...
//--------------------------------------------------------------------------------------------------------
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
         ...
         WHERE ( "OFERTA_V"."SERIE" = :serie ) AND ( "OFERTA_V"."NRO_OFERTA" = nro_oferta ) ; 
			...
			VALUES ( 'L'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'D'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:parque  );
...
WHERE ( "FACTURA_OFERTA_DV_M"."FECHA_CIERRE" = :fecha_cierre ) AND  
      ( "FACTURA_OFERTA_DV_M"."BASE" = 'L' ) AND  
...
//-----------------------------------------dv_m-----------------------------------------------------
			if tasa > 0 then
				...
				saldo_total		= saldo_total + pie_pag + ldb_saldo_pie
