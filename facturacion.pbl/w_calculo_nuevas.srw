forward
global type w_calculo_nuevas from window
end type
type cb_primera_fact from commandbutton within w_calculo_nuevas
end type
type hpb_1 from hprogressbar within w_calculo_nuevas
end type
type dw_2 from datawindow within w_calculo_nuevas
end type
type st_3 from statictext within w_calculo_nuevas
end type
type st_2 from statictext within w_calculo_nuevas
end type
type dw_1 from datawindow within w_calculo_nuevas
end type
type cb_3 from commandbutton within w_calculo_nuevas
end type
type st_1 from statictext within w_calculo_nuevas
end type
type cb_2 from commandbutton within w_calculo_nuevas
end type
type cb_1 from commandbutton within w_calculo_nuevas
end type
type p_1 from picture within w_calculo_nuevas
end type
type em_fecha_cierre from editmask within w_calculo_nuevas
end type
type gb_1 from groupbox within w_calculo_nuevas
end type
end forward
...
global w_calculo_nuevas w_calculo_nuevas
...
forward prototypes
public subroutine wf_terminadas ()
...
ids_1.DataObject = "dw_calculo_nuevas"
...
string text = "Actualiza Primer Factura"
end type
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_nuevas"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;OpenWithParm(w_imprime_saldo_nueva, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_nuevas)
end event
...
event clicked;long     	i,nro_oferta,cont,malos,ctas_pag_s,nro_cuotas,cuotas_mora,periodo,p1,p2,count,a,&
			facturado,ll_ctas_pie_parc
double   gastos_adm,factor_gc,g_cobranza,valor_presente,tasa,valor_cuota,saldo_insoluto,&
			uf_dia,capital,interes,uf_cierre,saldo_total,&
			saldo_12,saldo_12_mas,kcp,klp,icp,ilp,gccp,gclp,&
			monto_mora,pie,monto,cuota,pie_pac,pie_pag,precio,porce_venta,porce_sup,comision,&
			comprueba_saldo,existe,capital_inicial,interes_inicial,gc_inicial,ldb_saldo_pie
datetime fecha_prim,fecha_ult
datetime fecha_cierre,fecha_vto,fecha_oferta
date		fecha
string   serie,moneda,base,fecha_of,estado_comi,age,sup,jefe,estado_age,estado_sup,estado_jefe,&
			estado_comi_sup,estado_cadena,estatus_comision,estado_comi_jef
integer  ret, parque
...
if ret = 1 then 
	if ids_1.rowcount() > 0 then
		...
		for i = 1 to ids_1.rowcount()
			...
			if not isnull(cuota) then
				...
				estatus_comision 	 = ids_1.getitemstring(i,'oferta_v_estatus_comision')
				estado_comi_jef 	 = ids_1.getitemstring(i,'oferta_v_estado_comi_jv')
	...
	//--------------------------------------------------------------------------------------------------------
				...
				if moneda = '2' then
					...
					precio		= long(ids_1.getitemnumber(i,'precio') * uf_cierre)
					gastos_adm  = long(gastos_adm * uf_cierre)
				end if
...
//	DECLARE dept_proc PROCEDURE FOR SP_PRIMERA_FACTURACION();
//	EXECUTE dept_proc;
end if
setpointer(Arrow!)
end event
