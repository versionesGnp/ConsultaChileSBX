forward
global type w_calculo_pagare_resc from window
end type
type hpb_1 from hprogressbar within w_calculo_pagare_resc
end type
type cb_4 from commandbutton within w_calculo_pagare_resc
end type
type dw_2 from datawindow within w_calculo_pagare_resc
end type
type st_3 from statictext within w_calculo_pagare_resc
end type
type st_2 from statictext within w_calculo_pagare_resc
end type
type dw_1 from datawindow within w_calculo_pagare_resc
end type
type cb_3 from commandbutton within w_calculo_pagare_resc
end type
type st_1 from statictext within w_calculo_pagare_resc
end type
type cb_2 from commandbutton within w_calculo_pagare_resc
end type
type cb_1 from commandbutton within w_calculo_pagare_resc
end type
type p_1 from picture within w_calculo_pagare_resc
end type
type em_fecha_cierre from editmask within w_calculo_pagare_resc
end type
type gb_1 from groupbox within w_calculo_pagare_resc
end type
end forward
...
global w_calculo_pagare_resc w_calculo_pagare_resc
...
WHERE ( "FACTURA_PAGARE_INGRESO"."BASE" ='P'  ) AND  
      ...
      ( "FACTURA_PAGARE_INGRESO"."NUMERO" = :numero )  AND 
		("FACTURA_PAGARE_INGRESO"."ESTADO" ) ='R';
Return 1
end function

on w_calculo_pagare_resc.create
...
on w_calculo_pagare_resc.destroy
destroy(this.hpb_1)
destroy(this.cb_4)
destroy(this.dw_2)
destroy(this.st_3)
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_pagare_res"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;close(w_calculo_pagare_resc)
end event
...
SELECT count("FACTURA_PAGARE"."NUMERO")  
INTO :existe  
FROM "FACTURA_PAGARE"  
WHERE ( "FACTURA_PAGARE"."FECHA_CIERRE" = :fecha_cierre ) AND  
      ( "FACTURA_PAGARE"."BASE" = 'P' ) AND  
      ( "FACTURA_PAGARE"."ESTADO" = 'R' )   ;
if existe > 0 then
...
if dw_1.rowcount() > 0 then
	...
	//				pie_pac		= pie_pac * uf_dia
		...
		//------------------- > largo plazo mas de 12 meses <-------------------
			...
			VALUES ( 'P'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,null       ,null    ,null    ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'R'     ,:gdt_tiempo_servidor,:pie      ,:pie_pag    ,null        ,:monto        ,:comprueba_saldo ,:estado);
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
...
long textcolor = 255
long backcolor = 67108864
string text = "Saldos de Pagarés Resciliados."
end type
