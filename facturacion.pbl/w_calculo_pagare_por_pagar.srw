forward
global type w_calculo_pagare_por_pagar from window
end type
type hpb_1 from hprogressbar within w_calculo_pagare_por_pagar
end type
type cb_4 from commandbutton within w_calculo_pagare_por_pagar
end type
type dw_2 from datawindow within w_calculo_pagare_por_pagar
end type
type st_3 from statictext within w_calculo_pagare_por_pagar
end type
type st_2 from statictext within w_calculo_pagare_por_pagar
end type
type dw_1 from datawindow within w_calculo_pagare_por_pagar
end type
type cb_3 from commandbutton within w_calculo_pagare_por_pagar
end type
type st_1 from statictext within w_calculo_pagare_por_pagar
end type
type cb_2 from commandbutton within w_calculo_pagare_por_pagar
end type
type cb_1 from commandbutton within w_calculo_pagare_por_pagar
end type
type p_1 from picture within w_calculo_pagare_por_pagar
end type
type em_fecha_cierre from editmask within w_calculo_pagare_por_pagar
end type
type gb_1 from groupbox within w_calculo_pagare_por_pagar
end type
end forward
...
global w_calculo_pagare_por_pagar w_calculo_pagare_por_pagar
...
on w_calculo_pagare_por_pagar.create
...
on w_calculo_pagare_por_pagar.destroy
destroy(this.hpb_1)
destroy(this.cb_4)
destroy(this.dw_2)
destroy(this.st_3)
...
event clicked;close(w_calculo_pagare_por_pagar)
end event
...
//				fecha_vto   = datetime(fecha_ult)
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
			...
			VALUES ( 'P'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,null       ,null    ,null    ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'V'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado       );
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
...
long textcolor = 255
long backcolor = 67108864
string text = "Pagarés por Vencer"
end type
