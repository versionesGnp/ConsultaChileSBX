forward
global type w_calculo_resc from window
end type
type hpb_1 from hprogressbar within w_calculo_resc
end type
type cb_4 from commandbutton within w_calculo_resc
end type
type dw_2 from datawindow within w_calculo_resc
end type
type st_3 from statictext within w_calculo_resc
end type
type st_2 from statictext within w_calculo_resc
end type
type dw_1 from datawindow within w_calculo_resc
end type
type cb_3 from commandbutton within w_calculo_resc
end type
type st_1 from statictext within w_calculo_resc
end type
type cb_2 from commandbutton within w_calculo_resc
end type
type cb_1 from commandbutton within w_calculo_resc
end type
type p_1 from picture within w_calculo_resc
end type
type em_fecha_cierre from editmask within w_calculo_resc
end type
type gb_1 from groupbox within w_calculo_resc
end type
end forward
...
global w_calculo_resc w_calculo_resc
...
WHERE ( "FACTURA_OFERTA_INGRESO"."BASE" ='O'  ) AND  
      ...
      ( "FACTURA_OFERTA_INGRESO"."NUMERO" = :numero )  AND 
		("FACTURA_OFERTA_INGRESO"."ESTADO" ) ='R';
Return 1
end function

on w_calculo_resc.create
...
//	close(w_calculo_resc)
//end if	



ids_1 = Create DataStore
ids_1.DataObject = "dw_calculo_resc"
ids_1.SetTransObject(sqlca)
if ids_1.retrieve()  > 0 then
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_resc"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;OpenWithParm(w_imprime_saldo_resciliadas, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_resc)
end event
...
date		fecha
string   serie,moneda,base,fecha_of,estado_comi,age,estado_age,&
         estado_sup,estado_jefe,sup,jefe,estado_cadena,estatus_comision,estado_comi_jef
integer  ret, parque
...
ret = messagebox(gs_app_name,'¿ Esta seguro que desea procesar resciliadas ?~n~r~n~r~n~r   El proceso tardará algunos minutos.~n~r'&
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
		if  tasa = 0 then
			...
			if moneda = '2' then						
				...
				capital_inicial = ( ( f_vp( nro_cuotas, tasa, valor_cuota )  ) - gc_inicial + pie )
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
			...
			SELECT "OFERTA_V"."COD_AGE","OFERTA_V"."COD_SUP","OFERTA_V"."COD_JEF"  
         INTO :age,:sup,:jefe
         FROM "OFERTA_V"  
         WHERE ( "OFERTA_V"."SERIE" = :serie ) AND ( "OFERTA_V"."NRO_OFERTA" = :nro_oferta ) ; 
			...
			VALUES ( 'O'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'R'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,:estado_age ,:estado_sup ,:estado_jefe ,:porce_venta  ,:porce_sup,null              ,:capital_inicial ,:interes_inicial ,:gc_inicial , :parque     ,:estado_cadena,	:estatus_comision, :estado_comi_jef);
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
...
long textcolor = 255
long backcolor = 67108864
string text = "Saldos de Ofertas Resciliadas."
end type
