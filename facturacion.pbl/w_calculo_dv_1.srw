forward
global type w_calculo_dv_1 from window
end type
type cb_4 from commandbutton within w_calculo_dv_1
end type
type dw_2 from datawindow within w_calculo_dv_1
end type
type st_3 from statictext within w_calculo_dv_1
end type
type st_2 from statictext within w_calculo_dv_1
end type
type dw_1 from datawindow within w_calculo_dv_1
end type
type cb_3 from commandbutton within w_calculo_dv_1
end type
type st_1 from statictext within w_calculo_dv_1
end type
type ole_meter from uo_meter_control within w_calculo_dv_1
end type
type cb_2 from commandbutton within w_calculo_dv_1
end type
type cb_1 from commandbutton within w_calculo_dv_1
end type
type p_1 from picture within w_calculo_dv_1
end type
type em_fecha_cierre from editmask within w_calculo_dv_1
end type
type gb_1 from groupbox within w_calculo_dv_1
end type
end forward
...
global w_calculo_dv_1 w_calculo_dv_1
...
boolean border = true
borderstyle borderstyle = stylelowered!
string binarykey = "w_calculo_dv_1.win"
integer textsize = -8
end type
...
if dw_1.rowcount() > 0 then
	...
	for i = 1 to dw_1.rowcount()
		...
		//	ll_ctas_pie_parc			= dw_1.getitemnumber(ll_indi,'pago_oferta_ctas_pactadas_pie')
		...
		if not isnull(cuota) then 
			...
			periodo        = ctas_pag_s //+ cuotas_mora // <--------OJO-----<<
//	 		periodo        = nro_cuotas
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
			...
			VALUES ( 'O'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'X'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo);
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
