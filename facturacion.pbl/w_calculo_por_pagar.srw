forward
global type w_calculo_por_pagar from window
end type
type hpb_1 from hprogressbar within w_calculo_por_pagar
end type
type cb_4 from commandbutton within w_calculo_por_pagar
end type
type dw_2 from datawindow within w_calculo_por_pagar
end type
type st_3 from statictext within w_calculo_por_pagar
end type
type st_2 from statictext within w_calculo_por_pagar
end type
type dw_1 from datawindow within w_calculo_por_pagar
end type
type cb_3 from commandbutton within w_calculo_por_pagar
end type
type st_1 from statictext within w_calculo_por_pagar
end type
type cb_2 from commandbutton within w_calculo_por_pagar
end type
type cb_1 from commandbutton within w_calculo_por_pagar
end type
type p_1 from picture within w_calculo_por_pagar
end type
type em_fecha_cierre from editmask within w_calculo_por_pagar
end type
type gb_1 from groupbox within w_calculo_por_pagar
end type
end forward
...
global w_calculo_por_pagar w_calculo_por_pagar
...
on w_calculo_por_pagar.destroy
destroy(this.hpb_1)
destroy(this.cb_4)
destroy(this.dw_2)
destroy(this.st_3)
...
event clicked;OpenWithParm(w_imprime_saldo_p, em_fecha_cierre.text)
end event
...
event clicked;close(w_calculo_por_pagar)
end event
...
//-----------------------------------------08-05-2006-----------------------------------------------------
			if tasa > 0 then
				...
				g_cobranza     = 0
				if moneda = '2' then
					saldo_total		= (precio * uf_cierre) - (monto + ( (cuotas_mora * valor_cuota) * uf_cierre)) + ldb_saldo_pie
					capital        = (precio * uf_cierre) - (monto + ( (cuotas_mora * valor_cuota) * uf_cierre))
				else
					saldo_total		= precio - (monto + ( cuotas_mora * valor_cuota)) + ldb_saldo_pie
					capital        = precio - (monto + ( cuotas_mora * valor_cuota))
				end if
			end if
			if periodo       <= 0 then
				interes			= 0
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
         ...
         WHERE ( "OFERTA_V"."SERIE" = :serie ) AND ( "OFERTA_V"."NRO_OFERTA" = nro_oferta ) ; 
			...
			INSERT INTO "FACTURA_OFERTA"  
					 ( "BASE","SERIE","NUMERO"   ,"FEC_OFE"    ,"MORA"      ,"SALDO_TOTAL" ,"SALDO_CP" ,"SALDO_LP"   ,"CAPITAL" ,"CAPITAL_CP" ,"CAPITAL_LP" ,"INTERES" ,"INTERES_CP" ,"INTERES_LP" ,"GC"       ,"GC_CP" ,"GC_LP" ,"FECHA_PRIM" ,"CTAS_PAG" ,"PLAZO"    ,"MONTO_MORA" ,"PIE"     ,"VALOR_CTA" ,"FACTURADO" ,"FECHA_UF","FECHA_CIERRE","MONEDA","VALOR_UF","ESTADO","FECHA_PROCESO"     ,"MORA_PIE","PIE_PAGADO","COMISIONES","TOTAL_PAGADO","COMPRUEBA_SALDO","ESTADO_COMI","ESTADO_AGE","ESTADO_SUP","ESTADO_JEFE","COD_PARQUE","ESTADO_CADENA",	"ESTATUS_COMISION","ESTADO_COMI_JEF")   
			VALUES ( 'O'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'V'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,null        ,null        ,null         ,:parque     ,:estado_cadena,	:estatus_comision, :estado_comi_jef);
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
...
long textcolor = 255
long backcolor = 67108864
string text = "Ofertas por Vencer"
end type
