forward
global type w_elimina_pagare from window
end type
type dw_1 from datawindow within w_elimina_pagare
end type
type p_1 from picture within w_elimina_pagare
end type
type em_fecha from editmask within w_elimina_pagare
end type
type st_3 from statictext within w_elimina_pagare
end type
type cb_actualiza from commandbutton within w_elimina_pagare
end type
type st_2 from statictext within w_elimina_pagare
end type
type st_1 from statictext within w_elimina_pagare
end type
type sle_clave from singlelineedit within w_elimina_pagare
end type
type sle_usuario from singlelineedit within w_elimina_pagare
end type
type cb_aceptar from commandbutton within w_elimina_pagare
end type
type cb_cancelar from commandbutton within w_elimina_pagare
end type
type ln_1 from line within w_elimina_pagare
end type
type ln_2 from line within w_elimina_pagare
end type
type gb_1 from groupbox within w_elimina_pagare
end type
end forward
...
global w_elimina_pagare w_elimina_pagare
...
WHERE ( "FACTURA_PAGARE"."FECHA_CIERRE" = :FECHA1 ) ;
...
WHERE ( "FACTURA_PAGARE_DV"."FECHA_CIERRE" = :FECHA1 ) ;
If sqlca.sqlcode < 0 Then
	MessageBox(gs_app_name,"DataBase Error!.~n~n"+sqlca.sqlerrtext+".~n~nwf_actualiza()")
	ROLLBACK;
ELSE
...
WHERE ( "FACTURA_PAGARE_DV_M"."FECHA_CIERRE" = :FECHA1 ) ;
If sqlca.sqlcode < 0 Then
	MessageBox(gs_app_name,"DataBase Error!.~n~n"+sqlca.sqlerrtext+".~n~nwf_actualiza()")
	ROLLBACK;
ELSE
...
this.title = gs_app_name+' - ( Elimina Pagare )'
...
string facename = "Arial"
boolean enabled = false
string text = "~'Elimina &Pagares~'"
end type

event clicked;integer ret
string fec
ret = messagebox(gs_app_name,'          ¿ Esta seguro que desea Eliminar ?',Question!,YesNoCancel! ,3)
if ret = 1 then
	setpointer(HourGlass!)
	fec = trim(em_fecha.text)
	if wf_actualizar(fec) = 1 then messagebox(gs_app_name,'Eliminación terminada.',information!)
	setpointer(Arrow!)
	close(w_elimina_ofertas)
elseif ret = 2 then
	close(w_elimina_ofertas)
end if
end event
...
event clicked;close(w_elimina_pagare)
end event
...
long textcolor = 255
long backcolor = 79741120
string text = "Eliminación de Pagare. ( Factura_pagare )"
end type
