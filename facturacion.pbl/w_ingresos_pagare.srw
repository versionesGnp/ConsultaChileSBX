forward
global type w_ingresos_pagare from window
end type
type dw_1 from datawindow within w_ingresos_pagare
end type
type p_1 from picture within w_ingresos_pagare
end type
type em_fecha from editmask within w_ingresos_pagare
end type
type st_3 from statictext within w_ingresos_pagare
end type
type cb_actualiza from commandbutton within w_ingresos_pagare
end type
type st_2 from statictext within w_ingresos_pagare
end type
type st_1 from statictext within w_ingresos_pagare
end type
type sle_clave from singlelineedit within w_ingresos_pagare
end type
type sle_usuario from singlelineedit within w_ingresos_pagare
end type
type cb_aceptar from commandbutton within w_ingresos_pagare
end type
type cb_cancelar from commandbutton within w_ingresos_pagare
end type
type ln_1 from line within w_ingresos_pagare
end type
type ln_2 from line within w_ingresos_pagare
end type
type gb_1 from groupbox within w_ingresos_pagare
end type
end forward
...
global w_ingresos_pagare w_ingresos_pagare
...
this.title = gs_app_name+' - ( Ingresos Pagare )'
...
ret = messagebox(gs_app_name,'          ¿ Esta seguro que desea Actualizar los Ingresos de Pagares ?',Question!,YesNoCancel! ,3)
SetPointer ( HourGlass! )
if ret = 1 then
	fecha1 = em_fecha.text
	DELETE FROM "FACTURA_PAGARE_INGRESO" using sqlca;
	if sqlca.sqlcode = 0 then
		COMMIT using sqlca;
	else
		rollback;
	end if
	DECLARE x1 PROCEDURE FOR FAC_PAGARE_INGRE_DOS(to_date(:fecha1,'dd-mm-yyyy')) ;
	execute x1;
	if sqlca.sqlcode < 0 then
...
event clicked;close(w_ingresos_pagare)
end event
...
long textcolor = 255
long backcolor = 79741120
string text = "Ingresos de Pag. ( Factura_Pagare_Ingreso )"
end type
