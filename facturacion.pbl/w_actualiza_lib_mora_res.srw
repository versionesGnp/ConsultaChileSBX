forward
global type w_actualiza_lib_mora_res from window
end type
type dw_1 from datawindow within w_actualiza_lib_mora_res
end type
type cb_4 from commandbutton within w_actualiza_lib_mora_res
end type
type cb_3 from commandbutton within w_actualiza_lib_mora_res
end type
type st_1 from statictext within w_actualiza_lib_mora_res
end type
type cb_2 from commandbutton within w_actualiza_lib_mora_res
end type
type cb_1 from commandbutton within w_actualiza_lib_mora_res
end type
type p_1 from picture within w_actualiza_lib_mora_res
end type
type em_fecha_cierre from editmask within w_actualiza_lib_mora_res
end type
type gb_1 from groupbox within w_actualiza_lib_mora_res
end type
end forward
...
global w_actualiza_lib_mora_res w_actualiza_lib_mora_res
...
on w_actualiza_lib_mora_res.create
...
on w_actualiza_lib_mora_res.destroy
destroy(this.dw_1)
destroy(this.cb_4)
destroy(this.cb_3)
destroy(this.st_1)
...
event clicked;close(w_actualiza_lib_mora_res)
end event
...
for i = 1 to dw_1.rowcount()
	...
	INSERT INTO "RES_MORA_FACT" ( "MES","BASE","MORA_CERO","MORA_UNO","MORA_DOS","MORA_TRES_CUATRO","MORA_CINCO_DOCE","MORA_TRECE","CAPITAL_UNO","CAPITAL_DOS","CAPITAL_TRES_CUATRO","CAPITAL_CINCO_DOCE","CAPITAL_TRECE","COD_PARQUE" )  
								VALUES (:mes  ,'L'   ,:mora_cero ,:mora_uno ,:mora_dos ,:mora_tres_cuatro ,:mora_cinco_doce ,:mora_trece ,:capital_uno ,:capital_dos ,:capital_tres_cuatro ,:capital_cinco_doce ,:capital_trece ,:parque)  ;
	if sqlca.sqlcode <> 0 then
		rollback;
		messagebox('Error','Error al grabar la información ( '+sqlca.SQLErrText	+' )',stopsign!)
		close(w_actualiza_lib_mora_res)
	else
		commit;
...
long textcolor = 255
long backcolor = 67108864
string text = "Resumen de Mora de Liberadores"
end type
