forward
global type w_calculo_liberadores_por_pagar from window
end type
type dw_2 from datawindow within w_calculo_liberadores_por_pagar
end type
type st_3 from statictext within w_calculo_liberadores_por_pagar
end type
type st_2 from statictext within w_calculo_liberadores_por_pagar
end type
type dw_1 from datawindow within w_calculo_liberadores_por_pagar
end type
type cb_3 from commandbutton within w_calculo_liberadores_por_pagar
end type
type st_1 from statictext within w_calculo_liberadores_por_pagar
end type
type ole_meter from uo_meter_control within w_calculo_liberadores_por_pagar
end type
type cb_2 from commandbutton within w_calculo_liberadores_por_pagar
end type
type cb_1 from commandbutton within w_calculo_liberadores_por_pagar
end type
type p_1 from picture within w_calculo_liberadores_por_pagar
end type
type em_fecha_cierre from editmask within w_calculo_liberadores_por_pagar
end type
type gb_1 from groupbox within w_calculo_liberadores_por_pagar
end type
end forward
...
global w_calculo_liberadores_por_pagar w_calculo_liberadores_por_pagar
...
on w_calculo_liberadores_por_pagar.create
...
on w_calculo_liberadores_por_pagar.destroy
destroy(this.dw_2)
destroy(this.st_3)
destroy(this.st_2)
destroy(this.dw_1)
...
boolean border = true
borderstyle borderstyle = stylelowered!
string binarykey = "w_calculo_liberadores_por_pagar.win"
integer textsize = -8
end type
...
if dw_1.rowcount() > 0 then
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
			...
			monto			    = long(monto)
			comprueba_saldo = long(saldo_total - ( facturado - monto ) + pie)
			select distinct sysdate into :gdt_tiempo_servidor from agentes;
			...
			VALUES ( 'L'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'S'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo);
