forward
global type w_calculo_liberadores_resciliados from window
end type
type hpb_1 from hprogressbar within w_calculo_liberadores_resciliados
end type
type dw_2 from datawindow within w_calculo_liberadores_resciliados
end type
type st_3 from statictext within w_calculo_liberadores_resciliados
end type
type st_2 from statictext within w_calculo_liberadores_resciliados
end type
type dw_1 from datawindow within w_calculo_liberadores_resciliados
end type
type cb_3 from commandbutton within w_calculo_liberadores_resciliados
end type
type st_1 from statictext within w_calculo_liberadores_resciliados
end type
type cb_2 from commandbutton within w_calculo_liberadores_resciliados
end type
type cb_1 from commandbutton within w_calculo_liberadores_resciliados
end type
type p_1 from picture within w_calculo_liberadores_resciliados
end type
type em_fecha_cierre from editmask within w_calculo_liberadores_resciliados
end type
type gb_1 from groupbox within w_calculo_liberadores_resciliados
end type
end forward
...
global w_calculo_liberadores_resciliados w_calculo_liberadores_resciliados
...
on w_calculo_liberadores_resciliados.create
...
on w_calculo_liberadores_resciliados.destroy
destroy(this.hpb_1)
destroy(this.dw_2)
destroy(this.st_3)
destroy(this.st_2)
...
if dw_1.retrieve() > 0 then
	...
	close(w_calculo_liberadores_resciliados)
end if
end event
...
integer taborder = 50
boolean titlebar = true
string dataobject = "dw_calculo_liberador_resc"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
...
event clicked;close(w_calculo_liberadores_resciliados)
end event
...
double	gastos_adm,factor_gc,g_cobranza,valor_presente,&
			...
			comision,comprueba_saldo,existe,ldb_saldo_pie,CAPITAL_INICIAL,INTERES_INICIAL,GC_INICIAL
...
string   serie,moneda,base,fecha_of,estado_comi,age,estado_age,&
         estado_sup,estado_jefe,sup,jefe,estado_cadena
integer  ret,parque
...
//--------------------------------------------------------------------------------------------------------
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
			...
			SELECT "ANEXO_LIBERADOR"."COD_AGENTE","ANEXO_LIBERADOR"."COD_SUPER","ANEXO_LIBERADOR"."COD_JEF"    
			INTO :age,:sup,:jefe  
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
			...
			INSERT INTO "FACTURA_OFERTA"  
					 ( "BASE","SERIE","NUMERO"   ,"FEC_OFE"    ,"MORA"      ,"SALDO_TOTAL" ,"SALDO_CP" ,"SALDO_LP"   ,"CAPITAL" ,"CAPITAL_CP" ,"CAPITAL_LP" ,"INTERES" ,"INTERES_CP" ,"INTERES_LP" ,"GC"       ,"GC_CP" ,"GC_LP" ,"FECHA_PRIM" ,"CTAS_PAG" ,"PLAZO"    ,"MONTO_MORA" ,"PIE"     ,"VALOR_CTA" ,"FACTURADO" ,"FECHA_UF","FECHA_CIERRE","MONEDA","VALOR_UF","ESTADO","FECHA_PROCESO"     ,"MORA_PIE","PIE_PAGADO","COMISIONES","TOTAL_PAGADO","COMPRUEBA_SALDO","ESTADO_COMI","ESTADO_AGE","ESTADO_SUP","ESTADO_JEFE","CAPITAL_INICIAL","INTERES_INICIAL","GC_INICIAL" ,"COD_PARQUE","ESTADO_CADENA")   
			VALUES ( 'L'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'R'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,:estado_age ,:estado_sup ,:estado_jefe ,:CAPITAL_INICIAL ,:INTERES_INICIAL ,:GC_INICIAL  ,:parque     ,:estado_cadena ) ;
...
long textcolor = 255
long backcolor = 67108864
string text = "Saldos de Liberadores Resciliados."
end type
