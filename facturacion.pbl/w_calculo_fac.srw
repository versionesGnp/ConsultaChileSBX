forward
global type w_calculo_fac from window
end type
type cb_4 from commandbutton within w_calculo_fac
end type
type cb_primera_fact from commandbutton within w_calculo_fac
end type
type hpb_1 from hprogressbar within w_calculo_fac
end type
type dw_2 from datawindow within w_calculo_fac
end type
type st_3 from statictext within w_calculo_fac
end type
type st_2 from statictext within w_calculo_fac
end type
type dw_1 from datawindow within w_calculo_fac
end type
type cb_3 from commandbutton within w_calculo_fac
end type
type st_1 from statictext within w_calculo_fac
end type
type cb_2 from commandbutton within w_calculo_fac
end type
type cb_1 from commandbutton within w_calculo_fac
end type
type p_1 from picture within w_calculo_fac
end type
type em_fecha_cierre from editmask within w_calculo_fac
end type
type gb_1 from groupbox within w_calculo_fac
end type
end forward
...
global w_calculo_fac w_calculo_fac
...
if moneda = '2'  then									
//	if gc_inicial = 0 or interes_inicial = 0 then
//		gc_inicial      = 0
//		interes_inicial = 0
//	else
...
if isnull(cuota) then cuota = 0
Return 1
end function

on w_calculo_fac.create
...
double	gastos_adm,factor_gc,g_cobranza,valor_presente,&
			...
			capital_inicial,interes_inicial,gc_inicial
...
if ids_1.rowcount() > 0 then
	...
	hpb_1.position = 0
	for i = 1 to ids_1.rowcount()
		setnull(facturado)
		serie        			= ids_1.getitemstring(i,'serie')
		nro_oferta    			= ids_1.getitemnumber(i,'nro_oferta')
		estado_cadena 			= f_estado_cadena('O', serie,nro_oferta)
		wf_ingreso(date(fecha_cierre),serie,nro_oferta,monto,cuota,pie)
		pie_pag = pie
		if not isnull(cuota) then 
			pie_pac 				= ids_1.getitemnumber(i,'pie')
			moneda			 	= ids_1.getitemstring(i,'moneda')
			uf_dia			 	= ids_1.getitemnumber(i,'uf_dia')
			estado_comi		 	= ids_1.getitemstring(i,'estado_comi')
			estado_comi_sup 	= ids_1.getitemstring(i,'estado_comi_sup')
			precio			 	= ids_1.getitemnumber(i,'precio')
			if estado_comi = 'N' then
				porce_venta		= ids_1.getitemnumber(i,'porce_venta')
				porce_sup		= ids_1.getitemnumber(i,'porce_sup')
			...
			fecha_of		   	= string(ids_1.getitemdatetime(i,'fecha_of'),gs_formato_fecha)
			fecha_prim     	= ids_1.getitemdatetime(i,'fecha_prim')
			fecha_ult      	= ids_1.getitemdatetime(i,'fecha_ult')
			ctas_pag_s     	= cuota
			nro_cuotas     	= ids_1.getitemnumber(i,'nro_cuotas')
			...
			gastos_adm     = ids_1.getitemnumber(i,'gastos_adm')
			factor_gc      = gastos_adm / nro_cuotas
			if gastos_adm  = 0 then  factor_gc  = 0
			g_cobranza     = round(periodo * factor_gc,0)
			tasa				= ids_1.getitemnumber(i,'tasa')
//-----------------------------------------08-05-2006-----------------------------------------------------
			if tasa > 0 then
				valor_cuota		= ids_1.getitemnumber(i,'valor_cuota')
				pie				= ids_1.getitemnumber(i,'pie')
				valor_presente = round(f_vp( periodo, tasa, valor_cuota ),0)
				capital        = round(valor_presente - g_cobranza,0)
				interes			= round(( valor_cuota * periodo ) - valor_presente,0)
				saldo_total		= round(( valor_presente + interes ),0)
				if moneda = '2' then
...
//--------------------------------------------------------------------------------------------------------
			sup				= ids_1.getitemstring(i,'cod_sup')
			jefe				= ids_1.getitemstring(i,'cod_jef')
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
         ...
         WHERE ( "OFERTA_V"."SERIE" = :serie ) AND ( "OFERTA_V"."NRO_OFERTA" = :nro_oferta ) ; 
			...
			if saldo_total <= 0 then
				f_fecha_fin_pagos(fecha_cierre,serie,nro_oferta,saldo_total)
			end if
			parque = f_cadena_parque('O',serie,nro_oferta)
			...
			VALUES ( 'O'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'S'     ,:gdt_tiempo_servidor,:pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,:estado_age ,:estado_sup ,:estado_jefe ,:porce_venta ,:porce_sup ,:estado_comi_sup  ,:capital_inicial ,:interes_inicial ,:gc_inicial ,:parque     ,:estado_cadena );
...
WHERE 	( "FACTURA_OFERTA"."BASE" = "CADENA"."CODIGO" ) and  
			...
			( "CADENA"."FECHA_TERMINO_PAGO" is null) AND
/*NUEVO*/( "FACTURA_OFERTA"."PORCENTAJE_PAGO" >= 99.98)
...
event clicked;close(w_calculo_fac)
end event
...
event clicked;long     	nro_oferta,cont,malos,ctas_pag_s,nro_cuotas,&
			cuotas_mora,periodo,p1,p2,count,a,facturado	
long    	ll_indi, ll_ctas_pie_parc
double	gastos_adm,factor_gc,g_cobranza,valor_presente,&
			tasa,valor_cuota,saldo_insoluto,uf_dia,capital,&
			interes,uf_cierre,saldo_total,saldo_12,saldo_12_mas,&
			kcp,klp,icp,ilp,gccp,gclp,monto_mora,ldb_pie,monto,&
			cuota,pie_pac,pie_pag,precio,porce_venta,porce_sup,&
			comision,comprueba_saldo,existe,&
			capital_inicial,interes_inicial,gc_inicial,ldb_saldo_pie
datetime fecha_prim,fecha_ult
datetime fecha_cierre,fecha_vto,fecha_oferta
date		fecha
string   	serie,moneda,base,fecha_of,estado_comi,age,sup,jefe,&
         	estado_age,estado_sup,estado_jefe,estado_comi_sup,estado_cadena,&
			estatus_comision,estado_comi_jef
integer  	ret,parque
...
if ids_1.rowcount() > 0 then
	...
	for ll_indi = 1 to ids_1.rowcount()
		...
		nro_oferta    				= ids_1.getitemnumber(ll_indi,'nro_oferta')
		estado_cadena 		     = f_estado_cadena('O', serie,nro_oferta)
		wf_ingreso(date(fecha_cierre),serie,nro_oferta,monto,cuota,ldb_pie)
		pie_pag = ldb_pie
		...
		if not isnull(cuota) then 
			...
			estado_comi_sup 		= ids_1.getitemstring(ll_indi,'estado_comi_sup')
			estatus_comision 		= ids_1.getitemstring(ll_indi,'oferta_v_estatus_comision')
			estado_comi_jef 		= ids_1.getitemstring(ll_indi,'oferta_v_estado_comi_jv')
			...
			fecha_prim     			= ids_1.getitemdatetime(ll_indi,'fecha_prim')
			fecha_ult      			= ids_1.getitemdatetime(ll_indi,'fecha_ult')
			ctas_pag_s     			= cuota
			nro_cuotas     			= ids_1.getitemnumber(ll_indi,'nro_cuotas')
			cuotas_mora    		= funcion_mora_fac( fecha_prim,ctas_pag_s,nro_cuotas,fecha )
			if cuotas_mora < 0 then cuotas_mora = 0
			if estado_cadena = 'S' then cuotas_mora = 0
			periodo        			= nro_cuotas - ctas_pag_s //+ cuotas_mora )// <--------OJO-----<<
...
//-----------------------------------------08-05-2006-----------------------------------------------------
			if tasa > 0 then
				...
				ldb_pie				= ids_1.getitemnumber(ll_indi,'pie')
...
//--------------------------------------------------------------------------------------------------------
			sup						= ids_1.getitemstring(ll_indi,'cod_sup')
			jefe						= ids_1.getitemstring(ll_indi,'cod_jef')
			if nro_cuotas <=  ctas_pag_s then
				fecha_vto  	 		= datetime(fecha_ult)
			else
				fecha_vto   			= datetime(funcion_venc( fecha_prim , ctas_pag_s ))
			...
			if valor_presente <= 0 then
					saldo_total = 0;capital = 0;interes = 0;g_cobranza = 0 ; ldb_saldo_pie = 0
			end if	
			if periodo > 12 then 
	//-------------------> corto plazo hasta 12 meses <-------------------
				saldo_12 			= long((saldo_total/periodo)*12)
	...
	//------------------- > largo plazo mas de 12 meses <-------------------
			...
			facturado = long(( valor_cuota * nro_cuotas ) + ldb_pie)
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
			if  tasa = 0 then
				gc_inicial         			= 0
				interes_inicial    		= 0
...
///////////////////////////////////////////////26-12-2006///////////////////////////////////////////////////////////////////////
			fecha_oferta 			= datetime(date(fecha_of),time('00:00:00'))
			ldb_pie 					= long(pie_pac - pie_pag)
			if ldb_pie <= 10000 then ldb_pie = 0
			saldo_total     			= long(saldo_total)
			capital         			= long(capital)
			interes         			= long(interes)
			g_cobranza      		= long(g_cobranza)
			...
			monto			    		= long(monto)
			comprueba_saldo 		= long(( saldo_total - ( facturado - monto )) + ( ldb_pie + monto_mora ))
			select distinct sysdate into :gdt_tiempo_servidor from agentes;
			estado_age  			= f_estado_age_sup_jef(age,1)
			estado_sup  			= f_estado_age_sup_jef(sup,2)
			estado_jefe 				= f_estado_age_sup_jef(jefe,3)
			...
			parque 					= f_cadena_parque('O',serie,nro_oferta)
			INSERT INTO "FACTURA_OFERTA"  
					 ( "BASE","SERIE","NUMERO"   ,"FEC_OFE"    ,"MORA"      ,"SALDO_TOTAL" ,"SALDO_CP" ,"SALDO_LP"   ,"CAPITAL" ,"CAPITAL_CP" ,"CAPITAL_LP" ,"INTERES" ,"INTERES_CP" ,"INTERES_LP" ,"GC"       ,"GC_CP" ,"GC_LP" ,"FECHA_PRIM" ,"CTAS_PAG" ,"PLAZO"    ,"MONTO_MORA" ,"PIE"     ,"VALOR_CTA" ,"FACTURADO" ,"FECHA_UF","FECHA_CIERRE","MONEDA","VALOR_UF","ESTADO","FECHA_PROCESO"     ,"MORA_PIE","PIE_PAGADO","COMISIONES","TOTAL_PAGADO","COMPRUEBA_SALDO","ESTADO_COMI","ESTADO_AGE","ESTADO_SUP","ESTADO_JEFE","PORCE_VENTA","PORCE_SUP","ESTADO_COMI_SUP" ,"CAPITAL_INICIAL","INTERES_INICIAL","GC_INICIAL","COD_PARQUE","ESTADO_CADENA","ESTATUS_COMISION","ESTADO_COMI_JEF") 
			VALUES ( 'O'   ,:serie ,:nro_oferta,:fecha_oferta,:cuotas_mora,:saldo_total  ,:saldo_12  ,:saldo_12_mas,:capital  ,:kcp         ,:klp         ,:interes  ,:icp         ,:ilp         ,:g_cobranza,:gccp   ,:gclp   ,:fecha_prim  ,:ctas_pag_s,:nro_cuotas,:monto_mora  ,:pie_pac  ,:valor_cuota,:facturado  ,NULL      ,:fecha_cierre ,:moneda ,:uf_cierre,'S'     ,:gdt_tiempo_servidor,:ldb_pie     ,:pie_pag     ,:comision   ,:monto        ,:comprueba_saldo ,:estado_comi ,:estado_age ,:estado_sup ,:estado_jefe ,:porce_venta ,:porce_sup ,:estado_comi_sup  ,:capital_inicial ,:interes_inicial ,:gc_inicial ,:parque     ,:estado_cadena,	:estatus_comision, :estado_comi_jef);
			if sqlca.sqlcode = 0 then
				commit;
				cont ++
			else
...
long textcolor = 255
long backcolor = 67108864
string text = "Saldos de Promesas"
end type
