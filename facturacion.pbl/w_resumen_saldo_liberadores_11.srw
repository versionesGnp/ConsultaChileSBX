forward
global type w_resumen_saldo_liberadores_11 from window
end type
type dw_1 from datawindow within w_resumen_saldo_liberadores_11
end type
type cb_2 from commandbutton within w_resumen_saldo_liberadores_11
end type
type cb_1 from commandbutton within w_resumen_saldo_liberadores_11
end type
end forward
...
global w_resumen_saldo_liberadores_11 w_resumen_saldo_liberadores_11

on w_resumen_saldo_liberadores_11.create
...
on w_resumen_saldo_liberadores_11.destroy
destroy(this.dw_1)
destroy(this.cb_2)
destroy(this.cb_1)
end on
...
if isvalid(w_imprime_saldo_liberadores) then
	if w_imprime_saldo_liberadores.dw_1.rowcount() > 0 then
   	...
   	WHERE ( ESTADO = 'S' ) and ( fecha_cierre =:fecha1 ) and ( BASE = 'L' ) ;
		if sqlca.sqlcode <> 0 then 
			...
			dw_1.object.titulo.text = "Saldos Liberadores"
			dw_1.object.fecha_cierre.text = "Fecha de Cierre : '"+fecha+"'"
			dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(w_imprime_saldo_liberadores.dw_1.getitemnumber(1,'numero_s'))
			dw_1.object.titulo1.text = gs_empresa
		END IF
	end if
//----------------------------------------------------------------------------------------
elseif isvalid(w_imprime_saldo_liberadores_resciliados) then
		fecha1 = date(Message.StringParm)
		dw_1.insertrow(0)
		SELECT count(saldo_total),sum(saldo_total),sum(saldo_cp),sum(saldo_lp),sum(capital),sum(capital_cp),sum(capital_lp),sum(interes),sum(interes_cp),sum(interes_lp),sum(gc) ,sum(gc_cp),sum(gc_lp)  
    	INTO   :numero           ,:saldo          ,:saldocp     ,:saldolp     ,:capital    ,:capitalcp     ,:capitallp     ,:i          ,:icp           ,:ilp           ,:gc     ,:gccp     ,:gclp  
    	FROM FACTURA_OFERTA  
   	WHERE ( ESTADO = 'R' ) and ( fecha_cierre =:fecha1 ) and ( BASE = 'L' ) ;
		if sqlca.sqlcode <> 0 then 
			messagebox(gs_app_name,'No Existen datos.',stopsign!)
			close(w_resumen_saldo)
		else	
			...
			dw_1.object.titulo1.text = gs_empresa
			dw_1.object.fecha_cierre.text = "Fecha de Cierre : '"+fecha+"'"
			dw_1.object.titulo.text = "Liberadores Resciliados"
			dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(numero)
		end if	
//----------------------------------------------------------------------------------------	
elseif isvalid(w_imprime_saldo_nueva_liberadores) then
	if w_imprime_saldo_nueva_liberadores.dw_1.rowcount() > 0 then
		fecha1 = date(Message.StringParm)
		dw_1.insertrow(0)
		SELECT count(saldo_total),sum(saldo_total),sum(saldo_cp),sum(saldo_lp),sum(capital),sum(capital_cp),sum(capital_lp),sum(interes),sum(interes_cp),sum(interes_lp),sum(gc) ,sum(gc_cp),sum(gc_lp)  
    	INTO   :numero           ,:saldo          ,:saldocp     ,:saldolp     ,:capital    ,:capitalcp     ,:capitallp     ,:i          ,:icp           ,:ilp           ,:gc     ,:gccp     ,:gclp  
   	...
   	WHERE ( ESTADO = 'N' ) and ( fecha_cierre =:fecha1 ) and ( BASE = 'L' ) ;
		if sqlca.sqlcode <> 0 then 
			...
			dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(w_imprime_saldo_nueva_liberadores.dw_1.getitemnumber(1,'numero_s'))
			dw_1.object.titulo1.text = gs_empresa
		END IF			
	end if
elseif isvalid(w_saldo_nuevo_liberadores) then
   	...
   	WHERE (( ESTADO = 'N' ) OR ( ESTADO = 'S' )) and ( fecha_cierre =:fecha1 ) and ( BASE = 'L' ) ;
		if sqlca.sqlcode <> 0 then 
			...
			dw_1.object.titulo1.text = gs_empresa
			dw_1.object.fecha_cierre.text = "Fecha de Cierre : '"+fecha+"'"
			dw_1.object.titulo.text = "Saldos y Liberadores Nuevos"
			dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(numero)
		end if	
elseif isvalid(w_imprime_saldo_liberadores_dv) then 
	fecha1 = date(Message.StringParm)
	dw_1.insertrow(0)
	...
	WHERE FACTURA_OFERTA_DV.BASE   = FACTURA_OFERTA_DV_M.BASE AND 
         ...
         FACTURA_OFERTA_DV.NUMERO = FACTURA_OFERTA_DV_M.NUMERO AND 
		   ( FACTURA_OFERTA_DV.fecha_cierre =:fecha1 ) and ( FACTURA_OFERTA_DV.BASE = 'L' ) ;
	if sqlca.sqlcode <> 0 then 
		messagebox(gs_app_name,'No Existen datos.',stopsign!)
		close(w_resumen_saldo)
	else	
	...
	WHERE FACTURA_OFERTA_DV.BASE   = FACTURA_OFERTA_DV_M.BASE AND 
         ...
         FACTURA_OFERTA_DV.NUMERO = FACTURA_OFERTA_DV_M.NUMERO AND 
		   ( FACTURA_OFERTA_DV.fecha_cierre =:fecha1 ) and ( FACTURA_OFERTA_DV.BASE = 'L' ) ;
			...
			fecha = Message.StringParm
			dw_1.object.fecha_cierre.text = "Fecha de Cierre : '"+fecha+"'"
			dw_1.object.titulo.text = "Liberadores Devengados"
			dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(numero)
			dw_1.object.titulo1.text = 'Facturación '+	gs_empresa			
	end if		
	
elseif isvalid(w_imprime_saldo_liberadores_p) then
	fecha1 = date(Message.StringParm)
	dw_1.insertrow(0)
	SELECT count(saldo_total),sum(saldo_total),sum(saldo_cp),sum(saldo_lp),sum(capital),sum(capital_cp),sum(capital_lp),sum(interes),sum(interes_cp),sum(interes_lp),sum(gc) ,sum(gc_cp),sum(gc_lp)  
	INTO   :numero           ,:saldo          ,:saldocp     ,:saldolp     ,:capital    ,:capitalcp     ,:capitallp     ,:i          ,:icp           ,:ilp           ,:gc     ,:gccp     ,:gclp  
	FROM FACTURA_OFERTA  
	WHERE ( ESTADO = 'V' ) and ( fecha_cierre =:fecha1 ) and ( BASE = 'L' ) ;
	if sqlca.sqlcode <> 0 then 
		messagebox(gs_app_name,'No Existen datos.',stopsign!)
		close(w_resumen_saldo)
	else	
		...
		dw_1.object.titulo1.text = gs_empresa
		dw_1.object.fecha_cierre.text = "Fecha de Cierre : '"+fecha+"'"
		dw_1.object.titulo.text = "Liberadores por Pagar"
		dw_1.object.numero.text = "Cantidad de Liberadores calculados : "+string(numero)
	end if	
end if
end event
...
event clicked;close(w_resumen_saldo_liberadores_11)
end event
