forward
global type w_actualiza_pagare from window
end type
type dw_1 from datawindow within w_actualiza_pagare
end type
type p_1 from picture within w_actualiza_pagare
end type
type em_fecha from editmask within w_actualiza_pagare
end type
type st_3 from statictext within w_actualiza_pagare
end type
type cb_actualiza from commandbutton within w_actualiza_pagare
end type
type st_2 from statictext within w_actualiza_pagare
end type
type st_1 from statictext within w_actualiza_pagare
end type
type sle_clave from singlelineedit within w_actualiza_pagare
end type
type sle_usuario from singlelineedit within w_actualiza_pagare
end type
type cb_aceptar from commandbutton within w_actualiza_pagare
end type
type cb_cancelar from commandbutton within w_actualiza_pagare
end type
type ln_1 from line within w_actualiza_pagare
end type
type ln_2 from line within w_actualiza_pagare
end type
type gb_1 from groupbox within w_actualiza_pagare
end type
end forward
...
global w_actualiza_pagare w_actualiza_pagare
...
if dw_1.rowcount() > 0 then
	for i = 1 to dw_1.rowcount()
		...
		if estado = 'N' or estado = 'S' then
			UPDATE "PAGARE"  
		   SET "FACTURADO" = 99  
			WHERE ( "PAGARE"."SERIE_P" = :serie ) AND  
         ( "PAGARE"."NRO_PAGARE" = :contrato )   ;
...
this.title = gs_app_name+' - ( Actualiza Pagarés )'
...
integer taborder = 71
boolean titlebar = true
string dataobject = "dw_update_pagare"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
...
string facename = "Arial"
boolean enabled = false
string text = "~'Actualizar &Pagarés~'"
end type

event clicked;integer ret
string fec
ret = messagebox(gs_app_name,'          ¿ Esta seguro que desea actualizar ?~n~r~n~r~n~rLa Actualización cambiará de estado todos los Pagarés,~n~r'+&
		"    dejandolas como 'Facturados'.",Question!,YesNoCancel! ,3)
if ret = 1 then
	setpointer(HourGlass!)
	fec = trim(em_fecha.text)
	if wf_actualizar(fec) = 1 then messagebox(gs_app_name,'Actualización terminada',information!)
	setpointer(Arrow!)
	close(w_actualiza_pagare)
elseif ret = 2 then
	close(w_actualiza_pagare)
end if
end event
...
IF is_clave = Upper(sle_clave.text) and sle_clave.text<>'' THEN
	...
	END IF
	w_actualiza_pagare.height=1009
	w_actualiza_pagare.x = 988
	w_actualiza_pagare.y = 689
	this.enabled = false
...
event clicked;close(w_actualiza_pagare)
end event
...
long textcolor = 255
long backcolor = 79741120
string text = "Actualización de Pagarés"
end type
