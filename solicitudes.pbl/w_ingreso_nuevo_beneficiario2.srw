forward
global type w_ingreso_nuevo_beneficiario2 from window
end type
type st_contrato from statictext within w_ingreso_nuevo_beneficiario2
end type
type st_rechazado from statictext within w_ingreso_nuevo_beneficiario2
end type
type st_3 from statictext within w_ingreso_nuevo_beneficiario2
end type
type dw_detalle_original from datawindow within w_ingreso_nuevo_beneficiario2
end type
type dw_cliente_original from datawindow within w_ingreso_nuevo_beneficiario2
end type
type st_cliente from statictext within w_ingreso_nuevo_beneficiario2
end type
type dw_print from datawindow within w_ingreso_nuevo_beneficiario2
end type
type cb_ordenar from commandbutton within w_ingreso_nuevo_beneficiario2
end type
type cb_exportar from commandbutton within w_ingreso_nuevo_beneficiario2
end type
type cb_imprimir from commandbutton within w_ingreso_nuevo_beneficiario2
end type
type cb_grabar from commandbutton within w_ingreso_nuevo_beneficiario2
end type
type st_2 from statictext within w_ingreso_nuevo_beneficiario2
end type
type st_1 from statictext within w_ingreso_nuevo_beneficiario2
end type
type cb_cerrar from commandbutton within w_ingreso_nuevo_beneficiario2
end type
type dw_detalle from datawindow within w_ingreso_nuevo_beneficiario2
end type
type dw_cliente from datawindow within w_ingreso_nuevo_beneficiario2
end type
end forward
...
string title = "Antecedentes Herederos"
...
global w_ingreso_nuevo_beneficiario2 w_ingreso_nuevo_beneficiario2
...
on w_ingreso_nuevo_beneficiario2.create
...
on w_ingreso_nuevo_beneficiario2.destroy
...
event open;long		ll_new,ll_rut_ben,ll_renta,ll_grupo_f
string	ls_estado,ls_nom_ben,ls_ap_pat_ben,ls_ap_mat_ben,ls_dv_ben
String	ls_nom,ls_ap_pat,ls_ap_mat,ls_dir_p,ls_pob,ls_sector,ls_comuna,ls_ciudad,ls_fono_p,&
			ls_sexo,ls_est_civil,ls_dom_c,ls_fono_c,ls_dv,ls_nro_p,ls_depto_p,ls_block_p,ls_tipo_via
...
if is_tipo_sol='19' then
	...
	dw_print.dataobject	= 'dw_print_posesion_efectiva'
elseif is_tipo_sol='10' then
	st_3.text				= 'Detalle de Beneficiarios'
	st_2.text				= 'detalle de Beneficiarios'
	dw_print.dataobject	= 'dw_print_transferencia'
end if
st_contrato.text= 'CONTRATO Nº:  '+is_base+' - '+is_serie+' - '+string(il_numero,"###,###,###,###,##0")
gf_centrar(w_ingreso_nuevo_beneficiario2)
...
if dw_cliente.retrieve(il_rut,is_base,is_serie,il_numero,il_correlativo)=0 then
	...
	SELECT	"CLIENTE"."NOMBRE",	"CLIENTE"."A_PATERNO",	"CLIENTE"."A_MATERNO",	"CLIENTE"."DIRECCION_P",	"CLIENTE"."POBLACION",	"CLIENTE"."SECTOR",	"CLIENTE"."COMUNA",	"CLIENTE"."CIUDAD",	"CLIENTE"."FONO_P",	"CLIENTE"."SEXO",	"CLIENTE"."FECHA_NAC",	"CLIENTE"."ESTA_CIVIL",	"CLIENTE"."DOMICILIO_C",	"CLIENTE"."FONO_C",	"CLIENTE"."TOTAL_RENT",	"CLIENTE"."GRUPO_F",	"CLIENTE"."DV",	"CLIENTE"."NUMERO_PARTICULAR",	"CLIENTE"."DEPTO_PARTICULAR",	"CLIENTE"."BLOCK_PARTICULAR",	"CLIENTE"."TIPO_VIA"
	INTO		:ls_nom,					:ls_ap_pat,					:ls_ap_mat,					:ls_dir_p,						:ls_pob,						:ls_sector,				:ls_comuna,				:ls_ciudad,				:ls_fono_p,				:ls_sexo,			:ldt_fec_nac,				:ls_est_civil,				:ls_dom_c,						:ls_fono_c,				:ll_renta,					:ll_grupo_f,			:ls_dv,				:ls_nro_p,								:ls_depto_p,						:ls_block_p,						:ls_tipo_via
	FROM		"CLIENTE"  
	...
	if sqlca.sqlcode=0 then
		...
		dw_cliente.setitem(ll_new,'rut',il_rut)
		dw_cliente.setitem(ll_new,'dv',ls_dv)
		dw_cliente.setitem(ll_new,'nombres',ls_nom)
		dw_cliente.setitem(ll_new,'ap_paterno',ls_ap_pat)
		dw_cliente.setitem(ll_new,'ap_materno',ls_ap_mat)
		dw_cliente.setitem(ll_new,'tipo_via',ls_tipo_via)
		dw_cliente.setitem(ll_new,'direccion_p',ls_dir_p)
		dw_cliente.setitem(ll_new,'numero_particular',ls_nro_p)
		dw_cliente.setitem(ll_new,'depto_particular',ls_depto_p)
		dw_cliente.setitem(ll_new,'block_particular',ls_block_p)
		dw_cliente.setitem(ll_new,'poblacion',ls_pob)
		dw_cliente.setitem(ll_new,'sector',ls_sector)
		dw_cliente.setitem(ll_new,'comuna',ls_comuna)
		dw_cliente.setitem(ll_new,'ciudad',ls_ciudad)
		dw_cliente.setitem(ll_new,'fono_p',ls_fono_p)
		dw_cliente.setitem(ll_new,'sexo',ls_sexo)
		dw_cliente.setitem(ll_new,'fecha_nac',ldt_fec_nac)
		dw_cliente.setitem(ll_new,'estado_civil',ls_est_civil)
		dw_cliente.setitem(ll_new,'domicilio_c',ls_dom_c)
		dw_cliente.setitem(ll_new,'fono_c',ls_fono_c)
		dw_cliente.setitem(ll_new,'total_renta',ll_renta)
		dw_cliente.setitem(ll_new,'grupo_f',ll_grupo_f)
	...
	dw_detalle_original.retrieve(il_rut,is_base,is_serie,il_numero,il_correlativo)
end if
...
integer taborder = 10
string title = "none"
string dataobject = "dw_ingreso_detalle_copropietario_origen2"
...
integer width = 425
integer height = 464
string dataobject = "dw_print_posesion_efectiva"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
		...
		if dw_cliente.update()=1 then
			...
			if dw_detalle.update()=1 then
				...
				if dw_detalle_original.update()=1 then
					...
					close(w_ingreso_nuevo_beneficiario2)
				else
					rollback;
				end if
			else
...
if cb_grabar.enabled = true then
	...
	if ll_sw=0 then close(w_ingreso_nuevo_beneficiario2)
else
	close(w_ingreso_nuevo_beneficiario2)
end if	
end event
...
if ls_columna='rut' or ls_columna='dv' then
	ll_rut			= this.getitemnumber(1,'rut') //long(data)
	ls_val_dv		= String(gf_obtener_dv(ll_rut))
	ls_dv			= this.getitemstring(1,'dv')
	if (not isnull(ll_rut) or ll_rut>0 ) and (ls_val_dv <> ls_dv) then
		messagebox("Advertencia","Rut Inválido")
//		dw_cliente.setitem(1,'dv',ls_nulo)
	end if
end if
this.accepttext()
end event
...
event itemfocuschanged;Long		ll_rut
String		ls_val_dv,ls_dv,ls_columna,ls_nulo
...
ls_dv			= this.getitemstring(1,'dv')
if (not isnull(ll_rut) or ll_rut>0 ) and (ls_val_dv <> ls_dv) then
	dw_cliente.setitem(1,'nombres',ls_nulo)
	dw_cliente.setitem(1,'dv',ls_nulo)
	dw_cliente.setitem(1,'ap_paterno',ls_nulo)
	dw_cliente.setitem(1,'ap_materno',ls_nulo)
	dw_cliente.setfocus()
...
CHOOSE CASE ls_columna
	CASE 'rut','dv'
		ll_rut		= this.getitemnumber(1,'rut')
		ls_val_dv	= this.getitemstring(1,'dv')
		if (not isnull(ll_rut) or ll_rut>0 ) and (not isnull(ls_val_dv) or ls_val_dv<>'') then
			...
			elseif ls_val_dv <> gf_obtener_dv(ll_rut) then
				messagebox("Advertencia","Rut Inválido")
				setnull(ls_dv);setnull(ls_nom);setnull(ls_ap_pat);setnull(ls_ap_mat)
				dw_cliente.setitem(1,'dv',ls_dv);dw_cliente.setitem(1,'nombres',ls_nom)
				dw_cliente.setitem(1,'ap_paterno',ls_ap_pat);dw_cliente.setitem(1,'ap_materno',ls_ap_mat)
...
event itemfocuschanged;Long		ll_rut
String		ls_val_dv,ls_dv,ls_nom,ls_ap_pat,ls_ap_mat

this.accepttext()
ll_rut		= this.getitemnumber(1,'rut')
ls_val_dv	= this.getitemstring(1,'dv')
if ls_val_dv <> gf_obtener_dv(ll_rut) then
	setnull(ls_dv);setnull(ls_nom);setnull(ls_ap_pat);setnull(ls_ap_mat)
	dw_cliente.setitem(1,'dv',ls_dv);dw_cliente.setitem(1,'nombres',ls_nom)
	dw_cliente.setitem(1,'ap_paterno',ls_ap_pat);dw_cliente.setitem(1,'ap_materno',ls_ap_mat)
	dw_cliente.setfocus()
