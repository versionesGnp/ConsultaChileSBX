forward
global type w_repacta_mora_mantencion_reprog from window
end type
type st_cliente_tercero from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_saldo_ins from statictext within w_repacta_mora_mantencion_reprog
end type
type em_saldo_insoluto from statictext within w_repacta_mora_mantencion_reprog
end type
type cb_terceros from commandbutton within w_repacta_mora_mantencion_reprog
end type
type st_print_detalle from statictext within w_repacta_mora_mantencion_reprog
end type
type dw_print_ctto from datawindow within w_repacta_mora_mantencion_reprog
end type
type st_9 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_factor from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_valor_cta2 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_cuota_uf2 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_cuota_peso2 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_cob_adm from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_valor_cta from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_cuota_uf from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_cuota_peso from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_interes_adm_uf from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_interes_adm_pesos from statictext within w_repacta_mora_mantencion_reprog
end type
type rb_calc_uf from radiobutton within w_repacta_mora_mantencion_reprog
end type
type rb_calc_pesos from radiobutton within w_repacta_mora_mantencion_reprog
end type
type st_6 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_dscto from statictext within w_repacta_mora_mantencion_reprog
end type
type em_descuento from editmask within w_repacta_mora_mantencion_reprog
end type
type st_val_interes_tot_total from statictext within w_repacta_mora_mantencion_reprog
end type
type st_8 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_7 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_5 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_val_interes_total from statictext within w_repacta_mora_mantencion_reprog
end type
type em_mora_cta from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_total from statictext within w_repacta_mora_mantencion_reprog
end type
type st_moneda from statictext within w_repacta_mora_mantencion_reprog
end type
type em_moneda from statictext within w_repacta_mora_mantencion_reprog
end type
type st_uf_dia from statictext within w_repacta_mora_mantencion_reprog
end type
type st_monto_uf from statictext within w_repacta_mora_mantencion_reprog
end type
type st_deuda_uf from statictext within w_repacta_mora_mantencion_reprog
end type
type st_monto_en_uf from statictext within w_repacta_mora_mantencion_reprog
end type
type st_ctas_mant_pago from statictext within w_repacta_mora_mantencion_reprog
end type
type st_cta_txt from statictext within w_repacta_mora_mantencion_reprog
end type
type st_fecha_prox_pago from statictext within w_repacta_mora_mantencion_reprog
end type
type st_4 from statictext within w_repacta_mora_mantencion_reprog
end type
type cb_filtrar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type cb_ordenar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type cb_exportar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type p_fec from picture within w_repacta_mora_mantencion_reprog
end type
type em_fecha from editmask within w_repacta_mora_mantencion_reprog
end type
type st_fec_cuo from statictext within w_repacta_mora_mantencion_reprog
end type
type st_txt_real from statictext within w_repacta_mora_mantencion_reprog
end type
type cb_grabar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type st_cliente from statictext within w_repacta_mora_mantencion_reprog
end type
type em_cuotas from editmask within w_repacta_mora_mantencion_reprog
end type
type st_3 from statictext within w_repacta_mora_mantencion_reprog
end type
type st_numero from statictext within w_repacta_mora_mantencion_reprog
end type
type st_serie from statictext within w_repacta_mora_mantencion_reprog
end type
type st_base from statictext within w_repacta_mora_mantencion_reprog
end type
type st_2 from statictext within w_repacta_mora_mantencion_reprog
end type
type cb_proceso from commandbutton within w_repacta_mora_mantencion_reprog
end type
type st_1 from statictext within w_repacta_mora_mantencion_reprog
end type
type cb_cerrar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type em_val_presente from statictext within w_repacta_mora_mantencion_reprog
end type
type em_gastos_adm from statictext within w_repacta_mora_mantencion_reprog
end type
type dw_lista from datawindow within w_repacta_mora_mantencion_reprog
end type
type cb_imprimir from commandbutton within w_repacta_mora_mantencion_reprog
end type
type gb_2 from groupbox within w_repacta_mora_mantencion_reprog
end type
type cb_eliminar from commandbutton within w_repacta_mora_mantencion_reprog
end type
type gb_1 from groupbox within w_repacta_mora_mantencion_reprog
end type
type dw_detalle from datawindow within w_repacta_mora_mantencion_reprog
end type
type gb_3 from groupbox within w_repacta_mora_mantencion_reprog
end type
end forward

global type w_repacta_mora_mantencion_reprog from window
integer width = 2597
integer height = 2176
boolean titlebar = true
string title = "Reprogramar un Contrato Base ~"R~""
boolean controlmenu = true
boolean minbox = true
long backcolor = 67108864
st_cliente_tercero st_cliente_tercero
st_txt_saldo_ins st_txt_saldo_ins
em_saldo_insoluto em_saldo_insoluto
cb_terceros cb_terceros
st_print_detalle st_print_detalle
dw_print_ctto dw_print_ctto
st_9 st_9
st_factor st_factor
st_txt_valor_cta2 st_txt_valor_cta2
st_val_cuota_uf2 st_val_cuota_uf2
st_val_cuota_peso2 st_val_cuota_peso2
st_txt_cob_adm st_txt_cob_adm
st_txt_valor_cta st_txt_valor_cta
st_val_cuota_uf st_val_cuota_uf
st_val_cuota_peso st_val_cuota_peso
st_val_interes_adm_uf st_val_interes_adm_uf
st_val_interes_adm_pesos st_val_interes_adm_pesos
rb_calc_uf rb_calc_uf
rb_calc_pesos rb_calc_pesos
st_6 st_6
st_dscto st_dscto
em_descuento em_descuento
st_val_interes_tot_total st_val_interes_tot_total
st_8 st_8
st_7 st_7
st_5 st_5
st_val_interes_total st_val_interes_total
em_mora_cta em_mora_cta
st_txt_total st_txt_total
st_moneda st_moneda
em_moneda em_moneda
st_uf_dia st_uf_dia
st_monto_uf st_monto_uf
st_deuda_uf st_deuda_uf
st_monto_en_uf st_monto_en_uf
st_ctas_mant_pago st_ctas_mant_pago
st_cta_txt st_cta_txt
st_fecha_prox_pago st_fecha_prox_pago
st_4 st_4
cb_filtrar cb_filtrar
cb_ordenar cb_ordenar
cb_exportar cb_exportar
p_fec p_fec
em_fecha em_fecha
st_fec_cuo st_fec_cuo
st_txt_real st_txt_real
cb_grabar cb_grabar
st_cliente st_cliente
em_cuotas em_cuotas
st_3 st_3
st_numero st_numero
st_serie st_serie
st_base st_base
st_2 st_2
cb_proceso cb_proceso
st_1 st_1
cb_cerrar cb_cerrar
em_val_presente em_val_presente
em_gastos_adm em_gastos_adm
dw_lista dw_lista
cb_imprimir cb_imprimir
gb_2 gb_2
cb_eliminar cb_eliminar
gb_1 gb_1
dw_detalle dw_detalle
gb_3 gb_3
end type
global w_repacta_mora_mantencion_reprog w_repacta_mora_mantencion_reprog

type variables
long			il_cta_mora,ii_dmora,ii_mora_cobro,ii_inc_gastos,ii_cantidad,il_sw=0,il_cta_en_mora, il_val_int_cta, il_val_int,il_monto_cuota, il_por_descto,&
				il_monto, il_tot_reg,il_deuda,il_nro_ctas,il_corr,il_mora_inicial,il_val_presente,il_val_gtos_adm,il_val_mora_cta,il_val_saldo_insoluto,&
				il_cta_mora_o,il_cta_ori_mora,il_valor_cuota_ctto,il_sw_insoluto=0,il_ctas_pag_m,il_mora_mant,il_dias_mora_mant,il_cuotas_mora_base,&
				il_monto_inicial,il_mora_original_sin_mod,il_modif=0
Double		id_valor_cuota,id_gc,id_im,id_monto_uf, id_val_tasa_anual_peso, id_val_tasa_anual_uf,	il_deuda_tot_uf, il_cuota_uf, il_int_adm_peso, il_int_adm_uf,&
				il_valor_cuota_m,id_interes_uf,	id_tasa,il_uf_inicial,il_numero,ll_num_ori,il_numero_aux
String			is_base, is_serie, is_opcion,is_string
Date			id_fecha_prox_pago, id_fecha_tope_pago,idt_fecha_hoy
Datetime		idt_fecha_venc_mant  
end variables

forward prototypes
public subroutine wf_grabar ()
public subroutine wf_eliminar_pie ()
end prototypes

public subroutine wf_grabar ();long		ll_val_ctas,ll_suma_cta, ll_cta_mora
Datetime	ldt_fecha_1, ldt_fecha_crea
date		ld_fecha_1, ld_fecha_crea
time		lt_hora
String	ls_base_aux,ls_barra,ls_barra_aux, ls_num
double	ld_val_cta_uf, ld_val_deuda_uf,ld_factor
cb_proceso.triggerevent(clicked!)
lt_hora				= Now()
ld_fecha_1			= date(trim(em_fecha.text))
ld_fecha_crea		= idt_fecha_hoy
ldt_fecha_1			= datetime(ld_fecha_1, lt_hora)
ldt_fecha_crea		= datetime(ld_fecha_crea, lt_hora)
ll_val_ctas			= long(st_val_cuota_peso2.text)
il_deuda				= long(long(st_val_cuota_peso2.text) * il_nro_ctas)
il_val_int				= long(st_val_interes_total.text)
ld_val_deuda_uf	= il_deuda / gd_uf_dia
ld_val_cta_uf		= double(st_val_cuota_uf2.text)
ls_barra				= 'L'+'R'+string(il_numero)+string(il_corr)
ls_num				= string(il_numero)+string(il_corr)
ll_num_ori			= il_numero
il_numero			= Double(ls_num)
ll_cta_mora			= long(st_ctas_mant_pago.text)
ll_suma_cta			= gl_ctas_pag_m + ll_cta_mora
ld_factor				= double(st_factor.text)

SELECT	"REPACTA_MANTENCION_L"."BASE"  
INTO 		:ls_base_aux  
FROM 		"REPACTA_MANTENCION_L"  
WHERE 	( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :is_base ) AND  
			( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :is_serie ) AND  
			( "REPACTA_MANTENCION_L"."NUMERO_ORIGINAL" = :il_numero ) AND
			( "REPACTA_MANTENCION_L"."CORRELATIVO" = :il_corr )
Using		sqlca ;
if sqlca.sqlcode=0 then
	if dw_lista.update()=1 then
		commit;
		il_sw=0
		w_detalle_repactacion_por_contrato.dw_lista.retrieve(is_base,is_serie,il_numero)
		messagebox("Grabar","Grabación Exitosa")
	else
		rollback;
		messagebox("Error","Error al Grabar")
	end if
else
	INSERT INTO "REPACTA_MANTENCION_L"  
				( "BASE",   "SERIE",   "NUMERO",   "BASE_ORIGINAL",   "SERIE_ORIGINAL",   "NUMERO_ORIGINAL",   "FECHA_CREA",   "CORRELATIVO","TOTAL_DEUDA",   "CUOTAS_PACTADAS",   "ESTADO_PAGO",   "VALOR_CUOTA",   "FECHA_PRIMERA_CTA",   "CUOTAS_EN_MORA",   "USUARIO",   "ABONO","SALDO","RUT_CLIENTE","DV_CLIENTE","UF_DIA",		"TIPO_MONEDA","TOTAL_DEUDA_UF",	"VALOR_CUOTA_UF","INTERES_CUOTA_PESOS","INTERES_DEUDA_PESOS","POR_DESCUENTO","INTERES_ADM_PESOS",	"FACTOR_APLICADO","TOTAL_PAGADO","FECHA_ULT",	"CTA_MORA_ORIGINAL",		"CTAS_PAGADAS" )  
  	VALUES 	( 'L',   	'R', 			:il_numero, :is_base,   			:is_serie,   		  :ll_num_ori,   		  :ldt_fecha_crea,:il_corr,		:il_deuda,   	 :il_nro_ctas,   		 'V',   				:ll_val_ctas,   	:ldt_fecha_1,   		 :gl_n_cuotas_pag,   :gs_user,    0,		0,			:gi_rut,		 :gs_dv,		  :gd_uf_dia,	:gs_moneda,		:ld_val_deuda_uf,	:ld_val_cta_uf,	:il_val_int_cta,		:il_val_int,			  :il_por_descto,:il_int_adm_peso,		:ld_factor,			0,					:ldt_fecha_1,	:gl_cta_mora_original,	0 )  
	Using	sqlca;
	if sqlca.sqlcode=0 then
		commit;
		il_tot_reg	= dw_lista.retrieve(is_base,is_serie,ll_num_ori,il_corr)
		if w_detalle_repactacion_por_contrato.dw_lista.retrieve(is_base,is_serie,ll_num_ori)>0 then w_detalle_repactacion_por_contrato.cb_pago.enabled=true
		il_sw=0
//		SELECT "CADENA"."BARRA"  
//		INTO :ls_barra_aux  
//		FROM "CADENA"  
//		WHERE ( "CADENA"."CODIGO" = 'R' ) AND  
//				( "CADENA"."SERIE" = :is_serie ) AND  
//				( "CADENA"."NUMERO" = :il_numero )   
//		Using	sqlca;
//		if sqlca.sqlcode=100 then
//			INSERT INTO	"CADENA"  
//							( "BARRA",   "RUT",   "CODIGO",   "SERIE",   "NUMERO",   "ESTADO",   "PRINT",   "EST_CUP",   "NRO_CARTA",   "FECHA_CARTA",   "ESTADO_CARTA",   "FECHA_ACT_ESTADO",   "CUOTA_CUPON",    "FECHA_RES",   "A_DEMANDAR",   "ESTADO_CONTABLE",   "ESTADO_TITULO",   "FECHA_TITULO",   "NRO_CARTA_M",   "FECHA_CARTA_M",   "ESTADO_CARTA_M",   "FECHA_ACT_ESTADO_M",	"COBRAN_C" )  
//			VALUES 		( :ls_barra, :gi_rut, 'R',   		 :is_serie, :il_numero, 'V',   		'N',   	  0,   			null,   			null,   			  0,   					:ld_fecha_crea,   	0,   					null,   			null,   			 null,   				 null,   			  null,   			  null,   			 null,   			  0,   					 null,   					null )  
//			Using			sqlca;
//			if sqlca.sqlcode=0 then
//				if is_base='C' then
//					commit;
//					UPDATE "CONTRATO"  
//					SET 	"ANEXO_REPACTA" = :ls_barra,   
//							"CTA_REPACTA" = :ll_cta_mora,  
//							"CTA_PAG_M" = :ll_suma_cta
//					WHERE ( "CONTRATO"."SERIE_C" = :is_serie ) AND  
//							( "CONTRATO"."NRO_CONTRATO" = :ll_num_ori )   
//					Using	sqlca ;
//					if sqlca.sqlcode=0 then
//						commit;
//					else
//						rollback;
//					end if
//				elseif is_base='O' then
//					UPDATE "OFERTA_V"  
//					SET 	"ANEXO_REPACTA" = :ls_barra,   
//							"CTA_REPACTA" = :ll_cta_mora,  
//							"CTA_PAG_M" = :ll_suma_cta
//					WHERE ( "OFERTA_V"."SERIE" = :is_serie ) AND  
//							( "OFERTA_V"."NRO_OFERTA" = :ll_num_ori )   
//					Using	sqlca ;
//					if sqlca.sqlcode=0 then
//						commit;
//					else
//						rollback;
//					end if
//				end if
//				if isvalid(w_boletas_cuotas) then w_boletas_cuotas.dw_datos_bl.retrieve(is_serie,ll_num_ori)
//			else
//				rollback;
//			end if
//		end if
	else
		rollback;
		messagebox("Error","Error al Grabar")
	end if
end if
cb_imprimir.triggerevent(clicked!)
end subroutine

public subroutine wf_eliminar_pie ();Long		ll_new

dw_lista.deleterow(1)
ll_new	= dw_lista.insertrow(0)
dw_lista.setitem(ll_new,'base','L')
dw_lista.setitem(ll_new,'serie','R')
dw_lista.setitem(ll_new,'numero',il_numero)
dw_lista.setitem(ll_new,'correlativo',il_corr)
dw_lista.setitem(ll_new,'base_original',is_base)
dw_lista.setitem(ll_new,'serie_original',is_serie)
dw_lista.setitem(ll_new,'numero_original',il_numero)
dw_lista.setitem(ll_new,'ctas_pagadas',0)
dw_lista.setitem(ll_new,'uf_dia',gd_uf_dia)
dw_lista.setitem(ll_new,'interes_adm_pesos',il_val_gtos_adm)
dw_lista.setitem(ll_new,'cuotas_pactadas',long(em_cuotas.text))
dw_lista.setitem(ll_new,'cuotas_en_mora',il_nro_ctas)
dw_lista.setitem(ll_new,'estado_pago','V')
dw_lista.setitem(ll_new,'fecha_primera_cta',date(em_fecha.text))
dw_lista.setitem(ll_new,'tipo_moneda',gs_moneda)
dw_lista.accepttext()
dw_lista.setcolumn('pie')
dw_lista.setfocus()
end subroutine

on w_repacta_mora_mantencion_reprog.create
this.st_cliente_tercero=create st_cliente_tercero
this.st_txt_saldo_ins=create st_txt_saldo_ins
this.em_saldo_insoluto=create em_saldo_insoluto
this.cb_terceros=create cb_terceros
this.st_print_detalle=create st_print_detalle
this.dw_print_ctto=create dw_print_ctto
this.st_9=create st_9
this.st_factor=create st_factor
this.st_txt_valor_cta2=create st_txt_valor_cta2
this.st_val_cuota_uf2=create st_val_cuota_uf2
this.st_val_cuota_peso2=create st_val_cuota_peso2
this.st_txt_cob_adm=create st_txt_cob_adm
this.st_txt_valor_cta=create st_txt_valor_cta
this.st_val_cuota_uf=create st_val_cuota_uf
this.st_val_cuota_peso=create st_val_cuota_peso
this.st_val_interes_adm_uf=create st_val_interes_adm_uf
this.st_val_interes_adm_pesos=create st_val_interes_adm_pesos
this.rb_calc_uf=create rb_calc_uf
this.rb_calc_pesos=create rb_calc_pesos
this.st_6=create st_6
this.st_dscto=create st_dscto
this.em_descuento=create em_descuento
this.st_val_interes_tot_total=create st_val_interes_tot_total
this.st_8=create st_8
this.st_7=create st_7
this.st_5=create st_5
this.st_val_interes_total=create st_val_interes_total
this.em_mora_cta=create em_mora_cta
this.st_txt_total=create st_txt_total
this.st_moneda=create st_moneda
this.em_moneda=create em_moneda
this.st_uf_dia=create st_uf_dia
this.st_monto_uf=create st_monto_uf
this.st_deuda_uf=create st_deuda_uf
this.st_monto_en_uf=create st_monto_en_uf
this.st_ctas_mant_pago=create st_ctas_mant_pago
this.st_cta_txt=create st_cta_txt
this.st_fecha_prox_pago=create st_fecha_prox_pago
this.st_4=create st_4
this.cb_filtrar=create cb_filtrar
this.cb_ordenar=create cb_ordenar
this.cb_exportar=create cb_exportar
this.p_fec=create p_fec
this.em_fecha=create em_fecha
this.st_fec_cuo=create st_fec_cuo
this.st_txt_real=create st_txt_real
this.cb_grabar=create cb_grabar
this.st_cliente=create st_cliente
this.em_cuotas=create em_cuotas
this.st_3=create st_3
this.st_numero=create st_numero
this.st_serie=create st_serie
this.st_base=create st_base
this.st_2=create st_2
this.cb_proceso=create cb_proceso
this.st_1=create st_1
this.cb_cerrar=create cb_cerrar
this.em_val_presente=create em_val_presente
this.em_gastos_adm=create em_gastos_adm
this.dw_lista=create dw_lista
this.cb_imprimir=create cb_imprimir
this.gb_2=create gb_2
this.cb_eliminar=create cb_eliminar
this.gb_1=create gb_1
this.dw_detalle=create dw_detalle
this.gb_3=create gb_3
this.Control[]={this.st_cliente_tercero,&
this.st_txt_saldo_ins,&
this.em_saldo_insoluto,&
this.cb_terceros,&
this.st_print_detalle,&
this.dw_print_ctto,&
this.st_9,&
this.st_factor,&
this.st_txt_valor_cta2,&
this.st_val_cuota_uf2,&
this.st_val_cuota_peso2,&
this.st_txt_cob_adm,&
this.st_txt_valor_cta,&
this.st_val_cuota_uf,&
this.st_val_cuota_peso,&
this.st_val_interes_adm_uf,&
this.st_val_interes_adm_pesos,&
this.rb_calc_uf,&
this.rb_calc_pesos,&
this.st_6,&
this.st_dscto,&
this.em_descuento,&
this.st_val_interes_tot_total,&
this.st_8,&
this.st_7,&
this.st_5,&
this.st_val_interes_total,&
this.em_mora_cta,&
this.st_txt_total,&
this.st_moneda,&
this.em_moneda,&
this.st_uf_dia,&
this.st_monto_uf,&
this.st_deuda_uf,&
this.st_monto_en_uf,&
this.st_ctas_mant_pago,&
this.st_cta_txt,&
this.st_fecha_prox_pago,&
this.st_4,&
this.cb_filtrar,&
this.cb_ordenar,&
this.cb_exportar,&
this.p_fec,&
this.em_fecha,&
this.st_fec_cuo,&
this.st_txt_real,&
this.cb_grabar,&
this.st_cliente,&
this.em_cuotas,&
this.st_3,&
this.st_numero,&
this.st_serie,&
this.st_base,&
this.st_2,&
this.cb_proceso,&
this.st_1,&
this.cb_cerrar,&
this.em_val_presente,&
this.em_gastos_adm,&
this.dw_lista,&
this.cb_imprimir,&
this.gb_2,&
this.cb_eliminar,&
this.gb_1,&
this.dw_detalle,&
this.gb_3}
end on

on w_repacta_mora_mantencion_reprog.destroy
destroy(this.st_cliente_tercero)
destroy(this.st_txt_saldo_ins)
destroy(this.em_saldo_insoluto)
destroy(this.cb_terceros)
destroy(this.st_print_detalle)
destroy(this.dw_print_ctto)
destroy(this.st_9)
destroy(this.st_factor)
destroy(this.st_txt_valor_cta2)
destroy(this.st_val_cuota_uf2)
destroy(this.st_val_cuota_peso2)
destroy(this.st_txt_cob_adm)
destroy(this.st_txt_valor_cta)
destroy(this.st_val_cuota_uf)
destroy(this.st_val_cuota_peso)
destroy(this.st_val_interes_adm_uf)
destroy(this.st_val_interes_adm_pesos)
destroy(this.rb_calc_uf)
destroy(this.rb_calc_pesos)
destroy(this.st_6)
destroy(this.st_dscto)
destroy(this.em_descuento)
destroy(this.st_val_interes_tot_total)
destroy(this.st_8)
destroy(this.st_7)
destroy(this.st_5)
destroy(this.st_val_interes_total)
destroy(this.em_mora_cta)
destroy(this.st_txt_total)
destroy(this.st_moneda)
destroy(this.em_moneda)
destroy(this.st_uf_dia)
destroy(this.st_monto_uf)
destroy(this.st_deuda_uf)
destroy(this.st_monto_en_uf)
destroy(this.st_ctas_mant_pago)
destroy(this.st_cta_txt)
destroy(this.st_fecha_prox_pago)
destroy(this.st_4)
destroy(this.cb_filtrar)
destroy(this.cb_ordenar)
destroy(this.cb_exportar)
destroy(this.p_fec)
destroy(this.em_fecha)
destroy(this.st_fec_cuo)
destroy(this.st_txt_real)
destroy(this.cb_grabar)
destroy(this.st_cliente)
destroy(this.em_cuotas)
destroy(this.st_3)
destroy(this.st_numero)
destroy(this.st_serie)
destroy(this.st_base)
destroy(this.st_2)
destroy(this.cb_proceso)
destroy(this.st_1)
destroy(this.cb_cerrar)
destroy(this.em_val_presente)
destroy(this.em_gastos_adm)
destroy(this.dw_lista)
destroy(this.cb_imprimir)
destroy(this.gb_2)
destroy(this.cb_eliminar)
destroy(this.gb_1)
destroy(this.dw_detalle)
destroy(this.gb_3)
end on

event open;Long		i,ll_new,ll_monto_cuota,ll_dia,ll_cta_pag, ll_interes_deuda,ll_por_descuento,ll_dia_aux,ll_mes,ll_anno, ll_dia_tope, &
			ll_mes_tope, ll_anno_tope,ll_rut_aux,	ll_mes_fecha, ll_mes_fecha_actual,ll_tot_real_cta,ll_rut,ll_nro_ctas_total,&
			ll_cant_mant,ll_cant_cred,ll_calc_cuotas
Double	ld_factor_uf
Date		ld_fecha,ld_fec_pago,ldt_fec_prox_pago
String		ls_fecha, ls_fecha_tope, ls_titulo,ls_dv,ls_nom,ls_ap_pat,ls_ap_mat

gf_centrar(w_repacta_mora_mantencion_reprog)
idt_fecha_hoy					= date(gdt_fec_sistema)
id_interes_uf					= f_factor_gasto_cobranza()
gs_moneda						= '2'
em_fecha.text					= string(idt_fecha_hoy)
if isvalid(w_detalle_repactacion_por_contrato) then
	if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
		dw_lista.dataobject	= 'dw_detalle_repacta_mantencion' //'dwe_cuotas_pactadas'
	else
		dw_lista.dataobject	= 'dw_detalle_repacta_mantencion_l'
	end if
else
	dw_lista.dataobject		= 'dw_detalle_repacta_mantencion_l'
end if
dw_lista.settransobject(sqlca)
dw_detalle.dataobject		= 'dwe_cuotas_pactadas'
dw_detalle.settransobject(sqlca)
dw_print_ctto.settransobject(sqlca)
is_string							= Message.StringParm
is_base 							= substr(1,1,Message.StringParm)
is_serie							= substr(1,2,Message.StringParm)
il_numero						= Double(substr(1,3,Message.StringParm))
ll_num_ori						= il_numero
il_monto							= Long(substr(1,4,Message.StringParm))
st_cliente.text					= trim(substr(1,5,Message.StringParm))
id_fecha_prox_pago			= date(substr(1,6,Message.StringParm))
il_nro_ctas						= long(substr(1,7,Message.StringParm))
il_mora_inicial					= il_nro_ctas
il_corr							= long(substr(1,8,Message.StringParm))
is_opcion							= substr(1,9,Message.StringParm)
gl_ctas_pag_m					= long(substr(1,10,Message.StringParm))
il_val_presente					= long(substr(1,11,Message.StringParm))
il_val_gtos_adm				= long(substr(1,12,Message.StringParm))
il_val_mora_cta				= long(substr(1,13,Message.StringParm))
il_val_saldo_insoluto			= long(substr(1,14,Message.StringParm))
il_cta_mora_o					= long(substr(1,15,Message.StringParm))
il_cuotas_mora_base			= long(substr(1,16,Message.StringParm))
st_base.text						= is_base
st_serie.text						= is_serie
ll_mes_fecha					= month(id_fecha_prox_pago)
ll_mes_fecha_actual			= month(idt_fecha_hoy)
il_valor_cuota_ctto				= 0
il_valor_cuota_m				= 0
ll_tot_real_cta					= il_cta_mora_o - il_mora_inicial
gl_cta_mora_original			= il_cta_mora_o
il_nro_ctas						= gl_ctas_pag_m + il_cta_mora_o //il_mora_inicial + il_cta_mora_o//nuevo
//if ll_tot_real_cta >= 2 then
//	il_nro_ctas					= il_cta_mora_o - 1
//end if
if idt_fecha_hoy >= date(id_fecha_prox_pago) then // ll_mes_fecha_actual>=ll_mes_fecha then 
	if ll_tot_real_cta>0 then il_nro_ctas++
	if is_base='C' then
		SELECT	"CONTRATO"."VALOR_CUOTA_M",	"CONTRATO"."CTA_PAG_M",	"CADENA_MORA"."MORA_MANT",	"CADENA_MORA"."DIAS_MORA_MANT",	"CADENA_MORA"."FECHA_VENC_MANT"  
		INTO 		:il_valor_cuota_m,				:il_ctas_pag_m,			:il_mora_mant,					:il_dias_mora_mant,					:idt_fecha_venc_mant  
		FROM 		"CONTRATO",	
					"CADENA",   
					"CADENA_MORA"  
		WHERE  ( "CONTRATO"."SERIE_C" = "CADENA"."SERIE" ) and  
				 ( "CONTRATO"."NRO_CONTRATO" = "CADENA"."NUMERO" ) and  
				 ( "CADENA"."CODIGO" = "CADENA_MORA"."BASE" ) and  
				 ( "CADENA"."SERIE" = "CADENA_MORA"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "CADENA_MORA"."NUMERO" ) and  
				 (("CADENA"."CODIGO" = 'C' ) AND  
				 ( "CONTRATO"."SERIE_C" = :is_serie ) AND  
				 ( "CONTRATO"."NRO_CONTRATO" = :il_numero ) )   
		USING		sqlca;

	elseif is_base='O' then
		SELECT	"CADENA_MORA"."MORA_MANT",	"CADENA_MORA"."DIAS_MORA_MANT",	"CADENA_MORA"."FECHA_VENC_MANT",	"OFERTA_V"."VALOR_CUOTA_M",	"OFERTA_V"."CTA_PAG_M"  
		INTO 		:il_mora_mant,					:il_dias_mora_mant,					:idt_fecha_venc_mant,				:il_valor_cuota_m,				:il_ctas_pag_m  
		FROM 		"CADENA",	"CADENA_MORA",	"OFERTA_V",	"PAGO_OFERTA"  
		WHERE  ( "CADENA"."CODIGO" = "CADENA_MORA"."BASE" ) and  
				 ( "CADENA"."SERIE" = "CADENA_MORA"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "CADENA_MORA"."NUMERO" ) and  
				 ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
				 ( "OFERTA_V"."NRO_OFERTA" = "PAGO_OFERTA"."NRO_OFERTA" ) and  
				 ( "OFERTA_V"."ULT_FOLIO" = "PAGO_OFERTA"."FOLIO" ) and  
				 ( "CADENA"."SERIE" = "OFERTA_V"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "OFERTA_V"."NRO_OFERTA" ) and  
				 (("CADENA"."CODIGO" = 'O' ) AND  
				 ( "CADENA"."SERIE" = :is_serie ) AND  
				 ( "CADENA"."NUMERO" = :il_numero ) )   
		USING		sqlca;
	end if
	
	il_valor_cuota_ctto			= il_valor_cuota_m * gd_uf_dia
end if
il_cta_ori_mora					= il_nro_ctas //+ il_mora_mant + il_cuotas_mora_base
//il_nro_ctas						= il_cta_ori_mora//Nuevo  + il_mora_mant
st_numero.text					= string(il_numero,"###,###,###,###,###,##0")
em_val_presente.text			= string(il_val_presente,"###,###,###,###,###,##0")
if il_nro_ctas > 15 then
	em_cuotas.text				= string(15,"###,###,###,###,##0")
else
	em_cuotas.text				= string(il_nro_ctas,"###,###,###,###,##0")
end if
il_mora_original_sin_mod	= il_nro_ctas
st_ctas_mant_pago.text		= string(il_nro_ctas,"###,###,###,###,##0")
st_fecha_prox_pago.text		= substr(1,6,Message.StringParm)
//ll_dia_tope						= day(id_fecha_prox_pago)
//ll_mes_tope						= month(id_fecha_prox_pago)
//ll_anno_tope					= year(id_fecha_prox_pago)
//ll_anno_tope					= ll_anno_tope + il_nro_ctas
SELECT "ESTADO_MORA_MANTENCION"."CANTIDAD_MESES", "ESTADO_MORA_CREDITO"."CANTIDAD_MESES"  
INTO   :ll_cant_mant,														:ll_cant_cred
FROM   "CADENA", "ESTADO_MORA_CREDITO", "ESTADO_MORA_MANTENCION"  
WHERE ("CADENA"."ESTADO_MORA_CREDITO" = "ESTADO_MORA_CREDITO"."ESTADO_MORA_CREDITO" ) and  
		( "CADENA"."ESTADO_MORA_MANTENCION" = "ESTADO_MORA_MANTENCION"."ESTADO_MORA_MANTENCION" ) and  
		(("CADENA"."CODIGO" = :is_base ) AND  
		( "CADENA"."SERIE" = :is_serie ) AND  
		( "CADENA"."NUMERO" = :il_numero ) )   ;
		
ll_calc_cuotas					= il_nro_ctas * ll_cant_mant
DECLARE sp_prox_pago PROCEDURE FOR sp_calcular_vencimiento( :id_fecha_prox_pago, :ll_calc_cuotas );
EXECUTE sp_prox_pago;
if sqlca.sqlcode=0 then
	fetch sp_prox_pago into :ldt_fec_prox_pago ;
	close sp_prox_pago;
end if
ls_fecha_tope					= string(ldt_fec_prox_pago,'dd/mm/yyyy')
//ls_fecha_tope					= string(ll_dia_tope,'00')+'/'+string(ll_mes_tope,'00')+'/'+string(ll_anno_tope,'0000')
id_fecha_tope_pago			= date(ls_fecha_tope)
em_gastos_adm.text			= string(il_val_gtos_adm,"###,###,###,###,###,##0")
em_mora_cta.text				= string(il_val_mora_cta,"###,###,###,###,###,##0")
il_monto							= il_val_saldo_insoluto //gd_val_deuda + gd_val_interes
em_saldo_insoluto.text		= string(il_val_saldo_insoluto,"###,###,###,###,###,##0")
st_monto_uf.text				= string(gd_uf_dia,"###,###,###,###,###,##0.00")
id_monto_uf						= il_monto / gd_uf_dia
st_deuda_uf.text				= string(id_monto_uf,"###,###,###,###,##0.00")
st_val_interes_tot_total.text	=string(id_monto_uf,"###,###,###,##0.00")
st_val_interes_total.text		= string(il_monto,"###,###,###,###,###,##0")

if is_opcion='C' then
	cb_proceso.enabled		= false
	cb_grabar.enabled			= false
	cb_eliminar.enabled		= false
	em_cuotas.enabled		= false
	em_fecha.enabled			= false
	p_fec.enabled				= false
	em_descuento.enabled	= false
	rb_calc_pesos.enabled	= false
	rb_calc_uf.enabled		= false
//	cb_terceros.enabled		= false
elseif is_opcion='N' then
	cb_proceso.enabled		= true
	cb_grabar.enabled			= false
	cb_eliminar.enabled		= true
	em_cuotas.enabled		= true
	em_fecha.enabled			= true
	p_fec.enabled				= true
	em_descuento.enabled	= true
	rb_calc_pesos.enabled	= true
	rb_calc_uf.enabled		= true
	cb_terceros.enabled		= true
end if
if is_base<>'' and not isnull(is_base) and is_serie<>'' and not isnull(is_serie) and il_numero>0 and il_corr > 0 then
	SELECT	"CLIENTE_DEUDOR_TERCERO"."RUT"  
	INTO 		:ll_rut_aux
	FROM 		"CLIENTE_DEUDOR_TERCERO"  
	WHERE  ( "CLIENTE_DEUDOR_TERCERO"."RUT_TITULAR" = :gi_rut ) AND  
			 ( "CLIENTE_DEUDOR_TERCERO"."BASE" = :is_base ) AND  
			 ( "CLIENTE_DEUDOR_TERCERO"."SERIE" = :is_serie ) AND  
			 ( "CLIENTE_DEUDOR_TERCERO"."NUMERO" = :il_numero ) AND  
			 ( "CLIENTE_DEUDOR_TERCERO"."CORRELATIVO" = :il_corr )   
	USING		sqlca;
	if sqlca.sqlcode=0 then
		cb_terceros.italic					= true
	else
		cb_terceros.italic					= false
	end if
	if isvalid(w_detalle_repactacion_por_contrato) then
		if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
			dw_lista.dataobject		= 'dw_detalle_repacta_mantencion'
		else
			dw_lista.dataobject		= 'dw_detalle_repacta_mantencion_l'
		end if
		dw_lista.settransobject(sqlca)
	end if
	il_tot_reg								= dw_lista.retrieve(is_base,is_serie,il_numero,il_corr)
	if il_tot_reg > 0 then
		st_val_interes_adm_pesos.text	= string(il_int_adm_peso,"###,###,###,###,###,##0")
		il_corr								= dw_lista.getitemnumber(1,'correlativo')
		gd_uf_dia							= dw_lista.getitemnumber(1,'uf_dia')
		
		st_ctas_mant_pago.text			= string(dw_lista.getitemnumber(1,'cuotas_en_mora'))
		em_cuotas.text						= string(dw_lista.getitemnumber(1,'cuotas_pactadas'))
		st_val_interes_adm_uf.text		= string((il_int_adm_peso / gd_uf_dia),"###,###,###,###,##0.00")

		ll_por_descuento					= dw_lista.getitemnumber(1,'por_descuento')
		em_descuento.text				= string(ll_por_descuento,"##0")
		ll_interes_deuda					= dw_lista.getitemnumber(1,'interes_deuda_pesos')
		st_val_interes_total.text			= string(ll_interes_deuda,'###,###,###,###,##0')

		st_val_interes_tot_total.text	= string(ll_interes_deuda / gd_uf_dia,'###,###,###,###,##0.00')

		ll_cta_pag							= dw_lista.getitemnumber(1,'ctas_pagadas')
		ld_fecha								= date(dw_lista.getitemdatetime(1,'fecha_primera_cta'))
		il_monto								= dw_lista.getitemnumber(1,'total_deuda')
		em_fecha.text						= string(ld_fecha)
		ll_monto_cuota						= dw_lista.getitemnumber(1,'valor_cuota')
		ll_dia									= day(ld_fecha)
		ll_mes								= month(ld_fecha)
		ll_anno								= year(ld_fecha)
		ld_factor_uf							= double(dw_lista.getitemnumber(1,'factor_aplicado'))
		st_factor.text						= string(ld_factor_uf,"###,##0.0000000")
		st_val_cuota_peso.text			= string(long(st_val_interes_total.text) + long(st_val_interes_adm_pesos.text),"###,###,###,###,##0")
		st_val_cuota_uf.text				= string(double(st_val_interes_tot_total.text) + double(st_val_interes_adm_uf.text),"###,###,###,###,##0.00")
		st_val_cuota_peso2.text			= string(ll_monto_cuota,"###,###,##0")

		st_val_cuota_uf2.text				= string(ll_monto_cuota / gd_uf_dia,"###,###,##0.0000")

		if gs_moneda = '1' then
			em_moneda.text				= 'PESO'
			rb_calc_pesos.checked		= true
			rb_calc_uf.checked			= false
		elseif gs_moneda = '2' then
			em_moneda.text				= 'U.F.'
			rb_calc_pesos.checked		= false
			rb_calc_uf.checked			= true
		end if
	else
		il_cta_en_mora			= long(substr(1,7,Message.StringParm))
		dw_lista.insertrow(0)
		dw_lista.setitem(1,'base','L')
	  	dw_lista.setitem(1,'serie','R')
		dw_lista.setitem(1,'numero',il_numero)
		dw_lista.setitem(1,'correlativo',il_corr)
		dw_lista.setitem(1,'base_original',is_base)
	  	dw_lista.setitem(1,'serie_original',is_serie)
		dw_lista.setitem(1,'numero_original',il_numero)
		dw_lista.setitem(1,'ctas_pagadas',0)
		dw_lista.setitem(1,'uf_dia',gd_uf_dia)
		dw_lista.setitem(1,'interes_adm_pesos',il_val_gtos_adm)
		if il_nro_ctas > 15 then
			dw_lista.setitem(1,'cuotas_pactadas',15)
		else
			dw_lista.setitem(1,'cuotas_pactadas',il_nro_ctas)
		end if
		dw_lista.setitem(1,'cuotas_en_mora',il_nro_ctas)
		dw_lista.setitem(1,'estado_pago','V')
		dw_lista.setitem(1,'fecha_primera_cta',idt_fecha_hoy)
		dw_lista.setitem(1,'tipo_moneda',gs_moneda)
		if gs_moneda = '1' then
			em_moneda.text				= 'PESO'
			rb_calc_pesos.checked		= true
			rb_calc_uf.checked			= false
		elseif gs_moneda = '2' then
			em_moneda.text				= 'U.F.'
			rb_calc_pesos.checked		= false
			rb_calc_uf.checked			= true
		end if
	end if
	if trim(gs_opera)='1' and is_opcion='N' then
		cb_grabar.enabled					= false
		cb_imprimir.enabled				= true
	elseif trim(gs_opera)<>'1' and is_opcion='N' then
		cb_grabar.enabled					= false
		cb_imprimir.enabled				= false
	end if
	if trim(gs_opera)='1' then
		st_print_detalle.enabled			= true
	elseif trim(gs_opera)<>'1' then
		st_print_detalle.enabled			= false
		cb_imprimir.enabled				= false
	end if
	dw_lista.accepttext()
	dw_detalle.reset()
	ll_nro_ctas_total						= long(em_cuotas.text)
	if ll_nro_ctas_total > 0 then
		for i=1 to ll_nro_ctas_total
			ll_new							= dw_detalle.insertrow(0)
			dw_detalle.setitem(ll_new,'monto',long(st_val_cuota_peso2.text))
			dw_detalle.setitem(ll_new,'uf',gd_uf_dia)
			if ll_dia > 28 and ll_mes =2 then
				ll_dia_aux					= 28
			elseif ll_dia > 30 and (ll_mes=4 or ll_mes=6 or ll_mes=9 or ll_mes=11) then
				ll_dia_aux					= 30
			else
				ll_dia_aux					= ll_dia
			end if
			ls_fecha							= string(ll_dia_aux,'00')+'/'+string(ll_mes,'00')+'/'+string(ll_anno,'0000')
			ld_fec_pago						= date(ls_fecha)
			dw_detalle.setitem(ll_new,'fecha',ld_fec_pago)
			ll_mes++
			if ll_mes>12 then
				ll_mes						= 1
				ll_anno++
			end if
			if i > ll_cta_pag then
				dw_detalle.setitem(ll_new,'estado','V')
			else
				dw_detalle.setitem(ll_new,'estado','C')
			end if
		next
		
		if isvalid(w_detalle_repactacion_por_contrato) then
			if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
				SELECT	"REPACTA_MANTENCION"."RUT_CLIENTE",   
							"REPACTA_MANTENCION"."DV_CLIENTE"  
				INTO 		:ll_rut,   
							:ls_dv  
				FROM 	"REPACTA_MANTENCION"  
				WHERE  ( "REPACTA_MANTENCION"."BASE_ORIGINAL" = :is_base ) AND  
						 ( "REPACTA_MANTENCION"."SERIE_ORIGINAL" = :is_serie ) AND  
						 ( "REPACTA_MANTENCION"."NUMERO_ORIGINAL" = :il_numero ) AND  
						 ( "REPACTA_MANTENCION"."CORRELATIVO" = :il_corr )   
				USING		sqlca;
			else
				SELECT	"REPACTA_MANTENCION_L"."RUT_CLIENTE",   
							"REPACTA_MANTENCION_L"."DV_CLIENTE"  
				INTO 		:ll_rut,   
							:ls_dv  
				FROM 	"REPACTA_MANTENCION_L"  
				WHERE  ( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :is_base ) AND  
						 ( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :is_serie ) AND  
						 ( "REPACTA_MANTENCION_L"."NUMERO_ORIGINAL" = :il_numero ) AND  
						 ( "REPACTA_MANTENCION_L"."CORRELATIVO" = :il_corr )   
				USING		sqlca;
			end if
		else
			SELECT	"REPACTA_MANTENCION_L"."RUT_CLIENTE",   
						"REPACTA_MANTENCION_L"."DV_CLIENTE"  
			INTO 		:ll_rut,   
						:ls_dv  
			FROM 	"REPACTA_MANTENCION_L"  
			WHERE  ( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :is_base ) AND  
					 ( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :is_serie ) AND  
					 ( "REPACTA_MANTENCION_L"."NUMERO_ORIGINAL" = :il_numero ) AND  
					 ( "REPACTA_MANTENCION_L"."CORRELATIVO" = :il_corr )   
			USING		sqlca;
		end if
		SELECT	"CLIENTE"."NOMBRE",   
					"CLIENTE"."A_PATERNO",   
					"CLIENTE"."A_MATERNO"  
		INTO 		:ls_nom,   
					:ls_ap_pat,   
					:ls_ap_mat  
		FROM 	"CLIENTE"  
		WHERE 	"CLIENTE"."RUT" = :ll_rut   ;
		dw_detalle.object.t_titulo.text		= 'Detalle Cuotas Repactación Nº:   R - '+is_serie+' - '+string(il_numero,"###,###,###,###,###")+'-'+string(il_corr,"###,###,###,###,###")
		dw_detalle.object.t_titulo_1.text	= 'CLIENTE  '+string(ll_rut,"###,###,###,##0")+' - '+ls_dv+'     '+ls_nom+' '+ls_ap_pat+' '+ls_ap_mat
		dw_detalle.object.t_titulo_2.text	= 'CONTRATO ORIGINAL:   '+is_base+' - '+is_serie+ ' - '+string(il_numero,"###,###,###,###,##0")
		dw_detalle.object.t_titulo_3.text	= ls_titulo
	end if
else
	messagebox("Advertencia","No Registra Dato Contrato "+is_base+' - '+is_serie+' - '+string(il_numero,"###,###,###,###,##0"))
	close(w_repacta_mora_mantencion_reprog)
end if
gs_ventana									= 'w_repacta_mora_mantencion_reprog'
f_valida_objeto()
end event

event close;setnull(gi_rut_tercero)
end event

type st_cliente_tercero from statictext within w_repacta_mora_mantencion_reprog
integer x = 27
integer y = 228
integer width = 2491
integer height = 60
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 79741120
boolean focusrectangle = false
end type

type st_txt_saldo_ins from statictext within w_repacta_mora_mantencion_reprog
integer x = 1998
integer y = 784
integer width = 494
integer height = 56
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Verdana"
long textcolor = 33554432
long backcolor = 67108864
string text = "Saldo Insoluto"
alignment alignment = center!
boolean focusrectangle = false
end type

type em_saldo_insoluto from statictext within w_repacta_mora_mantencion_reprog
integer x = 1998
integer y = 844
integer width = 494
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 255
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type cb_terceros from commandbutton within w_repacta_mora_mantencion_reprog
integer x = 1920
integer y = 1092
integer width = 567
integer height = 88
integer taborder = 40
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Ingresar NO &Titular (3º)"
end type

event clicked;string	ls_string
//ls_string	= is_base+'~t'+is_serie+'~t'+string(il_numero)+'~t'+string(gi_rut)+'~t'+string(il_corr)+'~t'+is_opcion
//openwithparm(w_ingreso_cliente_tercero,'M')
//openwithparm(w_ingreso_deudores_terceros,ls_string)
ls_string	= 'M'+'~t'+string(il_corr)
openwithparm(w_ingreso_cliente_tercero,ls_string)
end event

type st_print_detalle from statictext within w_repacta_mora_mantencion_reprog
integer x = 69
integer y = 1928
integer width = 713
integer height = 84
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
boolean underline = true
long backcolor = 79741120
string text = "Imprimir Detalle Vcto."
alignment alignment = center!
boolean focusrectangle = false
end type

event clicked;Double	ll_numero,ldb_pie
String		ls_base
datetime	ldt_fecha_pie
if dw_detalle.rowcount() > 0 then
	ll_numero								= dw_lista.getitemnumber(1,'numero')
	ldb_pie									= dw_lista.getitemnumber(1,'pie')
	ldt_fecha_pie							= dw_lista.getitemdatetime(1,'fecha_pie')
	
	dw_detalle.object.t_titulo.text		= 'Detalle Cuotas Repactación Nº:   L - '+'R'+' - '+string(il_numero,"###,###,###,###,###")	//+'-'+string(il_corr,"###,###,###,###,###")
	dw_detalle.object.t_titulo_1.text	= 'CLIENTE  '+string(gi_rut,"###,###,###,##0")+' - '+gs_dv+'     '+trim(st_cliente.text)
	dw_detalle.object.t_titulo_2.text	= 'CONTRATO ORIGINAL:   '+is_base+' - '+is_serie+ ' - '+string(gi_numero,"###,###,###,###,##0")
	dw_detalle.object.t_titulo_3.text	= ''	//ls_titulo
	dw_detalle.object.t_valor_pie.text	= string(ldb_pie,'###,###,###,##0.0###')
	dw_detalle.object.t_fecha_pie.text	= string(ldt_fecha_pie,'dd/mm/yyyy')
	dw_detalle.accepttext()
	
	if ll_numero>0 and not isnull(ll_numero) then
		SELECT 	"REPACTA_MANTENCION_L"."BASE"  
		INTO 		:ls_base  
		FROM 	"REPACTA_MANTENCION_L"  
		WHERE 	"REPACTA_MANTENCION_L"."NUMERO" = :ll_numero   
		Using	sqlca;
		if sqlca.sqlcode=0 then
			f_Print( dw_detalle )
		else
			messagebox("Advertencia","Reprogramación Inexistente debe Grabar antes de Imprimir")
		end if
	end if
else
	messagebox("Advertencia","No registra datos")
end if
end event

type dw_print_ctto from datawindow within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 160
integer y = 2080
integer width = 558
integer height = 388
integer taborder = 100
string dataobject = "dw_print_contrato_c_stgo_todos"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type

type st_9 from statictext within w_repacta_mora_mantencion_reprog
integer x = 928
integer y = 1920
integer width = 471
integer height = 68
integer textsize = -10
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 79741120
string text = "Factor Aplicado:"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_factor from statictext within w_repacta_mora_mantencion_reprog
integer x = 1413
integer y = 1920
integer width = 658
integer height = 68
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
boolean focusrectangle = false
end type

type st_txt_valor_cta2 from statictext within w_repacta_mora_mantencion_reprog
integer x = 1650
integer y = 1568
integer width = 448
integer height = 72
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Valor Cuota"
alignment alignment = center!
boolean focusrectangle = false
end type

type st_val_cuota_uf2 from statictext within w_repacta_mora_mantencion_reprog
integer x = 1650
integer y = 1656
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_val_cuota_peso2 from statictext within w_repacta_mora_mantencion_reprog
integer x = 1650
integer y = 1772
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_txt_cob_adm from statictext within w_repacta_mora_mantencion_reprog
integer x = 745
integer y = 1568
integer width = 448
integer height = 72
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Gastos Adm."
alignment alignment = center!
boolean focusrectangle = false
end type

type st_txt_valor_cta from statictext within w_repacta_mora_mantencion_reprog
integer x = 1198
integer y = 1568
integer width = 448
integer height = 72
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Total Reprog."
alignment alignment = center!
boolean focusrectangle = false
end type

type st_val_cuota_uf from statictext within w_repacta_mora_mantencion_reprog
integer x = 1198
integer y = 1656
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_val_cuota_peso from statictext within w_repacta_mora_mantencion_reprog
integer x = 1198
integer y = 1772
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_val_interes_adm_uf from statictext within w_repacta_mora_mantencion_reprog
integer x = 745
integer y = 1656
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_val_interes_adm_pesos from statictext within w_repacta_mora_mantencion_reprog
integer x = 745
integer y = 1772
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type rb_calc_uf from radiobutton within w_repacta_mora_mantencion_reprog
integer x = 101
integer y = 396
integer width = 549
integer height = 72
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 79741120
string text = "Calculo por U.F."
boolean checked = true
end type

event clicked;if rb_calc_uf.checked=true then
	gs_moneda 		= '2'
	em_moneda.text	= 'U.F.'
	dw_lista.setitem(1,'tipo_moneda',gs_moneda)
end if
end event

type rb_calc_pesos from radiobutton within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 82
integer y = 388
integer width = 539
integer height = 72
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 79741120
string text = "Calculo en Pesos"
end type

event clicked;if rb_calc_pesos.checked=true then
	gs_moneda 		= '1'
	em_moneda.text	= 'PESO'
	dw_lista.setitem(1,'tipo_moneda',gs_moneda)
end if
end event

type st_6 from statictext within w_repacta_mora_mantencion_reprog
integer x = 2043
integer y = 520
integer width = 78
integer height = 64
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "%"
boolean focusrectangle = false
end type

type st_dscto from statictext within w_repacta_mora_mantencion_reprog
integer x = 1413
integer y = 516
integer width = 334
integer height = 68
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Descuento"
alignment alignment = right!
boolean focusrectangle = false
end type

type em_descuento from editmask within w_repacta_mora_mantencion_reprog
integer x = 1765
integer y = 504
integer width = 261
integer height = 92
integer taborder = 30
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 31457278
alignment alignment = center!
borderstyle borderstyle = stylelowered!
string mask = "##0"
end type

event modified;il_por_descto		= long(this.text)
if il_por_descto< 0 then 
	messagebox("Advertencia","% no debe ser menor a 0")
	this.text			= '0'
	il_por_descto	= 0
elseif il_por_descto>100 then 
	messagebox("Advertencia","% no debe ser mayor a 100")
	this.text			= '100'
	il_por_descto	= 100
end if

end event

type st_val_interes_tot_total from statictext within w_repacta_mora_mantencion_reprog
integer x = 293
integer y = 1656
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_8 from statictext within w_repacta_mora_mantencion_reprog
integer x = 64
integer y = 1672
integer width = 215
integer height = 68
integer textsize = -11
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "U.F."
alignment alignment = center!
boolean focusrectangle = false
end type

type st_7 from statictext within w_repacta_mora_mantencion_reprog
integer x = 64
integer y = 1792
integer width = 215
integer height = 68
integer textsize = -11
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "PESO"
alignment alignment = center!
boolean focusrectangle = false
end type

type st_5 from statictext within w_repacta_mora_mantencion_reprog
integer x = 293
integer y = 1568
integer width = 448
integer height = 72
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Deuda en Mora"
alignment alignment = center!
boolean focusrectangle = false
end type

type st_val_interes_total from statictext within w_repacta_mora_mantencion_reprog
integer x = 293
integer y = 1772
integer width = 448
integer height = 108
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type em_mora_cta from statictext within w_repacta_mora_mantencion_reprog
integer x = 1362
integer y = 844
integer width = 494
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 255
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_txt_total from statictext within w_repacta_mora_mantencion_reprog
integer x = 1362
integer y = 784
integer width = 494
integer height = 56
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Verdana"
long textcolor = 33554432
long backcolor = 67108864
string text = "Mora Cta."
alignment alignment = center!
boolean focusrectangle = false
end type

type st_moneda from statictext within w_repacta_mora_mantencion_reprog
integer x = 87
integer y = 980
integer width = 251
integer height = 56
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 33554432
long backcolor = 67108864
string text = "Moneda"
alignment alignment = right!
boolean focusrectangle = false
end type

type em_moneda from statictext within w_repacta_mora_mantencion_reprog
integer x = 366
integer y = 964
integer width = 402
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_uf_dia from statictext within w_repacta_mora_mantencion_reprog
integer x = 901
integer y = 976
integer width = 270
integer height = 64
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 79741120
string text = "Valor UF día"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_monto_uf from statictext within w_repacta_mora_mantencion_reprog
integer x = 1193
integer y = 964
integer width = 462
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_deuda_uf from statictext within w_repacta_mora_mantencion_reprog
integer x = 2057
integer y = 964
integer width = 434
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 255
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_monto_en_uf from statictext within w_repacta_mora_mantencion_reprog
integer x = 1742
integer y = 976
integer width = 293
integer height = 64
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 79741120
string text = "Deuda en UF"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_ctas_mant_pago from statictext within w_repacta_mora_mantencion_reprog
integer x = 2203
integer y = 160
integer width = 311
integer height = 68
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 67108864
alignment alignment = center!
boolean focusrectangle = false
end type

type st_cta_txt from statictext within w_repacta_mora_mantencion_reprog
integer x = 1673
integer y = 164
integer width = 526
integer height = 64
integer textsize = -9
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 67108864
string text = "Cta.Mant.a Repactar"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_fecha_prox_pago from statictext within w_repacta_mora_mantencion_reprog
integer x = 2203
integer y = 60
integer width = 347
integer height = 68
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 67108864
boolean focusrectangle = false
end type

type st_4 from statictext within w_repacta_mora_mantencion_reprog
integer x = 1710
integer y = 64
integer width = 489
integer height = 64
integer textsize = -9
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 67108864
string text = "Fecha Vcto. Cta."
alignment alignment = right!
boolean focusrectangle = false
end type

type cb_filtrar from commandbutton within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 658
integer y = 1944
integer width = 306
integer height = 92
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "&Filtrar"
end type

event clicked;string nulo
setnull (nulo)
dw_detalle.SETfilter(NULO)
dw_detalle.filter()
end event

type cb_ordenar from commandbutton within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 347
integer y = 1944
integer width = 306
integer height = 92
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "&Ordenar"
end type

event clicked;if dw_detalle.rowcount() > 0 then
	string nulo
	setnull (nulo)
	dw_detalle.SETSORT(NULO)
	dw_detalle.SORT()
end if
end event

type cb_exportar from commandbutton within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 37
integer y = 1944
integer width = 306
integer height = 92
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "&Exportar"
end type

event clicked;datawindow	dw_paso
dw_paso	= dw_detalle
if dw_detalle.rowcount() > 0 then f_DWToExcel( dw_paso )

end event

type p_fec from picture within w_repacta_mora_mantencion_reprog
integer x = 1193
integer y = 504
integer width = 78
integer height = 88
string picturename = "Ddlb.bmp"
boolean focusrectangle = false
end type

event clicked;date	ld_fecha
ld_fecha	= date(em_fecha.text)
if f_valida_fecha(string(ld_fecha))=-1 then 
	em_fecha.text	= string(idt_fecha_hoy,gs_formato_fecha)
	return
end if
OpenWithParm(w_calendar,string(ld_fecha))
IF not isnull(Message.StringParm) THEN
	em_fecha.text	= string(date(Message.StringParm))
END IF
Date	ld_fecha_ing
ld_fecha_ing	= date(em_fecha.text)
ld_fecha			= RelativeDate(idt_fecha_hoy, 31)
if ld_fecha_ing > ld_fecha then
	messagebox("Advertencia","Fecha Tope es: "+string(ld_fecha,"dd/mm/yyyy"))
	em_fecha.text	= string(ld_fecha)
end if

end event

type em_fecha from editmask within w_repacta_mora_mantencion_reprog
integer x = 741
integer y = 504
integer width = 443
integer height = 92
integer taborder = 20
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 31457278
alignment alignment = center!
borderstyle borderstyle = stylelowered!
maskdatatype maskdatatype = datemask!
string mask = "dd/mm/yyyy"
end type

event modified;Date	ld_fecha, ld_fecha_ing

ld_fecha_ing	= date(em_fecha.text)
ld_fecha			= RelativeDate(idt_fecha_hoy, 31)
if ld_fecha_ing > ld_fecha then
	messagebox("Advertencia","Fecha Tope es: "+string(ld_fecha,"dd/mm/yyyy"))
	em_fecha.text	= string(ld_fecha)
end if
end event

type st_fec_cuo from statictext within w_repacta_mora_mantencion_reprog
integer x = 91
integer y = 516
integer width = 631
integer height = 64
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Fecha Pago 1ª Cuota"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_txt_real from statictext within w_repacta_mora_mantencion_reprog
integer x = 727
integer y = 784
integer width = 494
integer height = 56
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Verdana"
long textcolor = 33554432
long backcolor = 67108864
string text = "Gastos Adm."
alignment alignment = center!
boolean focusrectangle = false
end type

type cb_grabar from commandbutton within w_repacta_mora_mantencion_reprog
integer x = 2149
integer y = 1564
integer width = 334
integer height = 96
integer taborder = 60
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "&Grabar"
end type

event clicked;long		ll_val_ctas,ll_suma_cta, ll_cta_mora,ll_rut,ll_tot_renta,ll_grupo_f,ll_rut_aux,ll_est_resp=0,ll_rut_cont=0,ll_row=0,ll_sum_ctas_pag,ll_cta_pag_isa, ll_cta_repacta,&
			ll_plazo,ll_cod_parque,ll_count_vig,ll_ctas_mora_mant,ll_cta_pag_m,ll_count_anexo
Datetime	ldt_fecha_1, ldt_fecha_crea,ldt_fec_nac,ldt_fec_ing,ldt_fec_cob,ldt_fecha_hoy,	ldt_fecha_prim,ldt_fec_venc_mant
date		ld_fecha_1, ld_fecha_crea,ldt_fecha_ult,ldt_fecha_pie
time		lt_hora
String	ls_base_aux,ls_barra,ls_barra_aux, ls_num,ls_nom,ls_ap_pat,ls_ap_mat,ls_dir_p,ls_pob,ls_sec,ls_com,ls_ciu,ls_fono_p,ls_sexo,ls_est_civ,&
			ls_dom_c,ls_fono_c,ls_dv,ls_est_cob,ls_glosa_cob,ls_es_deudor='S',	ls_estado_deudor,ls_tipo='I',ls_est_titular='A',ls_dv_aux,ls_moneda,&
			ls_estado_cadena,ls_string_folio,ls_base='L'
double	ld_val_cta_uf, ld_val_deuda_uf,ld_factor,ldb_valor_cuota,ldb_valor_cuota_mant,ldb_pie,ld_saldo_finan,ldb_tasa,ldb_tasa_int_cta

connect using trans_1;
connect using trans_2;
//cb_proceso.triggerevent(clicked!)
ldt_fecha_hoy		= datetime(idt_fecha_hoy,time("00:00:00"))
if st_val_interes_tot_total.text='' or st_val_interes_adm_uf.text='' or &
	st_val_cuota_uf.text='' or st_val_cuota_uf2.text='' then
	messagebox("Advertencia","Antes de Grabar debe Procesar")
else
	ldb_pie					= dw_lista.getitemnumber(1,'pie')
	ldt_fecha_pie			= date(dw_lista.getitemdatetime(1,'fecha_pie'))
	if isnull(ldb_pie) then ldb_pie=0
	if isnull(ldt_fecha_pie) then ldt_fecha_pie=date('01/01/1900')
	if ldb_pie = 0 and isnull(ldt_fecha_pie) then
		messagebox("Advertencia","Debe Ingresar Fecha Pie")
		dw_lista.Setcolumn('fecha_pie')
	elseif ldb_pie > 0 and ldt_fecha_pie=date('01/01/1900') then
		messagebox("Advertencia","Debe Ingresar Pie")
		dw_lista.Setcolumn('pie')
	else
		gl_n_cuotas_pag		= long(st_ctas_mant_pago.text)
		lt_hora					= Now()
		il_nro_ctas				= long(em_cuotas.text)
		ld_fecha_1				= date(trim(em_fecha.text))
		ld_fecha_crea			= idt_fecha_hoy
		ldt_fecha_1				= datetime(ld_fecha_1, lt_hora)
		ldt_fecha_crea			= datetime(ld_fecha_crea, lt_hora)
		ll_val_ctas				= long(st_val_cuota_peso2.text)
		il_deuda					= long(long(st_val_cuota_peso2.text) * il_nro_ctas)
		il_val_int					= long(st_val_interes_total.text)
		ld_val_deuda_uf		= round(il_deuda / gd_uf_dia,4)
		ld_val_cta_uf			= double(st_val_cuota_uf2.text)
		ls_barra					= 'L'+'R'+string(il_numero)+string(il_corr)
		ls_num					= string(il_numero)+string(il_corr)
		ll_num_ori				= il_numero
		il_numero				= Double(ls_num)
		ll_cta_mora				= gl_n_cuotas_pag //long(st_ctas_mant_pago.text)
		ld_factor					= double(st_factor.text)
		is_base					= trim(st_base.text)
		ldt_fecha_ult			= date(dw_detalle.getitemdatetime(dw_detalle.rowcount(),'fecha'))
		gl_n_cuotas_pag		= long(st_ctas_mant_pago.text)
		il_val_presente			= long(em_val_presente.text)
		il_val_gtos_adm		= long(em_gastos_adm.text)
		il_val_mora_cta		= long(em_mora_cta.text)
		il_val_saldo_insoluto	= long(em_saldo_insoluto.text)
		CHOOSE CASE is_base
			CASE 'O'
				//gl_cod_parque
				SELECT	"CADENA"."COD_PARQUE",	"OFERTA_V"."CTA_PAG_M"
				INTO 		:gl_cod_parque,			:ll_cta_pag_m
				FROM 		"CADENA",   
							"OFERTA_V",   
							"PAGO_OFERTA"
				WHERE  ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
						 ( "OFERTA_V"."NRO_OFERTA" = "PAGO_OFERTA"."NRO_OFERTA" ) and  
						 ( "OFERTA_V"."ULT_FOLIO" = "PAGO_OFERTA"."FOLIO" ) and 
						 ( "CADENA"."SERIE" = "OFERTA_V"."SERIE" ) and  
						 ( "CADENA"."NUMERO" = "OFERTA_V"."NRO_OFERTA" ) and  
						 (("CADENA"."CODIGO" = :is_base ) AND  
						 ( "CADENA"."SERIE" = :is_serie ) AND  
						 ( "CADENA"."NUMERO" = :ll_num_ori ) )  
				USING		sqlca;
	
			CASE 'C'
				SELECT	"CADENA"."COD_PARQUE",	"CONTRATO"."CTA_PAG_M"  
				INTO 		:gl_cod_parque,			:ll_cta_pag_m
				FROM 		"CADENA",   
							"CONTRATO" 
				WHERE  ( "CADENA"."SERIE" = "CONTRATO"."SERIE_C" ) and  
						 ( "CADENA"."NUMERO" = "CONTRATO"."NRO_CONTRATO" ) and 
						 (("CADENA"."CODIGO" = :is_base ) AND  
						 ( "CADENA"."SERIE" = :is_serie ) AND  
						 ( "CADENA"."NUMERO" = :ll_num_ori ) )   
				USING		sqlca;
		END CHOOSE
		SELECT	"REPACTA_MANTENCION_L"."BASE"  
		INTO 		:ls_base_aux  
		FROM 		"REPACTA_MANTENCION_L"  
		WHERE 	( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :is_base ) AND  
					( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :is_serie ) AND  
					( "REPACTA_MANTENCION_L"."NUMERO_ORIGINAL" = :il_numero ) AND
					( "REPACTA_MANTENCION_L"."CORRELATIVO" = :il_corr )
		Using		sqlca ;
		if sqlca.sqlcode=0 then
			if dw_lista.update()=1 then
				commit;
				il_sw		= 0
				ll_row	= w_detalle_repactacion_por_contrato.dw_lista.getrow()
				if ll_row>0 then
					w_detalle_repactacion_por_contrato.dw_lista.setitem(ll_row,'estado_pago','P')
					if w_detalle_repactacion_por_contrato.dw_lista.update()=1 then
						commit;
					else
						rollback;
					end if
				end if
				w_detalle_repactacion_por_contrato.dw_lista.retrieve(is_base,is_serie,il_numero)
				messagebox("Grabar","Grabación Exitosa")
			else
				rollback;
				messagebox("Error","Error al Grabar")
			end if
		else
			ll_row				= w_detalle_repactacion_por_contrato.dw_lista.getrow()
			if ll_row>0 then
				w_detalle_repactacion_por_contrato.dw_lista.setitem(ll_row,'estado_pago','P')
			end if
			ld_saldo_finan		= double(st_deuda_uf.text)
			ldb_tasa				= f_tasa(il_nro_ctas,ld_val_cta_uf,ld_saldo_finan)
			ldb_tasa_int_cta	= ldb_tasa / 100
			if gi_rut_tercero>0 then
				INSERT INTO "REPACTA_MANTENCION_L"  
							( "BASE",   	"SERIE",   "NUMERO",   "BASE_ORIGINAL",   "SERIE_ORIGINAL",   "NUMERO_ORIGINAL",   	"FECHA_CREA",   	"CORRELATIVO",	"TOTAL_DEUDA",   "CUOTAS_PACTADAS",   "ESTADO_PAGO",   	"VALOR_CUOTA",   	"FECHA_PRIMERA_CTA",   	"CUOTAS_EN_MORA",   	"USUARIO",   	"ABONO",	"SALDO",	"RUT_CLIENTE",	"DV_CLIENTE",	"UF_DIA",	"TIPO_MONEDA",	"TOTAL_DEUDA_UF",	"VALOR_CUOTA_UF",	"INTERES_CUOTA_PESOS",	"INTERES_DEUDA_PESOS",	"POR_DESCUENTO",	"INTERES_ADM_PESOS",	"FACTOR_APLICADO",	"TOTAL_PAGADO",	"FECHA_ULT",	"CTA_MORA_ORIGINAL",		"CANT_PRINT",	"CTAS_PAGADAS","CTAS_PAG_M_ORIGINAL",	"VALOR_PRESENTE_PESOS",	"VALOR_GTOS_ADM_PESOS",	"VALOR_MORA_CTA_PESOS",	"VALOR_SALDO_INSOLUTO", 	"TIPO_REPROG",	"PIE",		"FECHA_PIE",	 	"TASA",		"TASA_INTERES_CUOTA")  
				VALUES 	( 'L',   		'R', 		:il_numero, 	:is_base,   			:is_serie,   				:ll_num_ori,   	  			:ldt_fecha_crea,	:il_corr,				:il_deuda,   	  		:il_nro_ctas,   	  			'V',   			 			:ll_val_ctas,   			:ldt_fecha_1,   		  			:gl_n_cuotas_pag,   		:gs_user,    		0,				0,			:gi_rut_tercero,	:gs_dv,			:gd_uf_dia,	:gs_moneda,		:ld_val_deuda_uf,		:ld_val_cta_uf,			:il_val_int_cta,					:il_val_int,						:il_por_descto,			:il_int_adm_peso,			:ld_factor,					0,							:ldt_fecha_ult,	:gl_cta_mora_original,		0,					0,						:gl_ctas_pag_m,		 		:il_val_presente,					:il_val_gtos_adm,		  			:il_val_mora_cta,		 			:il_val_saldo_insoluto, 			'RR',					:ldb_pie,	:ldt_fecha_pie,		:ldb_tasa,	:ldb_tasa_int_cta)   
				Using	sqlca;
			else
				INSERT INTO "REPACTA_MANTENCION_L"  
							( "BASE",   	"SERIE",   "NUMERO",   	"BASE_ORIGINAL",   	"SERIE_ORIGINAL",   "NUMERO_ORIGINAL", "FECHA_CREA",   	"CORRELATIVO",	"TOTAL_DEUDA",   "CUOTAS_PACTADAS",   "ESTADO_PAGO",   	"VALOR_CUOTA",  	"FECHA_PRIMERA_CTA",   	"CUOTAS_EN_MORA",   	"USUARIO",   	"ABONO",	"SALDO",	"RUT_CLIENTE",	"DV_CLIENTE",	"UF_DIA",	"TIPO_MONEDA",	"TOTAL_DEUDA_UF",	"VALOR_CUOTA_UF",	"INTERES_CUOTA_PESOS",	"INTERES_DEUDA_PESOS",	"POR_DESCUENTO",	"INTERES_ADM_PESOS",	"FACTOR_APLICADO",	"TOTAL_PAGADO",	"FECHA_ULT",	"CTA_MORA_ORIGINAL",		"CANT_PRINT",	"CTAS_PAGADAS","CTAS_PAG_M_ORIGINAL",	"VALOR_PRESENTE_PESOS",	"VALOR_GTOS_ADM_PESOS",	"VALOR_MORA_CTA_PESOS",	"VALOR_SALDO_INSOLUTO",	"TIPO_REPROG",	"PIE",		"FECHA_PIE",	"TASA",		"TASA_INTERES_CUOTA")  
				VALUES 	( 'L',   		'R', 		:il_numero, 	:is_base,   				:is_serie,   				:ll_num_ori,   	  		:ldt_fecha_crea,	:il_corr,				:il_deuda,   	  		:il_nro_ctas,   	  			'V',   						:ll_val_ctas,   			:ldt_fecha_1,   		  			:gl_n_cuotas_pag,   		:gs_user,    		0,				0,			:gi_rut,				:gs_dv,			:gd_uf_dia,	:gs_moneda,		:ld_val_deuda_uf,		:ld_val_cta_uf,			:il_val_int_cta,					:il_val_int,						:il_por_descto,			:il_int_adm_peso,			:ld_factor,					0,							:ldt_fecha_ult,	:gl_cta_mora_original,		0,					0,						:gl_ctas_pag_m,		 		:il_val_presente,					:il_val_gtos_adm,		  			:il_val_mora_cta,		 			:il_val_saldo_insoluto, 			'RR',					:ldb_pie,	:ldt_fecha_pie,	:ldb_tasa,	:ldb_tasa_int_cta)  
				Using	sqlca;
			end if
			
			if sqlca.sqlcode=0 then
				commit;
				if ll_row>0 then
					if w_detalle_repactacion_por_contrato.dw_lista.update()=1 then
						commit;
					else
						rollback;
					end if
				end if
				
				il_tot_reg	= dw_lista.retrieve(is_base,is_serie,ll_num_ori,il_corr)
				if w_detalle_repactacion_por_contrato.dw_lista.retrieve(is_base,is_serie,ll_num_ori)>0 then w_detalle_repactacion_por_contrato.cb_pago.enabled=true
				il_sw=0
				if cb_terceros.italic=true then
					SELECT	"CLIENTE_DEUDOR_TERCERO"."RUT",   "CLIENTE_DEUDOR_TERCERO"."NOMBRE",   "CLIENTE_DEUDOR_TERCERO"."A_PATERNO",   "CLIENTE_DEUDOR_TERCERO"."A_MATERNO",   "CLIENTE_DEUDOR_TERCERO"."DIRECCION_P",   "CLIENTE_DEUDOR_TERCERO"."POBLACION",   "CLIENTE_DEUDOR_TERCERO"."SECTOR",   "CLIENTE_DEUDOR_TERCERO"."COMUNA",   "CLIENTE_DEUDOR_TERCERO"."CIUDAD",   "CLIENTE_DEUDOR_TERCERO"."FONO_P",   "CLIENTE_DEUDOR_TERCERO"."SEXO",   "CLIENTE_DEUDOR_TERCERO"."FECHA_NAC",   "CLIENTE_DEUDOR_TERCERO"."ESTA_CIVIL",   "CLIENTE_DEUDOR_TERCERO"."DOMICILIO_C",   "CLIENTE_DEUDOR_TERCERO"."FONO_C",   "CLIENTE_DEUDOR_TERCERO"."FECHA_INGR",   "CLIENTE_DEUDOR_TERCERO"."TOTAL_RENT",   "CLIENTE_DEUDOR_TERCERO"."GRUPO_F",   "CLIENTE_DEUDOR_TERCERO"."DV",   "CLIENTE_DEUDOR_TERCERO"."ESTADO_COBRANZA",   "CLIENTE_DEUDOR_TERCERO"."FECHA_COBRANZA",   "CLIENTE_DEUDOR_TERCERO"."GLOSA_COBRANZA",   "CLIENTE_DEUDOR_TERCERO"."USUARIO",   "CLIENTE_DEUDOR_TERCERO"."ESTADO_DEUDOR"  
					INTO 		:ll_rut,   								 :ls_nom,   								  :ls_ap_pat,   									:ls_ap_mat,   									 :ls_dir_p,   										 :ls_pob,   									  :ls_sec,   									:ls_com,   									 :ls_ciu,   								  :ls_fono_p,   								:ls_sexo,   							  :ldt_fec_nac,   								:ls_est_civ,   								  :ls_dom_c,   									  :ls_fono_c,   								:ldt_fec_ing,   								  :ll_tot_renta,   								 :ll_grupo_f,   								:ls_dv,   								:ls_est_cob,   										 :ldt_fec_cob,   										 :ls_glosa_cob,   									 :gs_user,   									:ls_estado_deudor 
					FROM 		"CLIENTE_DEUDOR_TERCERO"  
					WHERE  ( "CLIENTE_DEUDOR_TERCERO"."RUT_TITULAR" = :gi_rut ) AND  
							 ( "CLIENTE_DEUDOR_TERCERO"."BASE" = :is_base ) AND  
							 ( "CLIENTE_DEUDOR_TERCERO"."SERIE" = :is_serie ) AND  
							 ( "CLIENTE_DEUDOR_TERCERO"."NUMERO" = :gi_numero ) AND  
							 ( "CLIENTE_DEUDOR_TERCERO"."CORRELATIVO" = :il_corr )   
					USING		sqlca;
					if sqlca.sqlcode=0 then
						if ls_estado_deudor="A" then
							SELECT	"CLIENTE"."RUT"  
							INTO 		:ll_rut_aux  
							FROM 		"CLIENTE"  
							WHERE 	"CLIENTE"."RUT" = :ll_rut
							USING		trans_1;
							if trans_1.sqlcode=100 then
								INSERT INTO "CLIENTE"  
											( "RUT",   "TIPO",   "NOMBRE",   "A_PATERNO",   "A_MATERNO",   "DIRECCION_P",   "POBLACION",   "SECTOR",   "COMUNA",   "CIUDAD",   "FONO_P",   "SEXO",   "FECHA_NAC",   "ESTA_CIVIL",   "DOMICILIO_C",   "FONO_C",   "FECHA_INGR",   "TOTAL_RENT",   "GRUPO_F",   "DV",   "ESTADO_COBRANZA",   "FECHA_COBRANZA",   "GLOSA_COBRANZA",   "USUARIO",   "ESTADO_TITULAR",   "RUT_CONTACTO",   "ESTADO_RESPONSO",   "ES_DEUDOR" )  
								VALUES 	( :ll_rut, :ls_tipo, :ls_nom,    :ls_ap_pat,   	:ls_ap_mat,   	:ls_dir_p,   	  :ls_pob,   	  :ls_sec,    :ls_com,    :ls_ciu,    :ls_fono_p, :ls_sexo, :ldt_fec_nac,  :ls_est_civ,    :ls_dom_c,  		:ls_fono_c, :ldt_fec_ing,   :ll_tot_renta,  :ll_grupo_f, :ls_dv, :ls_est_cob,   		  :ldt_fec_cob,   	 :ls_glosa_cob,   	:gs_user,    :ls_est_titular,   	:ll_rut_cont,   	:ll_est_resp,			:ls_es_deudor )  
								USING	trans_2;
								if trans_2.sqlcode=0 then
									commit;
								else
									rollback;
	//								messagebox("Error al Grabar","Error SQL: "+sqlca.sqlerrtext)
								end if
							end if
						end if
					end if
				end if
//				SELECT	"CADENA"."BARRA"  
//				INTO 		:ls_barra_aux  
//				FROM 		"CADENA"  
//				WHERE  ( "CADENA"."CODIGO" = 'R' ) AND  
//						 ( "CADENA"."SERIE" = :is_serie ) AND  
//						 ( "CADENA"."NUMERO" = :il_numero )   
//				Using	sqlca;
//				if sqlca.sqlcode=100 then
//					if gi_rut_tercero>0 then
//						INSERT INTO	"CADENA"  
//										( "BARRA",   "RUT",   			"CODIGO",   "SERIE",   "NUMERO",   "ESTADO",   "PRINT",   "EST_CUP",   "NRO_CARTA",   "FECHA_CARTA",   "ESTADO_CARTA",   "FECHA_ACT_ESTADO",   "CUOTA_CUPON",    "FECHA_RES",   "A_DEMANDAR",   "ESTADO_CONTABLE",   "ESTADO_TITULO",   "FECHA_TITULO",   "NRO_CARTA_M",   "FECHA_CARTA_M",   "ESTADO_CARTA_M",   "FECHA_ACT_ESTADO_M",	"COBRAN_C",	"COD_PARQUE" )  
//						VALUES 		( :ls_barra, :gi_rut_tercero, 'R',   		 :is_serie, :il_numero, 'V',   		'N',   	  0,   			null,   			null,   				0,   					:ld_fecha_crea,   	0,   					null,   			null,   			 null,   				 null,   				null,   				null,   			 null,   			  0,   					 null,   					null ,		:gl_cod_parque)  
//						Using			sqlca;
//					else
//						INSERT INTO	"CADENA"  
//										( "BARRA",   "RUT",   "CODIGO",   "SERIE",   "NUMERO",   "ESTADO",   "PRINT",   "EST_CUP",   "NRO_CARTA",   "FECHA_CARTA",   "ESTADO_CARTA",   "FECHA_ACT_ESTADO",   "CUOTA_CUPON",    "FECHA_RES",   "A_DEMANDAR",   "ESTADO_CONTABLE",   "ESTADO_TITULO",   "FECHA_TITULO",   "NRO_CARTA_M",   "FECHA_CARTA_M",   "ESTADO_CARTA_M",   "FECHA_ACT_ESTADO_M",	"COBRAN_C",	"COD_PARQUE" )  
//						VALUES 		( :ls_barra, :gi_rut, 'R',   		 :is_serie, :il_numero, 'V',   		'N',   	  0,   			null,   			null,   				0,   					:ld_fecha_crea,   	0,   					null,   			null,   			 null,   				 null,   				null,   				null,   			 null,   			  0,   					 null,   					null ,		:gl_cod_parque)  
//						Using			sqlca;
//					end if
//					if sqlca.sqlcode=0 then
//						il_numero_aux	= il_numero - 1
//						UPDATE	"CADENA"  
//						SET 		"ESTADO" = 'P',
//									"FECHA_RES" = :ldt_fecha_hoy
//						WHERE  ( "CADENA"."CODIGO" = 'R' ) AND  
//								 ( "CADENA"."SERIE" = :gs_serie ) AND  
//								 ( "CADENA"."NUMERO" = :il_numero_aux )   
//						using		sqlca;
//						if sqlca.sqlcode=0 then
//							commit;
//						else
//							rollback;
//						end if
//						if is_base='C' then //// Contrato ISACRUZ
//							commit;
//							// sumar ctas pagadas isacruz
//	//						SELECT	SUM("INGRESO"."CUOTAS_PAG")  
//	//						INTO 		:ll_sum_ctas_pag  
//	//						FROM 		"INGRESO"  
//	//						WHERE  ( "INGRESO"."SERIE" = :is_serie ) AND  
//	//								 ( "INGRESO"."CONTRATO" = :ll_num_ori ) AND  
//	//								 ( "INGRESO"."BASE" = :is_base ) AND  
//	//								 ( "INGRESO"."TIPO_MOV" = 'F' )   
//	//						USING		sqlca;
//	//						if sqlca.sqlcode=0 then
//	//							if isnull(ll_sum_ctas_pag) then ll_sum_ctas_pag=0
//	//						else
//	//							ll_sum_ctas_pag	= 0
//	//						end if
//							ll_ctas_mora_mant	= long(st_ctas_mant_pago.text)
//							ll_suma_cta				= (ll_cta_pag_m - il_cuotas_mora_base) + ll_ctas_mora_mant
//							UPDATE	"CONTRATO"  
//							SET 		"ANEXO_REPACTA" = :ls_barra,   
//										"CTA_REPACTA" = :ll_ctas_mora_mant,  
//										"CTA_PAG_M" = :ll_suma_cta
//							WHERE  ( "CONTRATO"."SERIE_C" = :is_serie ) AND  
//									 ( "CONTRATO"."NRO_CONTRATO" = :ll_num_ori )   
//							Using	sqlca ;
//							if sqlca.sqlcode=0 then
//								commit;
//								DECLARE sp_nuevo_cadena_mora_c PROCEDURE FOR sp_nuevo_cadena_mora('C',:is_serie,:ll_num_ori,'M');
//								EXECUTE sp_nuevo_cadena_mora_c;
//							else
//								rollback;
//							end if
//						elseif is_base='O' then //// PROMESAS
//	//						SELECT	SUM("INGRESO"."CUOTAS_PAG")  
//	//						INTO 		:ll_sum_ctas_pag  
//	//						FROM 		"INGRESO"  
//	//						WHERE  ( "INGRESO"."SERIE" = :is_serie ) AND  
//	//								 ( "INGRESO"."CONTRATO" = :ll_num_ori ) AND  
//	//								 ( "INGRESO"."BASE" = :is_base ) AND  
//	//								 ( "INGRESO"."TIPO_MOV" = 'F' )   
//	//						USING		sqlca;
//	//						if sqlca.sqlcode=0 then
//	//							if isnull(ll_sum_ctas_pag) then ll_sum_ctas_pag=0
//	//						else
//	//							ll_sum_ctas_pag=0
//	//						end if
//							ll_ctas_mora_mant	= long(st_ctas_mant_pago.text)
//							ll_suma_cta			= (ll_cta_pag_m - il_cuotas_mora_base) + ll_ctas_mora_mant
//	
//							UPDATE	"OFERTA_V"  
//							SET 		"ANEXO_REPACTA" = :ls_barra,   
//										"CTA_REPACTA" = :ll_ctas_mora_mant,  
//										"CTA_PAG_M" = :ll_suma_cta
//							WHERE  ( "OFERTA_V"."SERIE" = :is_serie ) AND  
//									 ( "OFERTA_V"."NRO_OFERTA" = :ll_num_ori )   
//							Using	sqlca ;
//							if sqlca.sqlcode=0 then
//								commit;
//								DECLARE sp_nuevo_cadena_mora_o PROCEDURE FOR sp_nuevo_cadena_mora('O',:is_serie,:ll_num_ori,'M');
//								EXECUTE sp_nuevo_cadena_mora_o;
//							else
//								rollback;
//							end if
//						end if
//	
//						DECLARE sp_anular_cupones_ori PROCEDURE FOR SP_ANULAR_CUP_CTTO_ORI_REPRO( :is_base, :is_serie, :ll_num_ori, :gl_cta_mora_original );
//						EXECUTE sp_anular_cupones_ori;
//						if isvalid(w_boletas_cuotas) then w_boletas_cuotas.dw_datos_bl.retrieve(is_serie,ll_num_ori)
//						if gi_rut_tercero>0 then
//							ll_rut				= gi_rut_tercero
//						else
//							ll_rut				= gi_rut
//						end if
//						ll_plazo					= il_nro_ctas
//						ldb_valor_cuota		= ll_val_ctas
//						ldt_fecha_prim			= ldt_fecha_1
//						Setnull(ldt_fec_venc_mant)
//						ldb_valor_cuota_mant	= 0
//						SELECT	"CLIENTE"."DV"  
//						INTO 		:ls_dv  
//						FROM 	"CLIENTE"  
//						WHERE 	"CLIENTE"."RUT" = :ll_rut   ;
//						ll_cod_parque			= gl_cod_parque
//						ls_moneda				= gs_moneda
//						SELECT	"CADENA"."ESTADO"  
//						INTO 		:ls_estado_cadena  
//						FROM 	"CADENA"  
//						WHERE  ( "CADENA"."CODIGO" = :ls_base ) AND  
//								 ( "CADENA"."SERIE" = :is_serie ) AND  
//								 ( "CADENA"."NUMERO" = :il_numero )   
//						USING		sqlca;
//						if ls_estado_cadena='V' then
//							SELECT	"CUPONERAS"."NUMERO"  
//							INTO 		:ll_count_vig  
//							FROM 	"CUPONERAS"  
//							WHERE  ( "CUPONERAS"."BASE" = :ls_base ) AND  
//									 ( "CUPONERAS"."SERIE" = :is_serie ) AND  
//									 ( "CUPONERAS"."NUMERO" = :il_numero ) AND  
//									 ( "CUPONERAS"."ESTADO_CUPONERA" = 'V' )   
//							USING		sqlca;
//							if sqlca.sqlcode=100 then
//								DECLARE sp_nuevo_cadena_mora PROCEDURE FOR sp_nuevo_cadena_mora(:ls_base,:is_serie,:il_numero,'N');
//								EXECUTE sp_nuevo_cadena_mora;
//								
//								DECLARE sp_proc_genera_cuponera PROCEDURE FOR SP_GENERAR_CUPONERA( :ls_base, :is_serie, :il_numero, :ll_plazo, :gs_user, :ll_rut, :ld_val_cta_uf, :ldb_valor_cuota_mant, :ls_dv, :ll_cod_parque, :ldt_fecha_prim, :ldt_fec_venc_mant, :ls_moneda );
//								EXECUTE sp_proc_genera_cuponera;
//							else
//								DECLARE sp_nuevo_cadena_mora1 PROCEDURE FOR sp_nuevo_cadena_mora(:ls_base,:is_serie,:il_numero,'M');
//								EXECUTE sp_nuevo_cadena_mora1;
//							end if
//						end if
//					else
//						rollback;
//					end if
//				end if
				SELECT	Count("CD_FOLIO"."BASE")
				INTO 		:ll_count_anexo  
				FROM 	"CD_FOLIO"  
				WHERE 	( "CD_FOLIO"."BASE" = 'L' ) AND  
							( "CD_FOLIO"."SERIE" = 'R' ) AND  
							( "CD_FOLIO"."NUMERO" = :il_numero )   ;
				if ll_count_anexo = 0 then
					ls_string_folio		= 'FOLIO LR '+string(il_numero,'###,###,###,###,##0')+', CREADO POR '+gs_user+' EL '+string(gdt_fec_sistema,'dd/mm/yyyy')
					INSERT INTO "CD_FOLIO"  
								( "BASE",		"SERIE", 		"NUMERO", 		"FECHA_CREA", 		"USUARIO_CREA",		"COD_AGENTE",	"COD_SUPERVISOR", 	"COD_JEFE_VENTA", 	"ULT_ESTADO",	"ULT_FECHA_ESTADO",	"ULT_GLOSA_ESTADO", 	"ULT_MEMO", 	"ULT_USUARIO_GESTION", 	"DEPTO_ASIGNADO", 	"ESTADO_REG", 	"ULT_REFERENCIA_MEMO",		"ULT_GLOSA_MEMO",		"ULT_EMAIL",	"ULT_GLOSA_EMAIL",		"USUARIO_ENTREGAR_FORM",		"ULT_TIPO_RECHAZO",	"ULT_TIPO_EXCEPCION",	"ULT_ESTADO_EXCEPCION",	"ULT_ESTADO_ASOCIADO_EXCEPCION",		"COD_PARQUE", 	"MES_CIERRE", 	"ANO_CIERRE",		"SW_SUP", 	"SW_VTA" )  
					VALUES	( 'L', 			'R',   			:il_numero,   	:gdt_fec_sistema,   	:gs_user,   				null,   				null,   					null,   					0,   					:gdt_fec_sistema,   		:ls_string_folio,   			null,   			:gs_user,   						null,   					'I',   					null,   								null,   						null,   			null,   						null,   									null,   						null,   							null,   								null,   												:gl_cod_parque,  	null,   				null,   				1,   			1 )  
					USING	sqlca;
					if sqlca.sqlcode=0 then
						commit;
						INSERT INTO "CD_FOLIO_DETALLE"  
									( "BASE", 	"SERIE", 		"NUMERO", 		"FECHA_CREA", 	"USUARIO_CREA", 	"ESTADO",	"FECHA_ESTADO", 	"GLOSA", 		"MEMO", 	"COD_AGENTE", 	"COD_SUPERVISOR", 		"COD_JEFE_VENTA", 	"REFERENCIA_MEMO",	"USUARIO_ENTREGAR_FORM", 	"TIPO_RECHAZO", 	"TIPO_EXCEPCION", 	"ESTADO_EXCEPCION", 	"ESTADO_ASOCIADO_EXCEPCION",	"COD_PARQUE", 	"ESTADO_MODIFICADO", 	"MES_CIERRE", 	"ANO_CIERRE" )  
						VALUES 	( 'L',   		'R',   			:il_numero,   	:gdt_fec_sistema,  :gs_user,   				0,   			:gdt_fec_sistema,   	:ls_string_folio,	'-',   			null,   				null,   						null,   					'-',   							null,   									null,   					null,   					null,   						null,   										:gl_cod_parque,  	'N',   								null,   				null )  
						USING	sqlca;
						if sqlca.sqlcode=0 then
							commit;
						else
							rollback;
							messagebox("Error","Error al Grabar CD_FOLIO Sql: "+sqlca.sqlerrtext)
						end if
					else
						rollback;
						messagebox("Error","Error al Grabar CD_FOLIO Sql: "+sqlca.sqlerrtext)
					end if
				end if
				messagebox("Grabar","Grabación Exitosa")
			else
				rollback;
				messagebox("Error","Error al Grabar")
			end if
		end if
		cb_imprimir.triggerevent(clicked!)
	end if
end if
disconnect using trans_1;
disconnect using trans_2;
end event

type st_cliente from statictext within w_repacta_mora_mantencion_reprog
integer x = 27
integer y = 140
integer width = 1646
integer height = 92
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
boolean focusrectangle = false
end type

type em_cuotas from editmask within w_repacta_mora_mantencion_reprog
integer x = 1765
integer y = 376
integer width = 261
integer height = 92
integer taborder = 10
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 31457278
alignment alignment = center!
borderstyle borderstyle = stylelowered!
string mask = "#########"
end type

event modified;Long		ll_mora

il_nro_ctas	= long(this.text)
ll_mora		= long(st_ctas_mant_pago.text)
if this.text<>'' then
	if il_nro_ctas<2 then
		messagebox("Advertencia","La Reprogramación deben ser entre 2 a 15 cuotas o 18 - 24 - 30 - 36")
		il_nro_ctas	= 2
		this.text		= string(il_nro_ctas)
	elseif  (il_nro_ctas < 2) or (il_nro_ctas > 15 and il_nro_ctas < 18) or (il_nro_ctas > 18 and il_nro_ctas < 24) or (il_nro_ctas > 24 and il_nro_ctas < 30) or &
			(il_nro_ctas > 30 and il_nro_ctas < 36) or (il_nro_ctas > 36 ) then
		messagebox("Advertencia","La Reprogramación deben ser entre 2 a 15 cuotas o 18 - 24 - 30 - 36")
		il_nro_ctas	= 15
		this.text		= string(il_nro_ctas)
		cb_proceso.triggerevent(clicked!)
		il_modif ++
	else
		cb_proceso.triggerevent(clicked!)
		il_modif ++
	end if
end if

end event

type st_3 from statictext within w_repacta_mora_mantencion_reprog
integer x = 1074
integer y = 384
integer width = 672
integer height = 68
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Nº de Cuotas a Pactar"
alignment alignment = right!
boolean focusrectangle = false
end type

type st_numero from statictext within w_repacta_mora_mantencion_reprog
integer x = 919
integer y = 32
integer width = 635
integer height = 96
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16776960
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_serie from statictext within w_repacta_mora_mantencion_reprog
integer x = 791
integer y = 32
integer width = 123
integer height = 96
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16776960
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_base from statictext within w_repacta_mora_mantencion_reprog
integer x = 663
integer y = 32
integer width = 123
integer height = 96
integer textsize = -12
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16776960
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type st_2 from statictext within w_repacta_mora_mantencion_reprog
integer x = 14
integer y = 48
integer width = 622
integer height = 72
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long textcolor = 33554432
long backcolor = 67108864
string text = "CONTRATO ORIGINAL"
alignment alignment = right!
boolean focusrectangle = false
end type

type cb_proceso from commandbutton within w_repacta_mora_mantencion_reprog
integer x = 2149
integer y = 504
integer width = 334
integer height = 92
integer taborder = 50
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "&Procesar"
end type

event clicked;String		ls_string, ls_fecha,ls_titulo,ls_fecha_tope,ls_anexo_rep,ls_nopasa='S'
Date		ld_fecha,ld_fecha_ini, ld_fecha_hoy,ld_fec_pago,ld_fecha_tope_pago,ldt_fec_prox_pago,ldt_fec_prox_pago1,ldt_fec_prox_pago2,ldt_fec_prox_pago5
long 		i,ll_cuotas, ll_mes,ll_dia_aux,ll_new,ll_cuotas_div, ll_val_cta_inicial,ll_year, ll_dia, ll_anno, ll_monto, ll_suma_interes_peso, ll_suma_interes_uf, ll_dias, &
			ll_dia_tope, ll_mes_tope, ll_anno_tope,ll_tot_real_cta,ll_sw_cta=0,ll_mes_fecha, ll_mes_fecha_actual,ll_por_descuento,ll_interes_deuda,ll_cta_pag, &
			ll_monto_cuota,ll_cta_final,ll_cta_original,ll_pie_peso,ll_cant_mant,ll_cant_cred
double	ld_total,ll_total, ld_int_cta,ld_tot_deuda_uf,ll_cuota_sin_int,ll_monto_cta,ld_resul, ld_val_int_uf, ld_int_cta_uf,ld_factor_peso, ld_factor_uf, ldb_valor_dif,&
			ldb_pie,ldb_calc_monto,ld_interes_uf,ld_factor_gc
Long		ll_cta_diferencia,ll_factor_gc_peso
datetime	ldt_fecha_pie

ldb_pie							= dw_lista.getitemnumber(1,'pie')
ldt_fecha_pie					= dw_lista.getitemdatetime(1,'fecha_pie')
if ldb_pie >= il_uf_inicial or ldb_pie > 500 or isnull(ldb_pie) or ldb_pie = 0 then
	dw_lista.Setitem(1,'pie',0)
	dw_lista.accepttext()
end if
ll_cta_original					= long(st_ctas_mant_pago.text)
if cb_terceros.italic = false then
	if il_modif = 0 then
		messagebox("Advertencia","Recuerde antes de Grabar Reprogramación, ingresar Nuevo Deudor si Procede")
	end if
end if
gs_moneda						= '2'
//dw_lista.dataobject			= 'dw_detalle_repacta_mantencion' //'dwe_cuotas_pactadas'
//dw_lista.settransobject(sqlca)
dw_detalle.dataobject		= 'dwe_cuotas_pactadas'
dw_detalle.settransobject(sqlca)
dw_print_ctto.settransobject(sqlca)
is_base 							= substr(1,1,is_string)
is_serie							= substr(1,2,is_string)
il_numero						= Double(substr(1,3,is_string))
ll_num_ori						= il_numero
il_monto							= Long(substr(1,4,is_string))
st_cliente.text					= trim(substr(1,5,is_string))
id_fecha_prox_pago			= date(substr(1,6,is_string))
il_nro_ctas						= long(substr(1,7,is_string))
il_mora_inicial					= il_nro_ctas
il_corr							= long(substr(1,8,is_string))
is_opcion							= substr(1,9,is_string)
gl_ctas_pag_m					= long(substr(1,10,is_string))
il_val_presente					= long(substr(1,11,is_string))
il_val_gtos_adm				= long(substr(1,12,is_string))
il_val_mora_cta				= long(substr(1,13,is_string))
il_val_saldo_insoluto			= long(substr(1,14,is_string))
il_cta_mora_o					= long(substr(1,15,is_string))
il_cuotas_mora_base			= long(substr(1,16,is_string))
st_base.text						= is_base
st_serie.text						= is_serie
st_ctas_mant_pago.text		= string(il_nro_ctas,"###,###,###,###,##0")
ll_mes_fecha					= month(id_fecha_prox_pago)
ll_mes_fecha_actual			= month(idt_fecha_hoy)
il_valor_cuota_ctto				= 0
il_valor_cuota_m				= 0
ll_tot_real_cta					= il_cta_mora_o - il_mora_inicial
gl_cta_mora_original			= il_cta_mora_o
if dw_lista.rowcount() > 0 then
	if isnull(ldb_pie) then ldb_pie=0
	ll_pie_peso					= round(ldb_pie * gd_uf_dia,0)
end if
SELECT "ESTADO_MORA_MANTENCION"."CANTIDAD_MESES", "ESTADO_MORA_CREDITO"."CANTIDAD_MESES"  
INTO   :ll_cant_mant,														:ll_cant_cred
FROM   "CADENA", "ESTADO_MORA_CREDITO", "ESTADO_MORA_MANTENCION"  
WHERE ("CADENA"."ESTADO_MORA_CREDITO" = "ESTADO_MORA_CREDITO"."ESTADO_MORA_CREDITO" ) and  
		( "CADENA"."ESTADO_MORA_MANTENCION" = "ESTADO_MORA_MANTENCION"."ESTADO_MORA_MANTENCION" ) and  
		(("CADENA"."CODIGO" = :is_base ) AND  
		( "CADENA"."SERIE" = :is_serie ) AND  
		( "CADENA"."NUMERO" = :il_numero ) )   ;
if idt_fecha_hoy >= date(id_fecha_prox_pago) then
	if is_base='C' then
		SELECT	"CONTRATO"."VALOR_CUOTA_M",	"CONTRATO"."CTA_PAG_M",	"CADENA_MORA"."MORA_MANT",	"CADENA_MORA"."DIAS_MORA_MANT",	"CADENA_MORA"."FECHA_VENC_MANT"  
		INTO 		:il_valor_cuota_m,							:il_ctas_pag_m,					:il_mora_mant,							:il_dias_mora_mant,							:idt_fecha_venc_mant  
		FROM 		"CONTRATO",	
					"CADENA",   
					"CADENA_MORA"  
		WHERE  ( "CONTRATO"."SERIE_C" = "CADENA"."SERIE" ) and  
				 ( "CONTRATO"."NRO_CONTRATO" = "CADENA"."NUMERO" ) and  
				 ( "CADENA"."CODIGO" = "CADENA_MORA"."BASE" ) and  
				 ( "CADENA"."SERIE" = "CADENA_MORA"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "CADENA_MORA"."NUMERO" ) and  
				 (("CADENA"."CODIGO" = 'C' ) AND  
				 ( "CONTRATO"."SERIE_C" = :is_serie ) AND  
				 ( "CONTRATO"."NRO_CONTRATO" = :il_numero ) )   
		USING		sqlca;

	elseif is_base='O' then
		SELECT	"CADENA_MORA"."MORA_MANT",	"CADENA_MORA"."DIAS_MORA_MANT",	"CADENA_MORA"."FECHA_VENC_MANT",	"OFERTA_V"."VALOR_CUOTA_M",	"OFERTA_V"."CTA_PAG_M"  
		INTO 		:il_mora_mant,					:il_dias_mora_mant,					:idt_fecha_venc_mant,				:il_valor_cuota_m,				:il_ctas_pag_m  
		FROM 		"CADENA",	"CADENA_MORA",	"OFERTA_V",	"PAGO_OFERTA"  
		WHERE  ( "CADENA"."CODIGO" = "CADENA_MORA"."BASE" ) and  
				 ( "CADENA"."SERIE" = "CADENA_MORA"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "CADENA_MORA"."NUMERO" ) and  
				 ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
				 ( "OFERTA_V"."NRO_OFERTA" = "PAGO_OFERTA"."NRO_OFERTA" ) and  
				 ( "OFERTA_V"."ULT_FOLIO" = "PAGO_OFERTA"."FOLIO" ) and  
				 ( "CADENA"."SERIE" = "OFERTA_V"."SERIE" ) and  
				 ( "CADENA"."NUMERO" = "OFERTA_V"."NRO_OFERTA" ) and  
				 (("CADENA"."CODIGO" = 'O' ) AND  
				 ( "CADENA"."SERIE" = :is_serie ) AND  
				 ( "CADENA"."NUMERO" = :il_numero ) )   
		USING		sqlca;
	end if
	il_valor_cuota_ctto		= il_valor_cuota_m * gd_uf_dia
end if
SELECT	"TASA"."TASA_INT_UF"  
INTO 		:id_tasa  
FROM 	"TASA"  
WHERE 	"TASA"."LOOK" = 1   ;
il_cta_ori_mora					= il_nro_ctas //+ il_mora_mant + il_cuotas_mora_base
il_nro_ctas						= il_cta_ori_mora
st_numero.text					= string(il_numero,"###,###,###,###,###,##0")
em_val_presente.text			= string(il_val_presente,"###,###,###,###,###,##0")
//st_ctas_mant_pago.text	= string(il_nro_ctas,"###,###,###,###,##0")
st_fecha_prox_pago.text		= substr(1,6,is_string)
ll_dia_tope						= day(id_fecha_prox_pago)
ll_mes_tope						= month(id_fecha_prox_pago)
ll_anno_tope					= year(id_fecha_prox_pago)
ll_anno_tope					= ll_anno_tope + il_nro_ctas
ls_fecha_tope					= string(ll_dia_tope,'00')+'/'+string(ll_mes_tope,'00')+'/'+string(ll_anno_tope,'0000')
id_fecha_tope_pago			= date(ls_fecha_tope)
em_gastos_adm.text			= string(il_val_gtos_adm,"###,###,###,###,###,##0")
em_mora_cta.text				= string(il_val_mora_cta,"###,###,###,###,###,##0")
il_monto_inicial					= il_val_saldo_insoluto
il_uf_inicial						= round(il_val_saldo_insoluto / gd_uf_dia,2)
il_monto_inicial					= round(il_uf_inicial * gd_uf_dia,0)
//messagebox("il_monto_inicial2 + il_uf_inicial",string(il_monto_inicial)+'+'+string(il_uf_inicial))
il_monto							= il_val_saldo_insoluto - ll_pie_peso	// PIE

ldb_calc_monto					= round(il_monto / gd_uf_dia,2)
il_monto							= round(ldb_calc_monto * gd_uf_dia,0)

//em_saldo_insoluto.text		= string(il_val_saldo_insoluto,"###,###,###,###,###,##0")
st_monto_uf.text				= string(gd_uf_dia,"###,###,###,###,###,##0.00")
id_monto_uf						= round(il_monto / gd_uf_dia,2)
st_deuda_uf.text				= string(id_monto_uf,"###,###,###,###,##0.00")
st_val_interes_tot_total.text	= string(id_monto_uf,"###,###,###,##0.00")
st_val_interes_total.text		= string(il_monto,"###,###,###,###,###,##0")
em_saldo_insoluto.text		= string(il_monto,"###,###,###,###,###,##0")
if il_modif = 0 then
	il_monto_inicial				= il_monto
	il_uf_inicial					= round(il_monto / gd_uf_dia,2)
end if
if is_opcion='C' then
	cb_proceso.enabled		= false
	cb_grabar.enabled			= false
	cb_eliminar.enabled		= false
	em_cuotas.enabled		= false
	em_fecha.enabled			= false
	p_fec.enabled				= false
	em_descuento.enabled	= false
	rb_calc_pesos.enabled	= false
	rb_calc_uf.enabled		= false
elseif is_opcion='N' then
	cb_proceso.enabled		= true
	cb_grabar.enabled			= false
	cb_eliminar.enabled		= true
	em_cuotas.enabled		= true
	em_fecha.enabled			= true
	p_fec.enabled				= true
	em_descuento.enabled	= true
	rb_calc_pesos.enabled	= true
	rb_calc_uf.enabled		= true
	cb_terceros.enabled		= true
end if
if is_base<>'' and not isnull(is_base) and is_serie<>'' and not isnull(is_serie) and il_numero>0 and il_corr > 0 then
//	il_tot_reg								= dw_lista.retrieve(is_base,is_serie,il_numero,il_corr)
	if dw_lista.rowcount() > 0 then
		il_int_adm_peso					= dw_lista.getitemnumber(1,'interes_adm_pesos')
		st_val_interes_adm_pesos.text	= string(il_int_adm_peso,"###,###,###,###,###,##0")
		gs_moneda 							= '2'
//		il_corr								= dw_lista.getitemnumber(1,'correlativo')
		gd_uf_dia							= dw_lista.getitemnumber(1,'uf_dia')
		if isnull(gd_uf_dia) then
			SELECT		"TAB_UF"."VALOR_UF"  
			INTO 			:gd_uf_dia  
			FROM 		"TAB_UF"  
			WHERE 		"TAB_UF"."FECHA_UF" = :idt_fecha_hoy   ;
		end if
		st_val_interes_adm_uf.text		= string((il_int_adm_peso / gd_uf_dia),"###,###,###,###,##0.00")
		ll_por_descuento					= dw_lista.getitemnumber(1,'por_descuento')
		em_descuento.text				= string(ll_por_descuento,"##0")
		ll_interes_deuda					= dw_lista.getitemnumber(1,'interes_deuda_pesos')
		ll_cta_pag							= dw_lista.getitemnumber(1,'ctas_pagadas')
		ld_fecha								= date(dw_lista.getitemdatetime(1,'fecha_primera_cta'))
//		il_monto								= dw_lista.getitemnumber(1,'total_deuda')  // VER
		ll_monto_cuota						= dw_lista.getitemnumber(1,'valor_cuota')
		ll_dia									= day(ld_fecha)
		ll_mes								= month(ld_fecha)
		ll_anno								= year(ld_fecha)
		ld_factor_uf							= double(dw_lista.getitemnumber(1,'factor_aplicado'))
		st_factor.text						= string(ld_factor_uf,"###,##0.0000000")
		st_val_cuota_peso.text			= st_val_interes_total.text	//  string(long(st_val_interes_total.text) + long(st_val_interes_adm_pesos.text),"###,###,###,###,##0")
		st_val_cuota_uf.text				= st_val_interes_tot_total.text  // string(double(st_val_interes_tot_total.text) + double(st_val_interes_adm_uf.text),"###,###,###,###,##0.00")
		st_val_cuota_peso2.text			= string(ll_monto_cuota,"###,###,##0")
		st_val_cuota_uf2.text				= string(ll_monto_cuota / gd_uf_dia,"###,###,##0.0000")
		if gs_moneda = '1' then
			em_moneda.text				= 'PESO'
			rb_calc_pesos.checked		= true
			rb_calc_uf.checked			= false
		elseif gs_moneda = '2' then
			em_moneda.text				= 'U.F.'
			rb_calc_pesos.checked		= false
			rb_calc_uf.checked			= true
		end if
		for i=1 to ll_cta_original	//il_nro_ctas
			ll_new							= dw_detalle.insertrow(0)
			dw_detalle.setitem(ll_new,'monto',long(st_val_cuota_peso2.text))
			dw_detalle.setitem(ll_new,'uf',gd_uf_dia)
			if ll_dia > 28 and ll_mes =2 then
				ll_dia_aux					= 28
			elseif ll_dia > 30 and (ll_mes=4 or ll_mes=6 or ll_mes=9 or ll_mes=11) then
				ll_dia_aux					= 30
			else
				ll_dia_aux					= ll_dia
			end if
			ls_fecha							= string(ll_dia_aux,'00')+'/'+string(ll_mes,'00')+'/'+string(ll_anno,'0000')
			ld_fec_pago						= date(ls_fecha)
			dw_detalle.setitem(ll_new,'fecha',ld_fec_pago)
			ll_mes++
			if ll_mes>12 then
				ll_mes						= 1
				ll_anno++
			end if
			if i > ll_cta_pag then
				dw_detalle.setitem(ll_new,'estado','V')
			else
				dw_detalle.setitem(ll_new,'estado','C')
			end if
		next
		dw_detalle.object.t_titulo.text		= 'Detalle Cuotas Repactación Nº:   L - '+'R'+' - '+string(il_numero,"###,###,###,###,###")	//+'-'+string(il_corr,"###,###,###,###,###")
		dw_detalle.object.t_titulo_1.text	= 'CLIENTE  '+string(gi_rut,"###,###,###,##0")+' - '+gs_dv+'     '+trim(st_cliente.text)
		dw_detalle.object.t_titulo_2.text	= 'CONTRATO ORIGINAL:   '+is_base+' - '+is_serie+ ' - '+string(gi_numero,"###,###,###,###,##0")
		dw_detalle.object.t_titulo_3.text	= ''	//ls_titulo
		dw_detalle.object.t_valor_pie.text	= string(ldb_pie,'###,###,###,##0.0###')
		dw_detalle.object.t_fecha_pie.text	= string(ldt_fecha_pie,'dd/mm/yyyy')
		dw_detalle.accepttext()
	else
		il_cta_en_mora							= long(substr(1,7,is_string))
//		dw_lista.insertrow(0)
		dw_lista.setitem(1,'base','L')
	  	dw_lista.setitem(1,'serie','R')
		dw_lista.setitem(1,'numero',il_numero)
		dw_lista.setitem(1,'correlativo',il_corr)
		dw_lista.setitem(1,'base_original',is_base)
	  	dw_lista.setitem(1,'serie_original',is_serie)
		dw_lista.setitem(1,'numero_original',il_numero)
		dw_lista.setitem(1,'ctas_pagadas',0)
		dw_lista.setitem(1,'cuotas_pactadas', long(em_cuotas.text)) //il_nro_ctas)
		dw_lista.setitem(1,'cuotas_en_mora',il_nro_ctas)
		dw_lista.setitem(1,'estado_pago','V')
		dw_lista.setitem(1,'fecha_primera_cta',idt_fecha_hoy)
		dw_lista.setitem(1,'tipo_moneda',gs_moneda)
		dw_lista.setitem(1,'uf_dia',gd_uf_dia)
		dw_lista.setitem(1,'interes_adm_pesos',il_val_gtos_adm)		
		if gs_moneda = '1' then
			em_moneda.text				= 'PESO'
			rb_calc_pesos.checked		= true
			rb_calc_uf.checked			= false
		elseif gs_moneda = '2' then
			em_moneda.text				= 'U.F.'
			rb_calc_pesos.checked		= false
			rb_calc_uf.checked			= true
		end if
	end if
	dw_lista.accepttext()
	if trim(gs_opera)='1' and is_opcion='N' then
		cb_grabar.enabled					= false
		cb_imprimir.enabled				= true
	elseif trim(gs_opera)<>'1' and is_opcion='N' then
		cb_grabar.enabled					= false
		cb_imprimir.enabled				= false
	end if
	if trim(gs_opera)='1' then
		st_print_detalle.enabled			= true
	elseif trim(gs_opera)<>'1' then
		st_print_detalle.enabled			= false
		cb_imprimir.enabled				= false
	end if
end if
//Iniciar Valores Fecha y Ctas
id_fecha_prox_pago						= date(substr(1,6,is_string))
il_nro_ctas									= il_cta_ori_mora
st_fecha_prox_pago.text					= substr(1,6,is_string)
ll_dia_tope									= day(id_fecha_prox_pago)
ll_mes_tope									= month(id_fecha_prox_pago)
ll_anno_tope								= year(id_fecha_prox_pago)
ll_anno_tope								= ll_anno_tope + il_nro_ctas
ls_fecha_tope								= string(ll_dia_tope,'00')+'/'+string(ll_mes_tope,'00')+'/'+string(ll_anno_tope,'0000')
id_fecha_tope_pago						= date(ls_fecha_tope)
// Fin validar Valores Fecha y Ctas
gl_n_cuotas_pag 							= 0 
ld_fecha_tope_pago						= id_fecha_prox_pago
il_nro_ctas									= long(em_cuotas.text)
ld_fecha										= date(em_fecha.text)
il_por_descto								= long(em_descuento.text)
ld_fecha_hoy								= idt_fecha_hoy
//if (il_nro_ctas > 0 and il_nro_ctas <= 15 and not isnull(ld_fecha) and il_por_descto>=0 and il_por_descto<=100 then
if ((il_nro_ctas > 1 and il_nro_ctas <= 15) or (il_nro_ctas=18 or il_nro_ctas=24 or il_nro_ctas=30 or il_nro_ctas=36)) and not isnull(ld_fecha) and il_por_descto>=0 and il_por_descto<=100 then
	dw_detalle.reset()
	SELECT 	"INTERES_CUOTAS"."FACTOR_PESOS",   "INTERES_CUOTAS"."FACTOR_UF"  ,	"INTERES_CUOTAS"."INTERES_UF"	,	"INTERES_CUOTAS"."FACTOR_GC",	"INTERES_CUOTAS"."FACTOR_GC_PESO"
	INTO 		:ld_factor_peso,   					  :ld_factor_uf  ,						:ld_interes_uf		, 					:ld_factor_gc,						:ll_factor_gc_peso
	FROM 	"INTERES_CUOTAS"  
	WHERE 	"INTERES_CUOTAS"."NRO_CUOTAS" = :il_nro_ctas   
	USING		sqlca;
	
	st_factor.text=''
	st_factor.text=string(ld_factor_uf,"###,###,##0.0000000")
	if il_nro_ctas > 0 and em_fecha.text<>'00/00/0000' and not isnull(ld_fecha) then
		IF gs_rezago <> "S" THEN	 
			id_valor_cuota 					= Round((il_valor_cuota_m * gd_uf_dia), 0)
			ii_dmora 						= il_dias_mora_mant
			IF ii_dmora > 5 THEN 
				ii_mora_cobro 				= ii_dmora 
				id_im 							= gf_int_mora(id_tasa, ii_mora_cobro, id_valor_cuota)
				IF ii_dmora > 30 THEN 
					id_gc 						= id_valor_cuota * id_interes_uf //005 // PASA DE .1 A .05 10/1/2000
					ii_inc_gastos 			= 1
				END IF
			ELSE
				id_gc 							= 0
				id_im 							= 0
				ii_mora_cobro				= 0
				ii_inc_gastos				= 0
			END IF
		ELSE
			id_valor_cuota 					= 0
			ii_dmora 						= 0
			id_gc 								= 0
			id_im 								= 0
			ii_mora_cobro 					= 0
			ii_inc_gastos 					= 0
		END IF
		
		ll_cuotas_div						= ll_cuotas / ll_cant_mant	//12
		ii_cantidad 							= il_cta_ori_mora	//ll_cta_original
		if date(em_fecha.text) <> ld_fecha_hoy and not isnull(date(em_fecha.text)) and em_fecha.text<>'00/00/0000' then
			ld_fecha_hoy					= date(em_fecha.text)
		end if
		ll_dia									= day(ld_fecha_hoy)
		ll_mes								= month(ld_fecha_hoy)
		ll_anno								= year(ld_fecha_hoy)
		for i=1 to il_nro_ctas + 1
			if ll_dia > 28 and ll_mes =2 then
				ll_dia_aux					= 28
			elseif ll_dia > 30 and (ll_mes=4 or ll_mes=6 or ll_mes=9 or ll_mes=11) then
				ll_dia_aux					= 30
			else
				ll_dia_aux					= ll_dia
			end if
			ls_fecha							= string(ll_dia_aux,'00')+'/'+string(ll_mes,'00')+'/'+string(ll_anno,'0000')
			ld_fec_pago						= date(ls_fecha)
			if i<>1 then ll_mes++
			if ll_mes>12 then
				ll_mes						= 1
				ll_anno++
			end if
		next
		CHOOSE CASE is_base
			CASE 'O'
				SELECT	"OFERTA_V"."ANEXO_REPACTA"  				
				INTO 		:ls_anexo_rep  
				FROM 		"OFERTA_V",   
							"PAGO_OFERTA",   
							"CADENA"  
				WHERE  ( "OFERTA_V"."SERIE" = "PAGO_OFERTA"."SERIE" ) and  
						 ( "OFERTA_V"."NRO_OFERTA" = "PAGO_OFERTA"."NRO_OFERTA" ) and  
						 ( "OFERTA_V"."ULT_FOLIO" = "PAGO_OFERTA"."FOLIO" ) and  
						 ( "PAGO_OFERTA"."SERIE" = "CADENA"."SERIE" ) and  
						 ( "PAGO_OFERTA"."NRO_OFERTA" = "CADENA"."NUMERO" ) and  
						 (("OFERTA_V"."SERIE" = :is_serie ) AND  
						 ( "OFERTA_V"."NRO_OFERTA" = :ll_num_ori ) AND  
						 ( "CADENA"."CODIGO" = 'O' ) )   
				USING		sqlca;
				if sqlca.sqlcode=0 then
					if isnull(ls_anexo_rep) or ls_anexo_rep='' then
						ll_year					= long(st_ctas_mant_pago.text) * ll_cant_mant
//						ld_fecha_tope_pago	= date(string(string(day(ld_fecha_tope_pago),"00")+'/'+string(month(ld_fecha_tope_pago),"00")+'/'+string(ll_year,"0000")))
						DECLARE sp_prox_pago1 PROCEDURE FOR sp_calcular_vencimiento( :ld_fecha_tope_pago, :ll_year );
						EXECUTE sp_prox_pago1;
						if sqlca.sqlcode=0 then
							fetch sp_prox_pago1 into :ldt_fec_prox_pago1 ;
							close sp_prox_pago1;
							if not isnull(ldt_fec_prox_pago1) then
								ld_fecha_tope_pago 	= ldt_fec_prox_pago1
							end if
						end if
						
					end if
				end if		

			CASE 'C'
				SELECT	"CONTRATO"."ANEXO_REPACTA"  
				INTO 		:ls_anexo_rep  
				FROM 		"CONTRATO"  
				WHERE  ( "CONTRATO"."SERIE_C" = :is_serie ) AND  
						 ( "CONTRATO"."NRO_CONTRATO" = :ll_num_ori )   
				USING		sqlca;
				if sqlca.sqlcode=0 then
					if isnull(ls_anexo_rep) or ls_anexo_rep='' then
						ll_year					= long(st_ctas_mant_pago.text) * ll_cant_mant
						DECLARE sp_prox_pago2 PROCEDURE FOR sp_calcular_vencimiento( :ld_fecha_tope_pago, :ll_year );
						EXECUTE sp_prox_pago2;
						if sqlca.sqlcode=0 then
							fetch sp_prox_pago2 into :ldt_fec_prox_pago2 ;
							close sp_prox_pago2;
							if not isnull(ldt_fec_prox_pago2) then
								ld_fecha_tope_pago 	= ldt_fec_prox_pago2
							end if
						end if
//						ld_fecha_tope_pago	= date(string(string(day(ld_fecha_tope_pago),"00")+'/'+string(month(ld_fecha_tope_pago),"00")+'/'+string(ll_year,"0000")))
					end if
				end if		
		END CHOOSE
		
		ll_tot_real_cta							= il_cta_mora_o - il_mora_inicial
		ld_fecha_ini								= ld_fecha
		if ll_tot_real_cta < 2 then
			if ld_fecha_ini <= ld_fec_pago then
				do while ld_fecha_ini <= ld_fec_pago
					DECLARE sp_prox_pago PROCEDURE FOR sp_calcular_vencimiento( :ld_fecha_ini, :ll_cant_mant );
					EXECUTE sp_prox_pago;
					if sqlca.sqlcode=0 then
						fetch sp_prox_pago into :ldt_fec_prox_pago ;
						close sp_prox_pago;
						if not isnull(ldt_fec_prox_pago) then
							ld_fecha_ini 	= ldt_fec_prox_pago
							ii_cantidad++
							ll_sw_cta	++
						end if
					end if
				loop
			end if
			st_ctas_mant_pago.text				= string(ii_cantidad,"###,###,###,###,##0")
			gl_n_cuotas_pag						= ii_cantidad 
			ll_cta_diferencia						= ii_cantidad - il_mora_original_sin_mod //ll_cta_original
			if isnull(ll_cta_diferencia) or ll_cta_diferencia<=0 then ll_cta_diferencia=0
			if ll_cta_diferencia > 0 then
				ldb_valor_dif						= round(ll_cta_diferencia * il_valor_cuota_m,2)
//				st_val_interes_tot_total.text		= string(double(st_val_interes_tot_total.text) + ldb_valor_dif,"###,###,###,##0.00")	//
//				st_val_interes_total.text			= string(double(st_val_interes_tot_total.text) * gd_uf_dia,"###,###,###,###,##0" )	//
//				id_monto_uf							= double(st_val_interes_tot_total.text)		// NUEVO
//				il_monto								= long(st_val_interes_total.text)				// NUEVO
				il_nro_ctas							= il_nro_ctas + ll_cta_diferencia
				il_val_saldo_insoluto				= il_monto
			end if
			st_fecha_prox_pago.text		= string(ld_fecha_tope_pago,"dd/mm/yyyy")
			id_valor_cuota 					= Round((il_valor_cuota_m * gd_uf_dia), 0)
			ii_dmora 						= il_dias_mora_mant
			ll_total							= ii_cantidad * id_valor_cuota
			id_gc								= 0
			id_im 								= 0
			ii_mora_cobro 					= 0
			ii_inc_gastos 					= 0
			FOR i = 1 TO ii_cantidad 
				IF (ii_dmora - (i - 1) * 365) > 5 THEN 
					ii_mora_cobro			= (ii_dmora - (i - 1) * 365)
					id_im 						= id_im + gf_int_mora(id_tasa, ii_mora_cobro, id_valor_cuota)
					IF (ii_dmora - (i - 1) * 365) > 30 THEN 
						 id_gc 					= id_gc + (id_valor_cuota * id_interes_uf) //005// CAMBIO DE 10 A 5%
						 ii_inc_gastos 		= 1
					END IF
				END IF
			NEXT
		else
			if ld_fecha_ini <= ld_fec_pago then
				do while ld_fecha_ini <= ld_fec_pago
					DECLARE sp_prox_pago5 PROCEDURE FOR sp_calcular_vencimiento( :ld_fecha_ini, :ll_cant_mant );
					EXECUTE sp_prox_pago5;
					if sqlca.sqlcode=0 then
						fetch sp_prox_pago5 into :ldt_fec_prox_pago5 ;
						close sp_prox_pago5;
						if not isnull(ldt_fec_prox_pago5) then
							ld_fecha_ini 	= ldt_fec_prox_pago5
							ii_cantidad++
							ll_sw_cta	++
						end if
					end if
				loop
			end if
			
			st_ctas_mant_pago.text					= string(ii_cantidad,"###,###,###,###,##0")
			gl_n_cuotas_pag							= ii_cantidad 
			ll_cta_diferencia							= ii_cantidad - ll_cta_original
			if isnull(ll_cta_diferencia) or ll_cta_diferencia <= 0 then ll_cta_diferencia=0
			if ll_cta_diferencia > 0 then
				ldb_valor_dif							= round(ll_cta_diferencia * il_valor_cuota_m,2)
//				st_val_interes_tot_total.text			= string(double(st_val_interes_tot_total.text) + ldb_valor_dif,"###,###,###,##0.00")		//
//				st_val_interes_total.text				= string(double(st_val_interes_tot_total.text) * gd_uf_dia,"###,###,###,###,##0" )		//
//				id_monto_uf								= double(st_val_interes_tot_total.text)		// NUEVO
//				il_monto									= long(st_val_interes_total.text)				// NUEVO
				il_nro_ctas								= il_nro_ctas + ll_cta_diferencia
				il_val_saldo_insoluto					= il_monto
			end if
			st_fecha_prox_pago.text					= string(ld_fecha_tope_pago,"dd/mm/yyyy")
			id_valor_cuota 								= Round((il_valor_cuota_m * gd_uf_dia), 0)
			ii_dmora 									= il_dias_mora_mant
			ll_total										= ii_cantidad * id_valor_cuota
			id_gc											= 0
			id_im 											= 0
			ii_mora_cobro 								= 0
			ii_inc_gastos 								= 0
			FOR i = 1 TO ii_cantidad 
				IF (ii_dmora - (i - 1) * 365) > 1 THEN 
					ii_mora_cobro						= (ii_dmora - (i - 1) * 365)
					id_im 									= id_im + gf_int_mora(id_tasa, ii_mora_cobro, id_valor_cuota)
					IF (ii_dmora - (i - 1) * 365) > 30 THEN 
						 id_gc 							= id_gc + (id_valor_cuota * id_interes_uf) //005 // CAMBIO DE 10 A 5%
						 ii_inc_gastos 					= 1
					END IF
				END IF
			NEXT
		end if

		if ls_nopasa='S' then
			gd_val_interes								= gd_val_interes - ((gd_val_interes * il_por_descto) / 100)
			em_val_presente.text						= string(il_val_presente,"###,###,###,###,###,##0")
			em_gastos_adm.text						= string(il_val_gtos_adm,"###,###,###,###,###,##0")
			em_mora_cta.text							= string(il_val_mora_cta,"###,###,###,###,###,##0")
			il_monto										= il_val_saldo_insoluto - ll_pie_peso
			ldb_calc_monto								= round(il_monto / gd_uf_dia,2)
			il_monto										= round(ldb_calc_monto * gd_uf_dia,0)
			id_monto_uf									= round(il_monto / gd_uf_dia,2)
			il_monto_cuota								= round(il_monto / il_nro_ctas,2)
			if gs_moneda = '1' then //peso
				il_val_int_cta							= ll_factor_gc_peso
				ld_int_cta_uf							= il_val_int_cta / gd_uf_dia
				il_val_int									= (il_val_int_cta * il_nro_ctas) + gd_val_interes
				ld_val_int_uf							= gd_val_interes / gd_uf_dia
				st_val_interes_tot_total.text			= string(ld_val_int_uf,"###,###,###,###,##0.00")
				ld_total									= ((ld_total + il_val_int) - gd_val_interes)
			elseif gs_moneda = '2' then //uf
				il_val_int_cta							= ld_factor_gc * gd_uf_dia //005 * gd_uf_dia
				ld_int_cta_uf							= il_val_int_cta / gd_uf_dia
				il_val_int									= (il_val_int_cta * il_nro_ctas) + gd_val_interes
				ld_val_int_uf							= gd_val_interes / gd_uf_dia
				ld_total									= ((ld_total + il_val_int) - gd_val_interes)
			end if
			il_int_adm_peso							= il_val_int_cta * il_nro_ctas
			il_int_adm_uf								= il_int_adm_peso / gd_uf_dia
			st_val_interes_adm_pesos.text			= string(il_int_adm_peso,"###,###,###,###,##0")
			st_val_interes_adm_uf.text				= string(il_int_adm_uf,"###,###,###,###,##0.00")
			il_deuda										= ld_total
			if date(em_fecha.text) <> ld_fecha_hoy and not isnull(date(em_fecha.text)) and em_fecha.text<>'00/00/0000' then
				ld_fecha_hoy							= date(em_fecha.text)
			end if
			ll_dia											= day(ld_fecha_hoy)
			ll_mes										= month(ld_fecha_hoy)
			ll_anno										= year(ld_fecha_hoy)
			if gs_moneda='1' then
		//		ll_val_cta_inicial						= ((gd_val_deuda + gd_val_interes + il_int_adm_peso) / il_nro_ctas)
		//		ll_monto_cta							= round(((ll_val_cta_inicial * ld_factor_peso) + ll_val_cta_inicial),0)
		//		st_val_cuota_peso.text				= string(ll_monto_cta,"###,###,###,###,##0")
		//		st_val_cuota_uf.text					= string((ll_monto_cta / gd_uf_dia),"###,###,###,##0.00")
			elseif gs_moneda='2' then
				ll_val_cta_inicial						= ((gd_val_deuda + gd_val_interes + il_int_adm_peso) / il_nro_ctas)
				ll_monto_cta							= id_monto_uf * ld_factor_uf
				st_val_cuota_peso.text				= st_val_interes_total.text
				st_val_cuota_uf.text					= st_val_interes_tot_total.text
			end if
			st_val_cuota_peso.text					= st_val_interes_total.text //string((il_int_adm_peso + il_monto),"###,###,###,###,##0")
			st_val_cuota_uf.text						= st_val_interes_tot_total.text//string((il_int_adm_peso + il_monto)/gd_uf_dia,"###,###,###,###,##0.00")
			st_val_cuota_peso2.text					= string(long(st_val_cuota_peso.text) * ld_factor_uf,"###,###,##0")
			st_val_cuota_uf2.text						= string(long(st_val_cuota_peso2.text) / gd_uf_dia,"###,###,##0.0000")
			dw_detalle.reset()
			ll_cta_final									= long(em_cuotas.text)
			for i=1 to ll_cta_final
				ll_new									= dw_detalle.insertrow(0)
				dw_detalle.setitem(ll_new,'monto',long(st_val_cuota_peso2.text))
				dw_detalle.setitem(ll_new,'uf',gd_uf_dia)
				if ll_dia > 28 and ll_mes =2 then
					ll_dia_aux							= 28
				elseif ll_dia > 30 and (ll_mes=4 or ll_mes=6 or ll_mes=9 or ll_mes=11) then
					ll_dia_aux							= 30
				else
					ll_dia_aux							= ll_dia
				end if
				ls_fecha									= string(ll_dia_aux,'00')+'/'+string(ll_mes,'00')+'/'+string(ll_anno,'0000')
				ld_fec_pago								= date(ls_fecha)
				dw_detalle.setitem(ll_new,'fecha',ld_fec_pago)
				ll_mes++
				if ll_mes>12 then
					ll_mes								= 1
					ll_anno++
				end if
				dw_detalle.setitem(ll_new,'estado','V')
			next
			if dw_detalle.rowcount() > 0 then
				il_monto_cuota							= dw_detalle.getitemnumber(1,'monto')
				ld_tot_deuda_uf						= ((il_monto_cuota * il_nro_ctas) / gd_uf_dia)
				dw_lista.setitem(1,'total_deuda_uf',ld_tot_deuda_uf)
				dw_detalle.object.t_titulo.text		= 'Detalle Cuotas Repactación Nº:   L - '+'R'+' - '+string(il_numero,"###,###,###,###,###")	//+'-'+string(il_corr,"###,###,###,###,###")
				dw_detalle.object.t_titulo_1.text	= 'CLIENTE  '+string(gi_rut,"###,###,###,##0")+' - '+gs_dv+'     '+trim(st_cliente.text)
				dw_detalle.object.t_titulo_2.text	= 'CONTRATO ORIGINAL:   '+is_base+' - '+is_serie+ ' - '+string(gi_numero,"###,###,###,###,##0")
				dw_detalle.object.t_titulo_3.text	= ls_titulo
				dw_detalle.object.t_valor_pie.text	= string(ldb_pie,'###,###,###,##0.0###')
				dw_detalle.object.t_fecha_pie.text	= string(ldt_fecha_pie,'dd/mm/yyyy')
				dw_detalle.accepttext()
			end if
			if dw_lista.rowcount() > 0 then
				dw_lista.setitem(1,'valor_cuota',il_monto_cuota)
				il_cuota_uf								=  il_monto_cuota / gd_uf_dia
				dw_lista.setitem(1,'valor_cuota_uf',il_cuota_uf)
				il_deuda									= il_monto_cuota * il_nro_ctas
				dw_lista.setitem(1,'total_deuda',il_deuda)
				dw_lista.setitem(1,'cuotas_pactadas', long(em_cuotas.text))  //il_nro_ctas)
				dw_lista.setitem(1,'fecha_primera_cta',ld_fecha)
				dw_lista.setitem(1,'cuotas_en_mora',ii_cantidad)
				il_sw										= 1
			end if
		end if
	else
		messagebox("Advertencia","Falta ingresar Datos")
		em_cuotas.setfocus()
	end if
else
	if il_nro_ctas=0 then 
		messagebox("Advertencia","Falta Ingresar Cuotas a Pactar")
		em_cuotas.setfocus()
	elseif il_nro_ctas<2 then
		messagebox("Advertencia","La Reprogramación deben ser entre 2 a 15 cuotas o 18 - 24 - 30 - 36")
		em_cuotas.setfocus()
	elseif  (il_nro_ctas < 2) or (il_nro_ctas > 15 and il_nro_ctas < 18) or (il_nro_ctas > 18 and il_nro_ctas < 24) or (il_nro_ctas > 24 and il_nro_ctas < 30) or &
			(il_nro_ctas > 30 and il_nro_ctas < 36) or (il_nro_ctas > 36 ) then
		messagebox("Advertencia","La Reprogramación deben ser entre 2 a 15 cuotas o 18 - 24 - 30 - 36")
		em_cuotas.setfocus()
	elseif il_por_descto<0 then
		messagebox("Advertencia","Rango Descuento Inválido (0 a 100)")
		em_descuento.setfocus()
	elseif isnull(ld_fecha) then
		messagebox("Advertencia","Fecha Pago 1º Cuota Inválida")
		em_fecha.setfocus()
	end if
end if
end event

type st_1 from statictext within w_repacta_mora_mantencion_reprog
integer x = 91
integer y = 784
integer width = 494
integer height = 56
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Verdana"
long textcolor = 33554432
long backcolor = 67108864
string text = "Valor Presente"
alignment alignment = center!
boolean focusrectangle = false
end type

type cb_cerrar from commandbutton within w_repacta_mora_mantencion_reprog
integer x = 2149
integer y = 1888
integer width = 334
integer height = 92
integer taborder = 90
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "&Cerrar"
end type

event clicked;//long	ll_resp, ll_cant_print
//date	ldt_fecha_hoy
//
//ldt_fecha_hoy		= date(gdt_fec_sistema)
//if trim(gs_opera)='1' then
//	if il_sw=1 then
//		ll_resp = MessageBox("Modificación", "Desea Grabar los Cambios",Exclamation!, YesNo!, 2)
//		IF ll_resp = 1 THEN
//			wf_grabar()
//		end if
//		SELECT 		"TAB_UF"."VALOR_UF"  
//		INTO 			:gd_uf_dia  
//		FROM 		"TAB_UF"  
//		WHERE 		"TAB_UF"."FECHA_UF" = :ldt_fecha_hoy   ;
		close(w_repacta_mora_mantencion_reprog)
//	else
//		if isvalid(w_boletas_cuotas) then w_boletas_cuotas.cb_repactar.triggerevent(clicked!)
//		SELECT 		"TAB_UF"."VALOR_UF"  
//		INTO 			:gd_uf_dia  
//		FROM 		"TAB_UF"  
//		WHERE 		"TAB_UF"."FECHA_UF" = :ldt_fecha_hoy   ;
//		close(w_repacta_mora_mantencion_reprog)
//	end if
//else
//	if isvalid(w_boletas_cuotas) then w_boletas_cuotas.cb_repactar.triggerevent(clicked!)
//	SELECT 		"TAB_UF"."VALOR_UF"  
//	INTO 			:gd_uf_dia  
//	FROM 		"TAB_UF"  
//	WHERE 		"TAB_UF"."FECHA_UF" = :ldt_fecha_hoy   ;
//	close(w_repacta_mora_mantencion_reprog)
//end if
end event

type em_val_presente from statictext within w_repacta_mora_mantencion_reprog
integer x = 91
integer y = 844
integer width = 494
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type em_gastos_adm from statictext within w_repacta_mora_mantencion_reprog
integer x = 727
integer y = 844
integer width = 494
integer height = 88
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 16777215
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type dw_lista from datawindow within w_repacta_mora_mantencion_reprog
integer x = 55
integer y = 1080
integer width = 2446
integer height = 316
string dataobject = "dw_detalle_repacta_mantencion_l"
boolean hscrollbar = true
boolean border = false
end type

event clicked;String		ls_fecha,ls_columna
date		ldt_fecha_dia,ldt_fecha_pie,ldt_fecha_ctto,ldt_fecha_ini,ldt_fecha_fin,ldt_nulo
Double	ldb_pie

Setnull(ldt_nulo)
ls_columna					= dwo.name
ldt_fecha_dia				= date(gdt_fec_sistema)
if ls_columna='p_1' then
	ldb_pie					= dw_lista.getitemnumber(1,'pie')
	if ldb_pie > 0 and not isnull(ldb_pie) then
		ls_fecha					= string(date(dw_lista.getitemdatetime(1,'fecha_pie')))
		if isnull(ls_fecha) then ls_fecha = string(ldt_fecha_dia,"dd/mm/yyyy")
		if f_valida_fecha(ls_fecha)=-1 then 
			dw_lista.setitem(1,'fecha_pie',datetime(string(ldt_fecha_dia,gs_formato_fecha)))
			return
		end if
		OpenWithParm(w_calendar,ls_fecha)
		IF not isnull(Message.StringParm) THEN
			ls_fecha				= trim(Message.StringParm)
			dw_lista.setitem(1,'fecha_pie',date(ls_fecha))
		END IF
		dw_lista.setfocus()
		dw_lista.setcolumn('fecha_pie')
		ldt_fecha_pie	= date(dw_lista.getitemdatetime(1,'fecha_pie'))
		ldt_fecha_ctto	= date(dw_lista.getitemdatetime(1,'fecha_crea'))
		if not isnull(ldt_fecha_pie) then
			ldt_fecha_ini	= RelativeDate(ldt_fecha_ctto, -90)
			ldt_fecha_fin	= RelativeDate(ldt_fecha_ctto, 90)
			if ldt_fecha_pie < ldt_fecha_ini and ldt_fecha_pie > ldt_fecha_fin then
				messagebox("Advertencia","Fecha Pie Inválida o Fuera del Plazo permitido")
				dw_lista.Setitem(1,'fecha_pie',ldt_nulo)
				dw_lista.accepttext()
			end if
		end if
	else
		messagebox("Advertencia","Debe Ingresar Pie")
	end if
end if
end event

event itemchanged;String		ls_columna
date		ldt_fecha_pie,ldt_fecha_ctto,ldt_fecha_ini,ldt_fecha_fin,ldt_nulo
Double	ldb_pie,ldb_nulo

this.accepttext()
Setnull(ldt_nulo);Setnull(ldb_nulo)
ls_columna				= dwo.name
if ls_columna='fecha_pie' then
	ldt_fecha_pie		= date(data)
	ldt_fecha_ctto		= date(dw_lista.getitemdatetime(1,'fecha_crea'))
	if not isnull(ldt_fecha_pie) then
		ldt_fecha_ini	= RelativeDate(ldt_fecha_ctto, -90)
		ldt_fecha_fin	= RelativeDate(ldt_fecha_ctto, 90)
		if ldt_fecha_pie < ldt_fecha_ini and ldt_fecha_pie > ldt_fecha_fin then
			messagebox("Advertencia","Fecha Pie Inválida o Fuera del Plazo permitido")
			dw_lista.Setitem(1,'fecha_pie',ldt_nulo)
			dw_lista.accepttext()
		end if
	end if
elseif ls_columna='pie' then
	ldb_pie			= Double(data)
	if ldb_pie >= il_uf_inicial then //id_monto_uf then
		messagebox("Advertencia","Pie No debe ser Superior al Monto Adeudado")
		wf_eliminar_pie()
		cb_proceso.triggerevent(clicked!)
	else
		il_modif ++
		if ldb_pie=0 or isnull(ldb_pie) then
			wf_eliminar_pie()
			cb_proceso.triggerevent(clicked!)
		elseif ldb_pie>500 then
			messagebox("Advertencia","Recuerde Pie debe ser Ingresado en U.F.")
			wf_eliminar_pie()
			cb_proceso.triggerevent(clicked!)
		elseif il_modif > 0 then
			cb_proceso.triggerevent(clicked!)
		end if
	end if
end if
end event

event itemfocuschanged;Double	ldb_pie,ldb_nulo

dw_lista.accepttext()
Setnull(ldb_nulo)
ldb_pie			= dw_lista.getitemnumber(1,'pie')
if ldb_pie >= il_uf_inicial or ldb_pie > 500 or isnull(ldb_pie) or ldb_pie = 0 then
	dw_lista.Setitem(1,'pie',ldb_nulo)
	dw_lista.accepttext()
	cb_proceso.triggerevent(clicked!)
end if

end event

type cb_imprimir from commandbutton within w_repacta_mora_mantencion_reprog
integer x = 2149
integer y = 1660
integer width = 334
integer height = 92
integer taborder = 80
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "&Imprimir Ctto"
end type

event clicked;long		ll_corr, ll_cont_print , ll_resp=0
Double	ll_ctto,ldb_pie
String		ls_base_ori, ls_serie_ori,ls_base_repro,ls_serie_repro
Datetime	ldt_fecha_pie

if dw_detalle.rowcount() > 0 then
	ls_base_ori								= trim(st_base.text)
	ls_serie_ori								= trim(st_serie.text)
	ls_base_repro							= dw_lista.getitemstring(1,'base')
	ls_serie_repro							= dw_lista.getitemstring(1,'serie')
	ll_ctto										= Double(st_numero.text)
	il_numero								= dw_lista.getitemnumber(1,'numero')
	ldb_pie									= dw_lista.getitemnumber(1,'pie')
	ldt_fecha_pie							= dw_lista.getitemdatetime(1,'fecha_pie')
	dw_detalle.object.t_titulo.text		= 'Detalle Cuotas Repactación Nº:   '+ls_base_repro+' - '+ls_serie_repro+' - '+string(il_numero,"###,###,###,###,###")	//+'-'+string(il_corr,"###,###,###,###,###")
	dw_detalle.object.t_titulo_1.text	= 'CLIENTE  '+string(gi_rut,"###,###,###,##0")+' - '+gs_dv+'     '+trim(st_cliente.text)
	dw_detalle.object.t_titulo_2.text	= 'CONTRATO ORIGINAL:   '+is_base+' - '+is_serie+ ' - '+string(gi_numero,"###,###,###,###,##0")
	dw_detalle.object.t_titulo_3.text	= ''	//ls_titulo
	if not isnull(ldb_pie) and ldb_pie > 0 then
		dw_detalle.object.t_valor_pie.text	= string(ldb_pie,'###,###,###,##0.0###')
		dw_detalle.object.t_fecha_pie.text	= string(ldt_fecha_pie,'dd/mm/yyyy')
	end if
	dw_detalle.accepttext()
	if isvalid(w_detalle_repactacion_por_contrato) then
		if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
			SELECT 	sum("REPACTA_MANTENCION"."CANT_PRINT")
			INTO 		:ll_cont_print  
			FROM 	"REPACTA_MANTENCION"  
			WHERE ( "REPACTA_MANTENCION"."BASE_ORIGINAL" = :ls_base_ori ) AND  
						( "REPACTA_MANTENCION"."SERIE_ORIGINAL" = :ls_serie_ori ) AND  
						( "REPACTA_MANTENCION"."NUMERO" = :il_numero )
			Using	sqlca;
		else
			SELECT 	sum("REPACTA_MANTENCION_L"."CANT_PRINT")
			INTO 		:ll_cont_print  
			FROM 	"REPACTA_MANTENCION_L"  
			WHERE ( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :ls_base_ori ) AND  
						( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :ls_serie_ori ) AND  
						( "REPACTA_MANTENCION_L"."NUMERO" = :il_numero )
			Using	sqlca;
		end if
	else
		SELECT 	sum("REPACTA_MANTENCION_L"."CANT_PRINT")
		INTO 		:ll_cont_print  
		FROM 	"REPACTA_MANTENCION_L"  
		WHERE ( "REPACTA_MANTENCION_L"."BASE_ORIGINAL" = :ls_base_ori ) AND  
					( "REPACTA_MANTENCION_L"."SERIE_ORIGINAL" = :ls_serie_ori ) AND  
					( "REPACTA_MANTENCION_L"."NUMERO" = :il_numero )
		Using	sqlca;
	end if
	if sqlca.sqlcode=0 then
		if ll_cont_print>0 then
			ll_resp	= messagebox("Advertencia","Contrato ya Impreso, desea imprimir nuevamente",Exclamation!,YesNo!,2)
			if ll_resp=1 then
				ll_cont_print ++
			else
				ll_resp=0
			end if
		elseif ll_cont_print=0 then
			ll_cont_print	= 1
			ll_resp	= messagebox("Advertencia","Imprimir Contrato",Exclamation!,YesNo!,2)
			if ll_resp<>1 then ll_resp=0
		elseif isnull(ll_cont_print) then
			messagebox("Advertencia","Debe Grabar antes de Imprimir Contrato")
			ll_resp	= 2
		end if
	elseif sqlca.sqlcode=100 then
		messagebox("Advertencia","Debe Grabar antes de Imprimir Contrato")
		ll_resp	= 2
	end if 
	if ll_resp=1 then
		if isvalid(w_detalle_repactacion_por_contrato) then
			if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
				if is_base='C' and gs_conexion="Parque El Prado" then //sigue igual solo cambios
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_stgo_todos'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_stgo_todos_tercero'
					end if
				elseif is_base='C' and gs_conexion="Parque La Foresta" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_serena_todos'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_serena_todos_tercero'
					end if
				elseif is_base='O' and gs_conexion="Parque La Foresta" and ll_num_ori >= 4251 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_mayor_4251'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_mayor_4251_3'
					end if
				elseif is_base='O' and gs_conexion="Parque La Foresta" and ll_num_ori < 4251 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_menor_4251'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_menor_4251_3'
					end if
				elseif is_base='O' and gs_conexion="Parque El Prado" and ll_num_ori >= 21506 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_mayor_21506'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_mayor_21506_3'
					end if
				elseif is_base='O' and gs_conexion="Parque El Prado" and ll_num_ori < 21506 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_menor_21506'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_menor_21506_3'
					end if
				elseif is_base='C' and gs_conexion="Parque Santiago" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_santiago'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_santiago'
					end if
				elseif is_base='C' and gs_conexion="Parque Concepción" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_conce'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_conce'
					end if
				end if
			else
				if is_base='C' and gs_conexion="Parque El Prado" then //sigue igual solo cambios
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_stgo_todos_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_stgo_todos_tercero_l'
					end if
				elseif is_base='C' and gs_conexion="Parque La Foresta" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_serena_todos_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_serena_todos_tercero_l'
					end if
				elseif is_base='O' and gs_conexion="Parque La Foresta" and ll_num_ori >= 4251 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_mayor_4251_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_mayor_4251_3_l'
					end if
				elseif is_base='O' and gs_conexion="Parque La Foresta" and ll_num_ori < 4251 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_menor_4251_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_serena_menor_4251_3_l'
					end if
				elseif is_base='O' and gs_conexion="Parque El Prado" and ll_num_ori >= 21506 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_mayor_21506_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_mayor_21506_3_l'
					end if
				elseif is_base='O' and gs_conexion="Parque El Prado" and ll_num_ori < 21506 then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_menor_21506_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_o_stgo_menor_21506_3_l'
					end if
				elseif is_base='C' and gs_conexion="Parque Santiago" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_santiago_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_santiago_l'
					end if
				elseif is_base='C' and gs_conexion="Parque Concepción" then
					if cb_terceros.italic=false then
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_conce_l'
					else
						dw_print_ctto.dataobject	= 'dw_print_contrato_c_parque_conce_l'
					end if
				end if
			end if
		end if
		dw_print_ctto.settransobject(sqlca)
		if dw_print_ctto.retrieve(ls_base_ori,ls_serie_ori,il_numero)=0 then //ll_num_ori
			messagebox("Advertencia","Debe Grabar antes de Imprimir Contrato")
		else
//			printsetup()
//			dw_print_ctto.print()
//			dw_print_ctto.print()
//			long ll_ret
			if f_printdlg2(dw_print_ctto,gstr_print,w_repacta_mora_mantencion_reprog)=0 then
				dw_print_ctto.print()
				dw_detalle.print()
				dw_detalle.print()
				if isvalid(w_detalle_repactacion_por_contrato) then
					if w_detalle_repactacion_por_contrato.cbx_reprog_historica.checked=true then
						UPDATE "REPACTA_MANTENCION"  
						SET "CANT_PRINT" = :ll_cont_print  
						WHERE ( "REPACTA_MANTENCION"."BASE" = :ls_base_repro ) AND  
								( "REPACTA_MANTENCION"."SERIE" = :ls_serie_repro) AND  
								( "REPACTA_MANTENCION"."NUMERO" = :il_numero )   
						Using	sqlca ;
					else
						UPDATE "REPACTA_MANTENCION_L"  
						SET "CANT_PRINT" = :ll_cont_print  
						WHERE ( "REPACTA_MANTENCION_L"."BASE" = :ls_base_repro ) AND  
								( "REPACTA_MANTENCION_L"."SERIE" = :ls_serie_repro ) AND  
								( "REPACTA_MANTENCION_L"."NUMERO" = :il_numero )   
						Using	sqlca ;
					end if
				else
					UPDATE "REPACTA_MANTENCION_L"  
					SET "CANT_PRINT" = :ll_cont_print  
					WHERE ( "REPACTA_MANTENCION_L"."BASE" = :ls_base_repro ) AND  
							( "REPACTA_MANTENCION_L"."SERIE" = :ls_serie_repro ) AND  
							( "REPACTA_MANTENCION_L"."NUMERO" = :il_numero )   
					Using	sqlca ;
				end if
				if sqlca.sqlcode=0 then
					commit;
				else
					rollback;
				end if
				cb_cerrar.triggerevent(clicked!)
			else
				close(w_repacta_mora_mantencion_reprog)
			end if
		end if
	elseif ll_resp=0 then
		cb_cerrar.triggerevent(clicked!)
	end if
end if
end event

type gb_2 from groupbox within w_repacta_mora_mantencion_reprog
integer x = 37
integer y = 1460
integer width = 2496
integer height = 572
integer textsize = -11
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Valores Reprogramación"
end type

type cb_eliminar from commandbutton within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 2149
integer y = 1736
integer width = 334
integer height = 92
integer taborder = 70
integer textsize = -10
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "&Eliminar"
end type

event clicked;//string	ls_base_aux
//integer	ll_resp
//Long		ll_count_reg
//Date		ld_fecha
//
//if dw_lista.rowcount()>0 then
//	ld_fecha	= idt_fecha_hoy
//	SELECT	"REPACTA_MANTENCION"."BASE"  
//	INTO 		:ls_base_aux  
//	FROM 		"REPACTA_MANTENCION"  
//	WHERE 	( "REPACTA_MANTENCION"."BASE_ORIGINAL" = :is_base ) AND  
//				( "REPACTA_MANTENCION"."SERIE_ORIGINAL" = :is_serie ) AND  
//				( "REPACTA_MANTENCION"."NUMERO_ORIGINAL" = :il_numero )   
//	Using		sqlca ;
//	if sqlca.sqlcode=0 then
//		ll_resp = MessageBox("Eliminar", "Está Seguro Eliminar Repactación Nº "+'R-'+is_serie+'-'+string(il_numero,"###,###,###,###,###"), &
//				Exclamation!, YesNo!, 2)
//		IF ll_resp = 1 THEN
//			SELECT sum("REPACTA_MANTENCION_DETALLE"."CORRELATIVO")
//			INTO :ll_count_reg
//			FROM "REPACTA_MANTENCION_DETALLE"  
//			WHERE ( "REPACTA_MANTENCION_DETALLE"."BASE" = 'R' ) AND  
//					( "REPACTA_MANTENCION_DETALLE"."SERIE" = :is_serie) AND  
//					( "REPACTA_MANTENCION_DETALLE"."NUMERO" = :il_numero )   
//			Using	sqlca ;
//			if sqlca.sqlcode=0 then
//				if ll_count_reg>0 then
//					messagebox("Advertencia","No Puede Eliminar Repactación, ya existen cuotas pagadas")
//				else
//					UPDATE "REPACTA_MANTENCION"
//					SET    "ESTADO_PAGO" = 'N'
//					WHERE ( "REPACTA_MANTENCION"."BASE_ORIGINAL" = :is_base ) AND  
//							( "REPACTA_MANTENCION"."SERIE_ORIGINAL" = :is_serie ) AND  
//							( "REPACTA_MANTENCION"."NUMERO_ORIGINAL" = :il_numero )   
//					Using	sqlca ;
//					if sqlca.sqlcode=0 then
//						commit;
//						UPDATE "CADENA"
//						SET    "ESTADO" = 'N',
//								 "FECHA_RES" = :ld_fecha
//						WHERE ( "CADENA"."CODIGO" = 'R' ) AND  
//								( "CADENA"."SERIE" = :is_serie) AND  
//								( "CADENA"."NUMERO" = :il_numero )   
//						Using	sqlca ;
//						if sqlca.sqlcode=0 then
//							commit;
//							messagebox("Eliminar","Repactación  L - "+'R'+' - '+string(il_numero,"###,###,###,###,##0")+"  Eliminada")
//						else
//							rollback;
//						end if
//						dw_lista.reset()
//						dw_detalle.reset()
//						em_cuotas.text	= ''
//						em_fecha.text	= string(idt_fecha_hoy)
//					else
//						rollback;
//					end if
//				end if
//			end if
//		END IF
//	end if
//end if
end event

type gb_1 from groupbox within w_repacta_mora_mantencion_reprog
integer x = 37
integer y = 280
integer width = 2496
integer height = 372
integer textsize = -11
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Ingrese Datos"
end type

type dw_detalle from datawindow within w_repacta_mora_mantencion_reprog
boolean visible = false
integer x = 960
integer y = 2112
integer width = 617
integer height = 356
string dataobject = "dwe_cuotas_pactadas"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type

event rowfocuschanged;if this.getrow() > 0 then
	This.SelectRow(0, FALSE)
	This.SelectRow(this.getrow(), TRUE)
end if
end event

event clicked;if row > 0 then
	This.SelectRow(0, FALSE)
	This.SelectRow(row, TRUE)
end if
end event

type gb_3 from groupbox within w_repacta_mora_mantencion_reprog
integer x = 37
integer y = 668
integer width = 2496
integer height = 780
integer textsize = -11
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
long backcolor = 79741120
string text = "Deuda Actual"
end type

