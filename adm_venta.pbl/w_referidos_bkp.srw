forward
global type w_referidos_bkp from window
end type
type st_3 from statictext within w_referidos_bkp
end type
type dw_agente_referidos from datawindow within w_referidos_bkp
end type
type cb_imprimir_resumen from commandbutton within w_referidos_bkp
end type
type dw_resumen from datawindow within w_referidos_bkp
end type
type st_2 from statictext within w_referidos_bkp
end type
type cb_exportar from commandbutton within w_referidos_bkp
end type
type cb_2 from commandbutton within w_referidos_bkp
end type
type cb_3 from commandbutton within w_referidos_bkp
end type
type cb_4 from commandbutton within w_referidos_bkp
end type
type cb_limpiar from commandbutton within w_referidos_bkp
end type
type st_1 from statictext within w_referidos_bkp
end type
type em_rut from editmask within w_referidos_bkp
end type
type dw_referidos from datawindow within w_referidos_bkp
end type
type dw_agente from datawindow within w_referidos_bkp
end type
type cb_cerrar from commandbutton within w_referidos_bkp
end type
type st_rut from statictext within w_referidos_bkp
end type
type gb_2 from groupbox within w_referidos_bkp
end type
end forward
...
global w_referidos_bkp w_referidos_bkp

type variables
long il_row
end variables

on w_referidos_bkp.create
...
ll_rut 						= long(em_rut.text)
	...
	SELECT  "AGENTES"."COD_AGE",	"AGENTES"."NOMBRE",	"AGENTES"."A_PATERNO",	"AGENTES"."A_MATERNO",	"AGENTES"."FECHA_INI",	"AGENTES"."FECHA_FIN", "AGENTES"."COD_SUP", "AGENTES"."ESTADO"  
	FROM	  "AGENTES"
	...
	if sqlca.sqlcode=0 then
		...
		fetch xage INTO	:ls_cod_age,	:ls_nombre_agente,	:ls_ap_pat_agente,	:ls_ap_mat_agente,	:ld_fecha_ingreso,	:ld_fecha_termino,	:ls_cod_sup,	:ls_estado_age;		  	
			if not isnull(ls_cod_age) and ls_cod_age<>'' then
				ll_new	=	dw_agente_referidos.insertrow(0)
				dw_agente_referidos.setitem(ll_new,'cod_age', ls_cod_age)
				dw_agente_referidos.getchild('cod_age',idw_detalle)
//				idw_detalle.reset()
				idw_detalle.insertrow(0)
//				dw_agente_referidos.scrolltorow(ll_new)
//				dw_agente_referidos.accepttext()
//				dw_agente_referidos.retrieve(ll_rut)
				dw_agente.retrieve(ll_rut)
				dw_resumen.retrieve(ls_cod_age)
				dw_resumen.object.usuario.text	 = gs_user
				if dw_referidos.retrieve(ls_cod_age) = 0 then
