forward
global type w_ver_agenda from window
end type
type cb_limpiar from commandbutton within w_ver_agenda
end type
type pb_filtrar from picturebutton within w_ver_agenda
end type
type pb_buscar from picturebutton within w_ver_agenda
end type
type em_ctto from editmask within w_ver_agenda
end type
type st_3 from statictext within w_ver_agenda
end type
type dw_evaluacion from datawindow within w_ver_agenda
end type
type dw_notificacion from datawindow within w_ver_agenda
end type
type dw_ruta from datawindow within w_ver_agenda
end type
type cb_ruta from commandbutton within w_ver_agenda
end type
type cb_grabar_formula from commandbutton within w_ver_agenda
end type
type cb_geo from commandbutton within w_ver_agenda
end type
type cb_historico from commandbutton within w_ver_agenda
end type
type cb_ctacte from commandbutton within w_ver_agenda
end type
type cb_imprimir from commandbutton within w_ver_agenda
end type
type cb_exportar from commandbutton within w_ver_agenda
end type
type cb_filtrar from commandbutton within w_ver_agenda
end type
type cb_ordenar from commandbutton within w_ver_agenda
end type
type st_1 from statictext within w_ver_agenda
end type
type st_2 from statictext within w_ver_agenda
end type
type em_fec_ini from editmask within w_ver_agenda
end type
type em_fec_fin from editmask within w_ver_agenda
end type
type pb_aceptar from picturebutton within w_ver_agenda
end type
type p_1 from picture within w_ver_agenda
end type
type p_2 from picture within w_ver_agenda
end type
type cb_cerrar from commandbutton within w_ver_agenda
end type
type dw_lista from datawindow within w_ver_agenda
end type
type gb_1 from groupbox within w_ver_agenda
end type
type gb_2 from groupbox within w_ver_agenda
end type
type dw_filtro from datawindow within w_ver_agenda
end type
end forward
...
global w_ver_agenda w_ver_agenda
...
event open;String	ls_usuario,ls_nombre,ls_titulo,ls_ciudad
Long	ll_fila
...
event clicked;String		ls_nulo,ls_ciudad
Long		ll_fila
...
event clicked;Long		ll_rut,ll_indi,ll_tot_reg,ll_new,ll_cod_parque,ll_cod_par_not,ll_new_not,ll_new_eva,ll_estado,ll_pasa,ll_fila
String		ls_base,ls_serie,ls_dv,ls_nom,ls_ap_pat,ls_ap_mat,ls_dir,ls_nro_p,ls_depto_p,ls_block_p,ls_pob,ls_sec,ls_fono_p,ls_fono_c,ls_nom_comp,ls_dir_comp,ls_descrip_comuna,ls_fec_palabra,&
			ls_nom_usuario,ls_tipo_via_descrip,ls_area,ls_sector,ls_sepultura,ls_glosa_tele,ls_glosa_pub,ls_celular
Date		ld_fecha_hoy
Double	ll_numero
		...
		for ll_indi=1 to ll_tot_reg
			...
			if ll_estado=0 then
				...
				WHERE	"AT_GESTION"."BASE" = "CADENA"."CODIGO" and  
							...
							"AT_GESTION"."USUARIO_ASIGNADO" = :gs_user AND // nuevo JP
...
integer height = 1500
integer taborder = 60
string dataobject = "dw_lista_agenda_visita_terreno"
...
Long		ll_status,ll_tot_reg,ll_indi
...
if row>0 then
	...
	elseif ls_columna='t_mayor_12' or ls_columna='t_mayor_12_2' then
		...
		dw_lista.filter()
		dw_lista.accepttext()
	elseif ls_columna='t_marcar' then
		if dw_lista.object.t_marcar.text='Marcar Todo' then
			ll_tot_reg	= dw_lista.rowcount()
			for ll_indi=1 to ll_tot_reg
				dw_lista.setitem(ll_indi,'estado_reg',0)
			next
			dw_lista.object.t_marcar.text='Desmarcar Todo'
			dw_lista.accepttext()
		elseif dw_lista.object.t_marcar.text='Desmarcar Todo' then
			ll_tot_reg	= dw_lista.rowcount()
			for ll_indi=1 to ll_tot_reg
				dw_lista.setitem(ll_indi,'estado_reg',1)
			next
			dw_lista.object.t_marcar.text='Marcar Todo'
			dw_lista.accepttext()
		end if
	end if
	dw_lista.SORT()
