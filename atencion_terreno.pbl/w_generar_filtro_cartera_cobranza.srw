forward
global type w_generar_filtro_cartera_cobranza from window
end type
type pb_buscar from picturebutton within w_generar_filtro_cartera_cobranza
end type
type em_ctto from editmask within w_generar_filtro_cartera_cobranza
end type
type st_1 from statictext within w_generar_filtro_cartera_cobranza
end type
type cbx_parque from checkbox within w_generar_filtro_cartera_cobranza
end type
type cb_1 from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_actualizar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_seleccionar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_mostrar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_limpiar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type st_buscar from statictext within w_generar_filtro_cartera_cobranza
end type
type cb_grabar_formula from picturebutton within w_generar_filtro_cartera_cobranza
end type
type pb_filtrar from picturebutton within w_generar_filtro_cartera_cobranza
end type
type cb_geo from commandbutton within w_generar_filtro_cartera_cobranza
end type
type tab_cobranza from uo_atencion_terreno within w_generar_filtro_cartera_cobranza
end type
type tab_cobranza from uo_atencion_terreno within w_generar_filtro_cartera_cobranza
end type
type st_porc from statictext within w_generar_filtro_cartera_cobranza
end type
type hpb_1 from hprogressbar within w_generar_filtro_cartera_cobranza
end type
type st_fondo from statictext within w_generar_filtro_cartera_cobranza
end type
type cb_ordenar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_filtrar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_imprimir from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_historico from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_ctacte from commandbutton within w_generar_filtro_cartera_cobranza
end type
type cb_asignar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type pb_ok from picturebutton within w_generar_filtro_cartera_cobranza
end type
type dw_seleccionar from datawindow within w_generar_filtro_cartera_cobranza
end type
type cb_cerrar from commandbutton within w_generar_filtro_cartera_cobranza
end type
type dw_lista from datawindow within w_generar_filtro_cartera_cobranza
end type
type gb_2 from groupbox within w_generar_filtro_cartera_cobranza
end type
type gb_4 from groupbox within w_generar_filtro_cartera_cobranza
end type
type dw_filtro from datawindow within w_generar_filtro_cartera_cobranza
end type
type gb_1 from groupbox within w_generar_filtro_cartera_cobranza
end type
end forward
...
string title = "Asignar Cartera Cobranza Atención Terreno"
...
global w_generar_filtro_cartera_cobranza w_generar_filtro_cartera_cobranza

type variables
Datetime	idt_fecha_hoy
Long		il_row,il_row_aux
Double	id_factor_gc
string		is_tipo_cob='CM'
end variables
...
for ll_indi=1 to ll_tot_reg
	...
	ls_estado_llamada			= dw_lista.getitemstring(ll_indi,'estado_llamada')
...
on w_generar_filtro_cartera_cobranza.create
...
on w_generar_filtro_cartera_cobranza.destroy
...
String		ls_ciudad,ls_pasa

SetPointer(HourGlass!)
dw_filtro.reset()
gs_formula											= ""
if gl_proceso=0 then
	messagebox("Advertencia","Usuario No Autorizado Para Asignar Cartera Cobranza")
	close(w_generar_filtro_cartera_cobranza)
else
	...
	gf_centrar(w_generar_filtro_cartera_cobranza)
	dw_lista.dataobject							= 'dw_lista_filtro_asigna_cart_terr_parq'
	dw_lista.settransobject(sqlca)
	dw_seleccionar.dataobject					= 'dwe_seleccionar_filtro_cobranza'
	...
	dw_seleccionar.object.parque_t.visible	= true
	dw_seleccionar.object.parque.visible		= true
	dw_seleccionar.object.t_tipo_cob.x		= 750
	dw_seleccionar.object.tipo_cob.x			= 1102
	pb_ok.x											= 1678
	gb_4.width										= 1819
	dw_seleccionar.width							= 1614
	...
	SELECT	"ENCARGADOS"."TIPO_COBRANZA_TERRENO"  
	INTO		:gs_tipo_cob  
	FROM		"ENCARGADOS"  
	WHERE	"ENCARGADOS"."CODIGO_USUARIO" = :gs_user
	USING	sqlca;
	if sqlca.sqlcode=0 then
		if not isnull(gs_tipo_cob) and gs_tipo_cob<>'' then
			ls_pasa														= 'S'
		else
			ls_pasa														= 'N'
		end if
	...
	if ls_pasa='S' then		
		dw_seleccionar.setitem(ll_new,'tipo_cob',gs_tipo_cob)
		dw_seleccionar.object.tipo_cob.protect					= 1
	else
		dw_seleccionar.setitem(ll_new,'tipo_cob','T')
	end if
...
if ll_numero>0 and ll_tot_reg>0 then
	ls_string							= "vista_at_terreno_cobranza_numero = "+string(ll_numero)
...
elseif cbx_parque.checked = false then
	...
	dw_seleccionar.object.parque_t.visible	= false
	dw_seleccionar.object.parque.visible		= false
	dw_seleccionar.object.t_tipo_cob.x		= 14
	dw_seleccionar.object.tipo_cob.x			= 375
	pb_ok.x											= 965
	gb_4.width										= 1093
	dw_seleccionar.width							= 901
...
if il_row>0 and dw_lista.rowcount()>0 then
	setnull(ls_nulo)
	gi_rut 																	= dw_lista.GetItemNumber(il_row, "vista_at_terreno_cobranza_rut")
	gs_base																	= dw_lista.getitemstring(il_row, "vista_at_terreno_cobranza_base")
	gs_serie																	= dw_lista.getitemstring(il_row, "vista_at_terreno_cobranza_serie")
	gi_numero																= dw_lista.getitemnumber(il_row, "vista_at_terreno_cobranza_numero")
...
w_generar_filtro_cartera_cobranza.triggerevent(open!)
dw_lista.reset()
end event
...
if not isnull(ls_ciudad) and ls_ciudad<>'' and not isnull(ls_comuna) and ls_comuna<>'' then
	ls_filtro				= "vista_at_terreno_cobranza_ciudad='"+ls_ciudad+"' and vista_at_terreno_cobranza_comuna='"+ls_comuna+"'"
//	if gs_formula<>'' and not isnull(gs_formula) then
//		ls_filtro			= gs_formula+" and vista_at_terreno_cobranza_ciudad='"+ls_ciudad+"' and vista_at_terreno_cobranza_comuna='"+ls_comuna+"'"
//	else
//	end if
	...
	if isnull(ls_ciudad) or ls_ciudad='' then
		...
		dw_filtro.setfocus()
		dw_filtro.setcolumn('vista_at_terreno_cobranza_ciudad')
	elseif isnull(ls_comuna) or ls_comuna='' then
		messagebox("Advertencia","Debe Seleccionar Comuna")
		dw_filtro.setfocus()
		dw_filtro.setcolumn('vista_at_terreno_cobranza_comuna')
	end if
end if
tab_cobranza.visible	= false
end event
...
event clicked;string ls_tipo_c,ls_tipo_new,ls_pasa='S'
long ll_fila,ll_tot_reg,ll_indi
...
if ll_fila=0 or isnull(ll_fila) then
	...
	gs_tipo_cob							= dw_seleccionar.getitemstring(1,'tipo_cob')
	if gs_tipo_cob='T' then
		dw_lista.SETfilter("estado_reg = 0")
		dw_lista.filter()
		ll_tot_reg						= dw_lista.rowcount()
		if ll_tot_reg>0 then
			ls_tipo_c						= dw_lista.getitemstring(1,'vista_at_terreno_cobranza_tipo_cobranza')
			gs_tipo_cob					= ls_tipo_c
			if ls_pasa='S' then
				for ll_indi=1 to ll_tot_reg
					ls_tipo_new			= dw_lista.getitemstring(ll_indi,'vista_at_terreno_cobranza_tipo_cobranza')
					if ls_tipo_c<>ls_tipo_new then
						ls_pasa			= 'N'
					end if
				next
			end if
...
if ls_pasa='S' then	
	...
	messagebox("Advertencia","Los Contratos Seleccionado No Corresponden al Mismo Tipo de Cobranza")
end if
end event
...
String		ls_base,ls_serie,ls_usuario_asignado,ls_tipo_cobr,ls_tipo_cobr1,ls_tipo_cobr2,ls_tipo_cobr3,ls_tipo_cob_lista,ls_pasa
Double	ll_numero
...
ls_tipo_cobr								= Trim(dw_seleccionar.getitemstring(1,'tipo_cob'))
if cbx_parque.checked=true then
	ll_parque								= dw_seleccionar.getitemnumber(1,'parque')
	dw_lista.dataobject				= 'dw_lista_filtro_asigna_cart_terr_parq'
	dw_lista.settransobject(sqlca)
	if not isnull(ll_parque) and ll_parque>0 and not isnull(ls_tipo_cobr) and ls_tipo_cobr<>'' then
		ls_pasa							= 'S'
	else
		ls_pasa							= 'N'
		...
		elseif isnull(ls_tipo_cobr) or ls_tipo_cobr='' then
			messagebox("Advertencia","Debe Ingresar Tipo Cobranza Terreno")
			dw_seleccionar.setfocus()
			dw_seleccionar.Setcolumn('tipo_cob')
		end if
	end if
elseif cbx_parque.checked=false then
	dw_lista.dataobject				= 'dw_lista_filtro_asigna_cart_terr_cob'
	dw_lista.settransobject(sqlca)
...
if ls_pasa='S' then
	if ls_tipo_cobr='T' then
		ls_tipo_cobr1					= 'C'
		ls_tipo_cobr2					= 'M'
		ls_tipo_cobr3					= 'E'
	elseif ls_tipo_cobr='C' then
		ls_tipo_cobr1					= 'C'
		ls_tipo_cobr2					= 'C'
		ls_tipo_cobr3					= 'C'
	elseif ls_tipo_cobr='M' then
		ls_tipo_cobr1					= 'M'
		ls_tipo_cobr2					= 'M'
		ls_tipo_cobr3					= 'M'
	elseif ls_tipo_cobr='E' then
		ls_tipo_cobr1					= 'E'
		ls_tipo_cobr2					= 'E'
		ls_tipo_cobr3					= 'E'	
	end if
	...
	if cbx_parque.checked=true then
		ll_tot_reg							= dw_lista.retrieve(ll_parque,ls_tipo_cobr1,ls_tipo_cobr2,ls_tipo_cobr3)
	elseif cbx_parque.checked=false then
		ll_tot_reg							= dw_lista.retrieve(ls_tipo_cobr1,ls_tipo_cobr2,ls_tipo_cobr3)
	end if
	if ll_tot_reg=0 then
		...
		for ll_indi=1 to ll_tot_reg
			ll_rut							= dw_lista.getitemnumber(ll_indi,'vista_at_terreno_cobranza_rut')
			ls_base						= dw_lista.getitemstring(ll_indi,'vista_at_terreno_cobranza_base')
			ls_serie						= dw_lista.getitemstring(ll_indi,'vista_at_terreno_cobranza_serie')
			ll_numero					= dw_lista.getitemnumber(ll_indi,'vista_at_terreno_cobranza_numero')
			ls_tipo_cob_lista			= dw_lista.getitemstring(ll_indi,'vista_at_terreno_cobranza_tipo_cobranza')
			...
			WHERE	( "AT_GESTION"."BASE" = :ls_base ) AND  
						...
						( "AT_GESTION"."TIPO_COBRANZA" = :ls_tipo_cob_lista )  
			USING	sqlca;
			if sqlca.sqlcode=0 then
...
integer taborder = 10
string title = "none"
string dataobject = "dwe_seleccionar_filtro_cobranza"
...
event clicked;close(w_generar_filtro_cartera_cobranza)
end event
...
integer height = 1608
integer taborder = 70
string dataobject = "dw_lista_filtro_asigna_cart_terr_cob"
...
if row>0 then
	...
	gs_serie					= dw_lista.getitemstring(il_row, "vista_at_terreno_cobranza_serie")
	gi_numero				= dw_lista.getitemnumber(il_row, "vista_at_terreno_cobranza_numero")
	ll_status				= dw_lista.GetItemNumber(il_row, "vista_at_terreno_cobranza_status_geo")
	if ll_status>0 then
		cb_geo.enabled		= true
	else
