forward
global type w_informe_por_agentes_graf from window
end type
type dw_3 from datawindow within w_informe_por_agentes_graf
end type
type p_4 from picture within w_informe_por_agentes_graf
end type
type p_3 from picture within w_informe_por_agentes_graf
end type
type cb_2 from commandbutton within w_informe_por_agentes_graf
end type
type dw_1 from datawindow within w_informe_por_agentes_graf
end type
type cb_1 from commandbutton within w_informe_por_agentes_graf
end type
type pb_consultar from picturebutton within w_informe_por_agentes_graf
end type
type st_1 from statictext within w_informe_por_agentes_graf
end type
type em_fec_ini from editmask within w_informe_por_agentes_graf
end type
type em_fec_fin from editmask within w_informe_por_agentes_graf
end type
type st_2 from statictext within w_informe_por_agentes_graf
end type
type dw_2 from datawindow within w_informe_por_agentes_graf
end type
type gb_1 from groupbox within w_informe_por_agentes_graf
end type
type cb_limpiar from commandbutton within w_informe_por_agentes_graf
end type
type gb_2 from groupbox within w_informe_por_agentes_graf
end type
type cb_anual from commandbutton within w_informe_por_agentes_graf
end type
type cb_semestral from commandbutton within w_informe_por_agentes_graf
end type
type cb_mensual from commandbutton within w_informe_por_agentes_graf
end type
end forward
...
global w_informe_por_agentes_graf w_informe_por_agentes_graf

type variables
string is_cod_jef,is_cod_sup,is_cod_age
end variables

on w_informe_por_agentes_graf.create
...
on w_informe_por_agentes_graf.destroy
destroy(this.dw_3)
destroy(this.p_4)
destroy(this.p_3)
destroy(this.cb_2)
...
event open;if gs_depto='I' or gs_depto='G' then	 	
	gf_centrar(w_informe_por_agentes_graf)
...
elseif gs_depto = 'X' or gs_depto='B' then
	gf_centrar(w_informe_por_agentes_graf)
	...
	dw_3.insertrow(0)
	dw_3.setitem(1,'cod_jef',gs_user)
	dw_3.setitem(1,'cod_sup','')
	dw_3.setitem(1,'agente','')
//	dw_3.getchild('cod_jef',idw_detalle)
//	idw_detalle.settransobject(sqlca)
//	if idw_detalle.retrieve() = 0 then
//		idw_detalle.insertrow(0)
...
//	dw_3.insertrow(0)
	...
	close(w_informe_por_agentes_graf)
end if	
...
integer taborder = 10
string title = "none"
string dataobject = "dwe_jefe_supervisor_agente"
...
event clicked;close(w_informe_por_agentes_graf)
end event
...
if isnull(is_cod_jef) then
	...
	if ldt_fech_ini <= ldt_fech_fin then	
		...
		if dw_1.retrieve(ldt_fech_ini,ldt_fech_fin,is_cod_jef,is_cod_sup,is_cod_age)=0 then
			 Messagebox("Mensaje","No Se Puede Generar Gráfico, Aun No Existen Datos en el Rango Consultado")
			 dw_2.reset()
		else
			dw_2.settransobject(sqlca)
			if dw_2.retrieve(ldt_fech_ini,ldt_fech_fin,is_cod_jef,is_cod_sup,is_cod_age)=0 then	
			else
			end if
		end if	
	else
...
em_fec_ini.text			= string(gdt_fec_sistema,'dd/mm/yyyy')
em_fec_fin.text			= string(gdt_fec_sistema,'dd/mm/yyyy')
w_informe_por_agentes_graf.triggerevent(open!)
