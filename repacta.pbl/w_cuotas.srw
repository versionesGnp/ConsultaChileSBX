forward
global type w_cuotas from window
end type
type dw_seguro from datawindow within w_cuotas
end type
type rb_anticipado from radiobutton within w_cuotas
end type
type rb_normal from radiobutton within w_cuotas
end type
type cb_aceptar from commandbutton within w_cuotas
end type
type em_numero from editmask within w_cuotas
end type
type dw_print from datawindow within w_cuotas
end type
type st_1 from statictext within w_cuotas
end type
type st_protesto from statictext within w_cuotas
end type
type st_castigado from statictext within w_cuotas
end type
type cb_2 from commandbutton within w_cuotas
end type
type cb_ver_cuenta from commandbutton within w_cuotas
end type
type cb_3 from commandbutton within w_cuotas
end type
type cb_4 from commandbutton within w_cuotas
end type
type cb_5 from commandbutton within w_cuotas
end type
type cb_6 from commandbutton within w_cuotas
end type
type cb_7 from commandbutton within w_cuotas
end type
type cb_8 from commandbutton within w_cuotas
end type
type cb_pagare from commandbutton within w_cuotas
end type
type cb_est_cheque from commandbutton within w_cuotas
end type
type dw_ctto from datawindow within w_cuotas
end type
type cb_cerrar from commandbutton within w_cuotas
end type
type gb_1 from groupbox within w_cuotas
end type
type em_saldo_peso from editmask within w_cuotas
end type
type em_saldo_uf from editmask within w_cuotas
end type
type em_pie_peso from editmask within w_cuotas
end type
type em_pie_uf from editmask within w_cuotas
end type
type dw_repacta from datawindow within w_cuotas
end type
end forward
...
if gs_conexion = "Parque El Prado" then
	ll_tot_reg	= dw_seguro.retrieve(1, gs_base)	//'O')
elseif gs_conexion = "Parque La Foresta" then
	ll_tot_reg	= dw_seguro.retrieve(11, gs_base)	//'O')
end if
...
event clicked;if this.checked=true then
	...
	ldt_fecha_entre				= dw_ctto.getitemdatetime(1,'pago_oferta_fecha_entre')
	if gdt_fec_sistema >= ldt_fecha_entre and not isnull(ldt_fecha_entre) then
		messagebox("Advertencia","Esta Promesa NO tiene Periodo de Restricción Vigente")
	end if
	st_castigado.visible			= false
	st_protesto.visible			= false
	...
	dw_repacta.setitem(ll_new,'base',gs_base)	//'O')
	...
	if dw_ctto.rowcount()>0 then
		...
		next
		ldb_cap_pag						= ldb_total_pagado   //double(ldb_cap_pag + ldb_pie_pagado)
...
//		if ldb_interes_mora<=0 or isnull(ldb_interes_mora) 				then ldb_interes_mora=0
		...
		if ls_moneda='1' then //peso
			ldb_valor_presente	= round((ldb_precio + ldb_der_esp) / gd_uf,2)//ldb_precio + round((ldb_der_esp * gd_uf),0)
		else
...
//		dw_repacta.setitem(1,'saldo_precio',ldb_valor_presente)//ldb_valor_presente + ldb_valor_cuotas_mora + ldb_interes_mora)
...
//			ldb_saldo_precio		= (dw_repacta.getitemnumber(1,'saldo_precio') + ldb_gasto_adm) - ldb_pie_new
...
//		if ls_moneda='2' then
//			ldb_pie_pesos			= round(ldb_pie * gd_uf,0)
//			ldb_valor_presente	= ldb_valor_presente + (ldb_pie_pesos - ll_pie_pagado_peso)
//		else
//			ldb_valor_presente	= ldb_valor_presente + (ldb_pie - ll_pie_pagado_peso)
//		end if
...
Double	ldb_precio,ldb_der_esp,ldb_tot_pag,ldb_porce_10,ldb_nvo_pie,ldb_precio_calc,ldb_precio_ctto,ldb_precio_calc_peso,ldb_monto,&
			ll_numero,ll_numero_rep,ll_numero_ctto_sap_c,ll_numero_ctto_sap_m
Date		ld_fec_res			
...
if Double(em_numero.text) > 0 and em_numero.text <> '' and not isnull(em_numero.text) then
	...
	ldb_precio_ctto			= dw_ctto.getitemnumber(1,'pago_oferta_precio')
	ls_modena				= dw_ctto.getitemstring(1,'pago_oferta_moneda')
			...
			if ls_pasa='S' then
				...
				if Double(em_numero.text) > 0 then
					...
					if dw_repacta.update() = 1 then
						...
						VALUES 	( :ls_base, :ls_serie, :ll_numero, :gdt_fec_sistema, :ld_fec_res, 'V' ,'P', :gs_user, :ll_numero_ctto_sap_c, :ll_numero_ctto_sap_m )
						USING		sqlca;
						if sqlca.sqlcode=0 then
							commit;
						else
...
if ls_columna='moneda' or ls_columna='nvo_plazo' or ls_columna='nvo_pie' or &
	...
	elseif rb_anticipado.checked=true then
		if dw_repacta.getitemstring(1,'moneda')='1' then //peso
			if dw_ctto.getitemstring(1,'pago_oferta_moneda')='1' then //peso
				ldb_precio				= round( dw_ctto.getitemnumber(1,'pago_oferta_precio') / gd_uf,2)//dw_ctto.getitemnumber(1,'pago_oferta_precio')
				ldb_der_esp				= dw_ctto.getitemnumber(1,'pago_oferta_des_esp')// * gd_uf,0)
			elseif dw_ctto.getitemstring(1,'pago_oferta_moneda')='2' then //UF
				ldb_precio				= dw_ctto.getitemnumber(1,'pago_oferta_precio')
				ldb_der_esp				= dw_ctto.getitemnumber(1,'pago_oferta_des_esp')
