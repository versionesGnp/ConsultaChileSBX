forward
global type w_imprimir_detalle_pago_bkp from window
end type
type dw_seguro from datawindow within w_imprimir_detalle_pago_bkp
end type
type dw_abono from datawindow within w_imprimir_detalle_pago_bkp
end type
type st_help_tc_check from statictext within w_imprimir_detalle_pago_bkp
end type
type st_help_ch_check from statictext within w_imprimir_detalle_pago_bkp
end type
type st_help_tc from statictext within w_imprimir_detalle_pago_bkp
end type
type st_help_ch from statictext within w_imprimir_detalle_pago_bkp
end type
type st_saldo from statictext within w_imprimir_detalle_pago_bkp
end type
type st_1 from statictext within w_imprimir_detalle_pago_bkp
end type
type dw_tc_2 from datawindow within w_imprimir_detalle_pago_bkp
end type
type dw_cheque_2 from datawindow within w_imprimir_detalle_pago_bkp
end type
type em_dias_cheque from editmask within w_imprimir_detalle_pago_bkp
end type
type st_16 from statictext within w_imprimir_detalle_pago_bkp
end type
type rb_otra_fecha_ch from radiobutton within w_imprimir_detalle_pago_bkp
end type
type rb_ch_30_dias from radiobutton within w_imprimir_detalle_pago_bkp
end type
type rb_ch_dia from radiobutton within w_imprimir_detalle_pago_bkp
end type
type cb_limpiar from commandbutton within w_imprimir_detalle_pago_bkp
end type
type cbx_td from checkbox within w_imprimir_detalle_pago_bkp
end type
type cbx_tc from checkbox within w_imprimir_detalle_pago_bkp
end type
type cbx_ch from checkbox within w_imprimir_detalle_pago_bkp
end type
type cbx_ef from checkbox within w_imprimir_detalle_pago_bkp
end type
type em_cuotas_tc from editmask within w_imprimir_detalle_pago_bkp
end type
type st_cantidad_cuotas from statictext within w_imprimir_detalle_pago_bkp
end type
type dw_efectivo from datawindow within w_imprimir_detalle_pago_bkp
end type
type dw_td from datawindow within w_imprimir_detalle_pago_bkp
end type
type dw_tc from datawindow within w_imprimir_detalle_pago_bkp
end type
type em_cheques from editmask within w_imprimir_detalle_pago_bkp
end type
type st_cantidad_dcto from statictext within w_imprimir_detalle_pago_bkp
end type
type dw_cheque from datawindow within w_imprimir_detalle_pago_bkp
end type
type cb_cerrar from commandbutton within w_imprimir_detalle_pago_bkp
end type
type cb_imprimir from commandbutton within w_imprimir_detalle_pago_bkp
end type
type dw_detalle from datawindow within w_imprimir_detalle_pago_bkp
end type
type gb_cheque from groupbox within w_imprimir_detalle_pago_bkp
end type
type ln_1 from line within w_imprimir_detalle_pago_bkp
end type
type ln_2 from line within w_imprimir_detalle_pago_bkp
end type
end forward
...
global w_imprimir_detalle_pago_bkp w_imprimir_detalle_pago_bkp
...
ll_total	= dw_detalle.getitemnumber(1,'total_2')
if cbx_ef.checked=true then
	if dw_efectivo.rowcount()>0 then ll_total	= ll_total - dw_efectivo.getitemnumber(1,'monto')
end if
if cbx_ch.checked=true then
	if dw_cheque.rowcount()>0 then ll_total = ll_total - dw_cheque.getitemnumber(1,'monto_total')
end if
if cbx_tc.checked=true then
	if dw_tc.rowcount()>0 then ll_total	= ll_total - dw_tc.getitemnumber(1,'monto_total')
end if
if cbx_td.checked=true then
	if dw_td.rowcount()>0 then ll_total	= ll_total - dw_td.getitemnumber(1,'monto')
end if
st_saldo.text	= string(ll_total,"###,###,###,###,###,##0.0###")
if ll_total<0 then
...
on w_imprimir_detalle_pago_bkp.create
...
on w_imprimir_detalle_pago_bkp.destroy
...
event open;Long		ll_tot_reg,ll_count,ll_indi,ll_new
String	ls_tipo_cob,ls_nom,ls_ap_pat,ls_ap_mat,ls_dv
Double	ll_suma_monto_otros,ll_suma_cuota,ll_monto_abono
...
if isvalid(w_repacta) then
	...
	if ll_tot_reg>0 then
		...
		if ll_count > 0 then
			...
			ll_count = w_repacta.dw_detalle_ci.Find("tipo_cob = 'E'",1, ll_tot_reg)
			if ll_count > 0 then
...
elseif isvalid(w_boletas_cuotas) then
	...
	if ll_tot_reg>0 then
		...
		if ll_count > 0 then
			...
			ll_count = w_boletas_cuotas.dw_detalle_bl.Find("tipo_cob = 'E'",1, ll_tot_reg)
			if ll_count > 0 then
...
if this.checked=true then
	...
	ll_td					= dw_detalle.getitemnumber(1,'total_2') - ll_suma
	dw_td.visible	= true
...
if this.checked=true then
	...
	ll_tc									= dw_detalle.getitemnumber(1,'total_2') - ll_suma
	em_cuotas_tc.text				= ''
	dw_tc.enabled					= false
	dw_tc.visible					= true
	...
	if ll_tc<=0 then
		messagebox("Advertencia","No es posible Ingresar Pago Tarjeta Crédito, Monto a Cancelar es US$ "+string(dw_detalle.getitemnumber(1,'total_2'),"###,###,###,###,###"))
...
if this.checked=true then
	...
	ll_cheque						= dw_detalle.getitemnumber(1,'total_2') - ll_suma
	em_cheques.text				= ''
...
if this.checked=true then
	...
	ll_efectivo				= dw_detalle.getitemnumber(1,'total_2') - ll_suma
...
CHOOSE CASE ls_columna
	CASE 'monto'
		ll_total_pagar	= dw_detalle.getitemnumber(1,'total_2')
		...
		if ll_monto_ch > ll_total_pagar then
			...
			if ll_suma > ll_total_pagar then
				...
				if this.getrow() = 1 and il_pasa_tc=0 then
					...
					il_pasa_tc ++
					if cbx_ef.checked=true then
						ll_suma_monto	= ll_suma_monto + dw_efectivo.getitemnumber(1,'monto')
						if isnull(ll_suma_monto) then ll_suma_monto=0
					end if
					if cbx_ch.checked=true then
						ll_suma_monto	= ll_suma_monto + dw_cheque.getitemnumber(1,'monto_total')
						if isnull(ll_suma_monto) then ll_suma_monto=0
					end if
					if cbx_td.checked=true then
						ll_suma_monto	= ll_suma_monto + dw_td.getitemnumber(1,'monto')
						if isnull(ll_suma_monto) then ll_suma_monto=0
					end if
...
CHOOSE CASE ls_columna
	CASE 'monto'
		ll_total_pagar	= dw_detalle.getitemnumber(1,'total_2')
		ll_monto_ch		= dw_cheque.getitemnumber(this.getrow(),'monto')
		if dw_efectivo.rowcount()>0 then
			ll_suma		= ll_suma + dw_efectivo.getitemnumber(1,'monto')
		end if
		if ll_monto_ch > ll_total_pagar then
...
//			dw_cheque.setitem(1,'monto',0)
			...
			if ll_suma > ll_total_pagar then
				...
				if this.getrow() = 1 and il_pasa_ch=0 then
					...
					if cbx_tc.checked=true then
						ll_suma_monto	= ll_suma_monto + dw_tc.getitemnumber(1,'monto_total')
						if isnull(ll_suma_monto) then ll_suma_monto=0
					end if
					if cbx_td.checked=true then
						ll_suma_monto	= ll_suma_monto + dw_td.getitemnumber(1,'monto')
...
event clicked;Long		ll_error=0,ll_tot_reg,ll_indi,ll_banco,ll_serie,ll_item
Date		ldt_fecha_vcto
String	ls_descrip
...
//				Limpiar objetos
				...
				if cbx_ef.checked=true then //Cargar Efectivo
					...
					if ll_monto>0 then
						...
						dw_detalle.Modify("t_monto_ef.Text='"+string(ll_monto,'###,###,###,###,##0.0###')+"'")
					end if
				end if
				...
				f_printdlg(dw_detalle,gstr_print,w_imprimir_detalle_pago)
			end if
		else
