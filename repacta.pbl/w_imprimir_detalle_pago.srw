forward
global type w_imprimir_detalle_pago from window
end type
type dw_seguro from datawindow within w_imprimir_detalle_pago
end type
type dw_abono from datawindow within w_imprimir_detalle_pago
end type
type st_help_tc_check from statictext within w_imprimir_detalle_pago
end type
type st_help_tc from statictext within w_imprimir_detalle_pago
end type
type st_help_ch from statictext within w_imprimir_detalle_pago
end type
type st_saldo from statictext within w_imprimir_detalle_pago
end type
type st_1 from statictext within w_imprimir_detalle_pago
end type
type dw_tc_2 from datawindow within w_imprimir_detalle_pago
end type
type dw_cheque_2 from datawindow within w_imprimir_detalle_pago
end type
type em_dias_cheque from editmask within w_imprimir_detalle_pago
end type
type st_16 from statictext within w_imprimir_detalle_pago
end type
type rb_otra_fecha_ch from radiobutton within w_imprimir_detalle_pago
end type
type rb_ch_30_dias from radiobutton within w_imprimir_detalle_pago
end type
type rb_ch_dia from radiobutton within w_imprimir_detalle_pago
end type
type cb_limpiar from commandbutton within w_imprimir_detalle_pago
end type
type cbx_td from checkbox within w_imprimir_detalle_pago
end type
type cbx_tc from checkbox within w_imprimir_detalle_pago
end type
type cbx_ch from checkbox within w_imprimir_detalle_pago
end type
type cbx_ef from checkbox within w_imprimir_detalle_pago
end type
type em_cuotas_tc from editmask within w_imprimir_detalle_pago
end type
type st_cantidad_cuotas from statictext within w_imprimir_detalle_pago
end type
type dw_efectivo from datawindow within w_imprimir_detalle_pago
end type
type dw_td from datawindow within w_imprimir_detalle_pago
end type
type dw_tc from datawindow within w_imprimir_detalle_pago
end type
type em_cheques from editmask within w_imprimir_detalle_pago
end type
type st_cantidad_dcto from statictext within w_imprimir_detalle_pago
end type
type dw_cheque from datawindow within w_imprimir_detalle_pago
end type
type cb_cerrar from commandbutton within w_imprimir_detalle_pago
end type
type cb_imprimir from commandbutton within w_imprimir_detalle_pago
end type
type gb_cheque from groupbox within w_imprimir_detalle_pago
end type
type ln_1 from line within w_imprimir_detalle_pago
end type
type ln_2 from line within w_imprimir_detalle_pago
end type
type dw_detalle from datawindow within w_imprimir_detalle_pago
end type
type st_help_ch_check from statictext within w_imprimir_detalle_pago
end type
end forward
...
global w_imprimir_detalle_pago w_imprimir_detalle_pago
...
on w_imprimir_detalle_pago.create
...
on w_imprimir_detalle_pago.destroy
...
event open;Long		ll_tot_reg,ll_count,ll_indi,ll_new,ll_cod_seguro,ll_fila,ll_monto_seg,ll_suma_seg,ll_monto_uni,ll_cant
...
gf_centrar(w_imprimir_detalle_pago)
...
if isvalid(w_repacta) then
	...
	if ll_tot_reg>0 then
		ll_count = w_repacta.dw_detalle_ci.Find("monto = 0 and monto_uf = 0",1, ll_tot_reg)
		if ll_count > 0 then
			messagebox("Advertencia","Debe Ingresar Correctamente Detalle de Pago")
			w_repacta.dw_detalle_ci.setfocus()
			w_repacta.dw_detalle_ci.scrolltorow(ll_count)
			w_repacta.dw_detalle_ci.setcolumn('monto')
			w_repacta.dw_detalle_ci.setfocus()
			close(w_imprimir_detalle_pago)
		else
			...
			ll_count = w_repacta.dw_detalle_ci.Find("tipo_cob = 'E' or tipo_cob = 'U'",1, ll_tot_reg)
			if ll_count > 0 then
				dw_detalle.setitem(ll_new,'cuotas_a_pagar',w_repacta.dw_detalle_ci.getitemnumber(ll_count,'cantidad'))
				dw_detalle.setitem(ll_new,'valor_cuota',w_repacta.dw_datos_ci.getitemnumber(1,'c_valor_cuota_pesos'))
				...
				if sqlca.sqlcode=0 then
					do while sqlca.sqlcode=0
						...
						if not isnull(ll_cod_seguro) then
							ll_fila		= w_repacta.dw_detalle_ci.Find("tipo_cob = '"+string(ll_cod_seguro)+"'",1,  w_repacta.dw_detalle_ci.RowCount())
							if ll_fila > 0 then
								ll_monto_seg	= w_repacta.dw_detalle_ci.getitemnumber(ll_fila,'c_total_linea')
//								ll_monto_uni	= w_repacta.dw_detalle_ci.getitemnumber(ll_fila,'monto')
//								ll_cant			= w_repacta.dw_detalle_ci.getitemnumber(ll_fila,'cantidad')
//								ll_monto_seg	= ll_monto_uni * ll_cant
			...
			ll_count = w_repacta.dw_detalle_ci.Find("tipo_cob = 'F'",1, ll_tot_reg)
			if ll_count > 0 then
				dw_detalle.setitem(ll_new,'cuotas_a_pagar_mant',w_repacta.dw_detalle_ci.getitemnumber(ll_count,'cantidad'))
				dw_detalle.setitem(ll_new,'valor_cuota_mant',w_repacta.dw_datos_ci.getitemnumber(1,'c_valor_cuota_mantencion_pesos'))
			else
			...
			dw_detalle.accepttext()
			dw_detalle.setitem(ll_new,'cod_parque', w_repacta.dw_datos_ci.getitemnumber(1,'cadena_cod_parque'))
			if w_repacta.dw_gastos_ci.getitemnumber(1,'incluir_gastos') = 1 then
				dw_detalle.setitem(ll_new,'gastos_cob', w_repacta.dw_gastos_ci.getitemnumber(1,'gastos_cob'))
				dw_detalle.setitem(ll_new,'intereses', w_repacta.dw_gastos_ci.getitemnumber(1,'mora'))
			end if
			dw_detalle.accepttext()
...
//			ll_suma_cuota			= dw_detalle.getitemnumber(1,'total_cuota')
			...
			ll_iva_p					= 0
			ll_iva_r					= 0
			dw_detalle.setitem(ll_new,'iva_cuota',0 )
			dw_detalle.setitem(ll_new,'iva_mora',0 )
			dw_detalle.setitem(ll_new,'iva_gastos',0 )
			dw_detalle.accepttext()
			for ll_indi=1 to ll_tot_reg
				...
				CHOOSE CASE ls_tipo_cob
					CASE 'A','Q'
						...
						dw_detalle.setitem(ll_new,'total_cuota_pie',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'F'
						dw_detalle.setitem(ll_new,'estado_mantencion',0)
						dw_detalle.setitem(ll_new,'total_cuota_mantencion',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'E','U'
						dw_detalle.setitem(ll_new,'estado_cuota',0)
						ll_suma_cuota	= ll_suma_cuota + round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						dw_detalle.setitem(ll_new,'total_cuota',ll_suma_cuota)
					CASE 'L'
						dw_detalle.setitem(ll_new,'cod_pago_otro',w_repacta.dw_detalle_ci.getitemstring(ll_indi,'tipo_cob_otro'))
						ll_suma_monto_otros	= ll_suma_monto_otros + round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
					CASE 'M'
						dw_detalle.setitem(ll_new,'derecho_sepulta',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'D'
						ll_iva_f					= round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_f) then ll_iva_f=0
						dw_detalle.setitem(ll_new,'iva_cuota',ll_iva_f )
					CASE 'V'
						ll_iva_p					= round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_p) then ll_iva_p=0
						dw_detalle.setitem(ll_new,'iva_mora',ll_iva_p)
					CASE 'X'
						ll_iva_r					= round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_r) then ll_iva_r=0
						dw_detalle.setitem(ll_new,'iva_gastos',ll_iva_r)
					CASE 'S'
				//		dw_detalle.setitem(ll_new,'cod_desc_otro',w_repacta.dw_detalle_ci.getitemstring(ll_indi,'tipo_cob_otro'))
				//		dw_detalle.setitem(ll_new,'descuento',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						ls_cod_otro			= w_repacta.dw_detalle_ci.getitemstring(ll_indi,'tipo_cob_otro')
						dw_detalle.setitem(ll_new,'cod_desc_otro',ls_cod_otro)
						if ls_cod_otro = '102' then
							dw_detalle.setitem(ll_new,'desct_mora_cred_int',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '118' then
							dw_detalle.setitem(ll_new,'desct_mora_cred_gc',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '130' then
							dw_detalle.setitem(ll_new,'desct_mora_mant_int',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '133' then
							dw_detalle.setitem(ll_new,'desct_mora_mant_gc',round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						else	
							ll_suma_descuento	= ll_suma_descuento + round(w_repacta.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
...
elseif isvalid(w_repacta_funeraria) then
...
//			ll_suma_cuota			= dw_detalle.getitemnumber(1,'total_cuota')
			...
			for ll_indi=1 to ll_tot_reg
				...
				CHOOSE CASE ls_tipo_cob
					...
					CASE 'D'
						ll_iva_f					= round(w_repacta_funeraria.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_f) then ll_iva_f=0
						dw_detalle.setitem(ll_new,'iva_cuota',ll_iva_f )
					CASE 'V'
						ll_iva_p					= round(w_repacta_funeraria.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_p) then ll_iva_p=0
						dw_detalle.setitem(ll_new,'iva_mora',ll_iva_p)
					CASE 'X'
						ll_iva_r					= round(w_repacta_funeraria.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0)
						if isnull(ll_iva_r) then ll_iva_r=0
						dw_detalle.setitem(ll_new,'iva_gastos',ll_iva_r)
...
//						dw_detalle.setitem(ll_new,'descuento',round(w_repacta_funeraria.dw_detalle_ci.getitemnumber(ll_indi,'c_total_linea'),0))
						ls_cod_otro			= w_repacta_funeraria.dw_detalle_ci.getitemstring(ll_indi,'tipo_cob_otro')
						dw_detalle.setitem(ll_new,'cod_desc_otro',ls_cod_otro)
						if ls_cod_otro = '102' then
...
elseif isvalid(w_boletas_cuotas) then
	...
	if ll_tot_reg>0 then
		ll_count = w_boletas_cuotas.dw_detalle_bl.Find("monto = 0 and monto_uf = 0",1, ll_tot_reg)
		if ll_count > 0 then
			messagebox("Advertencia","Debe Ingresar Correctamente Detalle de Pago")
			w_boletas_cuotas.dw_detalle_bl.setfocus()
			w_boletas_cuotas.dw_detalle_bl.scrolltorow(ll_count)
			...
			ll_count = w_boletas_cuotas.dw_detalle_bl.Find("tipo_cob = 'E' or tipo_cob = 'U'",1, ll_tot_reg)
			if ll_count > 0 then
				dw_detalle.setitem(ll_new,'cuotas_a_pagar',w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_count,'cantidad'))
				dw_detalle.setitem(ll_new,'valor_cuota',w_boletas_cuotas.dw_detalle_bl.getitemnumber(1,'c_valor_cuota_pesos'))
			else
				dw_detalle.setitem(ll_new,'cuotas_a_pagar',0)
				dw_detalle.setitem(ll_new,'valor_cuota',0)
				dw_detalle.setitem(ll_new,'otros_cobros',0)
			...
			dw_detalle.setitem(ll_new,'numero',gi_numero)
			dw_detalle.setitem(ll_new,'uf_dia',gd_uf_dia)
			ll_count = w_boletas_cuotas.dw_detalle_bl.Find("tipo_cob = 'F'",1, ll_tot_reg)
			if ll_count > 0 then
				dw_detalle.setitem(ll_new,'cuotas_a_pagar_mant',w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_count,'cantidad'))
				dw_detalle.setitem(ll_new,'valor_cuota_mant',w_boletas_cuotas.dw_datos_bl.getitemnumber(1,'c_valor_cuota_mantencion_pesos'))
			else
				dw_detalle.setitem(ll_new,'cuotas_a_pagar_mant',0)
				dw_detalle.setitem(ll_new,'valor_cuota_mant',0)
			end if
			...
			dw_detalle.setitem(ll_new,'derecho_sepulta',0)
			dw_detalle.accepttext()
			dw_detalle.setitem(ll_new,'cod_parque', w_boletas_cuotas.dw_datos_bl.getitemnumber(1,'cadena_cod_parque'))
			if w_boletas_cuotas.dw_gastos_bl.getitemnumber(1,'incluir_gastos') = 1 then
				dw_detalle.setitem(ll_new,'gastos_cob', w_boletas_cuotas.dw_gastos_bl.getitemnumber(1,'gastos_cob'))
				dw_detalle.setitem(ll_new,'intereses', w_boletas_cuotas.dw_gastos_bl.getitemnumber(1,'mora'))
			end if
			if gs_base='L' and gi_numero>0 then dw_detalle.setitem(ll_new,'estado_liberador',0)
			if gs_base='R' and gi_numero>0 then dw_detalle.setitem(ll_new,'reprog_mantencion',0)
			if gi_numero=0 then dw_detalle.setitem(ll_new,'estado_rezago',0)
...
//			ll_suma_cuota			= dw_detalle.getitemnumber(1,'total_cuota')
			...
			for ll_indi=1 to ll_tot_reg
				ls_tipo_cob	= w_boletas_cuotas.dw_detalle_bl.getitemstring(ll_indi,'tipo_cob')
				CHOOSE CASE ls_tipo_cob
					CASE 'A','Q'
						dw_detalle.setitem(ll_new,'estado_cuota',0)
						dw_detalle.setitem(ll_new,'total_cuota_pie',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'F'
						dw_detalle.setitem(ll_new,'estado_mantencion',0)
						dw_detalle.setitem(ll_new,'total_cuota_mantencion',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'E','U'
						dw_detalle.setitem(ll_new,'estado_cuota',0)
						ll_suma_cuota	= ll_suma_cuota + round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0)
						dw_detalle.setitem(ll_new,'total_cuota',ll_suma_cuota)
					CASE 'L'
						dw_detalle.setitem(ll_new,'cod_pago_otro',w_boletas_cuotas.dw_detalle_bl.getitemstring(ll_indi,'tipo_cob_otro'))
						ll_suma_monto_otros	= ll_suma_monto_otros + round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0)
					CASE 'M'
						dw_detalle.setitem(ll_new,'derecho_sepulta',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'D'
						dw_detalle.setitem(ll_new,'iva_cuota',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0) )
					CASE 'V'
						dw_detalle.setitem(ll_new,'iva_mora',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
					CASE 'X'
						dw_detalle.setitem(ll_new,'iva_gastos',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						
					CASE 'S'
//						dw_detalle.setitem(ll_new,'cod_desc_otro',w_boletas_cuotas.dw_detalle_bl.getitemstring(ll_indi,'tipo_cob_otro'))
//						dw_detalle.setitem(ll_new,'descuento',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						ls_cod_otro			= w_boletas_cuotas.dw_detalle_bl.getitemstring(ll_indi,'tipo_cob_otro')
						dw_detalle.setitem(ll_new,'cod_desc_otro',ls_cod_otro)
						if ls_cod_otro = '102' then
							dw_detalle.setitem(ll_new,'desct_mora_cred_int',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '118' then
							dw_detalle.setitem(ll_new,'desct_mora_cred_gc',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '130' then
							dw_detalle.setitem(ll_new,'desct_mora_mant_int',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						elseif	ls_cod_otro = '133' then
							dw_detalle.setitem(ll_new,'desct_mora_mant_gc',round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0))
						else	
							ll_suma_descuento	= ll_suma_descuento + round(w_boletas_cuotas.dw_detalle_bl.getitemnumber(ll_indi,'c_total_linea'),0)
							dw_detalle.setitem(ll_new,'descuento',ll_suma_descuento)
						end if
					CASE 'K'
						dw_detalle.setitem(ll_new,'estado_cuota',0)
...
event clicked;close(w_imprimir_detalle_pago)
end event
...
boolean enabled = false
string title = "none"
string dataobject = "dw_imprimir_detalle_pago"
boolean border = false
end type
