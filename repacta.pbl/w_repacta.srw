forward
global type w_repacta from window
end type
type dw_multiproducto from datawindow within w_repacta
end type
type st_1 from statictext within w_repacta
end type
type cbx_sin_gc from checkbox within w_repacta
end type
type cbx_sin_mora from checkbox within w_repacta
end type
type dw_dias_mora from datawindow within w_repacta
end type
type cb_imprimir_detalle from commandbutton within w_repacta
end type
type cb_3 from commandbutton within w_repacta
end type
type cb_2 from commandbutton within w_repacta
end type
type st_3 from statictext within w_repacta
end type
type st_4 from statictext within w_repacta
end type
type st_fecha from statictext within w_repacta
end type
type st_uf from statictext within w_repacta
end type
type cb_1 from commandbutton within w_repacta
end type
type cb_repacta from commandbutton within w_repacta
end type
type dw_datos_ci from datawindow within w_repacta
end type
type cb_ci_insertar from commandbutton within w_repacta
end type
type cb_ci_borrar_item from commandbutton within w_repacta
end type
type cb_ci_borrar from commandbutton within w_repacta
end type
type dw_encabezado_ingreso_cupon from datawindow within w_repacta
end type
type dw_abono from datawindow within w_repacta
end type
type dw_ci_buscar_cliente from datawindow within w_repacta
end type
type st_help_descuento from statictext within w_repacta
end type
type st_help_abono from statictext within w_repacta
end type
type dw_gastos_ci from datawindow within w_repacta
end type
type dw_detalle_ci from datawindow within w_repacta
end type
end forward
...
public subroutine wf_calcular_interes (string as_cobro);Long		ii_cantidad,ii_dmora,ii_mora_cobro,ii_inc_gastos,i,ll_gasto_cob,ll_mora,ll_gastos,	ll_tot_reg,ll_indi,ll_monto,ll_total_monto,ll_day,ll_month,ll_year,ll_ctas_mora,ll_new,&
			...
			ll_dia_prox_pago,ll_cant_mant,ll_cant_cred,ll_count_filas,ll_valor_int,ll_suma_acum_gc,ll_acum_id_gc,ll_acum_id_im,ll_count_f,ll_count_e,ll_cuota,ll_cant,ll_dias_mora_cred,&
			ll_acum_dias,ll_sumar_mes,ll_mora_cred,ll_dias
...
Double	ldb_tasa,ldb_val_cta_cred,id_monto
						...
						if ll_mora_cred > 0 then 
							if ll_sumar_mes <= ll_mora_cred then
								 IF ll_acum_dias > 5 or ll_dias_mora_cred > 5 THEN  
									  ...
									  id_im							= id_monto
									  IF ll_acum_dias > 30  THEN	//or i <= ll_mora_cred
//									  IF ll_acum_dias > 30 THEN
										id_gc 							= Round(ldb_val_cta_cred * id_factor_gc, 0 )
									  else
										id_gc							= 0
									  END IF;
...
//					if isnull(ll_mora) then ll_mora=0
					if cbx_sin_gc.checked=false then
						...
						if ii_cantidad <= w_repacta.dw_datos_ci.GetItemNumber(1,'c_cuotas_mora')  then   //// validar cantidad de cuotas mora a pagar
							w_repacta.dw_gastos_ci.SetItem(1,"gastos_cob",  ll_gasto_cob)
							w_repacta.dw_gastos_ci.SetItem(1,"gasto_cred", ll_gasto_cob)
							w_repacta.dw_gastos_ci.accepttext()
...
Double	ldb_uf_m,ldb_monto_item_m,ll_cupon_m,ll_suma_mora,ll_suma_gasto
...
if ll_tot_reg_m > 0 then
	if as_columna='tipo_cob' then
		...
		for ll_indi_m=1 to ll_tot_reg_m
			...
			if ll_indi_m > 1 and ls_tipo_mov_m<>'J' and ls_tipo_mov_m<>'G' and ls_tipo_mov_m<>'R' and ls_tipo_mov_m<>'P' then
				ll_fila_m				= dw_detalle_ci.insertrow(0)
				dw_detalle_ci.SetItem(ll_fila_m, 'uf', gd_uf)
			end if
			if ls_tipo_mov_m<>'J' and ls_tipo_mov_m<>'G' and ls_tipo_mov_m<>'R' and ls_tipo_mov_m<>'P' then
			...
			if ls_tipo_mov_m='G' then
				dw_gastos_ci.setitem(1,'mora_cred',ldb_monto_item_m)
				ll_suma_mora			= ll_suma_mora + ldb_monto_item_m
			elseif ls_tipo_mov_m='J' then
				dw_gastos_ci.setitem(1,'gasto_cred',ldb_monto_item_m)
				ll_suma_gasto			= ll_suma_gasto + ldb_monto_item_m
			elseif ls_tipo_mov_m='R' then
				dw_gastos_ci.setitem(1,'gasto_mant',ldb_monto_item_m)
				ll_suma_gasto			= ll_suma_gasto + ldb_monto_item_m
			elseif ls_tipo_mov_m = 'P' then
				dw_gastos_ci.setitem(1,'mora_mant',ldb_monto_item_m)
				ll_suma_mora			= ll_suma_mora + ldb_monto_item_m
			end if
		next
		dw_gastos_ci.setitem(1,'mora',ll_suma_mora)
		dw_gastos_ci.setitem(1,'gastos_cob',ll_suma_gasto)
	...
	elseif as_columna='cantidad' then
		for ll_indi_m=1 to ll_tot_reg_m
			...
			if ll_fila = 0 and ls_tipo_mov_m<>'J' and ls_tipo_mov_m<>'G' and ls_tipo_mov_m<>'R' and ls_tipo_mov_m<>'P' then
...
if ls_pasa='S' then
	...
	//					this.SetItem(row, "cantidad", 0)
					...
					if gs_base='L' then
						...
						if gs_base<>'M' then wf_calcular_interes(ls_tipo_cob)
...
//			this.accepttext()
			...
			if ls_tipo_cob = "A" or ls_tipo_cob = "Q" then
				IF ll_cantidad > 1 AND gs_rezago <> "S" and ll_ctas_pactadas_pie = 0 AND (gs_base='O' or gs_base='U' or gs_base='M') THEN
					messagebox("Advertencia","No es posible Ingresar más de 1 Cuota Pie, No es Parcelado")
					wf_borrar_fila(row)
				else
					IF (ls_tipo_cob = "A" or ls_tipo_cob = "Q") AND ll_cantidad = 1 AND gs_rezago <> "S" and ll_ctas_pactadas_pie = 0 AND (gs_base='O' or gs_base='U' or gs_base='M') THEN
						this.SetItem(row, "monto", dw_datos_ci.GetItemNumber(1,"c_valor_pie_pesos"))
					end if
				end if
			end if
		...
		if gs_base='M' then
			...
			wf_cargar_credito_m(gs_base+gs_serie+string(gi_numero),ls_tipo_cob,ii_cantidad,'CTO',row,is_columna)
		else
			if gs_base <> 'M' then wf_calcular_interes(ls_tipo_cob)
