forward
global type w_repacta_bkp_iva from window
end type
type st_1 from statictext within w_repacta_bkp_iva
end type
type cbx_sin_gc from checkbox within w_repacta_bkp_iva
end type
type cbx_sin_mora from checkbox within w_repacta_bkp_iva
end type
type dw_dias_mora from datawindow within w_repacta_bkp_iva
end type
type cb_imprimir_detalle from commandbutton within w_repacta_bkp_iva
end type
type cb_3 from commandbutton within w_repacta_bkp_iva
end type
type cb_2 from commandbutton within w_repacta_bkp_iva
end type
type st_3 from statictext within w_repacta_bkp_iva
end type
type st_4 from statictext within w_repacta_bkp_iva
end type
type st_fecha from statictext within w_repacta_bkp_iva
end type
type st_uf from statictext within w_repacta_bkp_iva
end type
type cb_1 from commandbutton within w_repacta_bkp_iva
end type
type cb_repacta from commandbutton within w_repacta_bkp_iva
end type
type dw_datos_ci from datawindow within w_repacta_bkp_iva
end type
type cb_ci_insertar from commandbutton within w_repacta_bkp_iva
end type
type cb_ci_borrar_item from commandbutton within w_repacta_bkp_iva
end type
type cb_ci_borrar from commandbutton within w_repacta_bkp_iva
end type
type dw_encabezado_ingreso_cupon from datawindow within w_repacta_bkp_iva
end type
type dw_abono from datawindow within w_repacta_bkp_iva
end type
type dw_ci_buscar_cliente from datawindow within w_repacta_bkp_iva
end type
type st_help_descuento from statictext within w_repacta_bkp_iva
end type
type st_help_abono from statictext within w_repacta_bkp_iva
end type
type dw_gastos_ci from datawindow within w_repacta_bkp_iva
end type
type dw_detalle_ci from datawindow within w_repacta_bkp_iva
end type
end forward
...
global w_repacta_bkp_iva w_repacta_bkp_iva
...
event itemchanged;//string	is_columna,ls_texto,ls_tipo_cob,ls_tipo_cob_aux,ls_pasa='S',ls_descrip_estado,ls_estado_cadena,ls_moneda_ctto,sw_ok='S',ls_tipo_comp
...
//			ll_cuota_ini,ll_cuota_fin,ll_monto_cuota_reg,ll_new_iva
//double 	id_im, id_gc, id_valor_cuota,ld_monto,ldb_uf,ldb_prima,ldb_iva,ll_iva_f
//
//
...
//				wf_calcular_interes(ls_tipo_cob)
//				if gs_base='O' or gs_base='C' then
//					ll_monto_cuota_reg		= dw_detalle_ci.getitemnumber(getrow(),'c_total_linea')
...
//					end if
//					
//					ll_mora_mant			= dw_gastos_ci.getitemnumber(1,'c_cuotas_en_mora')
...
//						  end if
//					end if
//					ll_gasto_mant				= dw_gastos_be.getitemnumber(1,'gasto_mant')
...
//					wf_borrar_fila(row)
//				else
//					IF (ls_tipo_cob = "A" or ls_tipo_cob = "Q") AND ll_cantidad = 1 AND gs_rezago <> "S" and ll_ctas_pactadas_pie = 0 AND gs_base='O' THEN
//						this.SetItem(row, "monto", dw_datos_ci.GetItemNumber(1,"c_valor_pie_pesos"))
//					end if
//				end if
//			end if
