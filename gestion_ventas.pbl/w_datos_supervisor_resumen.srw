forward
global type w_datos_supervisor_resumen from window
end type
type pb_1 from picturebutton within w_datos_supervisor_resumen
end type
type sle_1 from singlelineedit within w_datos_supervisor_resumen
end type
type cb_2 from commandbutton within w_datos_supervisor_resumen
end type
type cb_1 from commandbutton within w_datos_supervisor_resumen
end type
type dw_1 from datawindow within w_datos_supervisor_resumen
end type
type gb_1 from groupbox within w_datos_supervisor_resumen
end type
end forward
...
global w_datos_supervisor_resumen w_datos_supervisor_resumen
...
on w_datos_supervisor_resumen.create
...
on w_datos_supervisor_resumen.destroy
destroy(this.pb_1)
destroy(this.sle_1)
destroy(this.cb_2)
destroy(this.cb_1)
...
event open;gf_centrar(w_datos_supervisor_resumen)
dw_1.settransobject(sqlca)
dw_1.retrieve()
gs_ventana	= 'w_datos_supervisor_resumen'
f_valida_objeto_1()
...
event clicked;close(w_datos_supervisor_resumen)
end event
...
event doubleclicked;if this.getrow() > 0 then
	...
	OpenWithParm(w_resumen_canal,getitemstring(this.getrow(),'supervisor_cod_sup'))
end if
end event
