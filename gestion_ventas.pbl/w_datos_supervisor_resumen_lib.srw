forward
global type w_datos_supervisor_resumen_lib from window
end type
type pb_1 from picturebutton within w_datos_supervisor_resumen_lib
end type
type sle_1 from singlelineedit within w_datos_supervisor_resumen_lib
end type
type cb_2 from commandbutton within w_datos_supervisor_resumen_lib
end type
type cb_1 from commandbutton within w_datos_supervisor_resumen_lib
end type
type dw_1 from datawindow within w_datos_supervisor_resumen_lib
end type
type gb_1 from groupbox within w_datos_supervisor_resumen_lib
end type
end forward
...
string title = "Ofertas por Supervisor"
...
global w_datos_supervisor_resumen_lib w_datos_supervisor_resumen_lib
...
on w_datos_supervisor_resumen_lib.create
...
on w_datos_supervisor_resumen_lib.destroy
destroy(this.pb_1)
destroy(this.sle_1)
destroy(this.cb_2)
destroy(this.cb_1)
...
event open;gf_centrar(w_datos_supervisor_resumen_lib)
dw_1.settransobject(sqlca)
dw_1.retrieve()
gs_ventana	= 'w_datos_supervisor_resumen_lib'
f_valida_objeto_1()
...
event clicked;close(w_datos_supervisor_resumen_lib)
end event
...
event doubleclicked;if this.getrow() > 0 then
	...
	OpenWithParm(w_resumen_canal_lib,getitemstring(this.getrow(),'supervisor_cod_sup'))
end if 
end event
