forward
global type w_datos_supervisor from window
end type
type sle_1 from singlelineedit within w_datos_supervisor
end type
type pb_1 from picturebutton within w_datos_supervisor
end type
type cb_2 from commandbutton within w_datos_supervisor
end type
type cb_1 from commandbutton within w_datos_supervisor
end type
type dw_1 from datawindow within w_datos_supervisor
end type
type gb_1 from groupbox within w_datos_supervisor
end type
end forward
...
global w_datos_supervisor w_datos_supervisor
...
event open;gf_centrar(w_datos_supervisor)
dw_1.settransobject(sqlca)
dw_1.retrieve()
//wf_sort('a_supervisor_cod_sup')
gs_ventana	= 'w_datos_supervisor'
f_valida_objeto_1()
end event
...
event clicked;close(w_datos_supervisor)
end event
...
event doubleclicked;if this.getrow() > 0 then 
	...
	OpenWithParm(w_premios_supervisores,getitemstring(this.getrow(),'supervisor_cod_sup'))
end if
end event
