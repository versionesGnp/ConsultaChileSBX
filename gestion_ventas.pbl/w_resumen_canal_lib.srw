forward
global type w_resumen_canal_lib from window
end type
type p_8 from picture within w_resumen_canal_lib
end type
type p_7 from picture within w_resumen_canal_lib
end type
type st_registros from statictext within w_resumen_canal_lib
end type
type pb_antes from picturebutton within w_resumen_canal_lib
end type
type p_3 from picture within w_resumen_canal_lib
end type
type pb_sigue from picturebutton within w_resumen_canal_lib
end type
type p_2 from picture within w_resumen_canal_lib
end type
type cb_2 from commandbutton within w_resumen_canal_lib
end type
type cb_1 from commandbutton within w_resumen_canal_lib
end type
type dw_1 from uo_datawindow within w_resumen_canal_lib
end type
end forward
...
global w_resumen_canal_lib w_resumen_canal_lib

type variables
datawindow dw_paso
end variables

on w_resumen_canal_lib.create
this.p_8=create p_8
this.p_7=create p_7
...
on w_resumen_canal_lib.destroy
destroy(this.p_8)
destroy(this.p_7)
...
gf_centrar(w_resumen_canal_lib)
gs_ventana	= 'w_resumen_canal_lib'
f_valida_objeto2()
title			= gs_app_name
ls_cod 		= Message.StringParm
dw_1.settransobject(sqlca)
if isvalid(w_premios_ofertas_historica) then 
	dw_1.Object.rpt_1.DataObject 	= "dw_ofertas_periodo_5_lib_hist"
	titulo 								= 'Historicas'
end if
if dw_1.retrieve(date(gd_fecha_inicio),date(gd_fecha_fin),ls_cod) = 0 then
	...
	dw_1.object.titulo.text 					= 'Resumen de Liberadores por Supervisores '+titulo+' ( '+gs_empresa+' )'
...
event clicked;close(w_resumen_canal_lib)
end event
...
integer height = 1624
integer taborder = 10
string dataobject = "dw_premios_supervisores_55_lib"
boolean border = true
borderstyle borderstyle = stylelowered!
end type
