forward
global type w_agentes_print_lib from window
end type
type cb_ctacte from commandbutton within w_agentes_print_lib
end type
type pb_sigue from picturebutton within w_agentes_print_lib
end type
type pb_antes from picturebutton within w_agentes_print_lib
end type
type st_registros from statictext within w_agentes_print_lib
end type
type p_7 from picture within w_agentes_print_lib
end type
type p_8 from picture within w_agentes_print_lib
end type
type dw_1 from uo_datawindow within w_agentes_print_lib
end type
type cb_2 from commandbutton within w_agentes_print_lib
end type
type cb_1 from commandbutton within w_agentes_print_lib
end type
end forward
...
global w_agentes_print_lib w_agentes_print_lib

type variables
datawindow dw_paso
end variables

on w_agentes_print_lib.create
...
on w_agentes_print_lib.destroy
...
integer 	empresa
date 		fech,fecha
gf_centrar(w_agentes_print_lib)
gs_ventana	= 'w_agentes_print_lib'
f_valida_objeto_1()
title			= gs_app_name
...
if isvalid(w_premios_lib_historica) then 
	dw_1.DataObject 			= "dw_ofertas_periodo_33_lib_hist_2"
	dw_1.settransobject(sqlca)
	titulo 						= 'Historicas'
end if
if dw_1.retrieve(fech,fecha,trim(ls_cod),empresa) = 0 then
	...
	dw_1.object.titulo.text	= 'Unidades de Liberadores por Agentes '+titulo+' ( '+gs_empresa+' )'
...
integer height = 1792
integer taborder = 10
string dataobject = "dw_lib_periodo_33_sup_2"
boolean border = true
borderstyle borderstyle = stylelowered!
end type
...
event clicked;close(w_agentes_print_lib)
end event
