forward
global type w_muestra_detalle_gestion from window
end type
type hpb_1 from hprogressbar within w_muestra_detalle_gestion
end type
type st_porc from statictext within w_muestra_detalle_gestion
end type
type st_fondo from statictext within w_muestra_detalle_gestion
end type
type dw_lista_agente from datawindow within w_muestra_detalle_gestion
end type
type dw_lista from datawindow within w_muestra_detalle_gestion
end type
type st_cod_agente from statictext within w_muestra_detalle_gestion
end type
type em_cod_age from editmask within w_muestra_detalle_gestion
end type
type cb_sort from commandbutton within w_muestra_detalle_gestion
end type
type cb_exportar from commandbutton within w_muestra_detalle_gestion
end type
type cb_imprimir from commandbutton within w_muestra_detalle_gestion
end type
type cb_salir from commandbutton within w_muestra_detalle_gestion
end type
type st_volver from statictext within w_muestra_detalle_gestion
end type
type dw_muestra from datawindow within w_muestra_detalle_gestion
end type
end forward
...
global w_muestra_detalle_gestion w_muestra_detalle_gestion
...
public subroutine wf_volver_sup ();if is_opcion='O' then
	w_muestra_detalle_gestion.title		= "Resumen Actual de Agentes por Supervisor"
	dw_muestra.dataobject					= 'dw_resumen_promesas_por_gral_sup'
...
elseif is_opcion='A' then
	w_muestra_detalle_gestion.title		= "Resumen Actual de Agentes por Supervisor"
	dw_muestra.dataobject					= 'dw_resumen_aumento_por_gral_sup'
	dw_muestra.settransobject(sqlca)
	dw_muestra.object.titulo.text 		= 'Informe Gestion Actual por Supervisor'
	dw_muestra.retrieve(gd_fecha_inicio,gd_fecha_fin,trim(is_codigo),il_empresa)
	dw_muestra.object.usuario.text 		= gs_user
...
on w_muestra_detalle_gestion.create
...
on w_muestra_detalle_gestion.destroy
destroy(this.hpb_1)
...
gf_centrar(w_muestra_detalle_gestion)
em_cod_age.visible		= false
st_cod_agente.visible	= false
...
dw_muestra.object.usuario.text 				= gs_user
gs_ventana											= 'w_muestra_detalle_gestion'
f_valida_objeto_1()
end event
	...
	CHOOSE CASE dw_muestra.dataobject
		CASE 'dw_resumen_promesas_por_supervisor','dw_resumen_aumento_por_supervisor'
			...
			if ll_row > 0 then
				...
				OpenWithParm(w_muestra_detalle_gestion_agente,ls_string)
			else
...
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_gral_sup' or this.dataobject = 'dw_resumen_aumento_por_gral_sup' then
		...
		CHOOSE CASE ls_columna
			...
			CASE 'c_detalle'
				if is_opcion='O' then
					dw_muestra.dataobject	= 'dw_resumen_promesas_por_supervisor'
				elseif is_opcion='A' then
					dw_muestra.dataobject	= 'dw_resumen_aumento_por_supervisor'
				end if
				dw_muestra.settransobject(sqlca)
			...
			CASE 'c_moras'
				...
				cb_salir.visible			= false
				if is_opcion='O' then
					dw_lista.dataobject	= 'dw_ofertas_periodo_supervisor'
				elseif is_opcion='A' then
					dw_lista.dataobject	= 'dw_aumento_periodo_supervisor'
				end if 
				dw_lista.settransobject(sqlca)
	...
	elseif this.dataobject = 'dw_resumen_promesas_por_agente' or this.dataobject = 'dw_resumen_aumento_por_agente' then
		f_color_titulo(dw_muestra,ls_columna)
		CHOOSE CASE ls_columna
			CASE 'c_grafico'
				if this.object.c_grafico.text = 'Mostrar Gráfico' then
			...
			CASE 'c_moras'
				...
				cb_salir.visible			= false
				if is_opcion='O' then
					dw_lista_agente.dataobject	= 'dw_ofertas_periodo_agente'
				elseif is_opcion='A' then
					dw_lista_agente.dataobject	= 'dw_aumento_periodo_agente'
				end if
				dw_lista_agente.settransobject(sqlca)
	...
	elseif this.dataobject = 'dw_resumen_promesas_por_supervisor' then
		em_cod_age.setfocus()
	end if
end if
end event
...
ls_columna	= dwo.name
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_gral_sup' or dw_muestra.dataobject	= 'dw_resumen_promesas_por_agente' or &
		this.dataobject = 'dw_resumen_aumento_por_gral_sup' or dw_muestra.dataobject	= 'dw_resumen_aumento_por_agente' then
		ll_nro_caracter	= len(trim(is_codigo))
		ll_nro_caracter	= (5 - ll_nro_caracter)
		is_codigo			= trim(is_codigo) + space(ll_nro_caracter)
		if dw_muestra.dataobject = 'dw_resumen_promesas_por_gral_sup' or dw_muestra.dataobject = 'dw_resumen_aumento_por_gral_sup' then ls_cadena = '2'
		if dw_muestra.dataobject = 'dw_resumen_promesas_por_agente' or dw_muestra.dataobject = 'dw_resumen_aumento_por_agente' then	ls_cadena = '3'
