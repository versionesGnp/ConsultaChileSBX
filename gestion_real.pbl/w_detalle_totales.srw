forward
global type w_detalle_totales from window
end type
type cb_4 from commandbutton within w_detalle_totales
end type
type cb_consulta from commandbutton within w_detalle_totales
end type
type cb_3 from commandbutton within w_detalle_totales
end type
type cb_2 from commandbutton within w_detalle_totales
end type
type cb_filtrar from commandbutton within w_detalle_totales
end type
type dw_lista from datawindow within w_detalle_totales
end type
type cb_1 from commandbutton within w_detalle_totales
end type
end forward
...
global w_detalle_totales w_detalle_totales

type variables
long 		il_row
String	is_base,is_opcion_fecha
end variables
...
close(w_detalle_totales)
end subroutine

on w_detalle_totales.create
...
gf_centrar(w_detalle_totales)
gs_ventana			= 'w_detalle_totales'
f_valida_objeto_1()
ls_string			= trim(substr(1,1,Message.StringParm))
...
is_opcion_fecha	= substr(1,3,Message.StringParm)
...
CHOOSE CASE ls_tipo_cargo
	CASE '1'
		...
		if is_base='O' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe'
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe_cierre'
			end if
		elseif is_base='A' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe_aumento' 
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_jefe_aum_cierre'
			end if
		end if
	...
	CASE '2'
		...
		if is_base='O' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_supervisor'
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_sup_cierre'
			end if
		elseif is_base='A' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_supervisor_aume'
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_sup_aume_cierre'
			end if
		end if
		dw_lista.settransobject(sqlca)
	...
	CASE '3'
		...
		if is_base='O' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_agente'
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_agente_cierre'
			end if
		elseif is_base='A' then
			if is_opcion_fecha='U' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_agente_aume'
			elseif is_opcion_fecha='C' then
				dw_lista.dataobject	= 'dw_lista_detalle_gestion_age_aume_cierre'
			end if
		end if
		dw_lista.settransobject(sqlca)
	...
	CASE '%'
		if dw_lista.retrieve(ld_fec_ini,ld_fec_fin,ls_codigo,il_empresa,'%') = 0 then//%
			...
			if is_base='O' then
				...
				dw_lista.object.c_titulo.text = '( Total Aumento Capacidad )'
			end if
		end if
	CASE 'N'
...
integer height = 1804
integer taborder = 10
string dataobject = "dw_lista_detalle_gestion_age_aume_cierre"
