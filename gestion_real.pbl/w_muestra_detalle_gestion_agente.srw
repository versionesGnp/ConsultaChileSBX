forward
global type w_muestra_detalle_gestion_agente from window
end type
type hpb_1 from hprogressbar within w_muestra_detalle_gestion_agente
end type
type st_fondo from statictext within w_muestra_detalle_gestion_agente
end type
type st_porc from statictext within w_muestra_detalle_gestion_agente
end type
type dw_lista from datawindow within w_muestra_detalle_gestion_agente
end type
type cb_sort from commandbutton within w_muestra_detalle_gestion_agente
end type
type cb_exportar from commandbutton within w_muestra_detalle_gestion_agente
end type
type cb_imprimir from commandbutton within w_muestra_detalle_gestion_agente
end type
type cb_salir from commandbutton within w_muestra_detalle_gestion_agente
end type
type st_salir from statictext within w_muestra_detalle_gestion_agente
end type
type dw_muestra from datawindow within w_muestra_detalle_gestion_agente
end type
end forward
...
global w_muestra_detalle_gestion_agente w_muestra_detalle_gestion_agente
...
on w_muestra_detalle_gestion_agente.create
...
on w_muestra_detalle_gestion_agente.destroy
destroy(this.hpb_1)
...
gf_centrar(w_muestra_detalle_gestion_agente)
...
if is_conexion = '2' then // agente
	if is_opcion='O' then
		this.title									= "Resumen Promesas por Agente"
		dw_muestra.dataobject					= 'dw_resumen_promesas_por_agente'
	...
	elseif is_opcion='A' then
		this.title									= "Resumen Promesas por Agente"
		dw_muestra.dataobject					= 'dw_resumen_aumento_por_agente'
		dw_muestra.settransobject(sqlca)
		dw_muestra.object.titulo.text 		= 'Informe Gestion Actual por Agente'
...
gs_ventana											= 'w_muestra_detalle_gestion_agente'
f_valida_objeto_1()
end event
...
integer width = 302
integer height = 100
string dataobject = "dw_ofertas_periodo_agente"
...
integer height = 1972
integer taborder = 10
string dataobject = "dw_resumen_aumento_por_agente"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = stylelowered!
end type

event doubleclicked;String	ls_columna, ls_cadena, ls_formula,ls_opcion_fecha='U'
Int		li_new
Long		ll_nro_caracter, ll_valor
ls_columna	= dwo.name
if this.rowcount() > 0 then
	if this.dataobject = 'dw_resumen_promesas_por_agente' or this.dataobject = 'dw_resumen_aumento_por_agente' then
		...
		CHOOSE CASE ls_columna
			CASE 'ts_1','c_cien','tot_vtas_t'
				ls_cadena	= ls_cadena + '%'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'c_1_resc1','c_1_resc2','tot_nulas_n_t'
				ls_cadena	= ls_cadena + 'N'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'c_1_resueltas1','c_1_resueltas2','tot_resueltas_t'
				ls_cadena	= ls_cadena + 'R'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'c_1_reprog1','c_1_reprog2','tot_nulas_p_t'
				ls_cadena	= ls_cadena + 'P'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'c_1_cancel1','c_1_cancel2','tot_canceladas_t'
				ls_cadena	= ls_cadena + 'C'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'c_1_vigente1','c_1_vigente2','tot_vigentes_t'
				ls_cadena	= ls_cadena + 'V'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'age_al_dia1','age_al_dia2','tot_al_dia_t'
				ls_cadena	= ls_cadena + 'A'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
			CASE 'age_mora1','age_mora2','tot_en_mora_t'
				ls_cadena	= ls_cadena + 'M'+'~t'+is_opcion+'~t'+ls_opcion_fecha
				OpenWithParm(w_detalle_totales, ls_cadena)
		END CHOOSE
	elseif this.dataobject = 'dw_informe_external_agente' then
		if mid(ls_columna,1,1) = 'c' then
			...
			if ll_valor > 0 then
				...
				ls_formula			= ls_formula+'~t'+ls_cadena+'~t'+ls_columna+'~t'+is_opcion+'~t'+ls_opcion_fecha
	...
	if this.dataobject = 'dw_resumen_promesas_por_agente' or this.dataobject = 'dw_resumen_aumento_por_agente' then
		...
		CHOOSE CASE ls_columna
			...
			CASE 'c_moras'
				...
				cb_salir.visible			= false
				if is_opcion='O' then
					dw_lista.dataobject	= 'dw_ofertas_periodo_agente'
				elseif is_opcion='A' then
					dw_lista.dataobject	= 'dw_aumento_periodo_agente'
				end if
				dw_lista.settransobject(sqlca)
